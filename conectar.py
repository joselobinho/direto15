#!/usr/bin/env python
# -*- coding: utf-8 -*-
#https://www.blogopcaolinux.com.br/2016/12/Instalando-o-VMware-Workstation-Player-no-openSUSE.html
import MySQLdb
import wx
import datetime
import time
...
class sqldb:
	...
	def dbc(self, messa, op=1, fil="", sm=True, janela=""):
		...
		if op == 12:	shost, suser, spass, sqlbd, sqlpo = login.bloqueiosrv.split(";") #-: Servidor mysql do nosso cliente p/bloqueio e desbloqueio do sistema
		...
		#print shost,suser,spass,sqlbd,sqlpo
		#print sqlbd
		try:
			...
			conn = MySQLdb.connect( host = __h, user = suser, passwd = spass, db = sqlbd, connect_timeout = 25 )
			curs = conn.cursor()
...
class PrinterSocket:
    ...
    def connectPrinter(self,parent, src, port, filial ):
	...
	except Exception as erro:	rc= False
	del _mensagem
			
	if not rc:	alertas.dia( parent, "3-Erro na abertura de comunicação com o host "+str( src )+":"+str( port )+"\n\n"+str( erro )+"\n"+(" "*140),"Conexão com o host")
	return rc, conn

    def closePrinter(self,conn):	conn.close()
...
class estoque:
	...
	def fisico(self, es, sql, close, produto, qT, lista, qTDev, _filial, NegaTivo = 'F', auTorizado = False ):
		...
		#print 'ES: ',es
		#if es == "S" and sql.execute("SELECT pd_para,pd_cont FROM produtos WHERE pd_codi='"+ produto +"'"):
		#print '1: ',NegaTivo
		if es in ["S","A"] and sql.execute("SELECT pd_para,pd_cont FROM produtos WHERE pd_codi='"+ produto +"'"):
			
			__p = sql.fetchone()
			if __p[1]!='T':	NegaTivo = "T" #--// Se o produto seleciona nao tiver marca de controlado, permitir negativo
			permissao_negativo = True if len(login.filialLT[_filial][35].split(";"))>=5 and login.filialLT[_filial][35].split(";")[5] == "T" else False
			
			#if __p[0] and len( __p[0].split('|') ) >=9 and  __p[0].split('|')[8] == "T":	NegaTivo = "T" #//Vender negativo
			#if __p[0] and len( __p[0].split('|') ) >=26 and  __p[0].split('|')[25] == "T":	NegaTivo = "F" #//Nao vender negativo
			
			#if _para and len( _para.split('|') )>=26 and _para.split('|')[25]== "T":
			#if __p[1]!='T':	NegaTivo = "T"
			#print '2: ',NegaTivo,__p[1]
			#print nF.fu( _filial ),permissao_negativo
			if permissao_negativo and __p[1]=='T':	NegaTivo='F'

		#print "Negativo: ",NegaTivo, _filial
...
class numeracao:
	...
	def para_reais(self,valor):
	    
	    _valor,_decimal = format(Decimal(str(valor)),'.2f').split('.')
	    if len(_valor)==4:	_valor=_valor[:1]+'.'+_valor[1:]
	    elif len(_valor)==5:	_valor=_valor[:2]+'.'+_valor[2:]
	    elif len(_valor)==6:	_valor=_valor[:3]+'.'+_valor[3:]
	    
	    return _valor+','+_decimal
	...
	def numero(self,codifica,_informe,_painel,_fil): #-->[ Cria codigos do sistema ]
		...
		if sql[0] == True:
			...
			try:
				...
				elif codifica == "18": #-: Incremento do sequencial da mdfe por filial
							
					finding = sql[2].execute("SELECT ep_mdfe FROM cia where ep_inde='"+str(_fil)+"'")
					__valor = sql[2].fetchall()
					if __valor[0][0] == None:	__valor = ((0,),) #-: Comeca com valor zero
...
#-------------: Incremenata codigo de produtos
				...
				elif codifica == "18" and finding !=0:	_reTorno = ( int( __valor[0][0]  +1 ) ) #-: Sequencia da NFCe
				...
				elif codifica == "18":	ajuste = "UPDATE cia SET ep_mdfe='"+str(_reTorno)+"' WHERE ep_inde='"+str(_fil)+"'"

				if   codifica == "5":	sql[2].execute(ajuste) #-:Sequencia da NFe
				elif codifica == "15":	sql[2].execute(ajuste) #-:Sequencia da NFCe
				elif codifica == "18":	sql[2].execute(ajuste) #-:Sequencia da MDFe
				else:
	...
	def adicionarLista(self,lista, numero, novo):
	    
	    if len( lista ) < numero:
		for i in range(numero+1):
		    
		    if i > len(lista):	lista.append('')

		if len(lista)==numero:	lista[numero-1]=novo
	...
	def barras(self, codigo, pesquisa, _cursor, parent, _fil): #-->[ Retorna o digito do codigo de barras ]
		...
		if pesquisa in ['1','2','3']:
			...
			if pesquisa == "2" and len( codigo ) == 12:	validaDG = codigo[11]
			if pesquisa == "2" and len( codigo )  < 12:
			...
			elif pesquisa == "2" and validaDG != _DGretorno and len(codigo)>12: #-->[ Valida digito ]
	...
	def calcularProduto(self,_id,p, Filial = "", mostrar = True, retornar_valor = False, custo=(False,'') ):
		...
		try:
			...
			if _id == 240 or _id == 242 or custos:
				
				if Decimal( p.pd_pcus.GetValue() ) > 0:

					__margem = p.pd_marg.GetValue()
					if custos:	__margem=valor_custo

					valorp = Trunca.trunca(1, Decimal(p.pd_pcus.GetValue()) )
					margem = Trunca.trunca(1, Decimal(__margem) )
			...
			if login.filialLT[ Filial ][19]!="2":	valor = Trunca.trunca(1, Decimal(p.pd_tpr1.GetValue()) )
			if login.filialLT[ Filial ][19]=="2":	valor = Trunca.trunca(3, Decimal(p.pd_tpr1.GetValue()) )
			...
			if p.acrescim.GetValue():
				...
				dc = login.filialLT[ Filial ][19]
				if tipo_calculo and Decimal( p.pd_pcus.GetValue() ): #-: Calculo do preco em cima do custo

					valorp = Trunca.trunca(1, Decimal(p.pd_pcus.GetValue()) )

					if Decimal( p.pd_vdp2.GetValue() ) > 0:	pc2 = Trunca.trunca(1 if dc!='2' else 3, ( valorp + ( valorp * Decimal(p.pd_vdp2.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp3.GetValue() ) > 0:	pc3 = Trunca.trunca(1 if dc!='2' else 3, ( valorp + ( valorp * Decimal(p.pd_vdp3.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp4.GetValue() ) > 0:	pc4 = Trunca.trunca(1 if dc!='2' else 3, ( valorp + ( valorp * Decimal(p.pd_vdp4.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp5.GetValue() ) > 0:	pc5 = Trunca.trunca(1 if dc!='2' else 3, ( valorp + ( valorp * Decimal(p.pd_vdp5.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp6.GetValue() ) > 0:	pc6 = Trunca.trunca(1 if dc!='2' else 3, ( valorp + ( valorp * Decimal(p.pd_vdp6.GetValue()) / 100 ) ) )
					passei_calcu = True

				else: #-: Calculo do preco em cima do preco_1

					if Decimal( p.pd_vdp2.GetValue() ) > 0:	pc2 = Trunca.trunca(1 if dc!='2' else 3, ( valor + ( valor * Decimal(p.pd_vdp2.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp3.GetValue() ) > 0:	pc3 = Trunca.trunca(1 if dc!='2' else 3, ( valor + ( valor * Decimal(p.pd_vdp3.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp4.GetValue() ) > 0:	pc4 = Trunca.trunca(1 if dc!='2' else 3, ( valor + ( valor * Decimal(p.pd_vdp4.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp5.GetValue() ) > 0:	pc5 = Trunca.trunca(1 if dc!='2' else 3, ( valor + ( valor * Decimal(p.pd_vdp5.GetValue()) / 100 ) ) )
					if Decimal( p.pd_vdp6.GetValue() ) > 0:	pc6 = Trunca.trunca(1 if dc!='2' else 3, ( valor + ( valor * Decimal(p.pd_vdp6.GetValue()) / 100 ) ) )
					passei_calcu = True
			...
			if p.desconto.GetValue():
				...
				dc = login.filialLT[ Filial ][19]
				if Decimal( p.pd_vdp2.GetValue() ) > 0 and Decimal( p.pd_vdp2.GetValue() ) < 100:	pc2 = Trunca.trunca(1 if dc!='2' else 3, ( valor - ( valor * Decimal(p.pd_vdp2.GetValue() ) / 100 ) ) )
				if Decimal( p.pd_vdp3.GetValue() ) > 0 and Decimal( p.pd_vdp3.GetValue() ) < 100:	pc3 = Trunca.trunca(1 if dc!='2' else 3, ( valor - ( valor * Decimal(p.pd_vdp3.GetValue() ) / 100 ) ) )
				if Decimal( p.pd_vdp4.GetValue() ) > 0 and Decimal( p.pd_vdp4.GetValue() ) < 100:	pc4 = Trunca.trunca(1 if dc!='2' else 3, ( valor - ( valor * Decimal(p.pd_vdp4.GetValue() ) / 100 ) ) )
				if Decimal( p.pd_vdp5.GetValue() ) > 0 and Decimal( p.pd_vdp5.GetValue() ) < 100:	pc5 = Trunca.trunca(1 if dc!='2' else 3, ( valor - ( valor * Decimal(p.pd_vdp5.GetValue() ) / 100 ) ) )
				if Decimal( p.pd_vdp6.GetValue() ) > 0 and Decimal( p.pd_vdp6.GetValue() ) < 100:	pc6 = Trunca.trunca(1 if dc!='2' else 3, ( valor - ( valor * Decimal(p.pd_vdp6.GetValue() ) / 100 ) ) )
				passei_calcu = True
	...
	def fu( self, fl ):
		...
		rT = "F"
		try:
		    if len( login.filialLT[ fl ][35].split(";") ) >=5:
			
			rT=login.filialLT[ fl ][35].split(";")[4]

		except:
		    print 'Filial nao consta na base de filias',fl
		return rT
	...
	def fr( self, fl ):
		...
		return 'T' if login.filialLT[ fl ][30].split(";")[1]=='T' else 'F'
	...
	def retornoIBPT(self,tabela,ncm, opcao = 1):
			...
			if i != "":
				...
				if opcao ==2:
					...
					if ordem == 1:
						...
						data_final  = datetime.datetime.strptime( cd[9], '%d/%m/%Y').date() if cd[9] else ""
						data_hoje   = datetime.datetime.now().date()
						expiracao = format( data_inicio, '%d/%m/%Y')+' A '+format( data_final, '%d/%m/%Y')
						
						if data_inicio and data_final and data_final < data_hoje:
						    expiracao = format( data_inicio, '%d/%m/%Y')+' A '+format( data_final, '%d/%m/%Y')
						    expirada = True
		...
		return rTn, imp, expiracao,expirada
	...
	def numeroBanco( self, codigoBand ):
		...
		for i in login.pgBand:
			...
			if i !=None and i !="" and i.split("|")[0] == codigoBand.split("-")[0]:
				...
				if len( i.split("|") ) >=6:	numeroBancoBaixa = i.split("|")[5]
				break
		
		return numeroBancoBaixa
	...
	def vendasPisosCaixas(self, parametros, quantidade, p ):
		...
		if len( parametros ) >= 7 and parametros[6] and Decimal( parametros[6] ) and quantidade:
			...
			if int( numcax[1] ):
				...
				pedido = "Pedido: "+str( quantidade )+"M2 "+numcax[0] +'.'+numcax[1]+' Caixas\nNumero caixa '+numcax[0]+'X'+str( __mtcx )+ '=' + pd + 'M2'
				
				cs = ( int( numcax[0] ) + 1 )
				ms = format( ( cs * __mtcx ),'.4f' ) #--// Sergil 23-11-2018
				...
				if Decimal( pd ) or Decimal( ms ):
					...
					#if numcax[1] and int(numcax[1]):	p.caixa_sugerido.SetValue( True )
	...
	def recuperarXmlManual(self,xmlarquivo,status,geraPDF, arquivo_string):

		if arquivo_string:	doc=xml.parse(xmlarquivo)
		else:	doc=xml.parseString(xmlarquivo)
		_st=doc.toprettyxml()

		"""  Retira da consulta a parte com autoriazacao """
		documento=xml.parseString(status)
		strdocume=documento.toprettyxml()

		chave,b2 = geraPDF.XMLLeitura(documento,"infProt","chNFe") #-------: Nº da DANFE
		...
		motiv,b6 = geraPDF.XMLLeitura(documento,"infProt","xMotivo") #-----: Motivo
		...
		dados=chave,dtrec,proto,cst,motiv
		return xml_final, dados

	def autorizacaoRemota(self,informar='', autori='', autante='', cabeca='', tmpcmd='', modulo=0, filial=None, par=None, dados=''):

	    autorizacoes._inform = informar
	    autorizacoes._autori = autori #---: Relacao das autorizacoes
	    autorizacoes.auTAnTe = autante
	    autorizacoes._cabeca = cabeca #-: Dados do Recebimento
	    autorizacoes._Tmpcmd = tmpcmd #--: Numero da comanda temporario 
	    autorizacoes.moduloP = modulo
	    autorizacoes.dados_retorno=dados
					    
	    autorizacoes.filiala = filial
	    auto_frame = autorizacoes(parent=par,id=-1)
	    auto_frame.Centre()
	    auto_frame.Show()	
    
	def validacaoAlteracaoPreco(self,valores, data_inicial, data_final):
	    
	    """ Recebe String e transforma para data """
	    di=datetime.datetime.strptime(data_inicial, '%Y/%m/%d').date()
	    df=datetime.datetime.strptime(data_final, '%Y/%m/%d').date()
	    retorno = False
	    data_usuario_valores = ""
		...
		for i in valores.split('\n'):
			...
			p1 = x1[0].split("|")
			p2 = x1[1].split("|")
			pca1 = p1[0].split(';')[0].replace(",","")
			pca2 = p1[1].split(';')[0].replace(",","")
			pca3 = p1[2].split(';')[0].replace(",","")
			pca4 = p1[3].split(';')[0].replace(",","")
			pca5 = p1[4].split(';')[0].replace(",","")
			pca6 = p1[5].split(';')[0].replace(",","")

			paj1 = p2[4].split(';')[0].replace(",","")
			paj2 = p2[5].split(';')[0].replace(",","")
			paj3 = p2[6].split(';')[0].replace(",","")
			paj4 = p2[7].split(';')[0].replace(",","")
			paj5 = p2[8].split(';')[0].replace(",","")
			paj6 = p2[9].split(';')[0].replace(",","")
		
			
			usuario_alteracao=p2[3]
			data_alteracao=datetime.datetime.strptime(usuario_alteracao.split(' ')[0], '%d/%m/%Y').date()
			
			if data_alteracao >= di and data_alteracao <= df:
			    
			    pca1='' if not Decimal(pca1) else format(Decimal(pca1),',.3f')
			    pca2='' if not Decimal(pca2) else format(Decimal(pca2),',.3f')
			    pca3='' if not Decimal(pca3) else format(Decimal(pca3),',.3f')
			    pca4='' if not Decimal(pca4) else format(Decimal(pca4),',.3f')
			    pca5='' if not Decimal(pca5) else format(Decimal(pca5),',.3f')
			    pca6='' if not Decimal(pca6) else format(Decimal(pca6),',.3f')

			    paj1='' if not Decimal(paj1) else format(Decimal(paj1),',.3f')
			    paj2='' if not Decimal(paj2) else format(Decimal(paj2),',.3f')
			    paj3='' if not Decimal(paj3) else format(Decimal(paj3),',.3f')
			    paj4='' if not Decimal(paj4) else format(Decimal(paj4),',.3f')
			    paj5='' if not Decimal(paj5) else format(Decimal(paj5),',.3f')
			    paj6='' if not Decimal(paj6) else format(Decimal(paj6),',.3f')

			    passar = False
			    if pca1 and paj1 and Decimal(pca1.replace(',','')) != Decimal(paj1.replace(',','')):	passar = True
			    if pca2 and paj2 and Decimal(pca2.replace(',','')) != Decimal(paj2.replace(',','')):	passar = True
			    if pca3 and paj3 and Decimal(pca3.replace(',','')) != Decimal(paj3.replace(',','')):	passar = True
			    if pca4 and paj4 and Decimal(pca4.replace(',','')) != Decimal(paj4.replace(',','')):	passar = True
			    if pca5 and paj5 and Decimal(pca5.replace(',','')) != Decimal(paj5.replace(',','')):	passar = True
			    if pca6 and paj6 and Decimal(pca6.replace(',','')) != Decimal(paj6.replace(',','')):	passar = True
			    
			    if passar:
				data_usuario_valores=usuario_alteracao,(pca1,pca2,pca3,pca4,pca5,pca6),(paj1,paj2,paj3,paj4,paj5,paj6)
				retorno=True
			    break
			    
	    return retorno, data_usuario_valores
	
	def retornoCaixasEmbalagens(self,calculo, quantidade_metros, quantidade_entregue,dados, saldo_devolucao=Decimal()):

	    emb = dados.split('|')
	    memb = Decimal()
	    embalagens = Decimal()
	    if emb and len(emb)>=7 and emb[6] and Decimal(emb[6]):
		
		memb = Decimal(emb[6])
		#if calculo==1 and memb:	embalagens = Decimal( format( ((quantidade_metros - quantidade_entregue - saldo_devolucao) / memb ), '.2f') )
		if calculo==1 and memb:	embalagens = Decimal( format( ((quantidade_metros - quantidade_entregue) / memb ), '.2f') )
		if calculo==2 and memb:	embalagens = Decimal( format( (quantidade_metros * memb ), '.4f') ) 

	    return embalagens
	    
	def calcularDevolucoes(self, sql,  codigo, iddev, lista_devolucao):

	    quantidade_devolucao = Decimal()
	    devolucao_ocorrencias = 0
	    for ndev in lista_devolucao:

		if sql.execute("SELECT it_quan,it_dado FROM didavs WHERE it_ndav='"+ ndev[0] +"' and it_codi='"+ codigo +"' and it_dado like '"+iddev+"%'"):

		    for nqt in sql.fetchall():

			quantidade_devolucao +=nqt[0]
			devolucao_ocorrencias +=1

	    return quantidade_devolucao,devolucao_ocorrencias
...
class gerenciador(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.listaImpressoras.InsertColumn(7, 'Numero IP', width=100)
		self.listaImpressoras.InsertColumn(8, 'Porta', width=100)
		...
		enviar = wx.BitmapButton(self.painel, 221, wx.Bitmap("imagens/enviarp.png",  wx.BITMAP_TYPE_ANY), pos=(329,96), size=(35,33))
		self.whatsa = wx.BitmapButton(self.painel, 225, wx.Bitmap("imagens/whatsa20.png",  wx.BITMAP_TYPE_ANY), pos=(285,96), size=(35,33))
		...
		if self.WhatsApp:
		    ...
		    self.whatsa.Bind(wx.EVT_BUTTON,self.whatsAppEnviar)
		    if self.Filial and len( login.filialLT[ self.Filial ][35].split(";") ) >=153 and login.filialLT[ self.Filial ][35].split(";")[152] == 'T':	self.whatsa.Enable(True)
		    else:	self.whatsa.Enable(False)
		...
		if self.TIPORL in ['LITTUS1','LITTUS2']:	self.TexT.SetValue('Acesse o boleto atraves do link\n'+anexar_link)
	...
	def conectarPrinter( self, dominio ):
		...
		try:
		    ...
		    finalizar = False
		    if type(retorno) !=unicode:	retorno = str(retorno)
		del _mensagem

		if not finalizar:

			if dominio !="127.0.0.1":	self.retornoImpressaoLocal()
			self.alerta.dia( self,"{ Impressao: falha na conexão com o servidor remoto }\nIP-Dominio: "+ dominio +"\n\n"+ retorno  +"\n"+(" "*160),"Conexao remota de impressao")
	...
	def whatsAppEnviar(self,event):	

		self.WhatsApp.telefone = self.telefones
		self.WhatsApp.texto = 'Envio de documentos/compras' 
		self.WhatsApp.arquivo = str(self.Anexar)
		
    		xlg_frame=self.WhatsApp(parent=self,id=-1)
		xlg_frame.Centre()
		xlg_frame.Show()
	...
	def printing(self,event):
		...
		indice = self.listaImpressoras.GetFocusedItem()
		alias  = str(self.listaImpressoras.GetItem(indice,1).GetText())
		...
		if TermPr.strip() == "S":
			...
			if self.TIPORL=="EXTRATOCLIENTE":	arquivo=self.extrato_cliente
			else:
			    
			    if   modelo == "2" and TermMd.strip() == "1" and self.impressoras_termicas:	arquivo = self.impressoras_termicas['bematech1']
			    elif modelo == "2" and TermMd.strip() == "2" and self.impressoras_termicas:	arquivo = self.impressoras_termicas['bematech2'] 
			    elif modelo == "2" and TermMd.strip() == "3" and self.impressoras_termicas:	arquivo = self.impressoras_termicas['bematech3'] 
			    elif modelo == "1" and self.impressoras_termicas:	arquivo = self.impressoras_termicas['epsonTMT20'] 
			    elif modelo == "10":	arquivo = self.impressoras_termicas['epsondavs']
			    else:
				if TermMd and modelo and self.impressoras_termicas:	arquivo = self.impressoras_termicas['bematech1']
		...
		if self.TIPORL.split('-')[0]=="DAV" and len(self.TIPORL.split('-'))==2 and self.TIPORL.split('-')[1]=="D" and len(login.filialLT[self.Filial][35].split(";"))>=47 and len( login.filialLT[ self.Filial ][35].split(";")[46].split(',') ) >=2  and login.filialLT[ self.Filial ][35].split(";")[46].split(',')[0]:	ncopias = int(login.filialLT[ self.Filial ][35].split(";")[46].split(',')[1])
		if self.TIPORL.split('-')[0]=="DAV" and len(self.TIPORL.split('-'))==2 and self.TIPORL.split('-')[1]=="O" and len(login.filialLT[self.Filial][35].split(";"))>=47 and len( login.filialLT[ self.Filial ][35].split(";")[46].split(',') ) >=3  and login.filialLT[ self.Filial ][35].split(";")[46].split(',')[1]:	ncopias = int(login.filialLT[ self.Filial ][35].split(";")[46].split(',')[2])

		""" Se for reimpressao fazer apenas um copia """
		if login.reimpre:	ncopias = 1

		""" Modelo2 Impressao do dav com entrega programaca saindo a informacao para VIA ESTABELECIMENTO, VIA CLIENTE """
		if self.TIPORL.split('-')[0] == "DAV" and len( self.TIPORL.split('-') ) == 2 and self.TIPORL.split('-')[1] == "P" and len(login.filialLT[ self.Filial ][35].split(";"))>=147 and login.filialLT[ self.Filial ][35].split(";")[146]=='T':
		    __arq = open(arquivo).read()

		    if 'Via do Cliente' in __arq or 'Via do Estabelecimento' in __arq:
			ncopias=2
			self.via_impressao.SetValue(False) #--// Forcar emissao lpd
		...
		if self.via_impressao.GetValue():
			...
			if not impresso:
				...
				if self.TIPORL.split("-")[0] == "DAV":
					...
					self.parente.gravaImpressao(alias)
			...
			for nc in range( ncopias ):
			    ...
			    i2_via=arquivo.split('/')[-1].split('_')[1] if '_' in arquivo.split('/')[-1] else arquivo.split('/')[-1]
			    b2_via=arquivo[: len(arquivo)- len(arquivo.split('/')[-1])]
			    arq=b2_via+'2vias_'+i2_via
			    
			    if nc==1 and os.path.exists(arq):	arquivo=arq
		...
		if simm == True:
			...
			for i in impressoras:
				...
				if prinT == True:
					...
					self.listaImpressoras.SetStringItem(indice,7, i[11] )
					self.listaImpressoras.SetStringItem(indice,8, i[10] )
	...
	def enviaremial(self,to,subject,text,attach,secundario,parent,par, Filial = '' ):
		...
		if login.filialLT[ self.emFl ][29] == None or login.filialLT[  self.emFl ][29] == '':
			...
			if tipo_relatorio == "LITTUS" or tipo_relatorio == "LITTUS1":
				...
				if tipo_relatorio == "LITTUS1":	text = par.TexT.GetValue() #u"Boleto de cobrança"

			else:

			    sr,us,re,ps,pr = login.filialLT[ self.emFl ][29].split(';')
			    if tipo_relatorio=="LITTUS2":	text = par.TexT.GetValue() #u"Boleto de cobrança"
...
class truncagem:
	...
	def trunca(self,TipoRetorno,Numero):
		...
		if TipoRetorno == 1: #->[Truncagem com 3 casas decimais para quantidade]
			...
			_Formatando = format(Numero,'.5f') #-->Recebe Decimal e Formata retorna com <.>
			_vlrRetorno = Decimal( _Formatando[:( len(_Formatando) - 1)] ) #-->Trunca 
			#print('Lobo: ',Numero,_Formatando,_vlrRetorno)
	...
	def arredonda(self,TipoArredonda,NumeroValor):
		...
		if TipoArredonda ==5: #---->[ Arredonda a retorna em string]
			_vlrRetorno = format(NumeroValor,'.4f') #-->Recebe Decimal e Formata
...
class login:
	...
	"""
	nossoweb = ''
	if os.path.exists("srv/lykossrv.srv"):	nossoweb = open("srv/lykossrv.srv","r").read()+';'
	TempEmpr = {}
	
	
	simples_nacional=False
	simples_nacional_mensal=False
	segundaviaNfce = False
	...
	listaws  = {'1':u'PagHiper','2':u'Pagarme','3':u'IUGU','4':u'PJBank'}
	...
	padrscep = 0
	integracao = ["Integracao","1-Corte cloud","2-Woo Loja virtual"]

	remover_dav_romaneio = ['','01-Endereco nao localizado','02-Cliente nao estava o imovel para o recebimento',\
	'03-Cliente recusou a mercadoria','04-Caminha nao tinha espaco para carregar o dav','05-Ajuste da logistica']
	...
	prdMovim = [
		    ...
		    u'2-Ultimas vendas',
		    u'3-Ultimos acertos',
		    u'4-Estoque Negativo',
		    u'5-Sem Comissão',
		    u'6-Sem Referência',
		    u'7-Sem Fabricante',
		    u'8-Sem Endereço',
		    u'9-Sem Grupo',
		    u'10-Sem Sub-Grupo 1',
		    u'11-Sem Sub-Grupo 2',
		    u'12-Margens,Custos',
		    u'13-Filtrar Produtos Marcados',
		    u'14-Sem Código Fiscal',
		    u'15-Com Código Fiscal',
		    u'16-Produtos em Promoção',
		    u'17-Kits-Conjuntos',
		    u'18-Produto sem Vinculo',
		    u'19-Sem Endereço Imagem',
		    u'20-Sem código CEST',
		    u'21-Similares',
		    u'22-Preços separados p/filial',
		    u'23-Ultimas transferencias',
		    u'24-Pis/Cofins utilizar campo de pesquisa ex: digitar o PIS ou COFINS [01,02,03 etc...] ou [1000-para vazio]',
		    u'25-Fitrar produtos marcados para vender negativo',
		    u'26-Fitrar produtos marcados para não vender negativo',
		    u'27-Fitrar produtos com controle marcados',
		    u'28-Fitrar produtos com controle desmarcados'
		    ]
	...
	relaTDav = ['',u'01-Comissão sobre vendas',u'02-Comissão sobre recebido',u'03-Comissão sobre produtos',u'04-Consulta e reimpressão']
	...
	tipolancamento = [u'01-Deposito',u'02-Transferências',u'03-Pagamentos',u'04-Estorno de lançamentos',u'05-Outros lançamentos']
	rcontacorrente = ['00-Extraro da conta selecionada',u'01-Filtrar lançamentos por deposito',u'02-Filtrar lançamentos por transferências',u'03-Filtrar lançamentos por pagamentos',u'04-Filtrar lançamentos por estorno de lançamentos',u'05-Filtrar lançamentos por outros lançamentos',u'20-Filtrar lançamentos por liquidação do contas areceber',\
					  u'30-Filtrar lançamentos por baixa do contas apagar','90-Relacionar saldos das contas correntes']
	
	estados={"RJ":u"AC-Acre","AL":u"AL-Alagoas","AP":u"AP-Amapá","AM":u"AM-Amazonas","BA":u"BA-Bahia","CE":u"CE-Ceará","DF":u"DF-Distrito Federal","ES":u"ES-Espírito Santo","GO":u"GO-Goiás","MA":u"MA-Maranhão","MT":u"MT-Mato Grosso","MS":u"MS-Mato Grosso do Sul","MG":u"MG-Minas Gerais","PA":u"PA-Pará","PB":u"PB-Paraíba","PR":u"PR-Paraná","PE":u"PE-Pernambuco","PI":u"PI-Piauí","RJ":u"RJ-Rio de Janeiro","RN":u"RN-Rio Grande do Norte","RS":u"RS-Rio Grande do Sul","RO":u"RO-Rondônia","RR":u"RR-Roraima","SC":u"SC-Santa Catarina","SP":u"SP-São Paulo","SE":u"SE-Sergipe","TO":u"TO-Tocantins"}
	...
	corte_cloud_email="littus.clientes@gmail.com"
	corte_cloud_passw="dj38a6fn34218cssh42"
	corte_cloud_key_client=''
	corte_cloud_key_partner=b64encode(corte_cloud_email+':'+corte_cloud_passw)
	corte_cloud_url="https://apis.cortecloud.com.br/"
	...
	lista_zebra = ['',
	u'01 - Zebra 107x25 1 carreira',
	u'02 - Zebra 40x20  2 carreiras',
	u'03 - Zebra 34x23  3 carreiras',
	u'04 - Zebra 70x30  1 carreira {Opção p/CNPJ}',
	u'05 - Zebra 100x48 1 carreira perfil de estoque',
	u'06 - Zebra 107x65 1 carreira endereçamento de estoque',
	u'07 - Zebra 80x50  1 carreira endereçamento de estoque',
	u'08 - Zebra 50x30  2 carreira precos, descrição e codigo          ',
	u'09 - Zebra 100x50 1 carreira preços,descrição e codigo',
	u'10 - Zebra 100x67 1 carreira preços,descrição e codigo',
	u'11 - Zebra 80x43  1 carreira preços,descrição e codigo gondola',
	u'99 - Resetar e calibrar a impressora'
	]
	
	#oldservers { fica no cadfretes.py linha +- 3211
	...
	def validadeCertificado(self,certificado,senha):
		...
		if encontra == True:
			...
			try:
				...
				__c = open(certificado, 'rb').read()
				arq = diretorios.usPasta+"certificado"+login.usalogin.lower()+".pem"
				p12 = crypto.load_pkcs12(__c,senha)
...
class formasPagamentos:
	...
	def limparQrcode(self, qrcode):
		return qrcode.replace('<![CDATA[','').replace(']]>','')
		
	def saldoCC( self, _sql, _docC ):

		#_soma = _sql.execute("SELECT SUM(cc_credit),SUM(cc_debito) FROM conta WHERE cc_docume='"+_docC+"'")
		_soma = _sql.execute("SELECT SUM(cc_credit),SUM(cc_debito) FROM conta WHERE cc_docume='"+_docC+"' ORDER BY cc_regist DESC limit 1")
		_slcc = _sql.fetchall()
	...
	def saldoRC(self,_sql,_docC,_dAT, _fl ):
		...
		for i in _slcc:
			...
			if i[0][:2] in ['02','03','06','07','09','11','12']:
	...
	def listaprn(self,opcao):
		...
		if os.path.exists("srv/impressoras.cmd") == True:
			...
			for i in __arquivo.readlines():
				...
				if opcao == 3 and len( campo ) >= 4:	relacao[campo[1]] = campo[2]
				...
				if opcao == 1 and len( campo ) >=13:
					relacao[_registros]=campo[0],campo[1],campo[2],campo[3],campo[4],campo[5],campo[6],campo[7],campo[8],campo[9],campo[10],campo[11],campo[12]
					printers[campo[0]]=campo[1],campo[2],campo[3],campo[4],campo[5],campo[6],campo[7],campo[8],campo[9],campo[10],campo[11],campo[12]
					if login.impparao==campo[1]:	printer_default = campo[0],campo[1],campo[2],campo[3],campo[4],campo[5],campo[6],campo[7],campo[8],campo[9],campo[10],campo[11],campo[12]

					if len( campo ) >=14:
					    relacao[_registros]=campo[0],campo[1],campo[2],campo[3],campo[4],campo[5],campo[6],campo[7],campo[8],campo[9],campo[10],campo[11],campo[12],campo[13]
					    printers[campo[0]]=campo[1],campo[2],campo[3],campo[4],campo[5],campo[6],campo[7],campo[8],campo[9],campo[10],campo[11],campo[12],campo[13]

					if len( campo ) >=15:
					    relacao[_registros]=campo[0],campo[1],campo[2],campo[3],campo[4],campo[5],campo[6],campo[7],campo[8],campo[9],campo[10],campo[11],campo[12],campo[13],campo[14]
					    printers[campo[0]]=campo[1],campo[2],campo[3],campo[4],campo[5],campo[6],campo[7],campo[8],campo[9],campo[10],campo[11],campo[12],campo[13],campo[14]
		...
		if opcao ==1:	return reTorno,relacao.values(), printer_default, printers
		if opcao in [2,3]:	return reTorno,relacao, printer_default, printers
	...
	def fpg( self, futuros, TP, rclocal=False, filial=login.identifi ):
		...
		if TP == 1: #-: Adiciona Pagamentos do Cliente aos Pagamentos Avista sem Ressalva
			...
			if rclocal and len( login.filialLT[filial][35].split(";") ) >= 160 and login.filialLT[filial][35].split(";")[159]=="T":	formas = self.validaRcLocalfpg( formas, rclocal )
		...
		elif TP == 6 and login.pgForm !='': #-:Retorna o texto da forma de pagamento
			rTe = ''
			for plb in login.pgForm:
			    
			    if plb[0]=='P' and plb[1].split('|')[0].split('-')[0]=='09':	rTe = plb[1].split('|')[0]
			
			return( rTe )

	def validaRcLocalfpg( self, formas, rcl ):
...
class dialogos:
	...
	def ValoresEstaticos(self, secao='', objeto = '', valor ='', lergrava ='r' ):
		...
		if secao:
		    ...
		    if not os.path.exists( diretorios.srvconf +'valores.ini'):
			    
			_file = open(diretorios.srvconf +'valores.ini','w')
			_file.close()

		    if os.path.exists(diretorios.srvconf +'valores.ini'):
	...
	def deOlhonoImposto(self, uf = "", lista_ncms = [] ):
		...
		if i and i != anterior:
		    ...
		    requisicao = requests.get( playload, timeout=5 )
...
class TelNumeric(wx.Frame):
	...
	def atualizar(self,event):
		...
		if self.segundop:
			...
			if self.mdfe and self.identificar in [63,64,65]:	self.mdfe.Tvalores( self.saidaNumero.GetValue(),self.identificar )
	...
	def TeclarNumeros(self,event):
		...
		try:
			...
			elif self.decimais == 6 and self.NumerosTeclados != '':

				formatado = format(Decimal(self.NumerosTeclados),'.5f')
				formatado = format(Decimal(formatado),',')
...
class srateio(wx.Frame):
	...
	def Tvalores(self,valor,idfy):
		...
		try:	self.vR.SetValue(valor)
		except Exception as erro:	self.vR.SetValue(valor)
		self.registrar(wx.EVT_KEY_UP)
		...
		if sql[0] == True:
			...
			if rT !=0:
				...
				for i in lst:
					...
					if i[30] == "C":	Orcamentos = "CONSIGNACAO"
			...
			if regDav == 0 and emis == login.usalogin.upper() and Tipo !='DEVOLUCAO' and Tipo !='ENTREGA FUTURA':
				...
				if sql[0] == True:
					...
					if ITems !=0 and passar == True:
						...
						if Tipo == "CONSIGNACAO":	self.p.TComa5.SetValue(True)
						for i in _result:
							...
							if esFT !=0:
								...
								per_desconto = _perdescon, vlr_desconto = _valdescon, vinculado = "", estoque_local = "0", produto_servico='' )
...
class adNFe(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		if self._id==230:	__dados = self.par.parente.dadosObservacao
		else:	__dados = self.par.dadosA
		...
		self.dados = wx.TextCtrl(self.painel,-1,'',pos=(0,0), size=(296,80), style=wx.TE_MULTILINE|wx.TE_DONTWRAP)
		...
		self.dados.SetValue(__dados)
		self.dados.SetFocus()

	def voltar(self,event):

		if self._id == 230:	self.par.parente.dadosObservacao=self.dados.GetValue()
		else:	self.par.dadosA = self.dados.GetValue()
		self.par.Enable()
		self.Destroy()
...
class AbrirArquivos(wx.Frame):
	...
	def __init__(self, parent, id):
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			elif id == 912:	self.p.importarCnab()
...
class ultimas:
	...
	def cva(self, sql, Tipo, codigo, _dav, _cl, _qt, _pc, _sb, _cu, _ipi, _stb, _pis, _cof, _qte, _ctr, _fan, _es, _mg, _fl,_rp ):
		...
		if reTo !=0:
			...
			if Tipo == "VD":	_vendas.append(_cl+'|'+str(_dav)+'|'+str(UVD)+'|'+str(DHO)+'|'+str(login.usalogin)+'|'+str(_qt)+'|'+str(_pc)+'|'+str(_sb)+'|'+str( _fan )+'|'+_fl+'\n')
			if Tipo == "CM":

				_vendas.append(_cl+'|'+str(_dav)+'|'+str(UVD)+'|'+str(DHO)+'|'+str(login.usalogin)+'|'+str(_qt)+'|'+str(_pc)+'|'+str(_sb)+'|'+str(_cu)+'|'+str(_ipi)+'|'+str(_stb)+'|'+str(_pis)+'|'+str(_cof)+'|'+str(_qte)+'|'+str(_ctr)+'|'+str( _fan )+'|'+str(_rp)+'|'+str(_fl)+'\n')
				if _mg !='':	_margem.append( _mg )
				
			if Tipo == "AC":	_vendas.append(_cl+'|'+str(_dav)+'|'+str(UVD)+'|'+str(DHO)+'|'+str(login.usalogin)+'|'+str(_qt)+'|'+str(_pc)+'|'+str(_sb)+'|'+str(_cu)+'|'+str(_ipi)+'|'+str(_stb)+'|'+str(_pis)+'|'+str(_cof)+'|'+str(_qte)+'|'+str(_ctr)+'|'+str(_fan)+'|'+str(_es)+'|'+str(_fl)+'\n')
			if Tipo == "TR":	_vendas.append(_cl+'|'+str(_dav)+'|'+str(UVD)+'|'+str(DHO)+'|'+str(login.usalogin)+'|'+str(_qt)+'|'+str(_pc)+'|'+str(_sb)+'|'+str(_cu)+'|'+str(_ipi)+'|'+str(_stb)+'|'+str(_pis)+'|'+str(_cof)+'|'+str(_qte)+'|'+str(_ctr)+'|'+str(_fan)+'|'+str(_es)+'|'+str(_fl)+'\n')
			if Tipo == "DV":	_vendas.append(_cl+'|'+str(_dav)+'|'+str(UVD)+'|'+str(DHO)+'|'+str(login.usalogin)+'|'+str(_qt)+'|'+str(_pc)+'|'+str(_sb)+'|'+str(_cu)+'|'+str(_ipi)+'|'+str(_stb)+'|'+str(_pis)+'|'+str(_cof)+'|'+str(_qte)+'|'+str(_ctr)+'|'+str(_fan)+'|'+str(_es)+'|'+str(_fl)+'\n')
...
class autorizacoes(wx.Frame):
	...
	def finalRemoto(self,event):
		...
		if not self.envio_autorizacao:
		    ...
		    agua_frame.Centre()
		    agua_frame.Show()
		    self.envio_autorizacao +=1
		    self.fimrem.Enable(False)
		    
		else:
		    self.a.dia(self,'{ Pedido temporario ja enviado }\n'+(' '*120),'Autorizacao')
	...
	def AutorizadoFinal(self):
		...
		elif self.moduloP == 24:	self.p.emissaoNFCeAutorizacaoRemota(self.dados_retorno,self.loginuser.GetValue())
...
class liberaRemoto(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.voltar = wx.BitmapButton(self.painel, -1,  wx.Bitmap("imagens/voltam.png", wx.BITMAP_TYPE_ANY), pos=(11, 211), size=(37,36))				
	...
	def selecionar(self,id):
		...
		if sql[0] == True:
			...
			if id == 400:
			    usuario_logado=self.loginuser.GetValue()
			    achei = sql[2].execute("SELECT * FROM auremoto WHERE au_solius='"+ usuario_logado +"' and au_dtpedi='"+ hoje +"' and au_uslibi='' ORDER BY au_dtpedi,au_hrpedi")
	...
	def liberacao(self,event):
		...
		if self.ListaRemoto.GetItemCount() != 0:
			...
			self.voltar.Enable(False)
			self.minhas.Enable(False)
			self.ltodas.Enable(False)
			self.descar.Enable(False)
			self.autori.Enable(False)
			...
			self.voltar.Enable(True)
			self.minhas.Enable(True)
			self.ltodas.Enable(True)
			self.descar.Enable(True)
			self.autori.Enable(True)
...
class CodigoMunicipio(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		if self.estado:
			self.estados.SetValue('Estado:'+self.estado)
			self.selecionar(wx.EVT_BUTTON)
	...
	def enviarCodigo(self,event):
		...
		if self.i in [950,951]:	self.parente.MunicipioCodigo( self.ListaMunicipio.GetItem(indice, 1).GetText(), self.ListaMunicipio.GetItem(indice, 2).GetText(), self.i)
		else:	self.p.MunicipioCodigo(self.ListaMunicipio.GetItem(indice, 1).GetText(),self.i)
...
class MostrarHistorico(wx.Frame):	
	...
	def __init__(self,parent,id):
		...
		if self.TP.upper()=='LIQUIDACAO':	linhas=430
		wx.Frame.__init__(self,parent,id,"Ajuda, Historico",size=(900,linhas),style=wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX|wx.CAPTION)
		...
		self.hsT.SetFont(wx.Font(self.TM, wx.MODERN, wx.NORMAL, wx.BOLD))
		self.hsT.SetBackgroundColour("#4D4D4D")
		self.hsT.SetForegroundColour('#90EE90')
		self.hsT.SetValue( self.hs )

		if self.TP.upper()!="LIQUIDACAO" and self.AQ !='':	self.hsT.SetValue("Envio do Arquivo: "+self.AQ+"\n\n"+self.hs)

		voltar=wx.BitmapButton(self.painel, -1, wx.Bitmap("imagens/volta16.png", wx.BITMAP_TYPE_ANY), pos=(0, 2), size=(30,28))
		if self.TP.upper()!="LIQUIDACAO":
			
			if self.TP.upper()=="ETQ":	self.email=wx.BitmapButton(self.painel, -1, wx.Bitmap("imagens/printed.png", wx.BITMAP_TYPE_ANY), pos=(0,40), size=(30,28))
			else:	self.email=wx.BitmapButton(self.painel, -1, wx.Bitmap("imagens/emailp.png", wx.BITMAP_TYPE_ANY), pos=(0,40), size=(30,28))

		elif self.TP.upper()=="LIQUIDACAO":

			wx.StaticText(self.painel,-1, u"Liquidação para controle de cheques individualizados\nContas apagar e cheques do contas areceber",pos=(5,392)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))

			self.email=GenBitmapTextButton(self.painel,100,label='Click para liquidação dos cheques relacionados      ', pos=(540,392),size=(350,30), bitmap=wx.Bitmap("imagens/devolver.png", wx.BITMAP_TYPE_ANY))
			self.email.SetBackgroundColour('#D5ECD5')
		...
		if self.TP.upper() in ["XML","ETQ","LIQUIDACAO"]:	self.email.Enable( True )
		
		self.email.Bind(wx.EVT_BUTTON, self.enviarEmail)
	...
	def enviarEmail(self,event):
		...
		confima = wx.MessageDialog(self.painel,u"{ Liquidação do(s) cheque(s) selecionado(s) }\n\nConfirme p/Continuar\n"+(" "*140),u"Contas apagar: Liquidação do cheque selecionado",wx.YES_NO|wx.NO_DEFAULT)
		if confima.ShowModal() ==  wx.ID_YES:
		
		    if self.p.liquidacaoCheques(self.FL,self.GD):

			alertas=dialogos()
			alertas.dia( self, "Cheques liquidados com sucesso...\n"+(" "*140),"Liquidacao de cheques")
	...
	def desenho(self,event):
		...
		t=self.TT if type(self.TT)==unicode else self.TT.decode('UTF-8')
		dc = wx.PaintDC(self.painel)
		dc.SetTextForeground("#1C581C") 	
		dc.SetFont(wx.Font(10, wx.MODERN, wx.NORMAL,wx.BOLD,False,"Arial"))
		dc.DrawRotatedText(t, 0, 390, 90)
...
class diretorios:
	...
	busca_preco = os.getcwd()+"/" #---: Busca preco gertec
	...
	esMdfe   = os.getcwd()+"/" #---: MDFE
	corteCloude=os.getcwd()+"/" #--: Corte cloud
	dicones = d = os.getcwd()+"/" #---: Icones
	pasHome  = usFolder
	xsd = os.getcwd()+"/eletronicos/xsd/"
	...
	if os.path.exists("/home/"+usPrinci+"/direto/buscapreco") == False:	os.makedirs("/home/"+usPrinci+"/direto/buscapreco")
	if os.path.exists("/home/"+usPrinci+"/direto/buscapreco"):	busca_preco = "/home/"+usPrinci+"/direto/buscapreco/"
	...
	if os.path.exists("/home/"+usPrinci+"/direto/mdfe300")==False:	os.makedirs("/home/"+usPrinci+"/direto/mdfe300")
	if os.path.exists("/home/"+usPrinci+"/direto/mdfe300"):	esMdfe="/home/"+usPrinci+"/direto/mdfe300/"

	if os.path.exists("/home/"+usPrinci+"/direto/cortecloud")==False:	os.makedirs("/home/"+usPrinci+"/direto/cortecloud")
	if os.path.exists("/home/"+usPrinci+"/direto/cortecloud"):	corteCloude="/home/"+usPrinci+"/direto/cortecloud/"
	...
	"csv":"'%scsv.png'"%(d),"sim5":"'%sentrada_mercadoria.png'"%(d),"sms":"'%ssms20.png'"%(d),"save":"'%ssave.png'"%(d),"tree":"'%stree.png'"%(d),\
	"trans":"'%strans.png'"%(d),"trans1":"'%strans1.png'"%(d),"menu":"'%smenu.png'"%(d),"emdfe":"'%sdelivery_truck_return.png'"%(d),"ida":"'%sdelivery_ida.png'"%(d),\
	"processo":"'%stime-left.png'"%(d),"cheque1":"'%scheck.png'"%(d),"cheque2":"'%scheck1.png'"%(d),"cheque3":"'%scheck3.png'"%(d),"cheque4":"'%scheck4.png'"%(d),\
	"i_lob":"'%scontingencia.png'"%(d)}
