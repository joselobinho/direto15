...
#		self.certificado()
#
#		simm, impressoras, user = formas.listaprn( 1 )
#		
...
#		if   self.moduloenvio == 1:	incompativel, dados_cartao, self.forma_pagamentos = meia_nota_fical.calcularVinculado( self.moduloenvio, self.meian.GetValue(), self.p.listaPagamento, self.mnd, self.resul_dav, self.efilial )
#		elif self.moduloenvio == 2:	incompativel, dados_cartao, self.forma_pagamentos = meia_nota_fical.calcularVinculado( self.moduloenvio, self.meian.GetValue(), '', self.mnd, self.resul_dav, self.efilial )
#
#		if dados_cartao:
#
...
#				if not self.resul_clientes and len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True )
#				if self.resul_clientes:
#
#					self.falta_dados = ""
#					if not self.resul_clientes[0][1]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:Nome
#						self.falta_dados +="Nome do cliente\n"
#						
#					if not self.resul_clientes[0][8]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:Endereco
#						self.falta_dados +="Endereço\n"
#	
#					if not self.resul_clientes[0][13]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:complemento1
#						self.falta_dados +="Complemento, numero\n"
#
#					if not self.resul_clientes[0][9]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:Bairro
#						self.falta_dados +="Bairro\n"
#
#					if not self.resul_clientes[0][11]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:IBGE
#						self.falta_dados +="Codigo do municipio\n"
#
#					if not self.resul_clientes[0][10]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:Cidade
#						self.falta_dados +="Municipio\n"
#
#					if not self.resul_clientes[0][15]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:UF
#						self.falta_dados +="UF\n"
#
#					if not self.resul_clientes[0][12]:
#						if len( login.filialLT[self.efilial][35].split(';') ) >= 118 and login.filialLT[self.efilial][35].split(';')[117]=='T':	self.impressao_consumidor.SetValue( True ) #-:CEP
...
class CalcularMeiaNota:
	...
	def calcularVinculado(self, moduloenvio, meian, listaPagamento, mnd, resul_dav, filial ):
		...
		if meian:
			...
			if moduloenvio == 2:
				...
				if len( login.filialLT[filial][35].split(';') ) >= 126 and login.filialLT[filial][35].split(';')[125] == "T":	valor_pagamentos_cartao +=resul_dav[0][61]
				if mnd and valor_pagamentos_cartao > mnd[0][37]:	incompativel = True
					...
					dados_cartao = u'valor ['+format( mnd[0][37],',')+u"]  Dinheiro ["+format( valor_pagamentos_saldos,',' ) +u"]  Cartão/boleto ["+format( valor_pagamentos_cartao,',')+"]"
					#self.mnfe.SetForegroundColour("#256FB7")
			...
			#---------//Emissao com recebimento
				...
				for pgC in range( listaPagamento.GetItemCount() ):
					...
					formas = listaPagamento.GetItem( pgC, 2 ).GetText().split('-')[0]
					passar = True if  formas in ["04","05"] else False
					if len( login.filialLT[filial][35].split(';') ) >= 126 and login.filialLT[filial][35].split(';')[125] == "T" and formas=='06':	passar = True
	...
	def listaParaPagamento( self, moduloenvio, meian, r, forma_pagamentos, resul_dav ):
		...
		if moduloenvio == 2 and not meian and resul_dav[0][107]:
