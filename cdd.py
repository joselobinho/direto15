...
for i in arq:
	...
	documento_xml = "" 
	item = "0" 
	#valores = Decimal("2070000.00")
	valores = Decimal("210000.00")
...
for f in arq_final:
	...
	for i in ccd:
		...
		if cfo[ind] == mudar:
			#print( cfo[ind], vTp[ ind ] )
			total_final +=Decimal( vTp[ ind ] )
			#print( cfo[ind], vTp[ ind ], total_final )
			#print( cfo, vTp, f )
			#time.sleep(5)
		if cfo[ind] == _cfop:
			#print( cfo[ind], _cfop )
			total_proposto +=Decimal( vTp[ ind ] )
