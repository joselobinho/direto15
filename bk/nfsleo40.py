...
from pynfe.entidades.evento import EventoCancelarNota, EventoManifestacaoDest
...
#---{ Emissao NF }--------------------------// Confeccao da NF
	def nfe40( self, parent, tipo_emissao = 1 ):
		...
		if rtn:
			...
			"""  Incluir cliente  """
			dados_clientes, inf_clientes_gravacao, pessoa_juridica_fisica, uf_cli = self.informacoesCliente( tipo_emissao, amb, '55', parent )
			...
			dados_identificacao, numero_nota, __serie, __origem, __nfe_nfce, estado_destino, uf_ide = self.indentificacaoNotaFiscal( dados_emitente, dados_clientes, dados_transportadora, tipo_emissao, self.p.cTClie[0], amb, pessoa_juridica_fisica )
	...
	def adicionandoProdutos(self, nota_fiscal, regime_tributacao, pessoa, estado_destino ):
		...
		for i in range( self.p.editdanfe.GetItemCount() ):
			...
			nota_fiscal.adicionar_produto_servico(
				...
				ean = self.p.editdanfe.GetItem(i,2).GetText().strip(),
				ean_tributavel = self.p.editdanfe.GetItem(i,2).GetText().strip(),
...
#---{ Emissao NF }--------------------------// Identificacao da Nota Fiscal
	def indentificacaoNotaFiscal(self, informar_dados_emitente, informar_dados_clientes, informar_dados_transportadora, emissao, dados_cliente, ambiente, pessoa ):
		...
		ide_noperacao=self.p.natureza.GetValue()[:60].decode('UTF-8')
	...
	def relacionarDuplicatas( self, emissao ):
		...
		if emissao !=3 and self.p.listaQuan == '' and contas_areceber and ( len( contas_areceber.split('|') ) - 1 ) > 0:
			...
			for dpl in range( self.p.listaQuan ):
				...
				imprimir = False
				if formapagamento[:2] == "06":	imprimir = True
				if formapagamento[:2] == "07" and len( login.filialLT[ self.filialide ][35].split(";") ) >=37 and login.filialLT[ self.filialide ][35].split(";")[36] == "T":	imprimir = True	
...
#---{ Emissao NF }--------------------------// emitente
	def informacoesEmitente(self, filial, emissao ):
		...
		emi_razao = login.filialLT[ filial ][1].decode('UTF-8')
		emi_fantasia = login.filialLT[ filial ][14].decode('UTF-8')
		...
		emi_logradouro = login.filialLT[ filial ][2].decode('UTF-8')
		emi_numero = login.filialLT[ filial ][7].decode('UTF-8')
		emi_complemento = login.filialLT[ filial ][8].decode('UTF-8')
		emi_bairro = login.filialLT[ filial ][3].decode('UTF-8')
		emi_municipio = login.filialLT[ filial ][4].decode('UTF-8')
		emi_cmunicipio = login.filialLT[ filial ][13].decode('UTF-8')
		emi_uf = login.filialLT[ filial ][6].decode('UTF-8')
		emi_cep = login.filialLT[ filial ][5].decode('UTF-8')
...
#---{ Emissao NF }--------------------------// Adicionando o cliente
	def informacoesCliente(self, emissao, ambiente, modelo_nf, parent ):
		...
		#if modelo_nf == '55':	cli_consumidor_final = int( self.p.indconfina.GetValue().split('-')[0] )
		...
		if emissao == 3: #-// Devolucao de compra RMA
			...
			if enderecamento:
				...
				#if emissao in [1,2] and len( self.p.destinat.GetValue().split('-')[0] ) == 3 and cli_dados[51] and modelo_nf == '55':
		...
		if modelo_nf == '65' and not enderecamento:
			...
			#print('---------------2: ',cli_endereco, cli_numero, cli_complemento, cli_bairro, cli_municipio, cli_cdmunicipio, cli_uf, cli_cep)
		...
		return cliente, self.informacoes_clientes_gravacao, cli_pessoa, unidade_ferederativa
	...
	def retornarEnderecoEntregaLocal(self, cli_dados, opcao, emissao, cli_endere, modelo_nf ):
		...
		print( opcao, cli_endere, emissao )
...
#---{ Emissao NF }--------------------------// Salvando informacoes de emissao, inutitilizacao, cancelamento e rejeicao nas tabelas de controles
	...
	def retornarPagamentos(self, origem_emissao = '160', lista1_pagamentos = '' ):
		...
		valores_parcelados = Decimal('0.00')
		for i in self.forma_pagamento:

			"""
				Relaciona todas as formas de pagamentos do direto e envia para totalizar
			"""
			
			if origem_emissao == 'MNF':	self.totalizaFormasPagamentos( i, lista1_pagamentos, origem_emissao )
			if origem_emissao == 'POS':	self.totalizaFormasPagamentos( i, lista1_pagamentos[107].split('|'), origem_emissao )
			if origem_emissao == '160':	self.totalizaFormasPagamentos( i, lista1_pagamentos, origem_emissao )

		return self.listagem_pagamentos

	def totalizaFormasPagamentos( self, forma, lista, origem_emissao ):

		"""
			Totaliza as formas de pagamentos
		"""
		if   forma in ['08','11','12']:	forma_retorno, lista_somar = '08', ['08','11','12']
		elif forma in ['02','03']:	forma_retorno, lista_somar = '02',['02','03']
		else:	forma_retorno, lista_somar = forma, [forma]
		valor_parcelas = Decimal('0.00')
		
		if forma_retorno not in self.listagem_pagamentos:
			...
			if origem_emissao in ['POS','MNF']:
				for fp in lista:
					...
					if fp:
						...
						fpg, valor = fp.split(';')
						if origem_emissao == 'MFN':	fpg, valor = fp.split('-')[0], fp.split(';')[1]

						if fpg in lista_somar:	valor_parcelas += Decimal( valor )

			elif origem_emissao == '160':

				for i in range( lista.GetItemCount() ):

					fpg = lista.GetItem( i, 2 ).GetText()[:2]
					valor_recebido = lista.GetItem( i, 3 ).GetText().replace(",","")
					valor = lista.GetItem( i, 4 ).GetText().replace(",","")

					if fpg in lista_somar:	valor_parcelas += Decimal( valor )
			
			if valor_parcelas and forma_retorno:

				indPag = '1' if forma in ['06','07'] else '0'
				if forma in ['04','05']:	self.listagem_pagamentos[ self.forma_pagamento[forma_retorno] ] = ( str( valor_parcelas ), '2', indPag )
				else:	self.listagem_pagamentos[ self.forma_pagamento[forma_retorno] ] = ( str( valor_parcelas ),'', indPag )
...
class EmissaNfce400:
	...
	def emitirNfce400( self, parent, numero_dav, modulo_emissao ):
		...
		dados_cliente, informacoes_clientes_gravacao, pessoa_juridica_fisica, unidade_federativa = emissaoNfe400.informacoesCliente( 1, amb, '65', parent )
