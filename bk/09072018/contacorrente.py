...
class ControlerConta(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, '{ Conta corrente controler }', size=(950,475), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		wx.StaticText(self.painel,-1, u"Histórico de lançamento", pos=(32, 387)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		wx.StaticText(self.painel,-1, u"Selecione { Tipo de Lançamento }",   pos=(203,350)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.tipo_lancameto = wx.ComboBox(self.painel, -1, '', pos=(203,363), size=(278, 27),  choices = [""] + login.tipolancamento )
		self.historico_curto = wx.ComboBox(self.painel, -1, '', pos=(30,400), size=(410, 27),  choices = [""])
		self.fornecedores_bancos_transferencia = wx.ComboBox(self.painel, -1, '', pos=(30, 445), size=(410, 27),  choices = [""])
		...
		self.historico_comprido = wx.BitmapButton(self.painel, 222, wx.Bitmap("imagens/edit.png", wx.BITMAP_TYPE_ANY), pos=(445,400), size=(36,26))
		self.lanca_transferencia = wx.BitmapButton(self.painel, 225, wx.Bitmap("imagens/modulo.png", wx.BITMAP_TYPE_ANY), pos=(445,445), size=(36,26))
	...
	def transferenciaEntreContas(self, event):
		...
		if self.fornecedores_bancos.GetValue() and self.fornecedores_bancos_transferencia.GetValue():
			...
			voltar, valor, __hsc, __hsl, __lan, __doc, __plc, __bnc, lanca, tipo = self.retornaDados()
	...
	def retornaDados(self):
		...
		voltar = True if valor and __hsc and __doc and __bnc and tipo else False
		...
		return voltar, valor, __hsc, __hsl, __lan, __doc, __plc, __bnc, lanca, tipo 
	...
	def gravarLancamentos(self,event):
		...
		if voltar:
			...
			if grvl.ShowModal() ==  wx.ID_YES:
				...
				dados = str( valor ) +'|'+ __hsc +'|'+ __hsl +'|'+ __lan +'|'+ __doc +'|'+ __plc +'|'+ __bnc +'|'+ lanca +'|'+ self.modulo+'|'+tipo
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(490, 305, 1, 140, 1)
		...
		if not lancamento:	alertas.dia( self.p.painel,u"{ Tipo de lançamento não definido }\n\n1-Avise ao desenvolvimento, sobre essa ocorrência\n"+(" "*160),u"Lançamento em conta corrente")
			...
			if sql[0]:
				...
				try:
					...
					__g = "INSERT INTO bancoconta ( bc_docume, bc_dlanca, bc_hlanca, bc_idcada, bc_planoc, bc_valora, bc_valorc, bc_valord, bc_valors, bc_histor, bc_hiscur, bc_usuari, bc_origem, bc_tipola )\
					VALUES( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )"
							
					sql[2].execute( __g, ( __doc, data_emissao, hora_emissao, __bnc, __plc, str( saldo_anterior ), str( valor_credito ), str( valor_debito ), str( saldo_atual ), __hsl, __hsc, login.usalogin, modul, tipol ) )
