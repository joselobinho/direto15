...
#				if not Decimal( percentual_pFCP ) and len( self.icmsEsTado.split(' ') ) >= 4:	percentual_pFCP = login.aliquotaFcp[ self.icmsEsTado.split(' ')[3] ]
...
#---------: Baixa do Pedido no Estoque de Reserva-Virutal			
	...
	def fcpIcmsSt(self, rTos, fl, __pFCP ):
		...
		if rTos[1][2] and int( rTos[1][2] ) == 60 and self.clD.GetLabel() and len( self.clD.GetLabel().split(' ')[0] ) == 11 and len( self.icmsEsTado.split(' ') ) >= 3 and self.icmsEsTado.split(' ')[2] and regime_tributario == '3':
...
#				self.rcvVL.SetValue( format(str( sLan ) )
	...
	def Teclas(self,event):
		...
		if self.ListaCli.GetItem( self.ListaCli.GetFocusedItem(), 6 ).GetText() and self.ListaCli.GetItem( self.ListaCli.GetFocusedItem(), 6 ).GetText() != login.filialLT[self.fcFilial][30].split(';')[12]:
			...
			if len( self.ListaCli.GetItem( self.ListaCli.GetFocusedItem(), 1 ).GetText() ) == 11 and regime_tributario == '3':	self.calculcfp.Enable( True )
