...
#---{ Eventos }-------------------------------------------------// Status do sevidor	
	def status( self, parent, dados ):
		...
		rtn, snh, cer, sis, sie, amb, uf, fus, __cnpj, __nfce, regime_tributario = informes.certificados( dados[0], parent, nf_modelo=dados[1] )
...
#---{ Eventos }-------------------------------------------------// Inutilizacao da NF	
	def inutilizarNfe(self, parent, filial, ambiente, nota_fiscal, motivo, nfe_nfce, dav, id_lancamento, tipo_pedido, serie ):
		...
		rtn, snh, cer, sis, sie, amb, uf, fus, __cnpj, __nfce, regime_tributario = informes.certificados( filial, parent, nf_modelo=nfe_nfce )
		...
		if not erro:
			...
			diretorio_gravacao = diretorios.usFolder+'nfe400/' if nfe_nfce == 55 else diretorios.nfceacb+'nfce400/'
			__f = filial.lower()
			...
			if rt[4] in ['102','563']:
				...
				parent.NFEinuTiliza( rT = ( rt[5], rt[6], retorno, motivo, dav, id_lancamento, tipo_pedido, filial, xml_retorno ) )

				__arquivo = open( __d  +'/inutilizacao_autorizado.xml', "w" )
				__arquivo.write( xml_retorno )
				__arquivo.close()
				
			if rt[0]:
				
				_arquivo = open( __d + "/inutilizacao_nao_autorizado.xml", "w" )
				_arquivo.write( xml_retorno )
				_arquivo.close()
...
#---{ Eventos }-------------------------------------------------// Cancelamento da NF
	def cancelamentoNfe(self, parent, filial='', xml='', motivo='', origem = '', retorno_html = '' ):
		...
		if rt[0] and rt[4] in ['135','573']:
			...
			parent.cannfe( rt[4], rt[3], rt[5], motivo, filial, xml_retorno )
	...
	def downloadNfe(self, parent, filial, chave ):
		...
		mod = 'nfe'
		modelo = 1
		#uf = 'SC' #--// Para pesquisa no ambiente nacional
		
		con = ComunicacaoSefaz( uf, cer, snh, amb)
		#xml = con.download( __cnpj, chave )
		xml = con.consulta_distribuicao( cnpj=__cnpj, chave=chave )
		#print("-"*100)
		#print xml.text
		html = xml.text if '!DOCTYPE html' in xml.text else ''
		...
		rt = informes.retornosSefaz( xml.text.encode('utf-8'), ["retDistDFeInt"], parent, nome_evento = 'DOWNLOAD' )
		#if rt[0] and rt[4] !='135':	modulo = 3
		if rt[0]:
		
			if rt[4] !='135':	modulo = 3
			informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modelo, mod, rt[4], '', filial, '', '', html )
	...
	def manifestoNfe(self, parent, filial, chave ):
		...
		mod = 'nfe'
		modulo = 1
		uf = 'AN' #--// Para pesquisa no ambiente nacional
		...
		if rt[0]:
			...
			html = envio.text if '!DOCTYPE html' in envio.text else ''
	...
	def cartaCorrecao(self, parent, filial, __i ):
		...
		if rt[0]:
			...
			html = envio.text if '!DOCTYPE html' in envio.text else '' 
			informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulo, mod, rt[4], envio.text, filial, '', '', html )
	...
	def consultaNotaFiscal(self, parent, chave, filial ):
		...
		con = ComunicacaoSefaz( uf, cer, snh, amb )
		envio = con.consulta_nota( mod, chave ) # nfe ou nfce

		rt = informes.retornosSefaz( envio.text.encode('utf-8'), ["nfeResultMsg"], parent, nome_evento = 'CONSULTAR_NF' )
		if rt[0]:
			html = envio.text if '!DOCTYPE html' in envio.text else ''
			informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulo, mod, rt[4], '', filial, '', '', html )
...
#---{ Emissao NF }--------------------------// Identificacao da Nota Fiscal
	def indentificacaoNotaFiscal(self, informar_dados_emitente, informar_dados_clientes, informar_dados_transportadora, emissao, dados_cliente, ambiente, pessoa, endereco_entrega_retira ):
		...
		if emissao in [2,3]:	referenciar_nota = [self.p.NumeroChav.GetValue()]
		...
		if sql[0]:
			...
			if envio[0] == 0:
				...
				if rt[0]:
					...
					#if '!DOCTYPE html' in envio[1].content:

					#	hsu_frame=LerHtml(parent=parent,id=-1, inf = envio[1].content+'<h1>Verifique o seu certificado!!</h1>' )
					#	hsu_frame.Centre()
					#	hsu_frame.Show()
