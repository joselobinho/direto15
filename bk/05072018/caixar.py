...
from ecf     import daruma
from nfes    import GerenciarNFe
...
from relatorio  import relatorioSistema,vendas,sangrias,atendimentos
...
class Devolucoes(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.ToTalGeral = self.ToTalReceb = self.ToTalAbert = self.ToTalDevol = Decimal('0.00')
		...
		wx.Frame.__init__(self, parent, id, 'Caixa: Relação-Relatórios de Devoluções', size=(950,545), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		ms = wx.StaticText(self.painel,-1,u"{ M e n s a g e m }", pos=(820,485))
		ms.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		ms.SetForegroundColour("#7F7F7F")
		...
		self.relatorios = ['01-Relação-Relatório de Devoluções',  '02-Relação-Relatório de DAVs Emitidos','03-Relação de Recebimentos de Cartão por Bandeiras',
						...
						'08-Comissão Sobre Vendas','09-Movimento do Conta Corrente','10-Relação de Produtos p/Emissão de DOF','11-Comissão Sobre Produtos','12-Comissão Sobre Recebido','14-Relatório de Efetivados { Dinheiro, Contas Recebidas }','15-Posição de Vendas c/Filtro em Recebimentos','16-Relatorio de frete']
		...
		self.ordenar_valor = wx.RadioButton(self.painel, 117 , "Ordenar valor", pos=(268,513))
		self.grafico_desco = wx.CheckBox(self.painel, 118 , "Comparar descontos no grafico", pos=(383,513))
		self.dof_resumido  = wx.CheckBox(self.painel, 119 , "Relatorio do DOF resumido", pos=(630,513))
		...
		self.historico = wx.TextCtrl(self.painel,-1,value='', pos=(573,280), size=(368,120),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
	...
	def ModuloRelatorios(self,event):
		...
		elif self.relator.GetValue()[:2] == "07":	at.aTresumo( __di, __df, __vd, self, rFiliais = self.fFilial.GetValue(), Filial = self.fla )
	...
	def configura(self,ajusTar):
		...
		_rl = ['04','05','06','07','08','09','10','11','12','14','16']
		if self.relator.GetValue()[:2] in _rl:
			...
			if self.relator.GetValue()[:2] == "12":	self.relator.SetValue( self.relatorios[11] )
			...
			if self.relator.GetValue()[:2] == "12":	self.vendedo.Enable()
		...
		if self.relator.GetValue()[:2] != "10":	self.selecionar(wx.EVT_BUTTON)
	...
	def passagem(self,event):
		...
		if self.relator.GetValue()[:2] not in lsT:
			...
			status = str( self.DEVconfs.GetItem(indice,9).GetText() )
	...
	def ToTalizacao(self,event):
		...
		if self.relator.GetValue()[:2] == "08":
			...
			if self.vendedo.GetValue():
				...
				vendedor_nome = str( self.vendedo.GetValue().split("-")[1] )
						...
						if ndT <= ndF: 
							...
							if valor_devolucao !=0:
								...
								for d in devolucao:
									...
									if self.sql[2].execute("SELECT pd_coms FROM produtos WHERE pd_codi='"+str( d[5] )+"'") !=0:	per_comissao = self.sql[2].fetchone()[0]
									else:	per_comissao = Decimal("0.00")
									...
									if per_comissao:	valor_comissao = Truncar.trunca( 3, ( ( d[13] - d[28] ) * ( per_comissao / 100) ) )
									else:	valor_comissao = Decimal('0.00')
									
									if valor_comissao:	total_comissao_devolucao +=valor_comissao
									
									dados_devolucao +=str(d[48])+';'+str(d[2])+';'+format(d[67],"%d/%m/%Y")+' '+str(d[68])+' '+str(d[46])+';'+str(d[7])+';'+format(d[13],',')+';'+format(d[28],',')+';'+onde_pegou_comissao+' '+str(per_comissao)+';'+str(valor_comissao)+";devolucao\n"
					...
					for d in range( r ):
						...
						if ndT <= ndF: 
							...
							if ( vlt_vendas + vld_vendas + vlt_devolu + vld_devolu ) !=0:
								...
								self.com_saldo  = ( self.com_vendas_produtos - ( self.com_vendas_desconto + ( self.com_devolu - self.com_devolu_desconto ) ) )
								#self.com_saldo  = ( self.com_vendas_produtos - ( self.com_vendas_desconto + self.com_devolu_desconto + self.com_devolu ) )
...
#---------: Produtos para DOF					
				...
				_vendas = "SELECT * FROM idavs  WHERE it_lanc>='"+str(dI)+"' and it_lanc<='"+str(dF)+"' and it_pdof='T' and it_canc=''"
				_devolu = "SELECT * FROM didavs WHERE it_lanc>='"+str(dI)+"' and it_lanc<='"+str(dF)+"' and it_pdof='T' and it_canc=''"
...
#						for si in rgvendas:
						for sb in range( numero_registros ):
							...
							if codigo_cliente != si[2]: # or numero_registros == numero_rgistrado:
								...
								#if numero_registros == numero_rgistrado:
...
#---------: Comissão de vendas sobre recebido
				...
				if vD !="":
					...
					#parametro_usuario = login.parametros_usuarios[ venDedorN ]
					#if len( parametro_usuario.split(";") ) >=24 and parametro_usuario.split(";")[23]:	comissao_percentual = parametro_usuario.split(";")[23]
										
						#comissao_vendas = ( ( valor - comissao_cartao ) * Decimal( parametro_usuario.split(";")[23] ) /100 )
				...
				if lusuarios !=0:
					...
					for usn in rUsuarios:
						...
						if ( vDinheiro + vDevoluca + vcReceber ):
							...
							parametro_usuario = ""
							if usn[1] in login.parametros_usuarios:	parametro_usuario = login.parametros_usuarios[ usn[1] ]
...
#---------: Produtos para DOF					
				...
				""" Eliminando dados do Temporario """
				fretes = "SELECT cr_inde,cr_ndav,cr_nota,cr_edav,cr_erec,cr_nmcl,cr_tnot,cr_vfre FROM cdavs WHERE cr_edav>='"+str(dI)+"' and cr_edav <='"+str(dF)+"' and cr_tipo='1' and cr_reca='1' and cr_vfre!=0"
				...
				for i in [] if not self.sql[2].execute( fretes ) else self.sql[2].fetchall():
					...
					contabilizado = "1" if valor_frete else "2"
					self.relacao[self._registros] = str( i[0] ), str( i[1] ), str( i[2] ), str( i[3] ), str( i[4] ), str( i[5] ), format( i[6],',' ), format( i[7],',' ), contabilizado
					self._registros +=1

					if valor_frete:	self.vlrfrete += i[7]
				...
				#----: 02-Davs Emitidos 04-Posicao de vendas 05-Resumo de Vendas 06-Resumo Sangria 07-Relatorio de Atendimentos
					...
					if self.davCan == "T":	_dev = "SELECT * FROM cdavs WHERE cr_edav>='"+str(dI)+"' and cr_edav <='"+str(dF)+"' and cr_tipo='1' and cr_tfat!='2' and cr_reca!='3' ORDER BY cr_edav"
					else:	_dev = "SELECT * FROM cdavs WHERE cr_edav>='"+str(dI)+"' and cr_edav <='"+str(dF)+"' and cr_tipo='1' and cr_tfat!='2' ORDER BY cr_edav"
				...
				if _adicio == True:
					...
					if self.relator.GetValue()[:2] == "15":
						...
						if avanca == True:
							...
							format(i[53],','),format(i[50],','),str( i[74] ),format(i[65],',')
						...
						self.relacao[self._registros] = str(i[54]),str(i[2]),emissao,recebim,str(i[4]),format(vlr1,','),str(i[78]),str(i[79]),cancela,str(i[74]),str(i[9]),str(i[10]),str(i[40]),str(i[47]),str(i[59]),str(i[60]),rCartao,_pagamenTos,format(vlr2,','),_TP
	...
	def impresDav(self,event):
		...
		if self.relator.GetValue()[:2] in ["11","12","08"]:
			...
			#self.i.impressaoDav(numdav,self,True,False, __Tipo,"", servidor = self.fla, codigoModulo = "", enviarEmail = "" )
...
#			self.InsertColumn(0, 'Vendas-Recebimento',  format=wx.LIST_ALIGN_LEFT, width=140)
#			self.InsertColumn(1, 'Total vendas avista',  format=wx.LIST_ALIGN_LEFT,  width=140)
#			self.InsertColumn(2, 'Total contas areceber' ,format=wx.LIST_ALIGN_LEFT, width=140)
#			self.InsertColumn(3, 'Total devoluções', format=wx.LIST_ALIGN_LEFT, width=140)
#			self.InsertColumn(4, 'S a l d o',   format=wx.LIST_ALIGN_LEFT, width=140)
#			self.InsertColumn(5, 'Relacao de vendas avista', width=500)
#			self.InsertColumn(6, 'Relacao de contas areceber', width=500)
#			self.InsertColumn(7, 'Relacao de devolucoes', width=500)
	...
	def OnGetItemImage(self, item):
		...
		if self.itemIndexMap != []:
			...
			if self.di in ['12','09','11','14','08','10','16']:	return self.i_orc
			else:
