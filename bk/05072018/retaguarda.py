...
class contacorrente(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Conta corrente', pos=(70,100),size=(920,402), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		printa  = wx.BitmapButton(self.painel, 152, wx.Bitmap("imagens/printp.png",   wx.BITMAP_TYPE_ANY), pos=(880,326), size=(35,33))				

		wx.StaticText(self.painel,-1,"Período DT-Inicial", pos=(655,320)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,"DATA Final",         pos=(765,320)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		...
		self.histo = wx.TextCtrl(self.painel,-1,   pos=(490,365),size=(425,20), style=wx.TE_READONLY)
		...
		self.dindicial = wx.DatePickerCtrl(self.painel, 153, pos=(652,335), size=(110,25), style=wx.DP_DEFAULT|wx.DP_ALLOWNONE)
		self.datafinal = wx.DatePickerCtrl(self.painel, 154, pos=(763,335), size=(110,25))
		...
		printa. Bind(wx.EVT_BUTTON, self.ClienteExtrato)
		...
		if self.modulo == "CR":	printa.Enable( acs.acsm("308",False) )
	...
	def addConta(self,event):
		...
		#if event.GetId() !=155 and not adiciona.ClienteNome:
		
		#	alertas.dia(self.painel,u"{ Cliente estar vazio }\n\n1 - Selecione um cliente para lançamento de crédido-débito\n"+(" "*160),u"Conta corrente: lançamento de crédido-débito")
		#	return
	...
	def relacionar(self,event):
		...
		#if self.consulta.GetValue() and str( self.consulta.GetValue().strip() ).isdigit() == True:
		if __avancar:
			...
			if sql[0] == True:
				...
				#if dav_devolucao:	cConta = "SELECT * FROM conta WHERE cc_davlan='"+ dav_devolucao +"' ORDER BY cc_docume,cc_lancam,cc_horala"
					#cc_davlan
...
class adiciona(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Conta Corrente { Incluir-Consultar }', pos=(260,150),size=(500,342), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self,-1,style=wx.BORDER_SUNKEN)
		...
		procura = wx.BitmapButton(self.painel, 130, wx.Bitmap("imagens/procurap.png", wx.BITMAP_TYPE_ANY), pos=(457,207 ),size=(35,33))				
		...
		wx.StaticText(self.painel,-1,"Pesquisar",pos=(20,205) ).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,"Relação de filiais: ",pos=(20,312) ).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))

		nome_filial = ""
		for fl in login.ciaRelac:
			if fl.split("-")[0] == self.adFilial:	nome_filial = fl

		self.relacao_filial = wx.ComboBox(self.painel, -1, nome_filial,  pos=(110,305), size=(382,27), choices = login.ciaRelac,style=wx.NO_BORDER|wx.CB_READONLY)
		if self.TipoConsulta !="1":	self.relacao_filial.Enable( False )
		...
		self.consCliente = wx.TextCtrl(self.painel,122, pos=(16,215 ), size=(437,23), style=wx.TE_PROCESS_ENTER)
		...
		#if self.TipoConsulta == '2' and self.ClienteNome !='':

		#	self.consCliente.SetValue(self.ClienteNome)
		#	self.selecionarcliente(wx.EVT_BUTTON)	

		#if id == 155: #self.TipoConsulta == '2' and self.ClienteNome !='':
	...
	def selecionarcliente(self,event):
		...
		if nome:
			...
			if sql[0] == True:
				...
				if nome.isdigit() == True:	sql[2].execute("SELECT cl_nomecl,cl_docume,cl_fantas,cl_codigo,cl_indefi,cl_estado FROM clientes WHERE cl_docume='"+nome+"'")
				else:	sql[2].execute("SELECT cl_nomecl,cl_docume,cl_fantas,cl_codigo,cl_indefi,cl_estado FROM clientes WHERE cl_nomecl like '"+nome+"%'")
	...
	def gravacao(self,event):
		...
		if self.TipoConsulta == '2':
			...
			#if _nm == '':
			#	alertas.dia(self.painel,"Selecione um cliente para incluir...\n"+(' '*80),u"Conta Corrente: Lançamentos de Créditos-Débitos")
			#	return
	...
	def vincularCliente(self,_nmc,_doc,_fan,_cod,_fil,_est):
		...
		if self.parente.ndav != '':
			...
			if sql[0] == True:
				...
				#print "1: ",type( result[0][3] ),result[0][3]
				#print "2: ",type( result[0][4] ),result[0][4]
				#print "3: ",type( _cod ),_cod
				#print "4: ",type( _nmc ),_nmc
				#print "5: ",type( login.usalogin ),login.usalogin
...
class EntregaAvulsa(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Expedição avulso: Entrega de material', size=(854,448), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self,-1)

		self.painel.Bind(wx.EVT_PAINT,self.desenho)

		self.lsTiTems = wx.ListCtrl(self.painel, 112,pos=(34,1), size=(819,239),
									style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.lsTiTems.InsertColumn(1, 'Código',  width=100)
		self.lsTiTems.InsertColumn(2, 'Descrição dos Produtos', width=260)
		self.lsTiTems.InsertColumn(3, 'Quantidade', format=wx.LIST_ALIGN_LEFT, width=75)
		self.lsTiTems.InsertColumn(4, 'UN',         width=25)
		self.lsTiTems.InsertColumn(5, 'Entregue',   format=wx.LIST_ALIGN_LEFT, width=75)
		self.lsTiTems.InsertColumn(6, 'Devolução',   format=wx.LIST_ALIGN_LEFT, width=75)
		self.lsTiTems.InsertColumn(7, 'Retirando',  format=wx.LIST_ALIGN_LEFT, width=75)
		self.lsTiTems.InsertColumn(8, 'Saldo',      format=wx.LIST_ALIGN_LEFT, width=80)
		...
		self.retirada = wx.TextCtrl(self.painel,203,value="", pos=(0, 340), size=(505,60),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
		...
		self.portador = wx.TextCtrl(self.painel,204,value="", pos=(510, 360), size=(343,40),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
		...
		self.selecao = wx.BitmapButton(self.painel, 142, wx.Bitmap("imagens/selectall.png",wx.BITMAP_TYPE_ANY), pos=(778,272), size=(35,38))
		self.gravaca = wx.BitmapButton(self.painel, 143, wx.Bitmap("imagens/savep.png",    wx.BITMAP_TYPE_ANY), pos=(820,272), size=(35,38))
	...
	def Teclas(self,event):
		...
		controle = wx.Window_FindFocus()
		keycode  = event.GetKeyCode()
		if controle !=None and controle.GetId() == 240:	self.retiradaManual()

	def retiradaManual(self):
		...
		if self.lsTiTems.GetItemCount() != 0:
			...
			qTT = self.t.trunca(5, Decimal( self.qTEnTrg.GetValue() ) )
			...
			if sql[0] == True:
				...
				if achaDav and achaITe:
					...
					for i in self.pis:
						...
						self.lsTiTems.SetStringItem(indiTem,4, str( i[8]  ) )	
...
#-: Finalizacao da Entrega
	def FinalEntrega(self,event):
		...
		if self.portador.GetValue().strip() == "":
...
#-----: Filial local para filiais q ficam no mesmo local ou um unica loja
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(602, 274, 172, 36, 3) #-->[ Lykos ]
