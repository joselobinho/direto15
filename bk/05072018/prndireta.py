...
class ImpressaoTermica:
	...
	def printers( self, parent, filial, ibpt, e = '', d = '', p = '', t = '', pg = '', des = '', dn = '', port = '', src = '', prn = '', tipo_printer=0, _printers = '', dav='' ):
		...
		if tipo_printer == 3:	s = g
			...
			din = chq = cTc = cTd = cLj = ouT = vouT = bol = Decimal("0.00")
			...
			for pgt in range( len( pg[0] ) ):
				...
				if pg[0][pgt] == "15":	bol += Decimal( pg[1][pgt] )
				if pg[0][pgt] == "99":	ouT += Decimal( pg[1][pgt] )

			"""   Lancar Pagamentos
			      Posicionamento do lado esquerdo valor da forma de pagamento
			"""
			ajustar_colunas = 0 if prn.split('-')[0] == '2' else 10 #--// 0-Daruma 2-Bematech

			if din:	s += "Dinheiro:"       +(" " * ( ( 58 - ajustar_colunas ) - len(format( din,',')) ) )+format( din,',') + '\n'
			if chq:	s += "Cheque:"         +(" " * ( ( 60 - ajustar_colunas ) - len(format( chq,',')) ) )+format( chq,',') + '\n'
			if cTc:	s += "Cartao Credito:" +(" " * ( ( 52 - ajustar_colunas ) - len(format( cTc,',')) ) )+format( cTc,',') + '\n'
			if cTd:	s += "Cartao Debito:"  +(" " * ( ( 53 - ajustar_colunas ) - len(format( cTd,',')) ) )+format( cTd,',') + '\n'
			if cLj:	s += "Credito Loja:"   +(" " * ( ( 54 - ajustar_colunas ) - len(format( cLj,',')) ) )+format( cLj,',') + '\n'
			if bol:	s += "Boleto:"         +(" " * ( ( 60 - ajustar_colunas ) - len(format( bol,',')) ) )+format( bol,',') + '\n'
			if ouT:	s += "Outros:"         +(" " * ( ( 60 - ajustar_colunas ) - len(format( ouT,',')) ) )+format( ouT,',') + '\n'
	...
	def impressaoNfceTermica( self, parent, xml, filial, ibpt, impressora ='', termica_laser = 0, numero_dav='' ):
		...
		if termica_laser == 3:	envia_printer.printers( parent, filial, e="", d="", p="", t="", pg="", des="", dn="", port = impressora[1], src = impressora[2], prn = impressora[0], ibpt = "", tipo_printer = 3, _printers = impressora  )
			...
			if impressora[0].split('-')[0] in ["1"]:
				...
				if termica_laser == 2:	envia_a4.ContaCorrente( dados_emitente,ibpt,parent, rFiliais = False, Filial = filial, Tp = "2", iTems = dados_produtos, Davs = dados_totalizacao, ClienTe = dados_destinatario, nfes = dados_pagamentos, conTigencia = dados_nota, tipo_pagamento = pagamentos_tipo, valor_pagamento = pagamentos_valor )
