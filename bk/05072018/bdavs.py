...
class DavConsultar(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.p.Disable()
		wx.Frame.__init__(self, parent, id, u'DAVs: pedidos, orçamentos - consulta e reimpressão', size=(800,528), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		self.ocorr = wx.StaticText(self.painel,-1,pos=(282,365))
		...
		wx.StaticText(self.painel,-1,u"Alteração manual do frete: ",pos=(410,506)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.DvVoltar = wx.BitmapButton(self.painel, 221, wx.Bitmap("imagens/voltam.png",  wx.BITMAP_TYPE_ANY), pos=(277,300), size=(36,34))				
		self.Procurar = wx.BitmapButton(self.painel, 222, wx.Bitmap("imagens/procurap.png",wx.BITMAP_TYPE_ANY), pos=(318,300), size=(36,34))				
		self.Printar  = wx.BitmapButton(self.painel, 223, wx.Bitmap("imagens/printp.png",  wx.BITMAP_TYPE_ANY), pos=(360,300), size=(36,34))

		self.extratp = wx.BitmapButton(self.painel, 224, wx.Bitmap("imagens/cccl.png",     wx.BITMAP_TYPE_ANY), pos=(240,300), size=(28,28))				
		self.importa = wx.BitmapButton(self.painel, 225, wx.Bitmap("imagens/import16.png", wx.BITMAP_TYPE_ANY), pos=(240,330), size=(28,28))				
		self.davrefe = wx.BitmapButton(self.painel, 226, wx.Bitmap("imagens/edit.png",     wx.BITMAP_TYPE_ANY), pos=(240,360), size=(28,28))				
		...
		self.dindicial = wx.DatePickerCtrl(self.painel,-1, pos=(120,310), size=(108,23), style=wx.DP_DEFAULT|wx.DP_ALLOWNONE)
		self.datafinal = wx.DatePickerCtrl(self.painel,-1, pos=(120,350), size=(108,23))
		...
		self.pesqperio = wx.CheckBox(self.painel, -1, "Pesquisar p/Período", pos=(277,340))
		...
		self.frete_manual = mkn(self.painel, 888, value = '0.00', pos=(565,501),size=(100,15), style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth = 4, fractionWidth = 2, allowNone=False,groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = '#FFFFFF', validBackgroundColour = '#FFFFFF', invalidBackgroundColour = "Yellow",allowNegative = False)
	...
	def importacao(self,event):
		...
		if Decimal(self.ToDAV.GetValue().replace(',','')) == 0:	alertas.dia(self.painel,"Selecione um DAV p/Continuar...\n",u"Importação de DAVs")
			...
			if self.csFilial != idfili and not esf_unificado:
				...
				alFil = wx.MessageDialog(self,"{ Filiais Diferentes }\n\nFilial Atual...: "+str( fA )+"\nFilial do DAV: "+str( fD )+"\n\nO Sistema vai alterar a filial p/"+str( fD )+"\nConfirme para continuar...\n"+(" "*140),"Alteração da Filial",wx.YES_NO|wx.NO_DEFAULT)
			...
			if RegisTros == 0:
				...
				if impDav.davsImporTa(self, self.p, _fil, orcame, clCodi, regime, clDocu, clNome, vended, dav_reducao, self.devorcame.GetValue() ):	self.voltar(wx.EVT_BUTTON)
	...
	def impresDav(self,event):
		...
		self.impres.impressaoDav( NumeroDav, self.myfram, True, True, __modo,"", servidor = _fil, codigoModulo = "616", enviarEmail = emai )
	...
	def Teclas(self,event):
		...
		if controle !=None and controle.GetId() == 300:
			...
			if self.p.TComa3.GetValue() and self.ListaDavs.GetItem(indice,5).GetText() and Decimal( self.ListaDavs.GetItem(indice,5).GetText() ):	self.devolufrt.Enable( True )
			else:
		...
		if not self.p.TComa3.GetValue() and len( login.usaparam.split(";") ) >=1 and login.usaparam.split(";")[0] == "T" and self.ListaDavs.GetItemCount() and not self.ListaDavs.GetItem( self.ListaDavs.GetFocusedItem(), 23).GetText() and self.ListaDavs.GetItem( self.ListaDavs.GetFocusedItem(), 16).GetText() == '1':
			...
			self.vinculnfe.Enable( False )
			self.reducao.Enable( False )
			#try:	self.reducao.SetValue('0.00')
			#except Exception as erro:	self.reducao.SetValue('0.00')
	...
	def aTualizaImporTacao(self):
		...
		if self.ListaDavs.GetItem(indice, 16).GetText() == "2":

			self.Tipos.SetLabel(u'Orçamento: '+self.ListaDavs.GetItem(indice, 0).GetText()+ '  Filial: '+ self.ListaDavs.GetItem(indice, 20).GetText() )
...
class ImporTaDAvs:
	...
	def davsImporTa( self, parent, par, _fil, orcame, clCodi, regime, clDocu, clNome, vended, nfe_reducao, usar_orcamento ):
		...
		if sql[0] == True:
			...
			ITems   = sql[2].execute("SELECT *  FROM idavs WHERE it_ndav='"+str( orcame )+"'")
			_result = sql[2].fetchall()
			...
			if parent.atualCFis.GetValue() == True:
			...
			if ITems !=0:
				...
				for i in _result:
					...
					FilialITem = _iTeFil, per_desconto = "0.00", vlr_desconto = "0.00", vinculado = orcame, estoque_local = es_local )
	...
	def reCalcularTribuTos(self, parent, Temporario, addTemp, davaV, reducao ):
		...
		if itens > 0:
			...
			for i in range(itens):
				...
				if int( cst ) == 60:	BaseCalcIcms = ValorIcms = Decimal('0.00')
				...
				if Temporario == True and addTemp == True:
					...
					_unidad = str(self.p.ListaPro.GetItem(indice,  4).GetText())
...
class QuemComprouSelecionado(wx.Frame):
	...
	def davConsulta(self,event):
		...
		self.p.parente.impre.impressaoDav( numero_dav, self, True, True, "", "", servidor = self.filial, codigoModulo = "616", enviarEmail = "608" )
