...
#		self.dados_impressora = wx.StaticText(self.painel,-1,"Dados da impressora", pos=(575,424))
		...
		self.meian = wx.CheckBox(self.painel, -1,  " [ Orçamento vinculado ]         ", pos=(175,244))
	...
	def enviarSefazNfce( self, event ):
		...
		if len( login.filialLT[self.efilial] ) >= 31 and len( login.filialLT[self.efilial][30].split(';') ) >= 3 and login.filialLT[self.efilial][30].split(';')[2] == "4.00":
			...
			nfs40emissao.emitirNfce400( self, self.numedav, self.moduloenvio )
		...
		if self.numedav:
			...
			if sql[0] == True:
				...
				if self.resul_dav and self.resul_dav[0][112] :
					...
					self.mnd = "" if not sql[2].execute("SELECT * FROM cdavs WHERE cr_ndav='"+str( self.resul_dav[0][112] )+"'") else sql[2].fetchall() # if self.vincul else ""
					self.mni = "" if not sql[2].execute("SELECT * FROM idavs WHERE it_ndav='"+str( self.resul_dav[0][112] )+"'") else sql[2].fetchall() # if self.vincul else ""
					
					self.meian.SetLabel("Orçamento vinculado {"+str( self.resul_dav[0][112] )+"}")
	...
	def calcularVinculado(self):
		...
		if self.meian.GetValue():
			...
			if self.moduloenvio == 2:
				...
				if self.mnd and self.valor_pagamentos_cartao > self.mnd[0][37]:
