...
#------: CCe
		...
		self.ListaCCe.SetForegroundColour('#F9F2F2')
		...
		if not self.moTiv.GetValue().strip():	alertas.dia(self.painel,"Para download, manifesto insira a chave em motivo"+"\n"+(" "*140),"Herlp-Gerênciador de NFes")
			...
			if not self.relfilial.GetValue().strip():	alertas.dia(self.painel,"Selecione uma filial para download, manifesto insira a chave em motivo"+"\n"+(" "*140),"Herlp-Gerênciador de NFes")
				...
				if receb.ShowModal() ==  wx.ID_YES:
					...
					if ler_motivo.isdigit() and len( ler_motivo ) == 44 and ler_motivo[20:22] == "65":	nfce31man.manutencao( self, filial, 5, dados = ler_motivo, gerenciador = False )
					else:
						if event.GetId() == 123:	NFE31Man.manutencao( self, 6, self.moTiv.GetValue(), Filial = filial )
	...
	def statusNfe(self,event):
		...
		nome_modelo = "nfce" if modelo == "65" else "nfe"
		dados = filial, modelo, nome_modelo

		if len( login.filialLT[filial] ) >= 31 and len( login.filialLT[filial][30].split(';') ) >= 3 and login.filialLT[filial][30].split(';')[2] == "4.00":	nf40eventos.status( self, dados ) 
			
		else:	NFE31Man.manutencao( self, 1, '', Filial = filial )
		...
		if   nChave == '' and inutil !="4":	alertas.dia(self.painel,u"Nº Chave vazio, NFs Inutilizadas não Gera Chave!!\n"+(" "*110),"Gerênciador NFs")
			...
			if sql[0] == True:
				...
				if _id == 127:	_arqxml = _result[0][0]
					...
					_arqxml = _result[0][0] if _result[0][0] else _result[0][1]
		...
		if sql[0] == True:
			...
			if rT !=0:
				...
				if dTem !=None and Hoje > dTem:
					...
					self.cnfs.Disable()
					self.cnfd.Disable()
					self.ms.SetLabel("{ Fora do prazo de cancelamento }")
	...
	def cancela(self,event):
		...
		if len( login.filialLT[self.ffilia] ) >= 31 and len( login.filialLT[self.ffilia][30].split(';') ) >= 3 and login.filialLT[self.ffilia][30].split(';')[2] == "4.00":
			...
			if self.Modelo == "55":
				...
				#"""  Cancelamento versao 4.00  """
				#if len( login.filialLT[self.ffilia] ) >= 31 and len( login.filialLT[self.ffilia][30].split(';') ) >= 3 and login.filialLT[self.ffilia][30].split(';')[2] == "4.00":

				#	if not self.arquivo_xml40:	alertas.dia(self, u"{ Chave de nota fiscal nao localizada }\n"+self.nfCha.GetValue()+"\n"+(" "*180),"Cancelamento de NFe 4.00")
				#	else:
