...
class RelatorioDiversos(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Contas Areceber: Cheques Estornados', size=(950,500), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self,-1, style = wx.BORDER_SUNKEN)
		self.Bind(wx.EVT_CLOSE, self.sair)
		...
		#wx.StaticText(self.painel,-1, u"Período Inicial",     pos=(18, 396)).SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		#hs = wx.StaticText(self.painel,-1,u"Historico de seleção", pos=(15,365))
		#hs.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		#hs.SetForegroundColour('#7F7F7F')
	...
	def configura(self):
		...
		#self.pesquisa_lancanc.Enable( False )
		#self.pesquisa_lancanc.SetValue( False )
		...
		if RelatorioDiversos._id == "05": #5106:
			...
			self.historico.SetBackgroundColour('#596C81')
			self.CHQEstornado.SetBackgroundColour('#89AACC')
			#self.pesquisa_lancanc.Enable( True )
		...
		if RelatorioDiversos._id == "08":	simples = False
			...
			if sql[0] == True:	
				...
				if _car !=0:
					...
					for i in _rca:
						...
						if self._id == "03":	relacao[_registros] = str( i[24] ),str( i[1]+'/'+i[3]),emissao,vencime,pagamen,format( cartao_liquido,','),str(i[59]),str(i[12]),str(i[35]),str(i[54]),str(i[6]),str(i[21]),str(i[38]),__relaTorio
	...
	def desenho(self,event):
		...
		dc.DrawRotatedText("Contas Areceber: Relatorios { D i v e r s o s }", 0, 485, 90)
...
#-------: Relacionar TiTulos Vinculados
		...
		self.Tpr = wx.CheckBox(self.painel, -1, "Marcar o Título p/Estornar"+(" "*20), pos=(0,340))
			...
			if sql[0]:
				...
				if bTiTulos !=0:
					...
					for i in rTiTulos:
						...
						if len( login.filialLT[ i[24] ][35].split(";") ) >= 80 and int( login.filialLT[ i[24] ][35].split(";")[79] ):	self.ect = self.diasPassados( format( i[7], "%d/%m/%Y"), login.filialLT[ i[24] ][35].split(";")[79] )
	...
	def esTornarVinculados(self,event):
		...
		mensa = "Estorno em Grupo do Título Nº "+str( self.nTL.GetValue() )+" ID-"+str( self.nRG.GetLabel() )+"\nNome do Cliente: "+str( self.nCL.GetValue() )+"\n\nConfirme p/Estornar !!\n"+(" "*130)
			...
			if sql[0] == True:
				...
				try:
					...
					for i in range( nRVinc ):
						...
						rc_obsest='"+str( hisTo )+"' WHERE rc_regist='"+ idL +"'"
					...
					for r in range( nRTiTu ):
						...
						rc_obsest='"+str( hisTo )+"' WHERE rc_regist='"+ idT +"'"
...
class TitulosDemembrados(wx.Frame):
	...
	def impresDav(self,event):
		...
		if nTipo.upper() == "DAV":
			...
			impress.impressaoDav( nDavs.split('/')[0], self, True, True,  "", "", servidor = self.p.filrc, codigoModulo = "", enviarEmail = "" )
