...
class ClientesControles(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.list_ctrl = clListCtrl(self.painel,500,pos=(13,30),size=(955,465),
						style=wx.LC_REPORT
						|wx.BORDER_SUNKEN
						|wx.LC_VIRTUAL
						|wx.LC_HRULES
		...
		self.list_ctrl.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.editarIncluir)
		...
		if sql[0] == True:
			...
			if self.consultar.GetValue() !='':
				...
				if _pes.isdigit() == True:	__clT = __clT.replace("ORDER BY cl_nomecl","and cl_docume like '"+str(_pes)+"%' ORDER BY cl_nomecl")
					...
					else:	__clT = __clT.replace("ORDER BY cl_nomecl","and cl_nomecl like '"+str(_pes)+"%' ORDER BY cl_nomecl")
			...
			for row in _result:
				...
				relacao[_registros] = str( row[46] ),row[2],row[1],row[3],row[30],row[31],row[38],row[9],row[10],row[11],row[12],row[8],row[40]+'|'+row[39],row[3],row[49],row[0]
				_registros +=1
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(15, 500, 480, 97, 3) #-->[ Funções ]
...
class alTeraInclui(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Cliente: Cadastros e Controles {'+str(TipoCad.encode("UTF-8"))+'}', size=(778,620), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self)
		self.painel.Bind(wx.EVT_PAINT,self.desenho)
		...
		if sql[0] == True:
			...
			if self.p.idInc !=301 and sql[2].execute(procura) == 0:
				...
				conn.cls(sql[1])
				alertas.dia(self.painel,u"{ Código: "+str( self.p.codigo )+" }, Cliente não localizado...\n"+(' '*100),u"Clientes: Alterar,Consultar,Incluir")
	...
	def salvar(self,event):
		...
		if sql[0] == True:
			...
			if str( self.cl_pgfutu.GetValue() )[:1] != self.danterior.split("|")[0]:	self.adicionaAlteracao( "NEGA", self.danterior.split("|")[0], str( self.cl_pgfutu.GetValue() )[:1] )
			if str( self.cl_blcred.GetValue() )[:1] != self.danterior.split("|")[1]:	self.adicionaAlteracao( "CRED", self.danterior.split("|")[1], str( self.cl_blcred.GetValue() )[:1] )
			if str( self.dbldescri.GetValue() )[:1] != self.danterior.split("|")[2]:	self.adicionaAlteracao( "DESB", self.danterior.split("|")[2], str( self.dbldescri.GetValue() )[:1] )
			if str( self.consumido.GetValue() )[:1] != self.danterior.split("|")[3]:	self.adicionaAlteracao( "NFCE", self.danterior.split("|")[3], str( self.consumido.GetValue() )[:1] )
			if str( self.cl_aprove.GetValue() )[:1] != self.danterior.split("|")[4]:	self.adicionaAlteracao( "APOV", self.danterior.split("|")[4], str( self.cl_aprove.GetValue() )[:1] )
			if Decimal( self.cl_limite.GetValue() ) != Decimal( self.danterior.split("|")[5] ):	self.adicionaAlteracao( "LIMI", self.danterior.split("|")[5], str( self.cl_limite.GetValue() ) )
			if str( self.rateiofrt.GetValue() )[:1] != self.danterior.split("|")[6]:	self.adicionaAlteracao( "RATE", self.danterior.split("|")[6], str( self.rateiofrt.GetValue() )[:1] )
			if str( fpagamentos ) != str( self.danterior.split("|")[7] ):	self.adicionaAlteracao( "FPGT", self.danterior.split("|")[7], str( fpagamentos ) )
			if str( self.ClDocume.GetValue() ) != str( self.danterior.split("|")[8] ):	self.adicionaAlteracao( "DOCU", self.danterior.split("|")[8], str( self.ClDocume.GetValue() ) )
			if self.cl_nomecl.GetValue() != self.danterior.split("|")[9]:	self.adicionaAlteracao( "DESC", self.danterior.split("|")[9], self.cl_nomecl.GetValue() )
			...
			if self.p.idInc == 500: #->[ Alteracao ]
				...
				try:
					...
					cl_nmrede='"+ self.cl_nmrede.GetValue().upper()[:40]+"',cl_altera='"+self.alteracoes+"' WHERE cl_codigo = '"+str(self.cl_codigo)+"'")
				...
				try:
					...
					cl_dtincl,cl_hrincl,cl_incalt,cl_parame,cl_vended,cl_nmrede)\
					...
					'"+str( self.clFilial ) +"','"+self.cl_imunic.GetValue()+"','"+self.cl_revend.GetValue()+"','"+self.cl_seguim.GetValue().upper()+"','"+self.cl_refere.GetValue()+"',\
					'"+self.cl_emails.GetValue()+"','"+Tpagame+"','"+str(self.cl_limite.GetValue())+"','"+str( Rpagame )+"','"+Bloquei+"',\
					'"+str( EMI )+"','"+str( DHO )+"','I','"+ parameTrosCl +"','"+ self.cl_vended.GetValue() +"','"+ self.cl_nmrede.GetValue().upper()[:40]+"')")
					...
					gRegc = str( nRegc )+'-'+str( self.clFilial )
...
class ClientesRelatorios(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Produtos: Relação-Relatorios', size=(950,550), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT)
		...
		wx.StaticText(self.painel,-1,u"Período Inicial", pos=(18, 385)).SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,u"Período Final",   pos=(18, 435)).SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		'04-Emissão de Etiquetas { Termica }','05-Lista do Cadastro de Clientes',"06-Conciliar: Compra,Devolucao,Cancelamento","07-Gravar dados de email e nome em csv p/webmail"]
	...
	def mCombobox(self,event):
		...
		ClientesRelatorios._id = self.relator.GetValue().split("-")[0]
		self.ajustes()
		if event.GetId() in [600,610,611] and self.relator.GetValue().split("-")[0] in ["01","04","05","07"]:	self.selecionar()	
	...
	def ajustes(self):
		...
		if self.relator.GetValue().split("-")[0] == "05":
				...
				if self._id not in ["06","07"]:
					...
					for i in _rca:
						...
						if self._id == "01":
							...
							_relatorio = str(i[1])+"|"+str(i[8])+"|"+str(i[13])+"|"+str(i[14])+"|"+str(i[9])+"|"+str(i[10])+"|"+str(i[12])+"|"+str(i[15])+"|"+str(i[0])+"|"+str(i[31])+"|"+str(i[35])+"|"+str(i[36])+"|"+str(i[17])+"|"+str(i[2])+"|"+_dCad

							relacao[_registros] = i[46],i[3],i[2],i[1],i[30],i[31],'','','',_relatorio
							_registros +=1
...
class CLTListCtrl(wx.ListCtrl):
	...
	def OnGetItemImage(self, item):
		...
		if self.itemIndexMap != []:
			...
			elif _ID == "02":	return self.i_orc
			elif _ID == "03":	return self.i_orc
			elif _ID == "04":	return self.i_orc
			elif _ID == "05":	return self.i_orc
