	...
	def verIbptValidade(self):
		...
		if login.administrador.upper() in ['ADMINISTRACAO','ADMINISTRACAO-APAGAR']:
			...
			try:
				...
				_mensagem = mens.showmsg(u"{ O sistema não conseguiur verificar a TABELA IBPT }\n\nNOTIFICANDO AO DESENVOLVER SOBRE O OCORRIDO\n\nAguarde...")
	...
	def erroBackup(self ):
		...
		if os.path.exists(pasT+"/bksystem/bkerro/"+arer) == True:
			...
			if __servido and __servido.split('|')[0].upper() == "ERRO":
				...
				#	MostrarHistorico.hs = "Sistema de Backup Estar Apresentando Erro,\nAnote o erro e entre em contato com o suporte!!\n\n--Descrição do(s) Erro(s)--\n"+__servido
				#	MostrarHistorico.TT = "{ Error: Sistema de Backup }"
				#	MostrarHistorico.AQ = ""
				#	MostrarHistorico.FL = ""
				...
				#self.ctg.SetLabel("Sistema de backup ativo\n"+str( __servido ) )
		...
		#if os.path.exists(pasT+"/bksystem/online/"+"onlineerro.txt") == True:

		#	__arquivo = open(pasT+"/bksystem/online/"+"onlineerro.txt","r")
		#	__servido = __arquivo.read()
		#	__arquivo.close()

		#	if __servido and __servido.split("|")[0].upper() == "ERRO":

		#		login.notas_rejeitadas = "Sistema de backup on-line\n"+str( __servido )
		#		self.ctg.SetForegroundColour('#76B6F5')
	
		#	if __servido and __servido.split("|")[0].upper() == "SUCESSO":	self.TI.SetForegroundColour('#76B6F5')

		#if os.path.exists("/home/lykos/direto/configura/errosbackup.txt"):

		#	__arquivo  = open("/home/lykos/direto/configura/errosbackup.txt","r").readlines()
		#	if __arquivo:
				
		#		__dataerr = __arquivo[0].split(' ')[0]
		#		dr =  datetime.datetime.strptime(__dataerr, '%d-%m-%Y').date()
		#		hj = datetime.datetime.now().date()
		#		dd = ( hj - dr ).days
		#		login.notas_rejeitadas = str( dd )+" dia(s)\nBackup-cloud do banco de dados apresentando erro\nEntre em contato com o suporte"
