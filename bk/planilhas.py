	...
	def EstoqueFisico(self,par,mod):
		...
		if mod.upper() == 'DIA':
						...
						if _car:
							...
							for i in _rca:
								...
								if sql[2].execute( esToqueFisico ):
									...
									if _esf:	Tcom += Tru.trunca(5, ( Decimal( _prd[0][23] ) * Decimal( _esf ) ) )
									if _esf:	Tcus += Tru.trunca(5, ( Decimal( _prd[0][24] ) * Decimal( _esf ) ) )
									if _esf:	Tcum += Tru.trunca(5, ( Decimal( _prd[0][25] ) * Decimal( _esf ) ) )
									if _esf:	Tven += Tru.trunca(5, ( Decimal( _prd[0][28] ) * Decimal( _esf ) ) )
			...
			for r in range(__nReg):
				...
				ws0.write(Linhas, 5 , mCompra, horz_style)
				ws0.col(5).width = 250*20
            
				if _sTcp != "" and Decimal( _sTcp ) != 0:	_sTcp = Decimal( _sTcp )
				else:	_sTcp = ""
				ws0.write(Linhas, 6 , str( _sTcp ),horz_style)
