	...
	def bancosSelecionar(self, _sql ):
		...
		if sql[0]:
			...
			for i in [] if not nrg else sql[2].fetchall():
				...
				relacao[_registros] = str( i[0] ).zfill(4), i[1], i[2], i[3], i[7], i[8], i[4], i[5], i[6], i[9], i[10], i[11]
				_registros +=1
		...
		if self.contasreceber.GetItemCount():
			...
			for i in range( self.contasreceber.GetItemCount() ):
				...
				if self.contasreceber.GetItem( i, 10 ).GetText() and self.contasreceber.GetItem( i, 11 ).GetText():
					...
					t, l, p = self.contasreceber.GetItem( i, 10 ).GetText().split('|')
					...
					if l and t.split('-')[0] == "02":	l = l.split('/')[ ( len( l.split('/') ) -1 ) ] #-:Boleto cloud
					if p:	relacao.append( c +";"+ v +";"+ e +";"+ p +";"+ l + ";"+ t.split("-")[0] )
...
#-//Gera boletos p/o cliente
	def geradorBoletos(self,event):

		if not self.cadastrobancos.GetItemCount():
			
			alertas.dia( self, "Banco não selecionado e/ou lista de bancos vazio!!\n"+(" "*120),"Gerador de boletos")
			return

		if not self.contasreceber.GetItemCount():
			
			alertas.dia( self, "Contas areceber não selecionado e/ou lista do areceber vazio!!\n"+(" "*120),"Gerador de boletos")
			return

		if len( login.cadcedente.split('|') ) < 3:

			alertas.dia( self, "Sem cadastro do servidor de boletos!!\n"+(" "*100),"Servidor de boletos")
			return

		else:	self.geraBoletosClientes()
			
	def recuperarBoleto(self,event):

		indice = self.contasreceber.GetFocusedItem()
		id_cliente    = self.contasreceber.GetItem( indice, 0 ).GetText().strip()
		id_lancamento = self.contasreceber.GetItem( indice, 1 ).GetText().strip()
		dados_cliente = self.contasreceber.GetItem( indice,12 ).GetText().strip()

		emailc = dados_cliente.split('|')[11] if len( dados_cliente.split('|') ) >= 12 else ""
		...
		if len( login.cadcedente.split('|') ) >= 3	and login.cadcedente.split('|')[2].split(";")[0] == "01":

			alertas.dia( self, "Serviço exclusivo para web-services de boletos!!\n"+(" "*130),"Servidor de boletos")
			return		
		 
		if self.contasreceber.GetItemCount() and self.contasreceber.GetItem( indice, 10 ).GetText().strip():

			srv, pro, rec, tok, juro, multa = login.cadcedente.split('|')[2].split(";")
			conexao = { "url":str( pro ), "token":str( tok ) }

			dados = self.contasreceber.GetItem( indice, 10 ).GetText().strip().split('|')
			local= self.contasreceber.GetItem( indice, 10 ).GetText().strip()

			passar = True if not dados[2] else False
			if not passar:

				incl = wx.MessageDialog( self,"{ O boleto existe na pasta local, sem a necessidade de recuperação }\n\nConfirme para recuperação do boleto em 2a Via\n"+(" "*150),"Recuperação de boletos 2a Via, com boleto local",wx.YES_NO|wx.NO_DEFAULT)
				if incl.ShowModal() ==  wx.ID_YES:	passar = True

			if dados[0].split('-')[0] == "02" and dados[1] and passar: #-:Boleto cloud

				if dados[1].split('/')[ ( len(dados[1].split('/') ) - 1 ) ]:

					id_boleto = dados[1].split('/')[ ( len(dados[1].split('/') ) - 1 ) ]
					saida, localizacao = bc.pegarBoletoGerado( self, id_lancamento, id_boleto, local, **conexao )
															
					if saida:	nova_localizacao = dados[0]+'|'+dados[1]+'|'+localizacao
					if saida:	recuperacao = dados[0].split('-')[0] +'|'+  pro +'|'+ id_boleto +'|'+ tok +'|'+ id_lancamento +'|'+ emailc +'|'+ rec+'/'+id_boleto 
					if saida:	self.gravarRetornoBoletos( 1, nova_localizacao, id_cliente, id_lancamento, recuperacao )
					if not saida:	erros = localizacao

				else:	saida = False	

		else:	saida = False

		if not saida and not erros:	alertas.dia( self, u"Sem dados para recuperação do boleto...\n"+(" "*120),u"Recuperação do boleto")
		if not saida and erros:	alertas.dia( self, erros +"\n"+(" "*150),u"Recuperação do boleto")
		if saida:	alertas.dia( self, u"Boleto com pdf e localização recuperado...\n"+(" "*120),u"Recuperação do boleto")
		...
		for i in range( self.contasreceber.GetItemCount() ):
			...
			if self.contasreceber.GetItem( i, 11 ).GetText().strip() and not self.contasreceber.GetItem( i, 10 ).GetText().strip():
				...
				if retorno:
					...
					self.gravarRetornoBoletos( 1, gravar_saida, id_cliente, id_lancame, recuperacao )
	...
	def gravarRetornoBoletos( self, tipo, saidas, id_cliente, id_lancame, recuperar_2via ):
		...
		if sql[0]:
			...
			try:
				...
				if tipo == 1:
					...
					sql[2].execute("UPDATE creceber SET rc_boleto='"+str( saidas )+"', rc_bl2via='"+str( recuperar_2via )+"' WHERE rc_idclie='"+str( id_cliente )+"' and rc_regist='"+str( id_lancame )+"'")
					sql[1].commit()
...
class cadastrarBancos(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Bancos: Cadastros e controles ', size=(520,285), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self)
		self.painel.Bind(wx.EVT_PAINT,self.desenho)
	...
	def levantarBanco(self):
		...
		self.bc_instruc.SetValue( self.p.cadastrobancos.GetItem( indice, 5 ).GetText() )
...
class RelacionarBoletos(wx.Frame):
		...
		if self.relacao:
			...
			for i in self.relacao:
				...
				if i:
					...
					self.ListaBoleto.SetStringItem( indice, 1, i.split(';')[0] )
					self.ListaBoleto.SetStringItem( indice, 2, i.split(';')[1] )
					self.ListaBoleto.SetStringItem( indice, 3, i.split(';')[2])	
					self.ListaBoleto.SetStringItem( indice, 4, i.split(';')[3])	
					if len( i.split(";") ) >= 5:	self.ListaBoleto.SetStringItem( indice, 5, i.split(';')[4])	
					if len( i.split(";") ) >= 6:	self.ListaBoleto.SetStringItem( indice, 6, i.split(';')[5])	

					if indice % 2:	self.ListaBoleto.SetItemBackgroundColour(indice, "#918F8F")
					if not i.split(';')[2].strip():	self.ListaBoleto.SetItemBackgroundColour(indice, "#C2A0A0")
					if datetime.datetime.strptime( i.split(';')[1].strip(), "%d/%m/%Y").date() < datetime.datetime.now().date():	self.ListaBoleto.SetItemBackgroundColour(indice, "#BB6161")
	...
	def emailImpressaoBoleto(self,event):
		...
		arquiv = self.ListaBoleto.GetItem( indice, 4 ).GetText().strip()

		gerenciador.Anexar = arquiv
