		...
		def cabConta(_separa):
			...
			if _separa == False:	mdl.mtitulo(rel,cv,cb1,cb2,cb3,cb4,cb5,cb6,cb7,cb8,cb9,cb10,cb11,cb12,6,1)
				...
				if Tp == "2":
					...
					indice = 0
					linhas = 660
					for i in sorted( tipo_pagamento ):
						
						pgt, vlr = tipos_pagamentos[i] +':', valor_pagamento[indice]
						cv.drawString( 402, linhas, pgt )
						cv.drawRightString( 568, linhas, format( Decimal( vlr ),',' ) )
						linhas -= 8
