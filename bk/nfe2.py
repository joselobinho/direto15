...
#-----: Devolucao de RMA { Chave Referenciada }
		...
		self.tipo_pedido_consumidor = wx.StaticText(self.painel, -1,'Converter emissão:', pos=(590,369))
		...
		wx.StaticText(self.painel,-1,u"Modalidade do frete", pos=(677,563)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.vin = wx.StaticText(self.painel,-1,'', pos=(680,490))
		...
		self.orc.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
...
#-----: IE do destinatario indIEst
		...
		self.modalidade_frete = wx.ComboBox(self.painel, -1, self.lista_modalidade_frete[3], pos=(675,574), size=(232,27), choices = self.lista_modalidade_frete , style=wx.NO_BORDER|wx.CB_READONLY)

		"""  Aproveitamento do credito de icms  """
		self.aproveitamento_credito = wx.CheckBox(self.painel, 562, "Aproveitamento do credito\nde icms: {"+str( self.vinculado )+"}", pos=(675,520))
...
#---------: Buscando Tabelas de Devolucao
			...
			if self.pd == True:
				...
				if sql[2].execute("SELECT * FROM cdavs WHERE cr_ndav='"+dav_numero+"'")  != 0:	self.cTDavs = sql[2].fetchall()
				if sql[2].execute("SELECT * FROM idavs WHERE it_ndav='"+dav_numero+"'")  != 0:	self.cTItem = sql[2].fetchall()
				...
				"""    Informacoes da meia nf [ coleta o valor dos cartoes e subtrai do total da nf ]   """
				_rt, _vc = self.controlaRecebimentosCartao( 1, sql )
				self.meia_nf400 = _rt

				if _rt:

					dav_valorn = Decimal("0.00")
					dav_cartao = _vc #Decimal("0.00")
					dav_saldos = Decimal("0.00")

					dav_valorn  = self.cTDavs[0][37]

					dav_saldos  = ( dav_valorn - dav_cartao )
					self.vin.SetLabel( 'Cartão: '+format( dav_cartao,',')+'  Saldo: '+format( dav_saldos,',' ) )
					if dav_cartao > dav_valorn:
						
						self.valorsuperior = True
						self.orc.SetForegroundColour("#B82626")
						self.vin.SetForegroundColour("#B82626")
...
#---------: Parametros do sistema { dados adicionais }
#			if sql[2].execute("SELECT pr_nfrd FROM parametr WHERE pr_regi=1"):
				
#				dados_rodape = sql[2].fetchone()[0]
#				if dados_rodape:
				
#					self.dadosA = ''
#					for da in dados_rodape.split('\n'):
						
#						if da:	self.dadosA +=da +'|'
#					if self.dadosA:	self.dadosA = '|'+ self.dadosA
...
#-----------// Atualiza dados //
			...
			if self.identifca == "RMA":
				...
				if self.identifca == 'POS' and  not self.cTDavs[0][74]:	self.eminfe.Disable()
	...
	def envioEmissao(self,event):
		...
		if nfe1 == False or nfe2 == False:
			...
			if os.path.exists(certificaoArquivo) !=True or al[6] == "":
				...
				elif self.nfversao == "4.00":	NFE40Evc.nfe40( self, tipo_emissao = tEmissao )
