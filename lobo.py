...
from eletronicos.notafiscal400 import PreenchimentoNotaFiscal
...
class DadosCertificadoRetornos:
	...
	def dadosIdentificao(self,filial, modelo=55):
...
#---{ Dados-Certificados Retornos SEFAZ }--------------------------// Visualizar retornos
	...
	def gravarRetorno(self, modelo, serie, nota, ambiente, filial):
		...
		NotaFiscalParametros.gravar_envio_retorno = __d
...
class LykosEmissaoNFe:
	...
	def emissaoNFe400( self, parent = '', filial = '', modelo = 55, tipo_dav = '1', tipo_emissao = '1', tipo_impressao = '0', dados={}):
		
		self.p = parent

		"""  IDENTIFICACAO DA NOTA FISCAL  """
		"""  emissao { 1-Nota fiscal de vendas, 2-Nota fiscal de devolucao de vendas, 3-Nota fiscal de devolucao de compras RMA }  """
		retorno, uf, ambiente, serie_nfe, serie_nfce, id_csc, numero_csc, aplicativo, regime = informes.dadosIdentificao( filial, modelo=modelo)
		if not retorno: return
		serie = serie_nfe if modelo == 55 else serie_nfce

		retorno, numero_nota = numeros.VerificaNFE( self.p.numero_dav, Tnf = tipo_dav, Filial = filial ) #-// Numero de NF gerado ou utilizando um numero ja gerado
		...
		forma_emissao_nota  = str( tipo_emissao )
		codigo_municipio_ocorrencia = str( CODIGOS_UF[uf] )

		chave = codigo_municipio_ocorrencia + ano_mes_compoe_nota + documento_emissor + modelo_nota_fiscal + numero_serie_nota +\
				numero_nota_fiscalf + forma_emissao_nota + numero_compoe_chave
		...
		("nNF",numero_nota_fiscal), #--// Número do Documento Fiscal
		...
		("tpImp",tipo_impressao), #--// Formato de impressão do DANFE (0-sem DANFE;1-DANFe Retrato; 2-DANFe Paisagem;3-DANFe Simplificado; 4-DANFe NFC-e;5-DANFe NFC-e em mensagem eletrônica)
		("tpEmis",tipo_emissao), #--// Forma de emissão da NF-e 1 - Normal; 2 - Contingência FS 3 - Contingência SCAN 4 - Contingência DPEC 5 - Contingência FSDA 6 - Contingência SVC - AN 7 - Contingência SVC - RS 9 - Contingência off-line NFC-e
		...
		identificacao = nfe400.identificacaoNFe(ide)

		"""  Identificacao do emitente da nota fiscal  """
		ide_dados = OrderedDict([
		("CNPJ",""),
		("CPF",""),
		("xNome",""),
		("xFant","")
		...
		ide_documentos = OrderedDict([
		("IE",""),
		("IEST",""), #--// Inscricao Estadual do Substituto Tributário
		("CRT","") #--// Código de Regime Tributário. Este campo será obrigatoriamente preenchido com: 1 – Simples Nacional; 2 – Simples Nacional – excesso de sublimite de receita bruta; 3 – Regime Normal.
		])
	...
	def parametrosEmitente(self, filial):
