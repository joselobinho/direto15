...
class PlanoContas(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		if self.TipoAcesso.upper() == "CONSULTA":
			...
			wx.Frame.__init__(self, parent, id, 'Plano de Contas { Gerênciador-Consulta }', size=(730,330), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
			
		else:	wx.Frame.__init__(self, parent, id, 'Plano de Contas { Gerênciador }', size=(730,546), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		if self.TipoAcesso.upper() == "CONSULTA":
			...
			__plano = wx.StaticText(self.painel,-1,u"P l a n o de C o n t a s", pos=(10,500))
			__plano.SetFont(wx.Font(30,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
			__plano.SetForegroundColour('#BFBFBF')
			...
			wx.StaticText(self.painel,-1,u"5-Sub-Conta", pos=(430,500)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1,u"Descrição da Nova Conta [ Categoria ]", pos=(503,380)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1,u"Descrição da Nova Conta [ Titulo ]", pos=(503,420)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1,u"Descrição da Nova Conta [ Lançamentos - 1 ]", pos=(503,460)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1,u"Descrição da Nova Conta [ Lançamentos - 2 ]", pos=(503,500)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			...
			self.nvConta5 = wx.TextCtrl(self.painel,-1,'',pos=(427,513),size=(65,22))
			...
			self.ndConta5 = wx.TextCtrl(self.painel,-1,'',pos=(500,513),size=(224,22))
			...
			self.nvConta5.SetBackgroundColour('#E5E5E5')
			self.ndConta5.SetBackgroundColour('#E5E5E5')
			...
			self.ndConta5.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
			...
			self.nvConta5.SetMaxLength(3)
			self.ndConta5.SetMaxLength(60)
	...
	def aplicacoes(self,event):
		...
		if sql[0] == True:
			...
			_appen2 = _appen3 = _appen4 = _append5 = ""
			...
			for i in result:
				...
				elif _cns[0] == "4": #//----: Despesas
					if _n02 and despesas and _n03 == '' and _n04 == '' and _n05 == '':	_appen2 = self.tree.AppendItem(despesas, str(_cns[0])+'.'+str(_cns[1])+' - '+str(i[2])+':    {'+_reg+'}')
					if _n03 and _appen2 and _n04 == '' and _n05 == '':	_appen3 = self.tree.AppendItem(_appen2 , str(_cns[0])+'.'+str(_cns[1])+'.'+str(_cns[2])+' - '+str(i[3])+':    {'+_reg+'}')
					if _n04 and _appen3 and _n05 == '':	_appen4 = self.tree.AppendItem(_appen3 , str(_cns[0])+'.'+str(_cns[1])+'.'+str(_cns[2])+'.'+str(_cns[3])+' - '+str(i[4])+':    {'+_reg+'}')
					if _n05 and _appen4:	_appen5 = self.tree.AppendItem(_appen4 , str(_cns[0])+'.'+str(_cns[1])+'.'+str(_cns[2])+'.'+str(_cns[3])+'.'+str(_cns[4])+' - '+str(i[5])+':    {'+_reg+'}')
	...
	def AlterarConta(self,event):
		...
		_s5 = str( self.nvConta5.GetValue() )
		...
		if _s5 !='' and _s5.isdigit() == True and _d5 !='':	_cT5 = _cp+'.'+_s2+'.'+_s3+'.'+_s4.zfill(3)+'.'+_s5.zfill(3)

		if _cT2 !='':	_alTeracao = '2'	
		if _cT3 !='':	_alTeracao = '3'	
		if _cT4 !='':	_alTeracao = '4'	
		if _cT5 !='':	_alTeracao = '5'	
		...
		if _alTeracao != '' and _rg != '':
			...
			if _alTeracao == '5':	_numeroconta = _cT5+"-"+_d5
			...
			if segui.ShowModal() ==  wx.ID_YES:
				...
				if sql[0] == True:
					...
					if _alTeracao == '5':	alTerar = "UPDATE plcontas SET pl_nconta=%s,pl_dcont5=%s WHERE pl_regist=%s"
					...
					try:
						...
						lista_atualizadas +=EMD+'|'+numero_conta+' '+numero_subconta+' Descricao: '+_d2+'-'+_d3+'-'+_d4+'-'+_d5+'|'+nConTa+'|'+dConTa+str( _rg )+ '\n'
	...
	def IncluirConta(self,event):
		...
		if _s2 !='' and _s3 !='' and _s4 !='' and _s5 !='' and _s5.isdigit() == True:	_cT5 = _cp+'.'+_s2+'.'+_s3+'.'+_s4.zfill(3)+'.'+_s5.zfill(3)
		
		if _s2 !='' and _d2 =='':	_vl = True
		if _s3 !='' and _d3 =='':	_vl = True
		if _s4 !='' and _d4 =='':	_vl = True
		if _s5 !='' and _d5 =='':	_vl = True
		...
		if _s2 !='' and _cp and _d2 =='':	_vl = True
		if _s3 !='' and _s2 and _d3 =='':	_vl = True
		if _s4 !='' and _s3 and _d4 =='':	_vl = True
		if _s5 !='' and _s4 and _d5 =='':	_vl = True

		if _vl == True:	alertas.dia(self.painel,u'Nº de Conta com Descrição e/ou código da conta Vazia...\n'+(" "*100),"Plano de Contas: Inclusão")
		else:
			...
			if sql[0] == True:
				...
				if _cT5 !='':	acT5 = sql[2].execute("SELECT * FROM plcontas WHERE pl_nconta='"+str(_cT5)+"'")
				...
				if Adicion != '':
					...
					if segui.ShowModal() ==  wx.ID_YES:
						...
						try:
							...
							_conTa5 = "INSERT INTO plcontas (pl_nconta,pl_dcont5) VALUES(%s,%s)"
							...
							if _ic5 == True:	sql[2].execute(_conTa5,(_cT5,_d5))
	...
	def OnSelChanged(self, event):
		...
		if self.TipoAcesso.upper() == "CONSULTA":
			...
			if _quant == 1:
				...
				self.nvConta5.SetValue('')
				self.ndConta5.SetValue('')
			...
			if _quant == 5:
				...
				self.nvConta3.SetValue(_selec[2])
				self.nvConta4.SetValue(_selec[3])
				self.nvConta5.SetValue(_selec[4])
				self.ndConta5.SetValue(_conta[1].strip())
			...
			if _quant == 5:	self.ndConta5.SetBackgroundColour('#BFBFBF')
			
	def exporTarConta(self,event):
		
		if self.cConta.GetValue() == '':	alertas.dia(self.painel,u"Selecione uma sub-conta valida!!\n"+(" "*80),"Plano de Contas")
		else:
			numero_conta=3 if len(login.filialLT[login.identifi][35].split(";"))>=142 and login.filialLT[login.identifi][35].split(";")[141]=="T" else 4
			if len(self.cConta.GetValue().split('.')) < numero_conta:	alertas.dia(self.painel,u"Utilize uma conta para identificar o lançamento\n\n1-Contas de titulos não são validads para lancamentos\n"+(" "*160),"Plano de Contas")
			else:
	...
	def desenho(self,event):
		...
		if self.TipoAcesso.upper() == "CONSULTA":
			...
			dc.DrawRoundedRectangle(425,498,300,40,   3) #-->[ Lykos ]
