...
#		pD = wx.TextCtrl(self.painel,-1,value = str( self.produT ), pos=(190,210), size=(540,18),style=wx.TE_READONLY)
		pD = wx.TextCtrl(self.painel,-1,value = self.produT, pos=(190,210), size=(540,18),style=wx.TE_READONLY)
	...
	def analisaCurva(self, sql, parent, TP ):
		...
		if _car > 0:
			...
			for i in _rca:
				...
				""" Limpa os dados anteriores de curva abc para produtos sem movimento atual """
				if not codigo_verificado and len(login.filialLT[ self.p.pRFilial ][35].split(";")) >= 172 and login.filialLT[ self.p.pRFilial ][35].split(";")[171]=="T":

				    sql[2].execute("UPDATE produtos SET pd_dabc='' WHERE pd_codi='"+ i[2] +"'")
...
class TelaLisTaPrecoSeparado(wx.Frame):
	...
	def sair(self,event):	self.Destroy()
	def evTRadio(self,event):	nF.calcularProduto( 0, self, Filial = self.p.ppFilial, mostrar = True, retornar_valor = False, custo=(False,'') )
	def levantar(self):
	...
	def Tvalores(self,valor,idfy):
		...
		try:
			...
			nF.calcularProduto( idfy, self, Filial = self.p.ppFilial, mostrar = True, retornar_valor = False, custo=(False,'') )
	...
	def Teclas(self,event):
		...
		nF.calcularProduto( __id, self, Filial = self.p.ppFilial, mostrar = True, retornar_valor = False, custo=(False,'') )
...
class TrocaCodigosFiscais(wx.Frame):
	...
	def __init__(self,parent, id):
		...
		wx.Frame.__init__(self, parent, id, "Ajustes: { Substituição de códigos fiscais }", size=( 450,400 ), style = wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self, style=wx.BORDER_SUNKEN)
		self.painel.Bind(wx.EVT_PAINT,self.desenho)
...
#---------: Codigos Fiscais
		...
		self.copiacod=wx.CheckBox(self.painel, -1,  "Copiar codigos fiscais do campo codigo fiscal-primario\npara o campo codigo fiscal-regime normal", pos=(13,358))
		if login.usalogin.upper()!='LYKOS':	self.copiacod.Enable(False)
		...
		self.copiacod.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
	...
	def gravarCodigos(self,event):
		...
		if not continuar and not self.copiacod.GetValue():
		...
		if recalcular.ShowModal() ==  wx.ID_YES:
			...
			if sql[0]:
				...
				try:
					...
					for i in [] if not sql[2].execute("SELECT * FROM produtos ORDER BY pd_cfis") else sql[2].fetchall():
						...
						if compara:
							...
							grva4 = "UPDATE produtos SET pd_cfsc='"+str( i[47] )+"' WHERE pd_regi='"+str( nregistro )+"'"
							
							atua +="AN|"+str( i[1] )+"|"+str( i[2] )+"|"+str( i[3] )+"|"+str( i[47] )+"|"+str( i[82] )+"|"+str( i[53] )+"|"+str( i[8] )+"|"+str( i[61] )+"|"+str( i[62] )+"\n"

							if self.acionarg.GetValue():
								
								if not self.copiacod.GetValue() and self.primario.GetValue():
								    sql[2].execute( grva1 )
								if not self.copiacod.GetValue() and self.rgnormal.GetValue():
								    sql[2].execute( grva2 )

								if not self.copiacod.GetValue() and self.emisnfce.GetValue():
								    sql[2].execute( grva3 )
								    
								if self.copiacod.GetValue() and i[47]:
								    sql[2].execute(grva4)
														
							if not self.copiacod.GetValue():	codigos_alterados += "Sequencia: "+str( muda + 1 ).zfill(4)+"  Anterior: "+ncm1+'.'+cfo1+'.'+cst1+'.'+icm1+"  Substituido: "+codigo_fiscal+"  Produto: "+ i[2] +" - "+ i[3].decode("UTF-8") +"\n"
							if self.copiacod.GetValue() and i[47]:	codigos_alterados += "Regime simples: "+str( i[47] )+"  Copiado para o regime normal Produto: "+ i[2] +" - "+ i[3].decode("UTF-8") +"\n"
				...
				if muda and atua and ggva:
					...
					if not self.acionarg.GetValue():	codigos_alterados = "["+str(muda)+"] { TESTE,TESTE,TESTE,TESTE,TESTE,TESTE }\n\n"+codigos_alterados
	...
	def desenho(self,event):
		...
		dc.DrawRotatedText("Muda", 0, 396, 90)
...
class GerenciadorGrupos(wx.Frame):
	...
	def __init__(self, parent,id,ajustes=None):

		self.p = parent
		self.a = ajustes

		wx.Frame.__init__(self, parent, 209, 'Produtos: Gerenciador de grupos-fabricantes ', size=(530,637), style = wx.CAPTION|wx.CLOSE_BOX|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self)
		self.Bind(wx.EVT_CLOSE, self.sair)

		self.gruposub = wx.ListCtrl(self.painel, 209,pos=(15,1), size=(514,300),
									style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.gruposub.Bind(wx.EVT_LIST_ITEM_ACTIVATED,self.ajustesPrecos)
		...
		wx.StaticText(self.painel,-1,u"Click duplo para ajustes de preços [Conferência]", pos=(15,302) ).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		contagem=wx.StaticText(self.painel,-1, u"Opções exclusiva para contagem de estoque c/loja aberta", pos=(40,440))
		contagem.SetFont(wx.Font(12,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		contagem.SetForegroundColour('#7F7F7F')
		...
		#wx.StaticText(self.painel,-1, "{ Marcar-Desmacarca os produtos de grupos,fabricantes ou endereco para venda }", pos=(54,442) ).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.vender=wx.CheckBox(self.painel, -1 , "Bloqueio para vender",(4,464))
		self.negativo=wx.CheckBox(self.painel, -1 , "Permissão para vender negativo",(4,489))
		self.vender.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.negativo.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))

		self.avancar=GenBitmapTextButton(self.painel,800,label=u'   { Gravar } Bloqueio para vender\n   { Gravar } Permissão para vender negativo', pos=(255,464),size=(270,47), bitmap=wx.Bitmap("imagens/devolv.png", wx.BITMAP_TYPE_ANY))
		self.avancar.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.avancar.SetBackgroundColour('#528494')

		self.zerar_estoque=GenBitmapTextButton(self.painel,801,label=u'   Zerar estoque fisico e reserva do tipo selecionado { Apenas de produtos bloqueado para vender }', pos=(4,524),size=(522,27), bitmap=wx.Bitmap("imagens/devolv.png", wx.BITMAP_TYPE_ANY))
		self.zerar_estoque.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.zerar_estoque.SetBackgroundColour('#A6CFA6')

		self.todos_negativo=GenBitmapTextButton(self.painel,802,label=u'   Marcar/Desmarcar todos os produtos para vender com estoque negativo { Todas as filiais }', pos=(4,564),size=(522,27), bitmap=wx.Bitmap("imagens/estornop.png", wx.BITMAP_TYPE_ANY))
		self.todos_negativo.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.todos_negativo.SetBackgroundColour('#E5E5A8')

		self.zerar_local=GenBitmapTextButton(self.painel,803,label=u'   Zerar o estoque fisico local { estoque local da filial selecionada p/todos os produtos bloqueados }', pos=(4,604),size=(522,27), bitmap=wx.Bitmap("imagens/reler16.png", wx.BITMAP_TYPE_ANY))
		self.zerar_local.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.zerar_local.SetBackgroundColour('#B3B3E1')

		habilitar=True if len( login.usaparam.split(";") ) >=31 and login.usaparam.split(";")[30] == "T" else False
		self.vender.Enable(habilitar)
		self.negativo.Enable(habilitar)
		self.avancar.Enable(habilitar)
		self.zerar_estoque.Enable(habilitar)
		self.todos_negativo.Enable(habilitar)
		...
		self.avancar.Bind(wx.EVT_BUTTON, self.venderNaoVender)
		self.zerar_estoque.Bind(wx.EVT_BUTTON, self.venderNaoVender)
		self.todos_negativo.Bind(wx.EVT_BUTTON, self.venderNaoVender)
		self.zerar_local.Bind(wx.EVT_BUTTON, self.venderNaoVender)
		self.levantarGrupos(wx.EVT_BUTTON)
		
	def sair(self,event):	self.Destroy()
	def venderNaoVender(self,event):

	    if self.gruposub.GetItemCount():
		
		indice=self.gruposub.GetFocusedItem()
		tipo=self.gruposub.GetItem(indice,1).GetText().upper()
		tian=self.gruposub.GetItem(indice,2).GetText() #--// Grupo,Fabricante endereco
		
		saida1=u'{ Bloqueio/Desbloqueio de produtos para vender e Permissão para vender negativo }\n\n'
		saida2=u"Produtos: Bloqueio para vender e vender negativo"
		if event.GetId()==801:	saida1=u'{ Zerar o estoque fisico e estoque de reserva }\n\n'
		if event.GetId()==801:	saida2=u'Produtos: zerar o estoque fisico e estoque de reserva'
		if event.GetId()==802:	saida1=u'{ Permissão para vender negativo para todos os produtos }\n\n'
		if event.GetId()==802:	saida2=u'Produtos: Permissão para vender negativo'
		if event.GetId()==803:	saida1=u'Produtos: Zerar o estoque fisico local\n\n'
		if event.GetId()==803:	saida2=u'Para todos os produtos da filial selecionada'

		if tipo=='G':	__t='Grupo { '+tian+' }'
		elif tipo=='1':	__t='Sub-grupo 1: { '+tian+' }'
		elif tipo=='2':	__t='Sub-grupo 2: { '+tian+' }'
		elif tipo=='F':	__t='Fabricante: { '+tian+' }'
		elif tipo=='E':	__t='Endereco: { '+tian+' }'
		else:	__t='V a z i o'
		if event.GetId()==803:	__t+=' Filial['+self.p.ppFilial+']'

		if event.GetId() in [802]:
		    self.vender.SetValue(False)
		    __t='Todos os produtos'
		
		recalcular = wx.MessageDialog(self,saida1 + u"Ajuste de produtos p/"+__t+u"\n\nConfirme para contiuar\n"+(" "*180),saida2,wx.YES_NO|wx.NO_DEFAULT)

		if recalcular.ShowModal() ==  wx.ID_YES:

		    conn=sqldb()
		    sql=conn.dbc("Produtos: Bloqueio/Desbloqueio para vender", fil = self.p.ppFilial, janela = self )
		    if sql[0]:

			if event.GetId()==802:
			    
			    ng=sql[2].execute("SELECT pd_regi,pd_para,pd_codi FROM produtos")
			    if ng:
				
				for i in sql[2].fetchall():

				    lista=self.ajusteParametros(i,True)

				    __lista=''
				    for l in lista:
					__lista+=l+'|'

				    sql[2].execute('UPDATE produtos SET pd_para="'+ __lista[:-1] +'" WHERE pd_regi="'+ str(i[0]) +'"')

				sql[1].commit()
				
			    conn.cls(sql[1])
			    alertas.dia(self,'Processo finalizado com sucesso...\n' + (' '*160),u'Produtos: Permissão para vender negativo')
			    
			else:
			    ng=''
			    if tipo=='G':	ng=sql[2].execute("SELECT pd_regi,pd_para,pd_codi FROM produtos WHERE pd_nmgr='"+ tian +"'")
			    if tipo=='1':	ng=sql[2].execute("SELECT pd_regi,pd_para,pd_codi FROM produtos WHERE pd_sug1='"+ tian +"'")
			    if tipo=='2':	ng=sql[2].execute("SELECT pd_regi,pd_para,pd_codi FROM produtos WHERE pd_sub2='"+ tian +"'")
			    if tipo=='F':	ng=sql[2].execute("SELECT pd_regi,pd_para,pd_codi FROM produtos WHERE pd_fabr='"+ tian +"'")
			    if tipo=='E':	ng=sql[2].execute("SELECT pd_regi,pd_para,pd_codi FROM produtos WHERE pd_ende='"+ tian +"'")
				...
				for i in sql[2].fetchall():
					...
					lista=self.ajusteParametros(i,False)
					...
					sql[2].execute('UPDATE produtos SET pd_para="'+ __lista[:-1] +'" WHERE pd_regi="'+ str(i[0]) +'"')
				    elif event.GetId() in [801,803]:
					
					"""  Pesquisando na tabela de pedidos temporarios  """
					if sql[2].execute("SELECT tm_codi,tm_quan,tm_fili FROM tdavs WHERE tm_codi='"+i[2] +"'"):
					    for ac in sql[2].fetchall():

						lista_produtos+='Modulo de vendas { Reserva-Temporario } Quantidade: '+str( ac[1] )+'  Codigo: '+ac[0]+' Filial: '+ac[2]+'\n'
					
					if i[1] and len(i[1].split('|')) >= 12 and i[1].split('|')[11]=='T':	lista_zerar_estoque.append(i[2])
					else:	lista_produtos+='Modulo de produtos {Nao bloqueado} Produto: '+i[2]+'\n'
					
				if event.GetId()==800:	sql[1].commit()

			    if event.GetId() in [801,803] and not lista_produtos and lista_zerar_estoque:

				lista_guarda=''
				for z in lista_zerar_estoque:
				    
				    if sql[2].execute('SELECT * FROM estoque WHERE ef_codigo="'+ z +'"'):
					
					for e in sql[2].fetchall():
					    lista_guarda+=tian+'|'+e[1]+'|'+e[2]+'|'+e[3]+'|'+str(e[4])+'|'+str(e[5])+'|'+str(e[6])+'|'+str(e[7])+'|'+e[8]+'\n'
				    
					if   event.GetId()==801:	sql[2].execute("UPDATE estoque SET ef_fisico='0',ef_virtua='0' WHERE ef_codigo='"+ z +"'")
					elif event.GetId()==803:	sql[2].execute("UPDATE estoque SET ef_esloja='0' WHERE ef_idfili='"+self.p.ppFilial+"' and ef_codigo='"+ z +"'")
				
				sql[1].commit()
				
				if lista_guarda:
				    
				    hora=datetime.datetime.now().strftime("%d_%m_%Y_%T")+'_'+login.usalogin.lower()
				    arquivo=diretorios.logsPsT+'zerar_estoque_'+hora+'.txt'
				    
				    arquivo_zerar="\n{ Arquivo log } \n"+arquivo

				    __arquivo = open(arquivo,"w")
				    __arquivo.write(lista_guarda)
				    __arquivo.close()
				    
			    conn.cls(sql[1])
			    
			    if ng and event.GetId()==800:	alertas.dia(self,'Processo finalizado com sucesso...\n' + (' '*160),'Bloqueio p/vender, não vender e permissao para vender negativo')
			    if ng and event.GetId()in [801,803] and not lista_produtos:	alertas.dia(self,'Processo finalizado com sucesso...\n'+ arquivo_zerar +'\n'+(' '*170),'Zerar estoque fisico/reserva')
			    if ng and event.GetId() in [801,803] and lista_produtos:
				
				MostrarHistorico.hs = '{ Processo nao finalizado }\n\n'+lista_produtos
						    
				MostrarHistorico.TP = ""
				MostrarHistorico.TT = "Zerar estoque fisico/reserva"
				MostrarHistorico.AQ = ""
	...
	def ajustesPrecos(self,event):

	    if self.endere.GetValue():	alertas.dia(self,"Endereço não permitido...\n"+(130*' '),"Ajustes")
	    else:
		self.ajfabri = self.fabric
		self.ajgrupo = self.grupos
		self.ajsubg1 = self.grupo1
		self.ajsubg2 = self.grupo2
		self.precos = self.dapagar

		self.a.npedido = ""
		self.a.modulos = "AJUSTES"
		self.a.filialP = self.p.ppFilial
			    
		prc_frame=self.a(parent=self,id=-1)
		prc_frame.Centre()
		prc_frame.Show()
	    
	def ajusteParametros(self, i, negativo):

	    lista=[]
	    if i[1] and len(i[1].split('|'))>=1 and i[1].split('|')[0]:	lista.append(i[1].split('|')[0])
	    else:	lista.append('PIS:;0.0')

	    if i[1] and len(i[1].split('|'))>=2 and i[1].split('|')[1]:	lista.append(i[1].split('|')[1])
	    else:	lista.append('COF:;0.0')

	    if i[1] and len(i[1].split('|'))>=3 and i[1].split('|')[2]:	lista.append(i[1].split('|')[2])
	    else:	lista.append('F')

	    if i[1] and len(i[1].split('|'))>=4 and i[1].split('|')[3]:	lista.append(i[1].split('|')[3])
	    else:	lista.append('0.0')

	    if i[1] and len(i[1].split('|'))>=5 and i[1].split('|')[4]:	lista.append(i[1].split('|')[4])
	    else:	lista.append('0.0')
	    
	    if i[1] and len(i[1].split('|'))>=6 and i[1].split('|')[5]:	lista.append(i[1].split('|')[5])
	    else:	lista.append('0.0')

	    if i[1] and len(i[1].split('|'))>=7 and i[1].split('|')[6]:	lista.append(i[1].split('|')[6])
	    else:	lista.append('0.00')

	    if i[1] and len(i[1].split('|'))>=8 and i[1].split('|')[7]:	lista.append(i[1].split('|')[7])
	    else:	lista.append('F')
		
	    if i[1] and len(i[1].split('|'))>=9 and i[1].split('|')[8]:	lista.append(str(self.negativo.GetValue())[:1])
	    else:	lista.append(str(self.negativo.GetValue())[:1])

	    if i[1] and len(i[1].split('|'))>=10 and i[1].split('|')[9]:	lista.append(i[1].split('|')[9])
	    else:	lista.append('')

	    if i[1] and len(i[1].split('|'))>=11 and i[1].split('|')[10]:	lista.append(i[1].split('|')[10])
	    else:	lista.append('0.0')

	    if negativo:
		
		if i[1] and len(i[1].split('|'))>=12 and i[1].split('|')[11]:	lista.append(i[1].split('|')[11])
		else:	lista.append('F')

	    else:
		if i[1] and len(i[1].split('|'))>=12:	lista.append(str(self.vender.GetValue())[:1])
		else:	lista.append(str(self.vender.GetValue())[:1])
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(1, 460, 528, 55, 3) #-: Dados da NFE
		dc.DrawRoundedRectangle(1, 520, 528, 35, 3) #-: Dados da NFE
		dc.DrawRoundedRectangle(1, 560, 528, 35, 3) #-: Dados da NFE
		dc.DrawRoundedRectangle(1, 600, 528, 35, 3) #-: Dados da NFE
...
class CalcularConversao:
	...
	def calcularEmbalagensAutomatico( self, p, dados, gravar, tipo=1, calculo='1', valorTotal='0', unidade='UN' ):
		...
		if tipo==2 and unidade in ['KG','T','TON','TN','TO']: #--// Calculo de vara de ferro transforma de kg/Tonelada para unidade de vara

		    if unidade in ['T','TON','TN','TO']:	quantidade_compras=(quantidade_compras*1000) #--// Quando a compra for em tonelas { transforma em quilos }
		    quantidade_manual = ( quantidade_compras /quantidade_embalagens )
		    
		    if   calculo=='1':	quantidade_manual=Decimal( str(quantidade_manual).split('.')[0]+'.0000')
		    elif calculo=='2':	quantidade_manual=Decimal( format(quantidade_manual,'.4f') )
		    elif calculo=='3':	quantidade_manual=Decimal( format(quantidade_manual,'.0f')+'.0000' )

		    valor_manual = Trunca.trunca( 5, ( Decimal(valorTotal) / quantidade_manual ) )
		    
		else:
		    if tipo!=2:	quantidade_manual = ( quantidade_embalagens * quantidade_compras )
		    else:	quantidade_manual= valor_manual=Decimal()
		...
		percentual_frete = Decimal(p.ListaCMP.GetItem(indice_lista_compras,43).GetText()) if p.ListaCMP.GetItem(indice_lista_compras,43).GetText() else Decimal('0')
		valor = Decimal(p.ListaCMP.GetItem(indice_lista_compras,77).GetText()) if p.ListaCMP.GetItem(indice_lista_compras,77).GetText() else Decimal('0')

		""" Calculo do frete por valor unitario para produtos por embalagens """
		if gravar and percentual_frete and valor:
		    valorunita_frete = Trunca.arredonda(4, ( Decimal( valor ) * percentual_frete / 100 ) )
		    if valorunita_frete:	p.ListaCMP.SetStringItem( indice_lista_compras ,110, str( valorunita_frete ) )
