...
class danfeGerar:
	...
	def MontarDanfe(self, par, arquivo='', TexTo='', emails = [''], Filial = "", automatico = False, emissao='' ):
...
#-------: Endereco de Entrega
		...
		self.edcp,aT = self.XMLLeitura(doc,"entrega","CPF") #-----: CPF
...
#-------: Impostos
		...
		self.TvFC,aT = self.XMLLeitura(doc,"ICMSTot","vFCP")	#-:[ Valor do FCP ]
...
#-------: Classificar Produtos
		...
		self.pfc,aT = self.XMLLeitura(doc,"ICMS","pFCP")	#-:[ Percentual do FCP ]
		self.vfc,aT = self.XMLLeitura(doc,"ICMS","vFCP")	#-:[ Valor do FCP ]
...
#-------: Confecciona DANFE Vazio, Imprimir o cabecalho	
		...
		self.RelacionarProdutos(cv,Filial)
...
#		cv.drawString(472,465, self.converte( str( self.edce[0] ), 2) ) # : CEP
		...
		if self.edcu[0].strip():	cv.drawString(512,465, self.converte( str( self.edcu[0] ), 4) ) # : CNPJ/CPF
		if self.edcp[0].strip():	cv.drawString(512,465, self.converte( str( self.edcp[0] ), 4) ) # : CNPJ/CPF
#-------: Duplicata/Fatura
		if self.Dnum[0] !='':
...
#------: Dados Adicionais
		...
		if self.TvFC[0] and Decimal(self.TvFC[0]):	dd +='|Total do FCP: '+self.TvFC[0]
		...
		if self.pram[0] == "2" or self.prpr[0] == '':
			...
			if self.e == '3':	cv.drawString(200,90,"Nota fiscal cancelada")
			if self.e == '4':	cv.drawString(200,90,"Nota fiscal inutilizada")
			if self.e == '5':	cv.drawString(200,90,"Nota fiscal denegada")
			cv.rotate(-45) 

	def RelacionarProdutos(self,cv, filial):
		...
		tru = truncagem()
		ite = 0
		dav = self.npd[0].split('-')[0].zfill(13)
		...
		for i in range( len( self.ccd ) ): 
			...
			if lin < 23 :	
				...
				if len( self.ivb ) >= ( int(i) + 1 ) and Decimal( str( self.ivb[i] ) ):	cv.drawRightString(477, cow, format( Decimal( str( self.ivb[i] ) ),',' ) )
				cv.setFont('Helvetica', 4.5)

				if len( self.ivi ) >= ( int(i) + 1 ) and Decimal( str( self.ivi[i] ) ):	cv.drawRightString(510, cow, format( Decimal( str( self.ivi[i] ) ),',' ) )
				if len( self.pvl ) >= ( int(i) + 1 ) and Decimal( str( self.pvl[i] ) ):	cv.drawRightString(537, cow, format( Decimal( str( self.pvl[i] ) ),',' ) )
				if len( self.ipe ) >= ( int(i) + 1 ) and Decimal( str( self.ipe[i] ) ):	cv.drawRightString(557, cow, format( tru.trunca( 3, Decimal( str( self.ipe[i] ) ) ),',' ) )
				...
				continuar_adicionais=True if ad and ad.split("|")[0] !="PC" else False
				if self.pfc[i] and self.vfc[i] and Decimal(self.pfc[i]) and Decimal(self.vfc[i]):	continuar_adicionais=True #--// FCP

				#if ad and ad.split("|")[0] !="PC": #29-05-2019
				if continuar_adicionais:
					
					""" Na sair dados adicionais se for COM,LARG,EXPE { clientes estava calculando e apurriando } """
					avancar_adicionais=True
					if 'COMP:' in ad or 'LARG:' in ad or 'EXPE:' in ad:
					    if self.xmlFilial and len(login.filialLT[ self.xmlFilial ][35].split(";"))>=148 and login.filialLT[ self.xmlFilial ][35].split(";")[147]=='T':	avancar_adicionais=False
					""" Sair o FCP """
					if self.pfc[i] and self.vfc[i] and Decimal(self.pfc[i]) and Decimal(self.vfc[i]):
					    
					    if ad:	ad+='\nBase FCP: '+self.ivb[i]+' Percentual do FCP: '+self.pfc[i]+'% Valor FCP: '+self.vfc[i]
					    else:	ad='Base FCP: '+self.ivb[i]+' Percentual FCP: '+self.pfc[i]+'% Valor FCP: '+self.vfc[i]
					    
					if avancar_adicionais:
				...
				if lin == 22:
					...
					#print('-----1: ',lin,ds,pag,self.Tp)
					pag +=1
					if pag > self.Tp:	self.Tp=pag
					#print('-----2: ',lin,ds,pag,self.Tp)
					if pag <= self.Tp:
...
#-------: Local de Entrega
		...
		cv.drawString(512,475,"CPF/CNPJ")
...
class danfeCCe:
	...
	def cceDANFE( self, par, arqxml, danfe, _fil, _clie, _cnpj, _moTi, emails = [''] ):
		...
		nomeArquivo = diretorios.usPasta+str( danfe )+"-cce-"+_fil.lower()+".pdf"
		cv = canvas.Canvas(nomeArquivo, pagesize=A4)
		cv.setLineWidth(.3)
...
class DanfeMdfe:
    ...
    def gerarPdfMdfe(self,xml,filial,parent, abertura):

	""" Dados do XML """
	arquivo_xml=xml
	if abertura and xml:	doc=minidom.parse(xml)
	else:
	    doc=minidom.parseString(xml.encode('utf-8'))
	
	g=danfeGerar()
	_id,at01=g.XMLLeitura(doc,"infMDFe","Id") #----------: ID de Emissão 
	dta,at02=g.XMLLeitura(doc,"infProt","dhRecbto") #----------: ID de Emissão
	pro,at03=g.XMLLeitura(doc,"infProt","nProt") #----------: ID de Emissão
	
	amb,at11=g.XMLLeitura(doc,"ide","tpAmb") #----------: ID de Emissão 
	mod,at04=g.XMLLeitura(doc,"ide","mod") #----------: ID de Emissão
	ser,at05=g.XMLLeitura(doc,"ide","serie") #----------: ID de Emissão
	num,at06=g.XMLLeitura(doc,"ide","nMDF") #----------: ID de Emissão
	dhe,at08=g.XMLLeitura(doc,"ide","dhEmi") #----------: ID de Emissão
	ufc,at09=g.XMLLeitura(doc,"ide","UFIni") #----------: ID de Emissão
	ufd,at10=g.XMLLeitura(doc,"ide","UFFim") #----------: ID de Emissão

	qnf,at11=g.XMLLeitura(doc,"tot","qNFe") #----------: ID de Emissão
	qkg,at12=g.XMLLeitura(doc,"tot","qCarga") #----------: ID de Emissão

	pla,at13=g.XMLLeitura(doc,"veicTracao","placa")
	rnt,at14=g.XMLLeitura(doc,"prop","RNTRC")

	nom,at15=g.XMLLeitura(doc,"condutor","xNome")
	cpf,at16=g.XMLLeitura(doc,"condutor","CPF")
	ina,at16=g.XMLLeitura(doc,"infAdic","infCpl")
	qrc,at17=g.XMLLeitura(doc,"infMDFeSupl","qrCodMDFe") #------:[ Dados Adicionais Informacoes Comprlementares ]

	"""  Grava o XML para envio do email  """
	if not abertura:

	    arquivo_xml=diretorios.usPasta+at01+'-'+filial.lower()+".xml"
	    _arquivo=open(arquivo_xml, "w" )
	    _arquivo.write(xml.encode('utf-8'))
	    _arquivo.close()
	
	nomeArquivo=diretorios.usPasta+at01+'-'+filial.lower()+".pdf"
	cv = canvas.Canvas(nomeArquivo, pagesize=landscape(A4))
	cv.setLineWidth(.3)
		
	cv.roundRect(10, 470, 410, 115, 4, stroke=1,fill=0) #-: Quadro do emitente
	cv.roundRect(10, 417, 410, 50, 4, stroke=1,fill=0) #-: Quadro do modelo

	cv.roundRect(425, 555, 410, 30, 4, stroke=1,fill=0) #-: Quadro do DAMDFE
	cv.roundRect(425, 417, 410, 135, 4, stroke=1,fill=0) #-: Quadro do chave/Protocolo
	cv.rect(10,  10, 825, 400, fill=0) #-: Quadro inferior principal
	#cv.rect(780, 15, 50, 50, fill=0) #//--: QRCODE
	
	cv.line(755,552,755,478) #--// Linha divisao do qrcode
	cv.line(425,478,835,478) #--// Linha da chave de acesso
	cv.line(425,448,835,448) #--// Linha do protocolo

	cv.line(58,417,58,467) #--// Modelo
	cv.line(93,417,93,467) #--// Seriea do protocolo
	cv.line(138,417,138,467) #--// Linha do protocolo
	cv.line(178,417,178,467) #--// Linha do protocolo
	cv.line(286,417,286,467) #--// Linha do protocolo
	cv.line(346,417,346,467) #--// Linha do protocolo

	cv.line(10,390,835,390) #--// Linha do ModalRodoviario
	cv.line(10,350,835,350) #--// Linha do ModalRodoviario CIOT,CTe,CTRC,NFe,NF Peso
	cv.line(10,250,340,250) #--// Linha do Placa RTNRC
	cv.line(10,230,340,230) #--// Linha do vale pedagio
	cv.line(10,150,835,150) #--// Linha do vale pedagio

	cv.line(10,330,835,330) #--// Linha do veiculo,condutor
	cv.line(340,150,340,330)
	cv.line(200,250,200,330)

	cv.line(450,150,450,330) #--// Linha CPF,Nome do condutor

	cv.line(116,150,116,230) #--// Responsável CNPJ
	cv.line(226,150,226,230) #--// Fornecedor CNPJ - No Comprovante

	linha=145
	for i in range(5):
	    cv.line(linha,350,linha,390) #--// linhas de dados CIOT,CTe,CTRC,NFe,NF Peso
	    linha +=135

	cv.setFont('Helvetica-Bold',18)
	cv.drawString(430,563,"DAMDFE")
	cv.setFont('Helvetica',10)
	cv.drawString(515,566,"Documento Auxiliar de Manifesto Eletrônico de Documentos Fiscais")
	
	emi=login.filialLT[filial]
	
	"""  Dados do emitente  """
	logo=login.filialLT[filial][15].strip()
	if logo and os.path.exists("imagens/"+logo):	cv.drawImage(ImageReader("imagens/"+logo), 15,535, width=75, height=40)
	cv.setFont('Helvetica-Bold',12)
	cv.drawString(100,560,emi[14].title())

	cv.setFont('Helvetica',11)
	cv.drawString(100,540,"CNPJ: "+nF.conversao(emi[9],4)+'  IE: '+emi[11]+'  RNTRC: ')
	cv.drawString(100,525,emi[1].title())
	cv.drawString(100,510,emi[2].title()+'  No: '+emi[7])

	cv.setFont('Helvetica',10)
	cv.drawString(100,495,'Complemento: '+emi[8].title()+'  Bairro: '+emi[3].title()+'  UF: '+emi[6])
	cv.drawString(100,482,'Cidade:'+emi[4].title()+'  CEP: '+nF.conversao(emi[5],2))
		
	b128=code128.Code128(str( at01[4:] ), barHeight=45, barWidth=1.1,stop=1) if at01 and len(at01[4:])==44 else None
	if b128:	b128.drawOn(cv, 418, 485) #--------------------: Codigo de Barras
#	if b128:	b128.drawOn(cv, 445, 485) #--------------------: Codigo de Barras

	cv.setFont('Helvetica-Bold',12)
	cv.drawString(540,535,"CONTROLE DO FISCO")

	cv.setFont('Helvetica-Bold',9)
	cv.drawString(430,470,"Chave de Acesso")
	cv.drawString(430,438,"Protocolo de Autorização")
	
	if at01 and len(at01[4:])==44:
	    danf=''
	    for i in range( len( str( at01[4:] ) ) /3 ):
		danf += str( at01[4:] )[(i*4):( (i*4) +4 )]+" "
	    cv.setFont('Helvetica-Bold',14.6)
	    cv.drawString(430,453,danf)

	cv.setFont('Helvetica',12)
	if dta and pro and dta[0] and pro[0]:	cv.drawString(430,423,pro[0]+'     '+dta[0].split('T')[0].split('-')[2]+'/'+dta[0].split('T')[0].split('-')[1]+'/'+dta[0].split('T')[0].split('-')[0]+' '+dta[0].split('T')[1].split('-')[0])

	cv.setFont('Helvetica',11)
	cv.drawString(15, 453,"Modelo")
	cv.drawString(60, 453,u"Série")
	cv.drawString(95, 453,u"Número")
	cv.drawString(140,453,u"Folhas")
	cv.drawString(180,453,u"Data e Hora Emissão")
	cv.drawString(289,453,u"UF Carreg")
	cv.drawString(349,453,u"UF Descarreg")
	cv.setFont('Helvetica-Bold',10)

	cv.drawString(140, 435,"1/1")
	if mod and mod[0]:	cv.drawString(15, 435,mod[0])
	if ser and ser[0]:	cv.drawString(60, 435,ser[0])
	if num and num[0]:	cv.drawString(95, 435,str(int(num[0])))
	if dhe and dhe[0]:	cv.drawString(180,435,dhe[0].split('T')[0].split('-')[2]+'/'+dhe[0].split('T')[0].split('-')[1]+'/'+dhe[0].split('T')[0].split('-')[0]+' '+dhe[0].split('T')[1].split('-')[0])
	if ufc and ufc[0]:	cv.drawString(289,435,ufc[0])
	if ufd and ufd[0]:	cv.drawString(349,435,ufd[0])
	
	cv.setFont('Helvetica-Bold',11)
	cv.drawString(335,395,u"Modal Rodoviário de Carga")

	cv.setFont('Helvetica',11)
	cv.drawString(15, 376,"CIOT")
	cv.drawString(149,376,u"Quantidade CT-e")
	cv.drawString(284,376,u"Quantidade CTRC")
	cv.drawString(420,376,u"Quantidade NF-e")
	cv.drawString(554,376,u"Quantidade NF")
	cv.drawString(691,376,u"Peso Total (kg)")

	cv.setFont('Helvetica-Bold',10)
	if qnf and qnf[0]:	cv.drawString(420,360,qnf[0])
	if qkg and qkg[0]:	cv.drawString(691,360,qkg[0])

	cv.setFont('Helvetica-Bold',11)
	cv.drawString(15, 335,u"Veículo")
	cv.drawString(340,335,u"Condutor")
	cv.setFont('Helvetica',11)
	cv.drawString(15, 314,u"Placa")
	cv.drawString(204,314,u"RNTRC")
	cv.drawString(344,314,u"CPF")
	cv.drawString(454,314,u"Nome")

	cv.setFont('Helvetica-Bold',11)
	cv.drawString(15, 237,u"Vale Pedágio")
	cv.setFont('Helvetica',10)
	cv.drawString(15, 217,u"Responsável CNPJ")
	cv.drawString(120,217,u"Fornecedor CNPJ")
	cv.drawString(230,217,u"No Comprovante")

	cv.setFont('Helvetica-Bold',12)
	cv.drawString(15, 137,u"Observação")

	cv.setFont('Helvetica-Bold',10)
	if pla and pla[0]:	cv.drawString(15, 300,pla[0])
	if rnt and rnt[0]:	cv.drawString(204,300,rnt[0])

	if cpf:
	    linhas=300
	    for i in range(len(cpf)):
		cv.drawString(344,linhas,nF.conversao(cpf[i],4))
		cv.drawString(454,linhas,nom[i])
		linhas-=15
	
	cv.setFont('Helvetica',11)
	if ina and ina[0]:	cv.drawString(15, 120,ina[0]) #--// Informacoes complementares
	if amb and amb[0] and amb[0]=='2':
	    cv.setFillColor(HexColor('0xCD4646'))
	    cv.setFont('Helvetica-Bold', 30)
	    cv.rotate(45) 

	    cv.drawString(100,0, "Ambiente de Homologação { Sem Valor Fiscal }")
	    cv.rotate(-45) 

	"""  Impressao do QRCODE  """
	if at01 and len(at01[4:])==44 and qrc[0]:
	    qrw=QrCodeWidget( qrc[0] )
	    b=qrw.getBounds()

	    w=b[2]-b[0] 
	    h=b[3]-b[1] 

	    d=Drawing(100,100,transform=[80./w,0,0,80./h,0,0]) 
	    d.add(qrw)
	    renderPDF.draw(d, cv, 755, 475)
#	    renderPDF.draw(d, cv, 780, 15)
	
	cv.showPage()
	cv.save()
		
	gerenciador.Anexar = nomeArquivo,arquivo_xml
