...
from contacorrente  import ControlerConta,gravacaoLancamentos,LevantaBancos
...
lancamentocontacorrente = gravacaoLancamentos()
...
class contasApagar(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.formas_pagamentos.InsertColumn(0, 'Forma de Pagamento', width=260)
		self.formas_pagamentos.InsertColumn(1, 'Valor Pago',format=wx.LIST_ALIGN_LEFT,width=90)
		self.formas_pagamentos.InsertColumn(2, 'Instituicao Financeira', width=240)
		self.formas_pagamentos.InsertColumn(3, 'Nº Banco', width=80)
		self.formas_pagamentos.InsertColumn(4, 'Agência', width=80)
		self.formas_pagamentos.InsertColumn(5, 'Conta Corrente', width=140)
	...
	def APBaixa(self,event):
		...
		if self.Tsel.GetValue() !='' and self.Qsel.GetValue() !='':
		    ...
		    liquida_cheques, baixar_titulos, relacao_cheques=self.validarBaixaLiquidacao()
		    
		    if liquida_cheques and not baixar_titulos:

			historicos=u'\n\n{ Histórico de liquidação }\n'+self.HisT.GetValue() if self.HisT.GetValue() and self.HisT.GetValue().strip()!="[ Outros historicos ]" else ""
			MostrarHistorico.TP=u"LIQUIDACAO"
			MostrarHistorico.hs=u"Relação de cheques para liquidação\n"+('-'*115)+'\n'+relacao_cheques+('-'*115)+historicos
			MostrarHistorico.TT=u"Contas Apagar { Liquidar cheques }"
			MostrarHistorico.AQ=self.HisT.GetValue() if self.HisT.GetValue() and self.HisT.GetValue().strip()!="[ Outros historicos ]" else ""
			MostrarHistorico.GD=relacao_cheques
			MostrarHistorico.FL=self.flAp

			his_frame=MostrarHistorico(parent=self,id=-1)
			his_frame.Centre()
			his_frame.Show()

		    elif baixar_titulos and not liquida_cheques:
	...
	def liquidacaoCheques(self,filial,relacao):

	    grv=False
	    if relacao:

		conn=sqldb()
		sql=conn.dbc("Liquidacao de cheques", op=3, fil=filial, janela=self.painel )
		grv=False
		if sql[0]:

		    for i in relacao.split('\n'):
			if i:
			    
			    registro=str(int(i.split(' ')[0]))
			    ET='Liquidacao: '+datetime.datetime.now().strftime("%d/%m/%Y %T")+' '+login.usalogin
			    sql[2].execute("UPDATE individualcheque SET ac_liquid='1',ac_informd='"+ET+"' WHERE ac_regis='"+registro+"'")
	...
	def validarBaixaLiquidacao(self):
	    
	    l,b=False,False
	    r=''
	    if self.ListaApagar.GetItemCount():
		
		for i in range(self.ListaApagar.GetItemCount()):
		    
		    if self.ListaApagar.GetItem(i, 13).GetText().upper()=="LIQUIDAR":
			if len(self.ListaApagar.GetItem(i, 5).GetText().split('-'))==2 and self.ListaApagar.GetItem(i, 5).GetText().split('-')[1] in ['5','6']:

			    l=True

			    if self.ListaApagar.GetItem(i,31).GetText():
				t=self.ListaApagar.GetItem(i,31).GetText().split("|")
				if len(t)>=7:	r+=self.ListaApagar.GetItem(i, 0).GetText().split('-')[0]+' Banco: '+t[1][:25]+(' '*(25-len(t[1])))+' Conta: '+t[4][:10]+(' '*(10-len(t[4])))+' Numero Cheque: '+t[5]+(' '*(10-len(t[5])))+' Valor: '+t[6]+'\n'
			    
			else:	b=True

	    if l and b:	alertas.dia(self.painel,u"{ Utilize cheques e titulos do contas apagar separados para baixa }\n\n1-Controle individual de cheques apenas para liquidação\n"+(" "*160),"Contas Apagar { B a i x a-Liquidação }")
	    return l,b,r
	...
	def AlteraAp(self,event):
		...
		indice = self.ListaApagar.GetFocusedItem()
		if len(self.ListaApagar.GetItem(indice, 5).GetText().split('-'))>=2 and str(self.ListaApagar.GetItem(indice, 5).GetText().split('-')[1]) in ['5','6']:	pass
		else:
	...
	def marcar(self,event):
		...
		if self.ListaApagar.GetItem(registro_focu,37).GetText().strip()=='1':
		    alertas.dia(self.painel,u"Cheque selecionado ja foi liquidado...\n"+(" "*140),"Contas Apagar")
		    return
...
#------: Forma de Lançamento
		...
		if self.ListaApagar.GetItem( indice, 31 ).GetText():
			...
			indic = 0
			self.formas_pagamentos.SetBackgroundColour('#C2CEC2')
			self.formas_pagamentos.SetForegroundColour('#000000')
			for p in self.ListaApagar.GetItem(indice,31).GetText().split("\n"):

				if p:
					
					pg = {1:"Dinheiro",2:u"Pagamento eletrônico",3:u"Depósito",4:u"Transferência",5:"Cheque",6:"Cheque terceiros a receber",50:u"Liquidação contas a receber",51:u"Lançamento no contas areceber",52:"Baixa de credito conta corrente",53:u"Lançamento no contas areceber valor superior",60:u"Baixa com cheque da filial {Liquidação}",61:u"Baixa com cheque de terceiros {Liquidação}",62:u"Lançamento com cheque da filial {Liquidação}"}
					np = p.split('|')
					if len( np ) >= 8:
						...
						if int( np[0].split("PG:")[0] ) in [60,61,62]: #--// Controle individualizado do cheque

						    self.formas_pagamentos.SetBackgroundColour('#1A1A1A')
						    self.formas_pagamentos.SetForegroundColour('#0FAD0F')
						    if indic % 2:	self.formas_pagamentos.SetItemBackgroundColour(indic, "#4D4D4D")
...
#----------: Pesquisa apenas titulos vencidos
			...
			"""  Relacionar cheques predatados """
			cheques=0
			
			#if self.FGeral.GetValue():	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_vencim>='"+inicial+"' and ac_vencim<='"+final+"' ORDER BY ac_vencim")
			#elif self.FAbert.GetValue():	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_vencim>='"+inicial+"' and ac_vencim<='"+final+"' and ac_liquid='' ORDER BY ac_vencim")
			#elif self.FBaixa.GetValue():	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_vencim>='"+inicial+"' and ac_vencim<='"+final+"' and ac_liquid='2' ORDER BY ac_vencim")
			#else:	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_liquid='' ORDER BY ac_vencim")

			#if self.FGeral.GetValue():	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_vencim>='"+inicial+"' and ac_vencim<='"+final+"' ORDER BY ac_vencim")
			if not self.FCance.GetValue():
			    if not self.period.GetValue():	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_liquid='' ORDER BY ac_vencim")
			    elif self.FAbert.GetValue():	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_vencim<='"+final+"' and ac_liquid='' ORDER BY ac_vencim")
			    elif self.FBaixa.GetValue():	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_vencim>='"+inicial+"' and ac_vencim<='"+final+"' and ac_liquid='2' ORDER BY ac_vencim")
			    else:	cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_liquid='' ORDER BY ac_vencim")
			    
			    if cheques:	result_cheques=sql[2].fetchall()
			...
			for i in _resul:
				...
				_DTBai = _DTEmi = _Histo = _Cance = _Lista = _Confe = _avali = _pTerc = _DComp = ''

				if i[6]  !=None:	_DTEmi = i[6].strftime("%d/%m/%Y")+' '+str(i[7])
				if i[13] !=None:	_DTBai = i[13].strftime("%d/%m/%Y")+' '+str(i[14])
				if i[54] !=None:	_DComp = i[54].strftime("%d/%m/%Y")
			...
			if cheques:
			    for i in result_cheques:
				
				lan=str(i[0]).zfill(12)+'-'+i[8]
				relacao[_registros]=lan,'','',i[3].strftime("%d/%m/%Y")+' '+str(i[4])+' '+i[5],i[13].strftime("%d/%m/%Y"),'Cheque-'+i[8],i[2],format(i[6],','),format(i[6],','),\
				'','','','','','','','','','','','','','','','','','','','','','',i[11],'','','','','',i[9]
				indice     += 1
				_registros += 1
	...
	def MenuPopUp(self):
		...
		controle_contacorrente = True if len( login.usaparam.split(";") ) >=27 and login.usaparam.split(";")[26] == "T" else False
		self.popupmenu.Enable( id = 5037, enable = controle_contacorrente )
...
class ApagarCtrl(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.attr9.SetBackgroundColour("#E7E7E7")
		self.attr10.SetBackgroundColour("#D4C4C4")
		self.attr11.SetBackgroundColour("#FDF6F6")
		...
		self.InsertColumn(37,'Controle cheque vazio-aberto 1-liquidado', width=150)
		self.InsertColumn(38,'Data de competencia', width=150)
	...
	def OnGetItemAttr(self, item): #Ajusta cores sim/nao
		...
		index=self.itemIndexMap[item]
		if self.itemDataMap[index][5] and len(self.itemDataMap[index][5].split('-'))==2 and self.itemDataMap[index][5].split('-')[1] in ['5','6'] and self.itemDataMap[index][37]=="1":	return self.attr11
		if self.itemDataMap[index][13].upper()!="LIQUIDAR" and self.itemDataMap[index][5] and len(self.itemDataMap[index][5].split('-'))==2 and self.itemDataMap[index][5].split('-')[1] in ['5','6']:	return self.attr9
		if self.itemDataMap[index][13].upper()=="LIQUIDAR" and self.itemDataMap[index][5] and len(self.itemDataMap[index][5].split('-'))==2 and self.itemDataMap[index][5].split('-')[1] in ['5','6']:	return self.attr10
	...
	def OnGetItemImage(self, item):
		...
		if self.itemDataMap[index][5] and len(self.itemDataMap[index][5].split('-'))==2 and self.itemDataMap[index][5].split('-')[1] in ['5','6'] and self.itemDataMap[index][13].upper()=="LIQUIDAR":	return self.cheque3
		if self.itemDataMap[index][5] and len(self.itemDataMap[index][5].split('-'))==2 and self.itemDataMap[index][5].split('-')[1] in ['5','6'] and self.itemDataMap[index][37]=="1":	return self.cheque4
		if self.itemDataMap[index][5] and len(self.itemDataMap[index][5].split('-'))==2 and self.itemDataMap[index][5].split('-')[1]=='5':	return self.cheque1
		if self.itemDataMap[index][5] and len(self.itemDataMap[index][5].split('-'))==2 and self.itemDataMap[index][5].split('-')[1]=='6':	return self.cheque2
...
class baixarApagar(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.parcial_grupo = False
		self.incluir_titulos = True if len(login.filialLT[self.Filial][35].split(';')) >= 127 and login.filialLT[self.Filial][35].split(';')[126]=='T' else False
		...
		wx.Frame.__init__(self, parent, id, 'Contas APagar: Baixa de títulos', size=(920,595), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self,-1,style=wx.BORDER_SUNKEN)
		self.Bind(wx.EVT_CLOSE, self.sair)
		...
		self.ListafPaga.Bind(wx.EVT_LIST_ITEM_SELECTED,  self.passagem)
		...
		self.ListafPaga.InsertColumn(7, 'Banco: Lancamento automatico para o controle do conta corrente',   width=500)
		self.ListafPaga.InsertColumn(8, 'Vecimento cheque', width=200)
		...
		self.qtlanca = wx.StaticText(self.painel,-1,"Lançamentos:{ 000 }", pos=(352,397) )
		...
		wx.StaticText(self.painel,-1,"CHEQUE Vencimento:", pos=(18,235) ).SetFont(wx.Font(9, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1," Nº Cheque:", pos=(280,235) ).SetFont(wx.Font(9, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,u"Lista de bancos { Lançamentos automático no controle de conta corrente }", pos=(15,547) ).SetFont(wx.Font(9, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))

		lb = LevantaBancos()
		lista_bancos = lb.relacionarListasBancos( self.painel, modulo = 2 )

		self.pgTo = ['1-Dinheiro','2-Pagamento Eletrônico','3-Depósito','4-Transferência','5-Cheque','6-Cheque Terceiros']
		self.pgT = wx.ComboBox(self.painel, -1, '', pos=(15,320), size=(208,27), choices = self.pgTo,style=wx.NO_BORDER|wx.CB_READONLY)
		self.tlc = wx.ComboBox(self.painel, -1, '', pos=(15,362), size=(208,27), choices = login.TpDocume,style=wx.NO_BORDER|wx.CB_READONLY)
		self.cc_bancos = wx.ComboBox(self.painel, 533, '', pos=(13,560), size=(447,27), choices = lista_bancos,style=wx.NO_BORDER|wx.CB_READONLY)
		...
		adicio = wx.BitmapButton(self.painel, 200, wx.Bitmap("imagens/simadd20.png",   wx.BITMAP_TYPE_ANY), pos=(423, 415), size=(36,31))
		apagar = wx.BitmapButton(self.painel, 201, wx.Bitmap("imagens/simapaga16.png", wx.BITMAP_TYPE_ANY), pos=(423, 449), size=(36,31))
		aptudo = wx.BitmapButton(self.painel, 202, wx.Bitmap("imagens/cancela16.png",  wx.BITMAP_TYPE_ANY), pos=(423, 483), size=(36,31))
		altera = wx.BitmapButton(self.painel, 203, wx.Bitmap("imagens/relerp.png",     wx.BITMAP_TYPE_ANY), pos=(423, 515), size=(36,25))
		...
		self.vencimento_cheque=wx.DatePickerCtrl(self.painel,902, pos=(150,230), size=(111,21), style=wx.DP_DEFAULT|wx.DP_ALLOWNONE)
		...
		if len( login.filialLT[ self.Filial ][35].split(";") ) >=19 and login.filialLT[ self.Filial ][35].split(";")[18] == "T":	self.dpT.Enable( True )
		...
		#-: Observacao
		self.obs = wx.TextCtrl(self.painel,-1,value="", pos=(466, 474), size=(449,112),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
		...
		altera.Bind(wx.EVT_BUTTON, self.alterarBanco)
		...
		self.cc_bancos.Bind(wx.EVT_COMBOBOX,self.alterarBanco)
	...
	def sair(self,event):	self.Destroy()
	def alterarBanco(self,event):

		indice = self.ListafPaga.GetFocusedItem()
		pagame = self.ListafPaga.GetItem( indice, 0 ).GetText().strip().split('-')[0]

		if not self.cc_bancos.GetValue().strip():	alertas.dia(self,"Banco estar vazio, o sistema vai apagar o banco do lançamento selecioando...\n"+(" "*180),"Contas apagar: Alterar banco")
		if pagame not in ['2','3','4','5']:	alertas.dia(self,u"Forma de pagamento não comporta lançamento automático do controle de conta corrente...\n\nFormas de pagamentos permitidas: { 2,3,4,5 }\n"+(" "*180),"Contas apagar: Alterar banco")
		elif pagame in ['2','3','4','5']:

			alteracao = u'{ Alteração de todos os lançamentos para o banco selecionado }\n\n' if event.GetId() == 203 else ''
			continuar = wx.MessageDialog(self,alteracao + u"Confirme para alteração do banco...\n\n"+self.cc_bancos.GetValue().strip()+"\n"+(" "*200),"Contas apagar: Alterar banco",wx.YES_NO|wx.NO_DEFAULT)
			if continuar.ShowModal() ==  wx.ID_YES:

				if event.GetId() == 203:
					for i in range(self.ListafPaga.GetItemCount()):

						self.ListafPaga.SetStringItem(i,7,  self.cc_bancos.GetValue() )
						self.ListafPaga.SetItemBackgroundColour(i, "#4A8599" if self.ListafPaga.GetItem( i, 7 ).GetText() else "#CDCDE5")
				else:
					self.ListafPaga.SetStringItem(indice,7,  self.cc_bancos.GetValue() )
					self.ListafPaga.SetItemBackgroundColour(indice, "#4A8599" if self.ListafPaga.GetItem( indice, 7 ).GetText() else "#CDCDE5")
	...
	def marcar_receber( self, event ):
		...
		if self.ListaReceber.GetItemCount():
			...
			if __liquidar:
				...
				self.ListafPaga.SetStringItem( __registpg,7,  self.cc_bancos.GetValue() if self.pgT.GetValue().split('-')[0] in ['2','3','4','5'] else '' )
	...
	def adicionarValores(self, __id, valor_recebido = 0 ):
		...
		if fP == '':	alertas.dia(self.painel,"Selecione uma forma de pagamento...\n"+(" "*100),"Apagar: Adicionar Pagamentos")
			...
			if adicionar:
				...
				cheque_vencimento=datetime.datetime.strptime(self.vencimento_cheque.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y-%m-%d")
				...
				self.ListafPaga.SetStringItem(rG,7,  self.cc_bancos.GetValue() if __pagamento.split('-')[0] in ['2','3','4','5'] else '' )
				if __pagamento.split('-')[0] in ['5','6']:	self.ListafPaga.SetStringItem(rG,8,cheque_vencimento)

				if vP < 0:	self.ListafPaga.SetItemTextColour(rG, "#B31010")
				if self.ListafPaga.GetItem( rG, 7 ).GetText():	self.ListafPaga.SetItemBackgroundColour(rG, "#4A8599")
	...
	def OnEnterWindow(self, event):
		...
		elif event.GetId() == 203:	sb.mstatus(u"  { Alteração de todos os lançamentos para o banco selecionado } p/transferência automática do controle de conta corrente",0)
	...
	def passagem(self,event):
		...
		regis  = self.ListafPaga.GetFocusedItem()
		valju  = "0.00"
		valds  = "0.00"
		...
		self.cc_bancos.SetValue( self.ListafPaga.GetItem( regis, 7 ).GetText() )
	...
	def selecionados(self,event):
		...
		if self.p.regis_baixar:
			...
			for nrg in self.p.regis_baixar:
				...
				a12 = self.p.ListaApagar.GetItem(nrg,12).GetText().strip()
	...
	def finalizaBaixa(self,event):
		...
		__j=Decimal( str(self.jur.GetValue()).replace(',','') )
		__d=Decimal( str(self.des.GetValue()).replace(',','') )
		__s=Decimal( str(self.pag.GetValue()).replace(',','') )

		nForn = self.ListaBaixaAp.GetItem(self.ListaBaixaAp.GetFocusedItem(), 0).GetText()
		dForn = self.ListaBaixaAp.GetItem(self.ListaBaixaAp.GetFocusedItem(),11).GetText().strip()
		rForn = self.ListaBaixaAp.GetItem(self.ListaBaixaAp.GetFocusedItem(),12).GetText().strip()
		...
		if finalizar:
			...
			controle_conta = '\n\n[ Controle de conta-corrente sem banco selecionado ]' if self.incluir_titulos and not self.cc_bancos.GetValue().strip() else ''
			if self.incluir_titulos and self.cc_bancos.GetValue().strip():	controle_conta = '\n\n'+self.cc_bancos.GetValue().strip()

			#----: Gravacao da Baixa
			bxa = wx.MessageDialog(self.painel,historico+controle_conta+"\n\nConfirme para finalizar baixa !!\n"+(" "*200),u"Contas Apagar: Baixa de Títulos",wx.YES_NO|wx.NO_DEFAULT)
			if bxa.ShowModal() ==  wx.ID_YES:
				...
				if sql[0] == True:
					...
					try:
						...
						if relaca and rTiTul and rvalor:
							...
							for t in range( self.ListafPaga.GetItemCount() ):
								...
								if self.ListafPaga.GetItem(t, 4).GetText() and self.ListafPaga.GetItem(t, 0).GetText().split('-')[0] == '6':
									...
									rc_rginst='"+str(rForn)+"',rc_docins='"+dForn+"',\
						...
						""" Controle independente de cheques da empresa e terceiros  """
						controle_independente_cheque=True if len(login.filialLT[self.Filial][35].split(';'))>=139 and login.filialLT[self.Filial][35].split(';')[138]=='T' else False
						if controle_independente_cheque:

						    lista_favorecidos=''
						    lista_lancamentos=''
						    favorecido=self.ListaBaixaAp.GetItem(0,0).GetText()
						    
						    for fv in range(self.ListaBaixaAp.GetItemCount()):
							
							if self.ListaBaixaAp.GetItem(fv,0).GetText()!=favorecido:	favorecido='Diversos'
							lista_favorecidos+=self.ListaBaixaAp.GetItem(fv,0).GetText()+'|'
							lista_lancamentos+=self.ListaBaixaAp.GetItem(fv,4).GetText()+'|'
							
						    for ci in range(self.ListafPaga.GetItemCount()):

							if self.ListafPaga.GetItem(ci,0).GetText().split('-')[0] in ['5','6']:

							    tipo=self.ListafPaga.GetItem(ci,0).GetText().split('-')[0]
							    
							    banco=self.ListafPaga.GetItem(ci,2).GetText().split('|')[2]
							    agencia=self.ListafPaga.GetItem(ci,2).GetText().split('|')[3]
							    conta=self.ListafPaga.GetItem(ci,2).GetText().split('|')[4]
							    cheque=self.ListafPaga.GetItem(ci,2).GetText().split('|')[5]
							    valor=self.ListafPaga.GetItem(ci,2).GetText().split('|')[6]
							    vencimento=self.ListafPaga.GetItem(ci,8).GetText()
							    
							    if tipo=='5':	dados_cheque='60PG:'+self.ListafPaga.GetItem(ci,2).GetText().split(':')[1]
							    if tipo=='6':	dados_cheque='61PG:'+self.ListafPaga.GetItem(ci,2).GetText().split(':')[1]
							    valor_gravacao=valor.replace(',','')
							    
							    ind_grv="INSERT INTO individualcheque (ac_filial,ac_favore,ac_emissa,ac_horaem,ac_usuari,ac_valord,ac_numero,ac_tipola,ac_xmlmdf,ac_xmlenc,ac_lifavo,ac_vencim)\
							    VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
							    sql[2].execute( ind_grv, (self.Filial, favorecido, DR, HB, login.usalogin, valor_gravacao, cheque, tipo, lista_lancamentos, dados_cheque,lista_favorecidos, vencimento) )
					...
					if grv == True:
						...
						"""  Lancamento no controle de conta corrente  """
						numero_controle = 'LA-'+brdo #if nRegis > 1 else 'LA-'+self.ListaBaixaAp.GetItem(self.ListaBaixaAp.GetFocusedItem(), 4).GetText()
						bandeira = plano_conta = banco_origem = ""

						if (__j + __d + __s ):	self.incluir_titulos=False
						if self.par.GetValue() or not self.incluir_titulos: #--// Baixa parcial ou incluir as formas de pagamentos no controle conta-corrente

							for fc in range( self.ListafPaga.GetItemCount() ):

									if self.ListafPaga.GetItem(fc, 7).GetText():

										valor_liquido = self.ListafPaga.GetItem(fc, 1).GetText().replace(',','')
										forma_pagamento = self.ListafPaga.GetItem(fc, 0).GetText()
										filial = self.Filial
										tipo_lancamento = "30"
										lancamento_longo = "Baixa do contas apagar"
										id_banco = str( int( self.ListafPaga.GetItem(fc, 7).GetText().split('-')[0] ) )
										
										numero_lancamento = self.ListafPaga.GetItem(fc, 4).GetText()

										lancamento_curto = 'Baixa contas apagar {'+forma_pagamento+'}'
										credito_debito = u"Dédito"
										modulo = "6-Baixa do contas apagar"
										banco_destino = self.ListafPaga.GetItem(fc, 7).GetText().strip()
										numero_controles = numero_controle + '-'+ET.replace(':','').replace(' ','').replace('/','').replace('-','')
										data_lamento_documeto = datetime.datetime.now().strftime("%d%m%Y%T").replace(':','')
										data_referencia = format( datetime.datetime.strptime( self.dpT.GetValue().FormatDate(),'%d-%m-%Y'), "%Y/%m/%d" )
										if id_banco:
			
											dados = valor_liquido+'|'+lancamento_curto+'|'+lancamento_longo+'|D|'+numero_controles+'|'+plano_conta+\
											'|'+id_banco+'|'+credito_debito+'|'+modulo+'|'+tipo_lancamento+'|'+banco_origem+'|'+banco_destino+'|'+data_lamento_documeto+'|'+data_referencia
											lancamentocontacorrente.gravarLancamentosNovos( dados = dados, parent = self, mostrar = False, numero_apagar = numero_lancamento )

						else:

							for fc in range(nRegis):
	
								valor_liquido = self.ListaBaixaAp.GetItem(fc, 8).GetText().replace(',','')
								forma_pagamento = ''
								filial = self.Filial
								tipo_lancamento = "30"
								lancamento_longo = "Baixa do contas apagar"
								id_banco = str( int( self.cc_bancos.GetValue().split('-')[0] ) ) if self.cc_bancos.GetValue() else ''
								lancamento_curto = 'Contas apagar {'+self.ListaBaixaAp.GetItem(fc,0).GetText()+' '+numero_controle+'}'
								credito_debito = u"Dédito"
								modulo = "6-Baixa do contas apagar"
								banco_destino = self.cc_bancos.GetValue()
								numero_controles = numero_controle + '-'+ET.replace(':','').replace(' ','').replace('/','').replace('-','')
								data_lamento_documeto = datetime.datetime.now().strftime("%d%m%Y%T").replace(':','')
								data_referencia = datetime.datetime.now().strftime("%Y/%m/%d")

								numero_lancamento = self.ListaBaixaAp.GetItem(fc, 4).GetText()
	...
	def desenho(self,event):
		...
		dc.DrawRotatedText(u"Banco", 0, 590, 90)
...
class EstornoApagar(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		#self.ListaEstorno.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.selecionarEstorno)
		...
		self.ListaEstorno.InsertColumn(15,'Estornar', width=100)
	...
	def sair(self,event):	self.Destroy()
#	def selecionarEstorno(self,event):

#	    if self.Tipo != 105
#		apga = wx.MessageDialog(self.painel,u"Marcar titulo individual para estornar\n\nConfirme para continuar\n"+(" "*120),"Contas Apagar Estorno: Apagar Item",wx.YES_NO|wx.NO_DEFAULT)
			...
			if apga.ShowModal() ==  wx.ID_YES:
				...
				if sql[0] == True:
					...
					try:
						...
						""" Pesquisa lancamento do conta corrente """
						lista_contacorrente = []
						numeros_lancamentos = []

						numero_lancamento = 'LA-'+self.ListaEstorno.GetItem(0, 7).GetText() if self.ListaEstorno.GetItem(0, 7).GetText() else 'LA-'+self.ListaEstorno.GetItem(0, 5).GetText()
						numero_lancamento += '-'+self.ListaEstorno.GetItem(0, 2).GetText().replace(':','').replace('/','').replace(' ','').replace('-','')

						lancamentos_vazios=True
						if sql[2].execute("SELECT * FROM bancoconta WHERE bc_docume='"+ numero_lancamento+"'"):
						    lista_contacorrente = sql[2].fetchall()
    						    for iv in lista_contacorrente:
							if iv[19]:	lancamentos_vazios=False 


						indice = 0
						for i in range( regs ):
    
							numeros_lancamentos.append(self.ListaEstorno.GetItem(indice, 5).GetText())
					...
					if grv:
						...
						if lista_contacorrente and self.Tipo == 103:
						    #print numeros_lancamentos
						    for i in lista_contacorrente:

							estornar_conta = False
							if lancamentos_vazios:	estornar_conta = True
							elif numeros_lancamentos and i[19] and i[19] in numeros_lancamentos:	estornar_conta = True
								
							numero_dav = i[1].replace('LA','EA')
							valor_liquido = str( i[8] )
							forma_pagamento = '7-Estorno de contas apagar'
							bandeira = ''
							filial = i[12]
							tipo_lancamento = "31" #--// Estorno de contas apagar
							lancamento_longo = "Estorno de lancamento do contas apagar" + "\n\n"+his if his else ""+ "\n\n"+i[10] if i[10] else ""
							id_banco = i[4]
							lancamento_curto = i[11].decode('utf-8').replace(u'Baixa contas apagar',u'Estorno contas apagar') 
							plano_conta = i[5]
							credito_debito = u"Crédito"
							modulo = "7-Estorno contas apagar"
							banco_origem = ""
							banco_destino = i[16]
							documento_compor=i[1].split('-')[2][:14]
							data_referencia = datetime.datetime.now().strftime("%Y/%m/%d")

							if id_banco and estornar_conta:

							    estorno_contacorrente = valor_liquido+'|'+lancamento_curto+'|'+lancamento_longo+'|C|'+numero_dav+'|'+plano_conta+\
							    '|'+id_banco+'|'+credito_debito+'|'+modulo+'|'+tipo_lancamento+'|'+banco_origem+'|'+banco_destino+'|'+documento_compor+'|'+data_referencia
							    
							    lancamentocontacorrente.gravarLancamentosNovos( dados = estorno_contacorrente, parent = self, mostrar = False, numero_apagar=i[19] )

						    alertas.dia(self.painel,u"Estorno concluido !!\n"+(" "*80),_msg)	
						self.sair(wx.EVT_BUTTON)

	def estornoCheque(self):

	    conn=sqldb()
	    sql=conn.dbc("Contas Apagar: Estorno de cheques", op = 3, fil=self.Filial, janela=self.painel)
	    if sql[0]:
		
		for i in range(self.ListaEstorno.GetItemCount()):
		    
		    numero=self.ListaEstorno.GetItem(i,5).GetText()
		    sql[2].execute("DELETE FROM individualcheque WHERE ac_xmlmdf like '%"+ numero +"%'")
...
class IncluirApagar(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Contas APagar: Inclusão manual de títulos ]', size=(697,555), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		wx.StaticText(self.painel,-1,"Data de competencia:",  pos=(22,530)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		...
		self.compet = wx.DatePickerCtrl(self.painel, -1,   pos=(140,522),size=(110,27), style=wx.DP_DEFAULT|wx.DP_ALLOWNONE)
		self.agrupa = wx.ComboBox(self.painel, -1, '',     pos=(16,331), size=(173,27), choices = [],style=wx.NO_BORDER|wx.CB_READONLY)
#				wx.StaticText(self.painel,-1,"Data de competencia:",  pos=(500,530)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		...
		self.inclavulso = wx.CheckBox(self.painel, -1 , "Inclusão avulso de titulos {Obrigatorio nome fornecedor-credor [ 10 caracter-minimo]}", pos=(258,524))
...
#-----: Listar Filial
	...
	def gravarFechar(self,event):
		...
		if inc.ShowModal() ==  wx.ID_YES:
			...
			if sql[0] == True:
				...
				dtc = datetime.datetime.strptime(self.compet.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y-%m-%d")
				...
				try:
					...
					for i in range(nRegis):
						...
						ap_chbanc,ap_chagen,ap_chcont,ap_chnume,ap_histor,ap_pagame,ap_lisagr,ap_lanxml,ap_avalis,ap_contas,ap_rgforn,ap_vlorig,ap_hisusa,ap_dtcomp)\
						...
						chBco,chAge,chCon,chChq,hs,Ip,self.l,Td,ava,plc,idr,vL, hisusa, dtc  ) )
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(18, 520,676,  30, 3)
...
class AlterarTitulos(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Contas Apagar: Alteração manual de títulos', size=(980,300), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		wx.StaticText(self.painel,-1,"Data de competencia:", pos=(20,280)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		...
		self.comp = wx.DatePickerCtrl(self.painel, -1, pos=(150,  275), size=(129,22), style=wx.DP_DEFAULT|wx.DP_ALLOWNONE)
		...
		self.alTe = wx.CheckBox(self.painel, -1,"Marque essa opção p/alterar todo o grupo"+(" "*5),  pos=(18,250))
		...
		self.HisT = wx.TextCtrl(self.painel,-1,'', pos=(500,223),size=(478,67),style=wx.TE_MULTILINE|wx.TE_DONTWRAP)
		self.HisT.SetFont(wx.Font(10,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.HisT.SetBackgroundColour('#7F7F7F')
		self.HisT.SetForegroundColour('#D5D583')

		voltar = wx.BitmapButton(self.painel, 101, wx.Bitmap("imagens/volta16.png", wx.BITMAP_TYPE_ANY), pos=(398, 262),  size=(42,34))
		gravar = wx.BitmapButton(self.painel, 102, wx.Bitmap("imagens/save16.png",  wx.BITMAP_TYPE_ANY), pos=(446, 262),  size=(42,34))
	...
	def EditarTitulo(self):
		...
		if self.p.ListaApagar.GetItem(indice, 37).GetText():	pass
		else:
		    ...
		    if self.p.ListaApagar.GetItem(indice, 38).GetText().strip():
			cDia,cMes,cAno = self.p.ListaApagar.GetItem(indice, 38).GetText().split('/')
			self.comp.SetValue(wx.DateTimeFromDMY(int(cDia), ( int(cMes)- 1 ), int(cAno)))
	...
	def AtulizaTitulo(self,event):
		...
		if   self.TpLanc.GetValue().strip() == "":	alertas.dia(self,"Tipo de Documento, estar vazio...\n"+(" "*100),"Contas Apagar: Atualização de Títulos")
			...
			_dtc= datetime.datetime.strptime(self.comp.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y/%m/%d")
				...
				if sql[0] == True:
					...
					if self.alTe.GetValue() == True:
						...
						gravar = "UPDATE apagar SET ap_docume=%s,ap_nomefo=%s,ap_fantas=%s,ap_numenf=%s,ap_duplic=%s,ap_valord=%s,ap_filial=%s,\
						 ap_chcorr=%s,ap_chbanc=%s,ap_chagen=%s,ap_chcont=%s,ap_chnume=%s,ap_histor=%s,ap_pagame=%s,ap_lanxml=%s,ap_avalis=%s,\
						 ap_contas=%s,ap_jurosm=%s,ap_descon=%s,ap_hisusa=%s,ap_dtcomp=%s WHERE ap_ctrlcm=%s"

					else:

						gravar = "UPDATE apagar SET ap_docume=%s,ap_nomefo=%s,ap_fantas=%s,ap_numenf=%s,ap_dtvenc=%s,ap_duplic=%s,ap_valord=%s,ap_filial=%s,\
						 ap_chcorr=%s,ap_chbanc=%s,ap_chagen=%s,ap_chcont=%s,ap_chnume=%s,ap_histor=%s,ap_pagame=%s,ap_lanxml=%s,ap_avalis=%s,ap_contas=%s,\
						 ap_jurosm=%s,ap_descon=%s,ap_hisusa=%s,ap_dtcomp=%s WHERE ap_ctrlcm=%s and ap_parcel=%s"
					
					try:

						if self.alTe.GetValue() == True:	sql[2].execute(gravar,(_dc,_fr,_ft,_nf,_dp,_vl,_fl, chn,chb,cha,chc,_ch,self.h,_Ip,_Td,_av,_pc,_ju,_ds,hsu,_dtc,nLan))
						else:	sql[2].execute(gravar,(_dc,_fr,_ft,_nf,_vc,_dp,_vl,_fl,	chn,chb,cha,chc,_ch,self.h,_Ip,_Td,_av,_pc,_ju,_ds,hsu,_dtc,nLan,nPar))
...
class ApagarRelatorios(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.valor_individual_cheque=Decimal()
		
		self.pcontas = []
		
		wx.Frame.__init__(self, parent, id, 'Contas Apagar: Relação-Relatorios', size=(950,630), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self,-1, style = wx.BORDER_SUNKEN)
		self.Bind(wx.EVT_CLOSE, self.sair)
		...
		__m = wx.StaticText(self.painel,-1,u"D I R E T O\nContas apagar", pos=(825,593))
		...
		self.plcvenc = wx.CheckBox(self.painel, 122 , "[Relatorio 03]->Utilizar em plano de contas para fazer o relatorio pela data de competencia", pos=(10,600))
		...
		self.plcvenc.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.fornecedor = wx.TextCtrl(self.painel, -1, value="", pos=(520,464), size=(391,20), style=wx.TE_READONLY )
		...
		relato = wx.BitmapButton(self.painel, 101, wx.Bitmap("imagens/report14.png",  wx.BITMAP_TYPE_ANY), pos=(912,403), size=(30,28))				
		ToTali = wx.BitmapButton(self.painel, 103, wx.Bitmap("imagens/sum12.png",     wx.BITMAP_TYPE_ANY), pos=(912,432), size=(30,26))				
		self.planil = wx.BitmapButton(self.painel, 113, wx.Bitmap("imagens/report16.png",  wx.BITMAP_TYPE_ANY), pos=(912,459), size=(30,26))
		self.planil.Enable(False)			
		...
		self.planil.Bind(wx.EVT_BUTTON, self.planilhaPagas)
		...
		self.slplcon.Bind(wx.EVT_KEY_UP, self.selecaoPlanoContas)
	...
	def selecaoPlanoContas(self,event):
	    
	    nova_lista = login.rlplcon
	    if self.slplcon.GetValue():
		nova_lista = []
		numero = self.slplcon.GetValue().strip()
		for i in login.rlplcon:
		    if i[:len(numero)] == numero:
			nova_lista.append(i)
	    
	    self.slplcon.SetItems(nova_lista)
	    
	def planilhaPagas(self,event):
	    
	    avancar = True if self.APAContas.GetItemCount() else False
	    if self.relator.GetValue().split('-')[0]=="03" and len(self.pcontas):	avancar=True
	    len(self.pcontas)
	    
	    if avancar:
				
		periodo=datetime.datetime.strptime(self.dindicial.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y/%m/%d")+' A '+datetime.datetime.strptime(self.datafinal.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y/%m/%d")
		xls=OutrosRelatorios()
		
		if self.relator.GetValue().split('-')[0]=="02":	xls.contasPagas(self,periodo)
		if self.relator.GetValue().split('-')[0]=="03":	xls.plano_contas(self,periodo)
	...
	def fluxoCaixaRelatorio(self,event):
		...
		if event.GetId() == 118 and self.relator.GetValue().split("-")[0] =='04':	self.selecionar()
		if event.GetId() == 117 and self.relator.GetValue().split("-")[0] =='04':	self.selecionar()
		if event.GetId() == 119 and self.relator.GetValue().split("-")[0] =='04' and self.flAnali.GetValue() == True and self.APAContas.GetItemCount() !=0:	self.relatorios(wx.EVT_BUTTON)
	...
	def mCombobox(self,event):
		...
		if self._id in ["01","02"]:
			...
			if self._id=='02':	self.planil.Enable(True)

		if self._id=='03':	self.planil.Enable(True)
	...
	def relatorios(self,event):
		...
		if   self._id!="03" and not self.APAContas.GetItemCount():	alertas.dia(self.painel,u"Sem registro na lista...\n"+(" "*80),"Contas Apagar: Relatorios")
		elif self._id=="03" and not len(self.pcontas):	alertas.dia(self.painel,u"Sem registro na lista...\n"+(" "*80),"Contas Apagar: Relatorios")
		elif self._id=="03" and not self.slplcon.GetValue().strip() and self.plAnali.GetValue():	alertas.dia(self.painel,u"{ Relatorio de plano de contas analítico sem conta selecionada }\n\n1-Selecione uma conta valida do plano de contas\n"+(" "*160),"Contas Apagar: Relatorios")
		else:

			if self._id=="03" and self.plAnali.GetValue():	self.pcontas=self.tratarSequenciaData()
	...
	def tratarSequenciaData(self):
		...
		tipo_lancamento='PCAP_'+login.usalogin.upper()
		eliminar = "DELETE FROM tmpclientes WHERE tc_inform='"+ tipo_lancamento +"'"
		sql[2].execute( eliminar )
		sql[1].commit()
		reg=0

		for i in self.pcontas:

		    l=i.split('|')
		    d,m,y=i.split('|')[1].split(' ')[0].split('/')
		    dT = y+'-'+m+'-'+d

		    h=i.split('|')[1].split(' ')[1]

		    insercao = "INSERT INTO tmpclientes (tc_inndat,tc_nome, tc_obser1, tc_clifor, tc_varia1, tc_inform, tc_hora) VALUES( %s, %s, %s, %s, %s, %s, %s )"
		    sql[2].execute( insercao, ( dT, l[0],l[1], l[2], l[3] , tipo_lancamento , h ) )
		    reg+=1

		if reg:

		    sql[1].commit()
		    rg=sql[2].execute("SELECT tc_inndat,tc_nome, tc_obser1, tc_clifor, tc_varia1, tc_inform FROM tmpclientes WHERE tc_inform='"+ tipo_lancamento +"' ORDER BY tc_inndat,tc_hora")

		    if rg:
			for r in sql[2].fetchall():

			    lista_retorno.append(r[1]+'|'+r[2]+'|'+r[3]+'|'+r[4])
	...
	def selecionar(self):
		...
		if sql[0] == True:	
			...
			if self._id in ["01","02"]:
				...
				"""  Relacionar cheques de controle individualizado  """
				individualizada=0
				if self._id=="01":
				...
				"""  Controle individualizados de cheques  """
				cheques=0
				if self._id=="01":

				    cheques=sql[2].execute("SELECT * FROM individualcheque WHERE ac_vencim<='"+ dF +"' and ac_liquid='' ORDER BY ac_vencim")
				    if cheques:	result_cheques=sql[2].fetchall()
				...
				for i in _rca:
					...
					if passar == True:
						...
						banco=i[45].split('|')[1] if i[45] and len(i[45].split('|'))>=2 else ""
						agencia=i[45].split('|')[3] if i[45] and len(i[45].split('|'))>=4 else ""
						conta=i[45].split('|')[4] if i[45] and len(i[45].split('|'))>=5 else ""
							
						dados_banco="Banco: "+banco+" Agencia: "+agencia+" Conta: "+conta if banco and agencia and conta else ""

						_relatorio = _nmf+'|'+_ncT+'|'+_dLa+'|'+_hLa+'|'+_uLa+'|'+_dvc+'|'+_npc+'|'+_vld+'|'+_dbx+'|'+_hbx+'|'+_vlb+'|'+_usb+'|'+_fil+'|'+_cnf+'|'+_nfe+"|"+str(i[10])+"|"+_ban+_age+_nch+"|"+format(i[30],',')+"|"+format(i[42],',')+"|"+dados_banco
				...
				if cheques:
				    ...
				    self.valor_individual_cheque=Decimal()
				    for ch in result_cheques:

					banco=banco="Banco: "+ch[11].split('|')[0]+" No Cheque: "+ch[11].split('|')[3] if len(ch[11].split('|'))>=4 else ''
					
					_emi=ch[3].strftime("%d/%m/%Y")+" "+str(ch[4]) +" "+str(ch[5]) if ch[3] else ''
					_ven=ch[13].strftime("%d/%m/%Y") if ch[13] else ""
					
					_dLa=ch[3].strftime("%d/%m/%Y") if ch[3] else ''
					_hLa=str(ch[4]) if ch[4] else ''
					_uLa=ch[5]
					_dvc=ch[13].strftime("%d/%m/%Y") if ch[13] else ""

					_relatorio = ch[2]+'|'+str(ch[0]).zfill(10)+'|'+_dLa+'|'+_hLa+'|'+_uLa+'|'+_dvc+'|IND|'+str(ch[6])+'|||||'+ch[1]+'|||'+banco+'|'
					relacao[_registros] = ch[1],ch[7]+'-'+ch[8],'','',_emi,_ven,format(ch[6],','),ch[2],ch[9],'','','','',   _relatorio   ,'','CHEQUE INDIVIDUAL',''
					
					self.valor_individual_cheque +=ch[6]
					_registros +=1
					ordem +=1
				...
				if self.plAnali.GetValue() and not self.slplcon.GetValue().strip():	alertas.dia(self,"Selecione uma Conta no Plano de Contas!!\n"+(" "*120),"Contas Apagar")
				    
				if self.plAnali.GetValue() and self.slplcon.GetValue().strip():

					_conT = "SELECT * FROM apagar WHERE ap_dtbaix>='"+str( dI )+"' and ap_dtbaix<='"+str( dF )+"' and ap_status='1' ORDER BY ap_dtbaix,ap_contas"
					if self.plcvenc.GetValue(): #--// Totalizando pela data de vencimento
						_conT = "SELECT * FROM apagar WHERE ap_dtcomp>='"+str( dI )+"' and ap_dtcomp<='"+str( dF )+"' and ap_status='1' ORDER BY ap_dtcomp,ap_contas"
					
					if self.rFilial.GetValue():	_conT = _conT.replace("WHERE","WHERE ap_filial='"+str( self.Filial.strip() )+"' and")
					...
					"""  Individualizado plano de contas do controle de conta-corrente  """
					resul_conta=self.totalizaPlano(npConTa, sql[2], dI, dF, self.Filial.strip(), self.rFilial.GetValue(),lancamento=True)

					if resul_conta:

						for i in resul_conta:

							espaco = espacos
							if len( resul_conta[i][0].split(".") ) > espacos: espaco = len( resul_conta[i][0].split(".") )

							if resul_conta[i][4]:	self.pcontas.append( str(resul_conta[i][0])+"|"+format(resul_conta[i][1],"%d/%m/%Y")+" "+str(resul_conta[i][2])+"|"+str(resul_conta[i][3])+"|"+format(resul_conta[i][4],','))
							if resul_conta[i][4]:	rlTorio +=npConTa+(" "*espaco)+resul_conta[i][0]+(" "*5)+format(resul_conta[i][1],"%d/%m/%Y")+" "+str(resul_conta[i][2])+" "+format(resul_conta[i][4],',')+(" "*5)+str(resul_conta[i][3])+"\n"
					...
					for p in range( plc ):
						...
						if cnT:
							...
							if len( pla[p][1].split('.') ) == 5:	espaco = 20
							...
							if len( pla[p][1].split('.') ) == 5:	cvalor = self.ToTalizaPlContas( sql[2], nconTa, dI, dF )
	...
	def ToTalizaPrincipal(self, banco, ncP, _dI, _dF ):
		...
		if ncP != self.plconTg:
			...
			if self.plcvenc.GetValue(): #--// Totalizando pela data de vencimento
				_cn = "SELECT SUM( ap_valorb ) FROM apagar WHERE ap_dtcomp>='"+str( _dI )+"' and ap_dtcomp<='"+str( _dF )+"' and ap_status='1' and ap_contas like '"+str( ncP )+"%' ORDER BY ap_dtcomp"
				
			if self.rFilial.GetValue():	_cn = _cn.replace("WHERE","WHERE ap_filial='"+str( self.Filial.strip() )+"' and")

			cn = banco.execute( _cn )
			sm = banco.fetchone()[0]
			sm = sm if sm else Decimal()

			sm +=self.totalizaPlano(ncP, banco, _dI, _dF, self.Filial.strip(), self.rFilial.GetValue())
			if sm:	vl = format( sm, ',' )
	...
	def ToTalizaPlContas(self, banco, ncT, _dI,_dF ):
		...
		if self.plcvenc.GetValue(): #--// Totalizando pela data de vencimento
			_conTa = "SELECT SUM( ap_valorb ) FROM apagar WHERE ap_dtcomp>='"+str( _dI )+"' and ap_dtcomp<='"+str( _dF )+"' and ap_status='1' and ap_contas like '"+str( ncT )+"%' ORDER BY ap_dtcomp"

		if self.rFilial.GetValue():	_conTa = _conTa.replace("WHERE","WHERE ap_filial='"+str( self.Filial.strip() )+"' and")
		
		conTa = banco.execute( _conTa )	
		vlrcl = banco.fetchone()[0]

		vlrcl=vlrcl if vlrcl else Decimal()

		vlrcl +=self.totalizaPlano(ncT, banco, _dI, _dF, self.Filial.strip(), self.rFilial.GetValue(),lancamento=False)
		if vlrcl:	rTvalor = format( vlrcl, ',' )

		return str(rTvalor)

	def totalizaPlano(self, conta, sql, dI, dF, filial, com_filial, lancamento=False):

		cc = "SELECT * FROM bancoconta WHERE bc_dlanca>='"+str(dI)+"' and bc_dlanca<='"+str(dF)+"' and bc_tipola!='04' and bc_planoc like '"+str(conta)+"%' ORDER BY bc_dlanca"
		if com_filial:	cc=cc.replace("WHERE","WHERE bc_filial='"+ filial +"' and")
		vl = Decimal()
		if lancamento:	vl={}
		...
		if sql.execute(cc):
			...
			for i in sql.fetchall():
				...
				"""  Verifica se o lancamento nao foi estornado,se foi nao SOMA, se nao foi estornado SOMA """
				if sql.execute("SELECT bc_planoc FROM bancoconta WHERE bc_doccom='"+ i[17] +"' and bc_tipola='04'"):	pass #//-Nao Soma
				else:

					if lancamento:	vl[i[0]]=i[5],i[2],i[3],'Controle-CC: '+i[11].strip(),i[8] #--// Controle do conta-corrente
					else:	vl +=i[8]
	...
	def desenho(self,event):
		...
		dc.DrawRotatedText("Contas Apagar: Relatorios { D i v e r s o s } - Lista de Títulos", 0, 625, 90)
...
class APAListCtrl(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.attr4.SetBackgroundColour('#EBE6E6')
	...
	def OnGetItemAttr(self, item): #Ajusta cores sim/nao
		...
		if self.itemIndexMap != []:
			...
			index = self.itemIndexMap[item]
			if _ID=="01" and self.itemDataMap[index][15].upper()=="CHEQUE INDIVIDUAL":	return self.attr4
	...
	def OnGetItemImage(self, item):
		...
		if self.itemIndexMap != []:
			...
			if _ID == "01": #5037:
				...
				if self.itemDataMap[index][15].upper() == "CHEQUE INDIVIDUAL":	return self.cheque1
