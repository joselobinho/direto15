...
class GerenciadorSMS(wx.Frame):
	...
	def __init__(self,parent,id):
		...
		self.rfilial = wx.ComboBox(self.painel,444, '', pos=(750,105), size=(147,27), choices = [""]+login.ciaRelac,style=wx.NO_BORDER|wx.CB_READONLY)
	...
	def enviosGerencia(self,event):
	    
	    if event.GetId()==444:

		self.f = self.rfilial.GetValue().split('-')[0]
		self.verificaCreditos(wx.EVT_BUTTON)
	...
	def enviarSms(self,event):
		...
		if opcao in ["1","2"]:
			...
			if   opcao == '1' and not self.rtr:	alertas.dia( self, u"{ Informações incompleta para o envio de SMS }\n\n1 - Entre em contato com o administrador do sistema\n"+(" "*120),"Envio de SMS")
				...
				if opcao == "1":
				    ...
				    if self.telefone.GetValue().strip()[:2] !='55':	telefone = '55'+self.telefone.GetValue().strip()
				    else:	telefone = self.telefone.GetValue().strip()

				    GerenciadorWhats.telefone = telefone
				    GerenciadorWhats.texto = self.texto_sms.GetValue().strip()
				    GerenciadorWhats.outros_modulos = True
				    gw_frame = GerenciadorWhats(parent=self,id=-1)
				    gw_frame.Centre()
				    gw_frame.Show()
	...
	def consultarEnvios(self,event):
		...
		if sql[0]:
			...
			if self.rcampan.GetValue() and self.rcampan.GetValue().split('-')[0] == "2":	cons = cons.replace("WHERE","WHERE rl_nomecl='CAMPANHA WSP' and")
...
class EnvioSMS:
	...
	def enviarMenssagem( self, **kwargs ):
		...
		bloquear  = True
		enviar_whats = True if self.t or self.rl else False
		...
		if   self.mi == "EMA":	self.gravacaoSMSRetorno( ("","") )		
			...
			if not self.f.strip():	self.f = login.identifi 
		...
		elif self.mi == "WPA" and enviar_whats and self.m:
		    numero = []

		    if self.rl:

			for i in self.rl.split(";"):
			    if i:
				num = i
				if i[:2]!='55':	num = '55'+i
				numero.append(num)
		    elif not self.rl and self.t:

			    if self.t[:2]!='55':	num = '55'+self.t
			    numero.append(num)

		    if numero:
			
			acumular = True if self.r=='Expedicao' else False
			em = self.m.replace('\n',' ')
			
			rr = enviowhats.enviarChatApi(self.p, numero, em, self.i, acumular)
			self.gravacaoSMSRetorno( ("","") )
			if acumular and rr:	return rr
...
#--//Credito facilita				
	def creditoSMSFacilita(self):
		...
		try:
			...
			self.p.saldos_sms.SetValue( u'1Consultando créditos' )
...
#			url = "http://54.173.24.177/shortcode/api.ashx?"
			url = "http://209.133.205.2/shortcode/api.ashx?"
	...
	def enviarDadosCampanha(self,event):
		...
		if   not self.p.cadastro_clientes.GetItemCount():	alertas.dia( self.p, "Lista de clientes para envio estar vazia...\n"+(" "*160),"Envio de campanhas")
			...
			if _enviar.ShowModal() ==  wx.ID_YES:
				...
				elif self.opcao == "4": #//Campanha de whatsapp

					self.p.e.enviarMenssagem( parent=self.p, telefone="", texto=self.p.texto_sms.GetValue(),filial=self.f, referencia = "", data="",hora="",imagem=str(self.caminho_imagem.GetValue()),\
					credito=False, codigocliente="", nomecliente="CAMPANHA "+self.tipo_campanha.upper(), midia = "WPA", campanha=__arquivo, relacao=relacao_telefones )
