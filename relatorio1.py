	...
	def rldiversos(self, mdl, par, _di, _df, __id, __mod, Filiais = "", Filial = "" ):
		...
		if par.CHQEstornado.GetItemCount() !=0:
			...
			for i in range( nRegisT ):
				...
				if __mod == "CA" and par.carteira.GetValue() and __id == "03": #-: Contas AReceber

					cv.drawString(22, float(lcampo),par.CHQEstornado.GetItem(i,0).GetText())
					cv.drawRightString(124, float(lcampo),par.CHQEstornado.GetItem(i,1).GetText())
					cv.drawString(130, float(lcampo),par.CHQEstornado.GetItem(i,2).GetText())
					cv.drawString(290,float(lcampo),par.CHQEstornado.GetItem(i,3).GetText())
					cv.drawString(335,float(lcampo),par.CHQEstornado.GetItem(i,4).GetText())
					cv.drawRightString(640, float(lcampo),par.CHQEstornado.GetItem(i,5).GetText())
					
					cv.drawString(495,float(lcampo),par.CHQEstornado.GetItem(i,11).GetText())

					if len(par.CHQEstornado.GetItem(i,7).GetText()) > 32:	cv.setFont('Helvetica', 6)
					cv.drawString(645,float(lcampo), par.CHQEstornado.GetItem(i,7).GetText()[:47] )

					cv.setFont('Helvetica', 8)

					if par.CHQEstornado.GetItem(i,13).GetText().strip():

					    rc, pgto = par.CHQEstornado.GetItem(i,13).GetText().strip().split('<>')

					    linhas +=1
					    lcampo -= 10

					    cv.setFillColor(HexColor('#7F7F7F'))
					    cv.drawString(22, float(lcampo),'<Desmenbramento>')
					    cv.drawString(100, float(lcampo),'Numero lancamento')
					    cv.drawString(200, float(lcampo),'Vencimento')
					    cv.drawString(270, float(lcampo),'Valor')
					    cv.drawString(300, float(lcampo),'Forma de pagameto')
					    
					    formas_pagamentos = pgto.split('|')
					    numero_pagamentos = len( pgto.split('|') )
					    numero_lancamento = len( rc.split('|') )

					    if numero_pagamentos: # <= numero_lancamento:
						cv.setFillColor(HexColor('#404040'))
						cv.drawString(400, float(lcampo),'<Recebimentos>')
						cv.drawString(495, float(lcampo),'Valor recebido')
						cv.drawString(560, float(lcampo),'Forma de pagamento')
					    cv.setFillColor(HexColor('#000000'))

					    __lc = 1
					    for __rc in rc.split('|'):

						dc, vc, vl, d1, fp = __rc.split(';')

						linhas +=1
						lcampo -= 10

						cv.drawString(100, float(lcampo),dc)
						cv.drawString(200, float(lcampo),vc)
						cv.drawRightString(288,float(lcampo),vl)
						cv.drawString(300, float(lcampo),fp)

						if __lc <= numero_pagamentos:
						    
						    cv.drawRightString(544, float(lcampo),formas_pagamentos[__lc-1].split(';')[0])
						    cv.drawString(560, float(lcampo),formas_pagamentos[__lc-1].split(';')[1])
						    __lc +=1
						...
						for nl in formas_pagamentos:
							...
							linhas +=1
							lcampo -= 10
							cv.drawRightString(544, float(lcampo),vlp)
							cv.drawString(560, float(lcampo),frp)
					...
					else:	cv.line(20,( lcampo )-2,820,( lcampo )-2)

				elif __mod == "CA" and _rc[0] !='' and __id != "06" and __id != "07": #-: Contas AReceber
				...
				if __id == '03' and par.carteira.GetValue():	pass
				else:
				...
				lcampo -= 10
				comparacao = 48
				if __mod == "CA" and par.carteira.GetValue() and __id == "03":	comparacao = 46
				if linhas >= comparacao:
					...
					lcampo = 515
					ccampo = 525
					if __id in ['03']:	cabccnf( False )
					else:	cabccnf(False if par.totaliza_cliente.GetValue() else True )
			
			if __id == '03' and par.carteira.GetValue():	pass #	cabccnf( False )
			else:	cabccnf( False if par.totaliza_cliente.GetValue() else True )
			mdl.rodape(cv,rd1,'','','','','','','','',6)
			cv.save()
	...
	def rldiversos(self,mdl,par,_di,_df,__id, __pc, Filiais = False, Filial = "" ):
		...
		def cabccnf(_separa):
			...
			if __id == "01" and not par.extrato.GetValue():
				...
				cv.setFillColor(HexColor('#A3A3A3'))
				cv.drawString(200,33,"Cheques individual:")
				cv.drawRightString(365, 33,format(par.valor_individual_cheque,','))
				cv.setFillColor(HexColor('0x000000'))				
			...
			if __id == "03":
				...
				if self.totalizacao_plano:

				    cv.drawString(645,434,"{ Resumo }")	
				    numero_conta=[]
				    for rr in __pc:
					
					rCon = rr.split('|')
					if len( rCon[1].split(".") ) == 2:
					    if rCon[1] not in numero_conta and rCon[2]:	numero_conta.append(rCon[1]+'|'+rCon[2])

				    pllinha=420
				    for r1 in numero_conta:
					_r=r1.split('|')
					cv.drawString(650,pllinha,_r[0].split(' ')[0])	
					cv.drawRightString(773, pllinha, _r[1] )	
					cv.line(645,pllinha-2,775,pllinha-2)
					pllinha-=10
				    self.totalizacao_plano=False
				    cv.rect(645,80,130,350)
			...
			if __id == "03":
				...
				if par.plAnali.GetValue() == True:
					...
					for r in __pc:
						...
						rCon = r.split('|')
						if len(rCon)==4:	ToTalConTa += Decimal( rCon[3].replace(",","") )
				...
				for r in __pc:
					...
					if par.plAnali.GetValue() == True:
						...
						if len(rCon)>=4:	cv.drawRightString(625, float(lcampo),rCon[3])
				...
				for r in range(__nReg):
					...
					if par.extrato.GetValue():
						...
						if _rel[6]=='IND':	cv.setFillColor(HexColor('#A3A3A3'))
						else:	cv.setFillColor(HexColor('#000000'))
						...
						if __id == "01":
							...
							if _rel[15].strip() !="" or _rel[16].strip() !="":
								...
								__dp = __nf = __ch = ""
								if _rel[15] !="" and _rel[6]!='IND':	__dp = u"Nº Duplicata: "+ _rel[15] +"  "
								if _rel[15] !="" and _rel[6]=='IND':	__dp = u"Controle CHEQUE: "+ _rel[15] +"  "
						...
						if __id == "02":
							...
							dados_banco=_rel[19] if len(_rel)>=20 else ""
							cv.drawRightString(124, float(lcampo),_rel[1]+"-"+_rel[6])
							...
							if dados_banco:
							    ...
							    if len(dados_banco)>70:	cv.setFont('Helvetica', 5)
							    if not ToTaliza:
								linhas+=1
								lcampo-=10
						    
								cv.drawString(555,float(lcampo),dados_banco)
							    else:	cv.drawString(555,(float(lcampo)-10),dados_banco)
...
class RelatoriosProdutos:
	...
	def rldiversos( self, mdl, par, _di, _df, __id, Filiais = False, Filial = "" ):
		...
		carga = False
		receber_local=True if len(login.filialLT[Filial][35].split(';'))>=137 and login.filialLT[Filial][35].split(';')[136]=='T' else False
		...
		def cabccnf(_separa):
			...
			if __id == "25":	rls.cabecalhopadrao(cv,ImageReader,dh,pag,Filial,"Relacao de KITs",2)
			if __id == "26":	rls.cabecalhopadrao(cv,ImageReader,dh,pag,Filial,"Produtos NCM",2)
			if __id == "27":	rls.cabecalhopadrao(cv,ImageReader,dh,pag,Filial,"Alteração de Preços",2)
			...
			if __id == "05":
				...
				if __id not in ["900","06","07","08","11","12","13","15","16","17","18","19","20","22","26","27"]:
				...
				if __id == "16":
					...
					cv.drawString(500,33,"Valor total de compras:")
					cv.drawRightString(683,33,format(par.vod,','))
			...
			if   __id == "06":	rel =  u'Usuário: '+str(login.usalogin)+"  "+_Tipo+(" "*10)+Filial
			    ...
			    if __id=="08":	_Tipo +="   Produto: "+par.pdDs.GetLabel()
			    rel =  u'Usuário: '+str(login.usalogin)+u"  PERÌODO: "+cIni+" A "+cFim+(" "*5)+_Tipo+(" "*5)+"Filial: "+Filial
			...
			if __id == "01":
				...
				cb3 = "280|540|Loja/Deposito|#7F7F7F"
			...
			if __id in ["11","26","27"]:
				...
				if __id=="27":
					...
					cb6 = "560|540|Usuario alteração|"
					cb7 = "660|540|Antigo_1|"
					cb8 = "700|540|Antigo_2|"
					cb9 = "740|540|Atual_1|"
					cb10= "780|540|Atual_2|"
					...
					cb6 = "580|540|Venda|"
					cb7 = "620|540|Custo|"
					cb8 = "660|540|Comissão|"
					cb9 = "700|540|NCM-1 {P}|"
					cb10= "740|540|NCM-2 {N}|"
					cb11= "780|540|NCM-3 {S}|"
			...
			if __id=="17":
				if par.rl17gr.GetValue() and par.relatorio_vendas_grupo:
				    cb1 = " 22|540||#7F7F7F"
				    cb2 = " 50|540|||R"
				    cb3 = "132|540|Descrição dos grupos|"
				else:
				    cb1 = " 22|540|Nº DAVS|#7F7F7F"
				    cb2 = " 50|540|Codigo||R"
				    cb3 = "132|540|Descrição dos produtos"+(" "*108)+"[ Fisico/Minimo ]|"
			...
			if __id=="25":
				...
				cb1 =  "22|540|Descrição do KIT {Vinculados}|#7F7F7F"
				cb2 = " 527|540|Preço de venda||R"
				cb3 = " 627|540|Valor de custo||R"
				cb4 = " 727|540|Margem||R"
			...
			if __id=="900" and carga == False:
				...
				cb4 = "450|540||" if receber_local else "450|540|Receber no Local|"
				cb5 = "600|540|Vendedor|"
				cb6 = "650|540|Telefone(s)|"
				cb7 = "750|540|Assinatura|"
			...
			if _separa == False:	mdl.mtitulo(rel,cv,cb1,cb2,cb3,cb4,cb5,cb6,cb7,cb8,cb9,cb10,cb11,cb12, __tamanho, 2)
				...
				if __id == "25":	cv.line((float(cb4.split('|')[0])-2), float(ccampo), (float(cb4.split('|')[0])-2), float(lcampo))
				...
				if __id in ["01","06","11","900","15","17","19","20","22","26","27"]:

					if __id !="900":	cv.line((float(cb4.split('|')[0])-2), float(ccampo), (float(cb4.split('|')[0])-2), float(lcampo))
					cv.line((float(cb5.split('|')[0])-2), float(ccampo), (float(cb5.split('|')[0])-2), float(lcampo))
					cv.line((float(cb6.split('|')[0])-2), float(ccampo), (float(cb6.split('|')[0])-2), float(lcampo))

					if __id not in ["22","25"]:	cv.line((float(cb7.split('|')[0])-2), float(ccampo), (float(cb7.split('|')[0])-2), float(lcampo))
					...
					if __id in ["11","26","27"]:
						...
						cv.line((float(cb9.split('|')[0])-2), float(ccampo), (float(cb9.split('|')[0])-2), float(lcampo))
						cv.line((float(cb10.split('|')[0])-2),float(ccampo), (float(cb10.split('|')[0])-2),float(lcampo))
						if __id!="27":	cv.line((float(cb11.split('|')[0])-2),float(ccampo), (float(cb11.split('|')[0])-2),float(lcampo))
				...
				if __id in ["02","03","04","05","07","08","12","13","16"] or __rel == "18":
					...
					if __id == "07":
						...
						perce_custos = ( final_custos / final_vendas * 100 ) if final_custos and final_vendas else Decimal()
		...
		if qLisTa:
			...
			if __id in ["11","27"]:	nomeArquivo = diretorios.usPasta+"TabelaPreco_"+login.usalogin.lower()+".pdf"
			...
			if __id == "25":	nomeArquivo = diretorios.usPasta+"kits"+login.usalogin.lower()+".pdf"
			if __id == "26":	nomeArquivo = diretorios.usPasta+"ncms_"+login.usalogin.lower()+".pdf"
			...
			if __id == "25":	rd1 = '22|35|110|'+login.identifi+'-'+login.emfantas+' (Relatorio: Relacao de KITs|'
			if __id == "26":	rd1 = '22|35|110|'+login.identifi+'-'+login.emfantas+' (Relatorio: Produtos-NCM)|'
			if __id == "27":	rd1 = '22|35|110|'+login.identifi+'-'+login.emfantas+' (Relatorio: Alteração de preços|'
			...
			if __id == "900":
				...
				for le in relacao.split("[p]"):
					...
					if rlc[0]:
						...
						cv.setFont('Helvetica-Bold', 6)
						if rlc[5] and not receber_local:	cv.drawString(450, float(lcampo),rlc[5].split(";")[0]+"  "+rlc[5].split(";")[1])
				...
				if par.relacionarCargar and par.listar_romaneio:
					...
					cv.drawRightString( 650, 515, "PesoBruto" )
					cv.drawRightString( 710, 515, "PesoLiquido" )
					cv.drawRightString( 750, 515, "Metros" )
					cv.drawRightString( 818, 515, "Ocorrencias" )

					total_pesobruto = Decimal()
					total_pesoliquido = Decimal()
					for rc in par.listar_romaneio:
												
					    cg = rc.split(";")
					    if Decimal( cg[9] ):
						if cg[11] and Decimal(cg[11]):	total_pesobruto +=Decimal(cg[11])
						if cg[12] and Decimal(cg[12]):	total_pesoliquido +=Decimal(cg[12])

					if total_pesobruto:	cv.drawString(200,33, "Peso bruto: "+format(total_pesobruto,'.3f') )
					if total_pesoliquido:	cv.drawString(200,23, "Peso liquido: "+format(total_pesoliquido,'.3f') )
					...
					for rc in par.listar_romaneio:
						...
						if Decimal( cg[9] ):
							...
							#if cg[13] and Decimal(cg[13]) and not Decimal( cg[10] ):	cv.drawRightString( 590, float(lcampo-2), cg[13]+' CX(s)' )
							cv.drawRightString( 540, float(lcampo-2), cg[9]+' '+cg[2]) #//-[ Quantida unidade]
							if cg[13] and Decimal(cg[13]):	cv.drawRightString( 590, float(lcampo-2), cg[13]+' CX(s)') #//-[ Caixas ]
							...
							#    cv.drawRightString( 590, float(lcampo-2), cg[13]+' CX(s)' )
							    
							    #print '9: ',cg[9],'10: ',cg[10]
							    #caixa = ' { '+cg[13]+' CX(s) }' if cg[13] and Decimal(cg[13]) else ''
							    #saldo = "( Venda: "+cg[9]+" Devolucao: "+cg[10]+"  SALDO: "+ str( ( Decimal( cg[9] ) - Decimal( cg[10] ) ) )+" )"
							    #saldo = "Saldo para entrega: "+cg[9]
							    #colun = 700
							#else:
							    
							cv.setFont('Helvetica', 8)
							#cv.drawRightString( colun, float(lcampo-2), saldo+' '+cg[2] + caixa)
							#cv.drawRightString( 570, float(lcampo-2), cg[2])

							if cg[4].split('|')[0]:	cv.drawRightString( 590, float(lcampo-2), cg[4].split('|')[0] ) #//-[ Pecas ]
							if cg[11] and Decimal(cg[11]):	cv.drawRightString( 650, float(lcampo-2), cg[11] )
							if cg[12] and Decimal(cg[12]):	cv.drawRightString( 710, float(lcampo-2), cg[12] )
				...
				Indice = 0
				condicao = range( __nReg )
				""" Posicao ṕor grupo """
				if __id=="17" and par.rl17gr.GetValue() and par.relatorio_vendas_grupo:	condicao=sorted(par.relatorio_vendas_grupo)
				
				for r in condicao: #range( __nReg ):
					...
					if __id not in ["05","06","07","08","11","12","13","15","16","17","18","19","20","22","25","26","27"]:
					...
					if __id == "01":
						...
						deposito=Decimal('0')
						loja=lojas=Decimal(_rel[15]) if Decimal(_rel[15]) else Decimal('0')
						fisico=Decimal(_rel[2]) if Decimal(_rel[2]) else Decimal('0')
						if loja<0:	loja=Decimal(str(loja).replace('-',''))
						if fisico>loja:	deposito=(fisico-loja) if fisico>0 else Decimal('0')
						if loja>fisico:	deposito=(loja-fisico) if fisico>0 else Decimal('0')
						
						if Decimal(lojas) <0:cv.setFillColor(HexColor('0xFF0000'))
						cv.drawRightString(316, float(lcampo),format(Decimal(lojas),',') if Decimal(lojas) else '') #--// NCM
						cv.setFillColor(HexColor('0x133C63'))
						cv.drawRightString(357, float(lcampo),format(Decimal(deposito),',') if Decimal(deposito) else '') #--// NCM
						cv.setFillColor(HexColor('0x000000'))
						cv.setFont('Helvetica', 7)
						...
						if __id=="27":
						    cv.setFont('Helvetica', 6)
						    cv.drawString(560, float(lcampo), par.RLTprodutos.GetItem( Indice, 5 ).GetText() )
						    cv.setFont('Helvetica', 7)

						    cv.drawRightString(697, float(lcampo), par.RLTprodutos.GetItem( Indice, 6 ).GetText() )
						    cv.drawRightString(737, float(lcampo), par.RLTprodutos.GetItem( Indice, 7 ).GetText() )
						    cv.drawRightString(777, float(lcampo), par.RLTprodutos.GetItem( Indice, 9 ).GetText() )
						    cv.drawRightString(817, float(lcampo), par.RLTprodutos.GetItem( Indice, 10).GetText() )
							...
							if len( par.RLTprodutos.GetItem( Indice, 9 ).GetText().split('.') ) >= 1:	cv.drawRightString(736, float(lcampo), par.RLTprodutos.GetItem( Indice, 9 ).GetText().split('.')[0] )
							if len( par.RLTprodutos.GetItem( Indice,10 ).GetText().split('.') ) >= 1:	cv.drawRightString(776, float(lcampo), par.RLTprodutos.GetItem( Indice,10 ).GetText().split('.')[0] )
							if len( par.RLTprodutos.GetItem( Indice,11 ).GetText().split('.') ) >= 1:	cv.drawRightString(818, float(lcampo), par.RLTprodutos.GetItem( Indice,11 ).GetText().split('.')[0] )
						...
						if par.rl17gr.GetValue() and par.relatorio_vendas_grupo:
						    ...
						    __qtd = format( par.relatorio_vendas_grupo[r][0],',')
						    __cus = format( Decimal( format(par.relatorio_vendas_grupo[r][1],'.2f') ),',')
						    __vnd = format( Decimal( format(par.relatorio_vendas_grupo[r][2],'.2f') ),',')
						    __mar = format( Decimal( format(par.relatorio_vendas_grupo[r][3],'.2f') ),',')
						    __des = format( Decimal( format(par.relatorio_vendas_grupo[r][4],'.2f') ),',')

						    cv.drawString(22,float(lcampo), str(Indice+1).zfill(3))
						    cv.setFont('Helvetica-Bold', 8)
						    cv.drawRightString(450,float(lcampo), r )
						    cv.setFont('Helvetica', 8)
						    cv.drawRightString(537, float(lcampo), __qtd )
						    cv.drawRightString(607, float(lcampo), __cus )
						    cv.drawRightString(677, float(lcampo), __vnd )
						    cv.drawRightString(780, float(lcampo), __des )
						    
						else:
						    efisico = Decimal(par.RLTprodutos.GetItem(r, 10).GetText())
						    eminimo = Decimal(par.RLTprodutos.GetItem(r, 11).GetText())
						    if efisico <0 or eminimo>=efisico:
							tamanho = 10
							cv.setFillGray(0.1,0.1) 
							cv.rect(20,(lcampo-2),800,tamanho, fill=1) #--: Fundo do cabecalho
							cv.setFillColor(HexColor('0x000000'))
						    ...
						    cv.setFont('Helvetica', 5)
						    cv.drawRightString(475, float(lcampo), '['+str(efisico)+' {'+str(eminimo)+'}]')
						    cv.setFont('Helvetica', 8) if not par.ananal.GetValue() else cv.setFont('Helvetica-Bold', 8)
					...
					if __id == "25":
					    ...
					    for il in  range(qLisTa):

						codigo = cLisTa.GetItem(il, 0).GetText()
						deskit = cLisTa.GetItem(il, 1).GetText()
						prcvnd = cLisTa.GetItem(il, 2).GetText()
						vlrcus = cLisTa.GetItem(il, 3).GetText()
						vlmarg = cLisTa.GetItem(il, 4 ).GetText()
						lista_kit = cLisTa.GetItem(il, 5).GetText()
						
						cv.setFillColor(HexColor('0x1A1A1A'))
						cv.setFont('Helvetica', 9)
						cv.drawString(22,  float(lcampo),codigo)
						cv.drawString(105, float(lcampo),deskit)
						cv.drawRightString(623, float(lcampo),prcvnd)
						cv.drawRightString(723, float(lcampo),vlrcus)
						cv.drawRightString(815, float(lcampo),vlmarg)

						lcampo -= 10
						linhas +=1
						for vi in lista_kit.split('|'):
						    if vi:
							...
							lv = vi.split(';')
							cv.drawString(105,  float(lcampo),lv[0])
							cv.drawString(200, float(lcampo),lv[1])
							cv.drawRightString(623, float(lcampo),'Quantidade>-->')
							cv.drawRightString(723, float(lcampo),lv[2])
							lcampo -= 10
							linhas +=1
					...
					if linhas !=1 and __id not in ["05","07","13","17","18","25"]:	cv.line(20,( lcampo + 8 ),820,( lcampo + 8 ))
			...
			if __id == "17" and par.ananal.GetValue():
			    ...
			    cabccnf( True if __id not in ["05","900","18"] else False )
...
class RelatoriosCaixa:
	...
	def rldiversos(self,mdl,par,_di,_df,__id, Filiais = "" ):
		...
		def cabccnf(_separa):
			...
			if relator == "02":	rls.cabecalhopadrao(cv,ImageReader,dh,pag, Filiais ,"CAIXA: DAVs" if not par.emitidos_orcamento.GetValue() else 'CAIXA: Orcamentos',2)
			...
			if relator == "17":	rls.cabecalhopadrao(cv,ImageReader,dh,pag, Filiais ,"CAIXA: { Compras }" if par.comprac.GetValue() else "CAIXA: Consignação",2)
			...
			if relator == "02":
				...
				cv.setFillColor(HexColor('0x636363'))
				cv.drawString(480,33,"DAVs Total de vendas >--> Valor recebido")
				cv.drawRightString(695, 33,format(par.ToTalGeral,','))
				cv.drawRightString(745, 33,format(par.ToTalReceb,','))

				cv.drawString(170,33,"Total do acrescimo:")
				cv.drawString(170,23,"Total do desconto:")
				if par.acrescim:	cv.drawRightString(0, 33,format(par.acrescim,','))
				if par.desconto:	cv.drawRightString(300, 23,format(par.desconto,','))

				cv.drawString(320,33,"Total do frete:")
				if par.vlrfrete:	cv.drawRightString(430, 33,format( par.vlrfrete,','))
			...
			if relator == "08":
				...
				cv.drawString(530,33,"Comissão sobre vendas:")
				...
				cv.drawRightString(690,33,str(par.comissao_vendedor)+' % [ '+format( par.com_sobre_vendas,',.2f')+' ]')
				cv.drawRightString(690,23,format( par.com_devolu_desconto,','))
			...
			if relator == "12":
				...
				if par.cartao_comissao:	cv.drawRightString(765, 33,format( par.cartao_comissao,','))
			...
			if relator == "15":
				...
				cv.setFillColor(HexColor('0x636363'))
				cv.drawString(340,33,"Valor total de davs:")
				cv.setFillColor(HexColor('0x000000'))
				cv.drawRightString(470, 33,format( par.total_dav,','))
			...
			if relator == "17":
				...
				cv.setFillColor(HexColor('0x636363'))
				cv.drawString(718,33,"Valor Total:")
				cv.setFillColor(HexColor('0x000000'))
				cv.drawRightString(818, 33,format(par.com_vendas_produtos,','))
			...
			if   relator == "90":	_Tipo = "Notas fiscais: "+ par.nemitidas.GetValue() +"  "+par.TipoNotas.GetValue()+"  "+par.NFesNFCes.GetValue()
			    ...
			    _Tipo = par.relator.GetValue()
			    if relator=="02" and par.emitidos_orcamento.GetValue():	_Tipo="Orcamentos Emitidos"
			...
			if relator=="17" and par.comprac.GetValue() and len(par.nome_cliente.GetValue().split('|'))>=2:	_Tipo+="  [ Cliente de compras selecionado: "+par.nome_cliente.GetValue().split('|')[1]+" ]"
			
			if relator=="15" and par.fpagame.GetValue():	_Tipo +="  Pagamento: "+par.fpagame.GetValue()
			...
			if relator == "01":
				...
				if relator == "02":	cb5  =  "390|540|Descrição do Cliente"+(" "*78)+"Descontos|"
				else:	cb5  =  "390|540|Descrição do Cliente|"
				...
				cb6 = "270|540|Descrição dos clientes|"
				...
				cb2 =  "57|540|Nº DAV|"
				cb3 = "122|540|Emissão|"
				cb4 = "263|540|Descrição dos clientes|"
				cb5 = "772|540|Valor do DAV||R"
		...
		if registros !=0:
			...
			if relator == "17":	nomeArquivo = diretorios.usPasta+"consiginacao_"+login.usalogin.lower()+".pdf"
			...
			if passar == True:
				...
				for r in range( registros ):
					...
					cv.setFont('Helvetica', 7)
					if relator not in ["14","15","12","08","11","90","10","16","91","17"]:
						...
						cv.setFont('Helvetica', 6)
						if relator=='02':	cv.setFont('Helvetica', 6)
						cv.setFillColor(HexColor('#7F7F7F'))
						cv.drawString(50,  float(lcampo), par.DEVconfs.GetItem(Indice,1).GetText() )
						cv.setFillColor(HexColor('0x000000'))
						cv.setFont('Helvetica', 7)
						if relator=='02' and Decimal(par.DEVconfs.GetItem(Indice,21).GetText()):	cv.drawRightString(646, float(lcampo), format(Decimal(par.DEVconfs.GetItem(Indice,21).GetText()),',') )
					...
					if relator == "15":
						...
						cv.drawString(340, float(lcampo), par.DEVconfs.GetItem(r,4).GetText()[:30] )

						cv.setFillColor(HexColor('0x6B6666'))
						cv.drawString(340, float(lcampo)-10, "Valor Total do DAV:")
						cv.drawString(340, float(lcampo)-19, "Valor do Recebido:")
						cv.drawString(480, float(lcampo), "Valor Desconto:")
						cv.drawString(480, float(lcampo)-10, "Valor Troco:")
						cv.drawString(480, float(lcampo)-19, "Transferencia C/C:")
						...
						cv.setFont('Helvetica-Bold', 7)
						if Decimal( par.DEVconfs.GetItem(r,16).GetText().replace(',','') ) > 0:	cv.drawRightString(470, float(lcampo)-10, par.DEVconfs.GetItem(r,16).GetText() ) #-: Total DAV
						if Decimal( par.DEVconfs.GetItem(r,17).GetText().replace(',','') ) > 0:	cv.drawRightString(470, float(lcampo)-19, par.DEVconfs.GetItem(r,17).GetText() ) #-: Total Recebimento
						if Decimal( par.DEVconfs.GetItem(r,18).GetText().replace(',','') ) > 0:	cv.drawRightString(590, float(lcampo)-10, par.DEVconfs.GetItem(r,18).GetText() ) #-: Total Troco
						if Decimal( par.DEVconfs.GetItem(r,19).GetText().replace(',','') ) > 0:	cv.drawRightString(590, float(lcampo)-19, par.DEVconfs.GetItem(r,19).GetText() ) #-: Transferencia CC
						if Decimal( par.DEVconfs.GetItem(r,22).GetText().replace(',','') ) > 0:	cv.drawRightString(590, float(lcampo), par.DEVconfs.GetItem(r,22).GetText() ) #-: Transferencia CC
						cv.setFillColor(HexColor('0x000000'))
					...
					if relator == "16":
						...
						cv.setFont('Helvetica', 8)
						ed = par.DEVconfs.GetItem(r,3).GetText().split('-')
						...
						cv.drawString(166, float(lcampo), ed[2]+'/'+ed[1]+'/'+ed[0]) #par.DEVconfs.GetItem(r,3).GetText() )
						#cv.drawString(215, float(lcampo), par.DEVconfs.GetItem(r,4).GetText() )

						cv.setFont('Helvetica', 6)
						cv.drawString(213, float(lcampo), par.DEVconfs.GetItem(r,10).GetText() )
						cv.setFont('Helvetica', 8)
						
						if par.DEVconfs.GetItem(r,9).GetText().strip():
						
						    cv.setFont('Helvetica', 7)
						    cv.drawRightString(597,float(lcampo),par.DEVconfs.GetItem(r,9).GetText().strip())
						    cv.setFont('Helvetica', 6)
						
						cv.drawString(270, float(lcampo), par.DEVconfs.GetItem(r,5).GetText() )
						cv.setFont('Helvetica', 8)
						...
						if par.DEVconfs.GetItem(r,8).GetText().split('-')[0]=="3":
						    
							frete = par.DEVconfs.GetItem(r,8).GetText().split('-')[1].split('|')[0]
							acrescimo = par.DEVconfs.GetItem(r,8).GetText().split('-')[1].split('|')[1]
							
							cv.setFont('Helvetica', 6)
							cv.drawRightString(817, (float(lcampo)+2), "Frete: "+frete+"  Acrescimo: "+acrescimo)
							cv.setFont('Helvetica', 8)

						else:
						    cv.drawString(720, float(lcampo), "Sem rateio" if par.DEVconfs.GetItem(r,8).GetText().split('-')[0]=="2" else "" )
						cv.setFillColor(HexColor('0x000000'))

					if relator == "17":

						cancelado = ''
						if par.DEVconfs.GetItem(r,6).GetText() == '3':
    							
							cv.setFillColor(HexColor('#C21732'))
							cancelado = "   { CANCELADO }"

						cv.setFont('Helvetica', 8)
						cv.drawString(22, float(lcampo), par.DEVconfs.GetItem(r,0).GetText() )
						cv.drawString(57, float(lcampo), par.DEVconfs.GetItem(r,1).GetText() )
						cv.setFont('Helvetica', 7)
						cv.drawString(122,float(lcampo), par.DEVconfs.GetItem(r,2).GetText() )
						cv.setFont('Helvetica', 8)
						cv.drawString(263,float(lcampo), par.DEVconfs.GetItem(r,3).GetText()+cancelado )
						cv.drawRightString(817,float(lcampo), par.DEVconfs.GetItem(r,4).GetText() )
						cv.line(20,( lcampo + 9 ),820,( lcampo + 9 ))
						cv.setFillColor(HexColor('#767676'))
						cv.drawString(50, float(lcampo-10), 'Codigo do produto' )
						cv.drawString(120,float(lcampo-10), 'Descrição' )
						cv.drawRightString(550,float(lcampo-10), 'Quantidade' )
						cv.drawRightString(600,float(lcampo-10), 'Unidade' )
						cv.drawRightString(700,float(lcampo-10), 'Valor unitario' )
						cv.drawRightString(790,float(lcampo-10), 'Valor total' )

						for lp in par.DEVconfs.GetItem(r,5).GetText().split("\n"):

							cv.setFillColor(HexColor('#313131'))

							linhas +=1
							lcampo -= 10
							dd = lp.split('|')
							if dd[0]:
								cv.drawString(50,  float(lcampo-10), dd[0] )
								cv.drawString(120, float(lcampo-10), dd[1] )
								cv.drawRightString(550, float(lcampo-10), dd[3] )
								cv.drawRightString(600, float(lcampo-10), dd[2] )
								cv.drawRightString(700, float(lcampo-10), dd[5] )
								cv.drawRightString(790, float(lcampo-10), dd[4] )

#								cv.drawRightString(700, float(lcampo-10), dd[4] )
#								cv.drawRightString(790, float(lcampo-10), dd[5] )
...
#										if __id == "13":	lcampo += 8
					...
					if relator in ["11","12","08",'17'] and par.comissao_analitico.GetValue():	intercalar_linhas = False
...
class RelatoriosCliente:
	...
	def cldiversos(self,mdl,par,_di,_df,__id, Filial = "" ):
		...
		def cabccnf( _separa ):
			...
			if par.relator.GetValue().split("-")[0] == "08":	rls.cabecalhopadrao(cv,ImageReader,dh,pag,login.identifi,"CLIENTE: Inativos",2)
			if par.relator.GetValue().split("-")[0] == "09":	rls.cabecalhopadrao(cv,ImageReader,dh,pag,login.identifi,"Aniversariantes",2)
			if par.relator.GetValue().split("-")[0] == "10":	rls.cabecalhopadrao(cv,ImageReader,dh,pag,login.identifi,"Utimos cadastrados",2)
			if par.relator.GetValue().split("-")[0] == "11":	rls.cabecalhopadrao(cv,ImageReader,dh,pag,login.identifi,"Maiores compras",2)
			...
			elif par.relator.GetValue().split("-")[0] == "08":
				cb1  =   "22|540|Codigo|#7F7F7F"
				cb2  =   "80|540|Descrição do cliente|#7F7F7F"
				cb3  =  "400|540|Ultima compra|"
				cb4  =  "540|540|Numero DAV|"
				cb5  =  "600|540|Valor||R"
				cb6  =  "650|540|Telefones|#7F7F7F"
				#cb7  =  "900|540|Telefones|#7F7F7F"

			elif par.relator.GetValue().split("-")[0] == "09":
				cb1  =  "22|540|Descrica do cliente|#7F7F7F"
				cb2  =  "340|540|Aniversario|"
				cb3  =  "380|540|Seguimento|"
				cb4  =  "500|540|Telefones|"

			elif par.relator.GetValue().split("-")[0] == "10":
				cb1  =  "22|540|Descrica do cliente|#7F7F7F"
				cb2  =  "340|540|Cadastro||R"
				cb3  =  "440|540|Ultima compra|"
				cb4  =  "750|540|"+(' '*25)+"Valor|"

			elif par.relator.GetValue().split("-")[0] == "11":
				cb1  =  "22|540|Descrica do cliente|#7F7F7F"
				cb2  =  "340|540|Ultima compra||R"
				cb3  =  "395|540|Valor||R"
				cb4  =  "450|540|QT Davs||R"
				cb5  =  "490|540|Total de comrpas||R"
				cb6  =  "560|540|Telefone(s)|"

			if _separa == False:	mdl.mtitulo(rel,cv,cb1,cb2,cb3,cb4,cb5,cb6,cb7,cb8,cb9,cb10,cb11,cb12,7,2)
			else:

				if par.relator.GetValue().split("-")[0] in ["01","06","08","11"]:
					...
					cv.line((float(cb5.split('|')[0])-2), float(ccampo), (float(cb5.split('|')[0])-2), float(lcampo))
					cv.line((float(cb6.split('|')[0])-2), float(ccampo), (float(cb6.split('|')[0])-2), float(lcampo))
					if par.relator.GetValue().split("-")[0] not  in ["08","11"]:	cv.line((float(cb7.split('|')[0])-2), float(ccampo), (float(cb7.split('|')[0])-2), float(lcampo))
				...
				if par.relator.GetValue().split("-")[0] in ["02","09","10"]:
		...
		if par.CLTContas.GetItemCount() !=0:
			...
			if par.relator.GetValue().split("-")[0] == "08":	nomeArquivo = diretorios.usPasta+"inativos_"+login.usalogin.lower()+".pdf"
			if par.relator.GetValue().split("-")[0] == "09":	nomeArquivo = diretorios.usPasta+"aniversario_"+login.usalogin.lower()+".pdf"
			if par.relator.GetValue().split("-")[0] == "10":	nomeArquivo = diretorios.usPasta+"cadastrados_"+login.usalogin.lower()+".pdf"
			if par.relator.GetValue().split("-")[0] == "11":	nomeArquivo = diretorios.usPasta+"compras_"+login.usalogin.lower()+".pdf"
			...
			rd1 = '22|35|110|'+login.identifi+'-'+login.emfantas+' (Relatorio:'+par.relator.GetValue()[2:]+')|'
			...
			for r in range(__nReg):
				...
				if par.relator.GetValue().split("-")[0] == "01" and _cli[0] !=0:
					...
					cv.drawString(300, float(lcampo), "CPF-CPNJ: ")
					...
					cv.setFont('Helvetica', 6)
					cv.drawString(340, float(lcampo), nF.conversao(_cli[15],4) if _cli[15] else '' )
					cv.setFont('Helvetica', 7)
					...
					cv.drawString(560, float(lcampo), par.CLTContas.GetItem( r, 6 ).GetText())
				...
				elif par.relator.GetValue().split("-")[0] == "08": # and _cli[0] !=0:
					
					valor_dav = par.CLTContas.GetItem( r, 3 ).GetText().split('-') if par.CLTContas.GetItem( r, 3 ).GetText() else	['','']
					cv.drawString(22,  float(lcampo), par.CLTContas.GetItem( r, 0 ).GetText() )
					cv.drawString(80,  float(lcampo), par.CLTContas.GetItem( r, 1 ).GetText() )
					cv.drawString(402, float(lcampo), par.CLTContas.GetItem( r, 2 ).GetText() )
					cv.drawString(542, float(lcampo), valor_dav[1] )
					cv.drawRightString(645, float(lcampo), valor_dav[0] )
					cv.drawString(652, float(lcampo), par.CLTContas.GetItem( r, 5 ).GetText() )

				elif par.relator.GetValue().split("-")[0] == "09":

					cv.drawString(22,  float(lcampo), par.CLTContas.GetItem(r,0).GetText() )
					cv.drawString(340,  float(lcampo), par.CLTContas.GetItem(r,1).GetText() )
					cv.drawString(382,  float(lcampo), par.CLTContas.GetItem(r,2).GetText() )
					cv.drawString(502, float(lcampo), par.CLTContas.GetItem(r,3).GetText() )

				elif par.relator.GetValue().split("-")[0] == "10":

					cv.drawString(22,  float(lcampo), par.CLTContas.GetItem(r,0).GetText() )
					cv.drawString(342,  float(lcampo), par.CLTContas.GetItem(r,1).GetText() )
					cv.drawString(442,  float(lcampo), par.CLTContas.GetItem(r,2).GetText() )
					cv.drawRightString(817, float(lcampo), par.CLTContas.GetItem(r,3).GetText() )


				elif par.relator.GetValue().split("-")[0] == "11":

					cv.drawString(22,  float(lcampo), par.CLTContas.GetItem(r,0).GetText() )
					cv.drawRightString(390,  float(lcampo), par.CLTContas.GetItem(r,1).GetText() )
					cv.drawRightString(445,  float(lcampo), par.CLTContas.GetItem(r,2).GetText() )
					cv.drawRightString(485, float(lcampo), par.CLTContas.GetItem(r,3).GetText() )
					cv.drawRightString(555, float(lcampo), par.CLTContas.GetItem(r,4).GetText() )
					cv.drawString(565, float(lcampo), par.CLTContas.GetItem(r,5).GetText() )
...
#				cb3  =  "395|540|Valor||R"
#				cb4  =  "450|540|QT Davs||R"
#				cb5  =  "490|540|Total de comrpas|"
#				cb6  =  "540|540|Telefone(s)|"
