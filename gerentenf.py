...
#--------------: CCe
		...
		wx.StaticText(self.painel,-1,u"Vendas",    pos=(502,301)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.sefaz = wx.TextCtrl(self.painel, value="Retorno SEFAZ!!", pos=(500,350), size=(420,121),style=wx.TE_MULTILINE|wx.TE_DONTWRAP)
		...
		self.inTipoNs.SetBackgroundColour("#CFDBCF")
		...
		self.inNserie.SetBackgroundColour("#CFDBCF")
		...
		self.inNCsTaT.SetBackgroundColour("#CFDBCF")
		...
		self._nfTP = ['1-NFes Todos','2-Vendas','3-Devoluções',"4-R M A","5-Transferência","6-Compras Entrada","7-Simples Faturamento","8-Entrega Futura","9-Complementar","10-Emiisao do check out"]
		...
		self.enviasele = wx.CheckBox(self.painel, -1, "Enviar xmls para o contador e/ou cliente apenas\ndo cliente selecionado", pos=(12,268))
		...
		self.fFilial   = wx.CheckBox(self.painel, -1, "Filtro p/filial { Marque a filial: "+str( self.fla )+" }    ", pos=(702,474))
		...
		enviar_continge = GenBitmapTextButton(self.painel,705,label='   Enviar para a sefaz\n   Contingencias\n   Notas para inutilizar',  pos=(335,271),size=(162,42), bitmap=wx.Bitmap("imagens/nfe.png", wx.BITMAP_TYPE_ANY))
		enviar_continge.SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
		enviar_continge.SetBackgroundColour('#E5E5E5')
		...
		self.inTipoNs.Bind(wx.EVT_LEFT_DCLICK, self.ajusteInutilizacao)
		self.inNserie.Bind(wx.EVT_LEFT_DCLICK, self.nfForaSequencia)
		self.inNCsTaT.Bind(wx.EVT_LEFT_DCLICK, self.listaSeries)
		self.atualxml.Bind(wx.EVT_BUTTON, self.xmlDanfe)
		
		self.chave.Bind( wx.EVT_LEFT_DCLICK, self.xmlDanfe )
		enviar_continge.Bind(wx.EVT_BUTTON, self.enviarContingencia)
	...
	def enviarContingencia(self,event):

	    con_frame=RelacionarContingencia(parent=self,id=-1)
	    con_frame.Centre()
	    con_frame.Show()
	    
	def listaSeries(self,event):

	    if not self.relfilial.GetValue():	alertas.dia(self,'Selecione uma filial e periodo para fazer a varredura...\n'+(' '*180),'Notas fiscais fora da sequncia')
	    elif not self.NFesNFCes.GetValue().split('-')[0] in ['2','3']:	alertas.dia(self,'Utilize a opcao 2-Relacionar NFes ou 3-Relacionar NFCes...\n'+(' '*180),'Notas fiscais fora da sequncia')
	    else:

		tiponf = self.NFesNFCes.GetValue().split('-')[0]
		...
		__add = wx.MessageDialog(self.painel,"{ Verificacao das series utilizadas }\n\nFilial: "+filial+"\nTipo: "+self.NFesNFCes.GetValue()+"\nPeriodo: "+inicial+" A "+final+"\n\nConfirme p/continuar\n"+(" "*180),"Gerenciador de notas fiscais",wx.YES_NO)
		if __add.ShowModal() ==  wx.ID_YES:
			...
			pesquisar="SELECT nf_nserie FROM nfes WHERE nf_idfili='"+ filial +"' and nf_envdat>='"+str( inicial )+"' and nf_envdat<='"+str( final )+"' and nf_nfesce='"+str(int(tiponf)-1)+"' ORDER BY nf_nserie"
			
			lista_series={}
			if sql[2].execute(pesquisar):
			    
			    for i in sql[2].fetchall():
				
				if i[0]:	lista_series[str(int(i[0]))]=i[0]

			    conn.cls(sql[1])
	
			if lista_series:
			    for i in sorted(lista_series):
				
				series_utilizadas+="Serie: "+i + '\n'

		    if not series_utilizadas:	alertas.dia(self,'Sem series registrada...\n'+(' '*130),'Gerenciador de notas fiscais')
		    else:
			
			MostrarHistorico.hs = "Relacao de series utilizadas\n\n"+series_utilizadas
			
			MostrarHistorico.TP = "Series de NOTAS"
			MostrarHistorico.TT = "Gerenciador de notas fiscais"
			MostrarHistorico.GD = '' 
	...
	def nfForaSequencia(self,event):

	    if not self.relfilial.GetValue():	alertas.dia(self,'Selecione uma filial e periodo para fazer a varredura...\n'+(' '*180),'Notas fiscais fora da sequncia')
	    elif not self.NFesNFCes.GetValue().split('-')[0] in ['2','3']:	alertas.dia(self,'Utilize a opcao 2-Relacionar NFes ou 3-Relacionar NFCes...\n'+(' '*180),'Notas fiscais fora da sequncia')
	    elif not self.inNserie.GetValue():	alertas.dia(self,'Selecione um lancamento c/a serie desejada...\n'+(' '*180),'Notas fiscais fora da sequncia')
	    else:
		
		
		tiponf = self.NFesNFCes.GetValue().split('-')[0]
		serie  = self.inNserie.GetValue()
		inicial= datetime.datetime.strptime(self.dindicial.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y/%m/%d")
		final  = datetime.datetime.strptime(self.datafinal.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y/%m/%d")

		filial=self.relfilial.GetValue().split('-')[0]

		__add = wx.MessageDialog(self.painel,"{ Verificacao de notas fora de sequencia }\n\nFilial: "+filial+"\nTipo: "+self.NFesNFCes.GetValue()+"\nSerie: "+serie+"\nPeriodo: "+inicial+" A "+final+"\n\nConfirme p/continuar\n"+(" "*180),"Gerenciador de notas fiscais",wx.YES_NO)
		if __add.ShowModal() ==  wx.ID_YES:

		    notas_fora_sequencia=""
		    conn = sqldb()
		    sql  = conn.dbc("NFE: Gerenciador de NFe,NFCe", fil = filial, janela = self.painel )

		    if sql[0]:

			pesquisar="SELECT nf_nnotaf FROM nfes WHERE nf_idfili='"+ filial +"' and nf_envdat>='"+str( inicial )+"' and nf_envdat<='"+str( final )+"' and nf_nfesce='"+str(int(tiponf)-1)+"' and nf_nserie='"+str(int(serie))+"' ORDER BY nf_nnotaf"
			
			lista_sequencia=[]
			if sql[2].execute(pesquisar):
			    for i in sql[2].fetchall():
				lista_sequencia.append(int(i[0]))
			
			conn.cls(sql[1])

			if lista_sequencia:
			    
			    primeira_nota=lista_sequencia[0]
			    ultima_nota=lista_sequencia[-1]
			    
			    nota_fiscal=primeira_nota
			    for i in range(( ultima_nota - primeira_nota ) + 1):
				
				if not nota_fiscal in lista_sequencia:	notas_fora_sequencia+="Nota fiscal: "+str(nota_fiscal)+'\n'
				nota_fiscal+=1

		    if not notas_fora_sequencia:	alertas.dia(self,'Notas em sequencia normal...\n'+(' '*130),'Gerenciador de notas fiscais')
		    else:
			
			MostrarHistorico.hs = "Relacao de notas fiscais fora de sequencia\n\n"+notas_fora_sequencia
			
			MostrarHistorico.TP = "Sequencial de NOTAS"
			MostrarHistorico.TT = "Gerenciador de notas fiscais"
			MostrarHistorico.GD = '' 
	...
	def imprimirDav(self,event):
		...
		if self.gerenciaNfe.GetItemCount():
			...
			if tipo_emissao == "4":	comprar.compras( self, numero_controle, "3", Filiais = filial, email = acs.acsm("212",True) ) #-:Compras RMA
				...
				cdavspr.impressaoDav( numero_controle, self, True, True,  __Dev, "", servidor = filial, codigoModulo = cdDav, enviarEmail = cdEma,recibo=False )
	...
	def TrocaFilial(self,event):
		...
		self.fFilial.SetLabel("Filtro p/filial { Marque a filial: "+str( self.fla )+" }")
	...
	def ajusteInutilizacao(self,event):
	    ...
	    modelo = '65' if TipoNF == '2' else '55'
	    
	    registro = filial +'|'+ ambiente  +'|'+ Numernf +'|'+ modelo +'|'+ nSerie +'|Escreva aqui o motivo|1-grava,2-nao gravar' 

	    self.moTiv.SetValue(registro)
	    alertas.dia(self,"Filial: "+filial+\
			   "\nAmbiente: "+ambiente+\
			   "\nNumeroNF: "+Numernf+\
			   "\nModelo: "+modelo+\
			   "\nSerie: "+nSerie+\
			   "\nMotivo: Escreva aqui o motivo"+\
			   "\nGravar: 1-grava,2-nao gravar\n"+(" "*190),"Inutilizacao manual da nota selecionada")
	    
	def consultarNfe(self,event):

	    if not self.moTiv.GetValue().strip():	alertas.dia(self.painel,"Para download, manifesto insira a chave em motivo"+"\n"+(" "*140),"Gerênciador de NFes")
	    else:
		...
		if not self.relfilial.GetValue().strip():	alertas.dia(self.painel,"Selecione uma filial para download, manifesto insira a chave em motivo"+"\n"+(" "*140),"Gerênciador de NFes")
			...
			if event.GetId() == 123:
			    ...
			    else:	nfe400eventos.consultaNotaFiscal( self, self.moTiv.GetValue().strip(), filial )
			    if event.GetId() == 124:	nfe400eventos.downloadNfe( self, filial, self.moTiv.GetValue().strip() ) 
			    if event.GetId() == 125:	nfe400eventos.manifestoNfe( self, filial, self.moTiv.GetValue().strip() )

	def recuperacaoXmlConsulta(self):

		recuperar_tudo=False
		if self.moTiv.GetValue() and len(self.moTiv.GetValue().strip())==44 and self.moTiv.GetValue().strip().isdigit() and not self.chave.GetValue().strip():

		    recuperar_tudo=True
		    self.chave.SetValue(self.moTiv.GetValue().strip())
		    
		if   len( login.usaparam.split(";") ) < 22:	alertas.dia( self, u"{ Opção não habilitada para esse usuario }\n\n1 - Veja com o administrador para habilitar essa opção para o seu usuario...\n"+(" "*160),u"Recuperação de XML via consulta")
			...
			id_lancamento=self.gerenciaNfe.GetItem( indice, 18 ).GetText()
			if not numero_chave.strip() and self.chave.GetValue():	numero_chave=self.chave.GetValue()
			...
			if verificacao:
				...
				if ch1 and 'NFe' in ch1 and len(ch1[3:])==44 and self.moTiv.GetValue().strip()==ch1[3:] and self.a:

				    documento=xml.dom.minidom.parse(self.a).toxml()
				    
				    conn=sqldb()
				    sql=conn.dbc("NFE: Gerenciador de NFe,NFCe, recuperacao do XML", fil = filial, janela = self.painel )
				    if sql[0]:

					if sql[2].execute("SELECT no_chave FROM nfeoriginal WHERE no_chave='"+ch1[3:]+"'"):
					    i1=sql[2].execute("UPDATE nfeoriginal SET no_arxml='"+documento+"' WHERE no_chave='"+ch1[3:]+"'")
					    
					else:
					    i1=sql[2].execute("INSERT INTO nfeoriginal (no_chave,no_arxml) VALUES('"+ch1[3:]+"', '"+documento+"')")

					i2=sql[2].execute("UPDATE nfes SET nf_urlqrc='"+ch1[3:]+"' WHERE nf_regist='"+id_lancamento+"'")
					sql[1].commit()
					conn.cls(sql[1])
					
					if i1 and i2:	alertas.dia( self, u"Base do gerenciador de NF e base backup original atualizados...\n"+(" "*140),u"Gerenciador: recuperação do XML")
					else:	alertas.dia( self, u"Base do gerenciador retorno: {"+str(i1)+"}\nBase do backup original retorno: {"+str(i2)+"}...\n"+(" "*140),u"Gerenciador: recuperação do XML")
	...
	def abrirDanfe(self,event):
		...
		if self.fla != '':
			...
			geraPDF.MontarDanfe(self, arquivo=self.a, automatico = False, emissao = inutil )
	...
	def nfDenegada(self,event):
		...
		if nDeneg in ["301","302","303"] and TpLanc == "5":	alertas.dia(self,"Nota Denegada, DAV ja restaurada!!\n"+(" "*100),"NFe: Notas Denegadas")
		if nDeneg in ["301","302","303"] and TpLanc != "5":
	...
	def cancelarNFe(self,event):
		...
		nSerie  = self.gerenciaNfe.GetItem(indice,22).GetText() #-: Numero de Serie
		ambiente= self.gerenciaNfe.GetItem(indice,24).GetText() #-: Ambiente
		contigencia= self.gerenciaNfe.GetItem(indice,25).GetText() #-: Contingencia
		Motivo  = self.moTiv.GetValue().strip()
		modelo = '65' if TipoNF == '2' else '55'
		...
		if   len(Motivo.strip()) < 15:	alertas.dia(self.painel,u"{ 15 Caracter Minimo }\n\nMotivo p/Cancelamento\nInutilização\nCarta de Correção...\n"+(" "*110),"Gerênciador: Cancelamento/Inutilização")
			...
			""" Permissao para lykos inutilizar NFCe em contingencia  """
			if event.GetId()==109 and contigencia=="1" and login.usalogin.upper()=="LYKOS" and Inulti !="2":
			    
			    inutilizar_passa = wx.MessageDialog(self,u"{ Inuitilização de NFCe em contingência, Apenas para [L Y K O S] }\n\nConfirme para continuar\n"+(" "*160),u"Gerênciador de NF",wx.YES_NO|wx.NO_DEFAULT)
			    if inutilizar_passa.ShowModal() ==  wx.ID_YES:	Inulti='2'
			...
			if event.GetId()==109 and Inulti !="2":	alertas.dia(self.painel,u"Dav não estar marcado para ser inutilizado...\n"+(" "*100),u"Gerênciador: Cancelamento/Inutilização")
				...
				if '|' in self.moTiv.GetValue():

					if len( self.moTiv.GetValue().split('|') ) == 7:

						filial = self.moTiv.GetValue().split('|')[0].upper()
						ambiente = self.moTiv.GetValue().split('|')[1]
						Numernf = self.moTiv.GetValue().split('|')[2]
						modelo = self.moTiv.GetValue().split('|')[3]
						nSerie = self.moTiv.GetValue().split('|')[4]
						Motivo = self.moTiv.GetValue().split('|')[5]

						if self.moTiv.GetValue().split('|')[6] in ['1','2']:	numdav = _id = Tpd = 'MANUAL'
						alertas.dia(self,'{ Inutilizacao avulso de nfe/nfce }\n'+(' '*150),'Inutilizacao manual de nota')

					else:
						alertas.dia(self,'{ Inutilizacao avulso de nfe/nfce }\n\nEm motivos descreva os parametros abaixo separado por pipe ["|"]\n\nExemplo:\n          ID-Filial|Ambiente{ 1-producao,2-homologacao }|Numero da Nota Fiscal|Modelo{ 55,65 }|Numero de Serie|Motivo|gravar no dav selecioando{ 1-gravar,2-nao gravar }\n'+(' '*350),'Inutilizacao manual de nota')
						return

				if len( login.filialLT[filial] ) >= 31 and len( login.filialLT[filial][30].split(';') ) >= 3 and login.filialLT[filial][30].split(';')[2] == "4.00":

					nfe400eventos.inutilizarNfe( self, filial = filial, ambiente = ambiente, nota_fiscal = Numernf, motivo = Motivo, nfe_nfce = modelo, dav = numdav, id_lancamento = _id, tipo_pedido = Tpd, serie = nSerie ) 
				...
				if sql[0] == True:
					...
					if rcce.ShowModal() ==  wx.ID_YES:
						...
						if al[2] == "4.00":	nfe400eventos.cartaCorrecao( self, filial, inf ) 
	...
	def passagem(self,event):
		...
		if self.gerenciaNfe.GetItem(indice, 23).GetText() in ["301","302","303"]:
		...
		if self.gerenciaNfe.GetItem(indice, 20).GetText() == "5" or self.gerenciaNfe.GetItem(indice, 20).GetText() in ["301","302","303"]:	TpEmissao = "Denegada"
	...
	def GerarXmlPdf(self,event):
		...
		idlanc = self.gerenciaNfe.GetItem(indice,18).GetText()
		inutil = self.gerenciaNfe.GetItem(indice,20).GetText()
		contin = self.gerenciaNfe.GetItem(indice,25).GetText()
		...
		if   nChave == '' and inutil !="4":	alertas.dia(self.painel,u"Nº Chave vazio, NFs Inutilizadas não Gera Chave!!\n"+(" "*110),"Gerênciador NFs")
			...
			if sql[0] == True:
				...
				if _d and _i:
				    ...
				    if self.inNCsTaT.GetValue().strip() == "100" or contin=='1':

					"""  Pedi autorizacao para emissao em segunda via"""
					dd=self,filial, '', _i, {'ibpt':d[109].strip()}, 701,True
					if len(login.filialLT[filial][35].split(';'))>=143 and login.filialLT[filial][35].split(';')[142]=='T':	

					    nF.autorizacaoRemota(informar="Emissao de 2a Via de NFCe em A4\nNumero DAV: "+numDav+"\nChave: "+nChave, autori='', autante='', cabeca='', tmpcmd='', modulo=24, filial=filial, par=self,dados=dd)

					else:	nfce_emissao.nfce(self,filial=filial, printar='', xml=_i, inform = {'ibpt':d[109].strip()}, emissao=701, segunda=False, autorizador='' )
			...
			if sql[0] == True:
				...
				if self.TipoNotas.GetValue() and self.TipoNotas.GetValue().split('-')[0] == '6':	rSefaz = "SELECT nc_arqxml FROM comprasxml WHERE nc_regist='"+str( idlanc )+"'"
					...
					else:	rSefaz = "SELECT sf_xmlarq,sf_arqxml FROM sefazxml WHERE sf_nchave='"+str( nChave )+"'"
				...
				if _reTorno == 0:	alertas.dia(self.painel,_ref,"Gerenciador: NFes")
					...
					if passa == True:	
						...
						if   _id == 105 or _id == 113:
							...
							geraPDF.MontarDanfe( self, arquivo=_Arq, TexTo=_arqxml, emails = emails, automatico = False, emissao = inutil )
	...
	def emissaoNFCeAutorizacaoRemota(self,dd,autorizado):

	    """ Envia chamada para a classe q cuida de impressao da NFCe A4 c/Autorizacao remota  """
	    nfce_emissao.nfce(dd[0],filial=dd[1], printar=dd[2], xml=dd[3], inform = dd[4], emissao=dd[5], segunda=dd[6], autorizador=autorizado )
		...
		if sql[0] == True:
			...
			if self.TipoNotas.GetValue() and self.TipoNotas.GetValue().split('-')[0] == '6':
				...
				if consulT == "":
					...
					if TipoLan=="2":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='1' and nf_oridav!='3' ORDER BY nf_envdat") #-:Vendas
					if TipoLan=="3":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='2' and nf_oridav!='3' ORDER BY nf_envdat") #-:Devolucao de Vendas
					if TipoLan=="4":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='4' and nf_oridav!='3' ORDER BY nf_envdat") #-:RMA Devolucao de Vendas
					if TipoLan=="5":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='5' and nf_oridav!='3' ORDER BY nf_envdat") #=:TransFerencia
					if TipoLan=="6":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='3' and nf_oridav!='3' ORDER BY nf_envdat") #=:NFe de Entrada de Mercadorias
					if TipoLan=="7":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='6' and nf_oridav!='3' ORDER BY nf_envdat") #=:Simples Faturamento
					if TipoLan=="8":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='7' and nf_oridav!='3' ORDER BY nf_envdat") #=:Remessa p/Entrega Futura
					if TipoLan=="9":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_tipnfe='8' and nf_oridav!='3' ORDER BY nf_envdat") #=:NF Complementar
					if TipoLan=="10":	_sl=_sl.replace("ORDER BY nf_envdat","and nf_oridav='3' ORDER BY nf_envdat") #=:NF Checkout
				...
				for i in _rs:
					...
					if i[37] !=None and i[37] !="":	conTi = i[37]
					...
					emDav = "SELECT cr_edav,cr_tnot,cr_vnfe,cr_vnpr,cr_cdev FROM cdavs WHERE cr_ndav='"+str( i[10] )+"'"
	...
	def EnvioContabilidade(self,event):
		...
		try:
			...
			if self.TipoNotas.GetValue() and self.TipoNotas.GetValue().split('-')[0] == '6': #-: Envio das notas fiscais de compras
				...
				compl = pasTa+"/xml_7_complementar"
				...
				rcmpl = pasTa+"/__6NC_xmlConferencia_complementar"
				...
				pcmpl = pasTa+"/xml_protocolo_cancelamento_complementar"
				...
				if os.path.exists( diretorios.contador+str( filial ) ) == True:
					...
					if os.path.exists(compl) == False:	os.makedirs(compl)
					if os.path.exists(rcmpl) == False:	os.makedirs(rcmpl)
					if os.path.exists(pcmpl) == False:	os.makedirs(pcmpl)
					...
					lis_cmp = glob.glob(_cmp)
					lis_rcm = glob.glob(_rcm)
					lis_ccm = glob.glob(_ccm)
					...
					for ar17 in lis_cmp:	os.remove(ar17)
					for ar18 in lis_rcm:	os.remove(ar18)
					for ar19 in lis_ccm:	os.remove(ar19)
		...
		if grv == True:
			...
			qTSFI = qTSFC = qTEFI = qTEFC = qIRMA = qcRMA  = qdene = qcmpl= qccmp=0
			...
			if sql[0] == True:
				...
				try:
					...
					if self.TipoNotas.GetValue() and self.TipoNotas.GetValue().split('-')[0] == '6': #-: Envio das notas fiscais de compras
						...
						_nfEm = _nfCa = _nfDv = _nfDc = _nfSf = _nfEf = _nfRM = _nfde = _ncmp = _nccm = Decimal("0.00")
						...
						if _pr !=0:
									...
									if i[2] == "1" and i[3] == "1" and i[24] !='' and i[25] !='':
										...
										if sql[2].execute( xml ):
											...
											if conteudo:
												...
												if '</nfeProc>' not in conteudo:	conteudo+='>'
										...
										if i[3] == "3" and i[24] !='' and i[25] !='':
											...
											if sql[2].execute(xml) !=0:
												...
												if conteudo !=None:
													...
													_arquivo = rcmpl+"/"+i[25]+"-nfe.xml"
													_emitida = open(_arquivo,'w')
													_emitida.write(conteudo)
													_emitida.close()
													qccmp += 1

										"""	XML-Protocolo de cancelamento de notas complementar """
										if i[40] != None and i[40] != '':
											_nccm += Decimal( i[41] )
											_arquivo = pcmpl+"/"+i[25]+"cancelamento-complementar-nfe.xml"
											_emitida = open(_arquivo,'w')
											_emitida.write(i[40])
											_emitida.close()
							...
							NfInu +="\n\n{ Totalizacao }\n"+\
									...
									"\nComplementar.......: "+format( _ncmp,',' )+\
									"\nComplementarCancela: "+format( _nccm,',' )
				...
				try:
					...
					if self.TipoNotas.GetValue() and self.TipoNotas.GetValue().split('-')[0] == '6': #-: Envio das notas fiscais de compras
						...
						qTidades =  "Período [ Inicial,Final ]......: "+str(dTi)+" A "+str(dTf)+" { "+str(hoj)+" }\n"+\
									...
									"Notas complementar.............: "+str(qcmpl)+"\n"+\
									"Notas complementar canceladoas.: "+str(qccmp)+"\n"+\
...
#---: Inutilizacao
	def NFEinuTiliza( self, rT = "" ):
		...
		if sql[0] == True:
			...
			davaTual = "UPDATE cdavs SET cr_nota='', cr_tnfs='', cr_chnf='', cr_nfem='' WHERE cr_ndav='"+str( dav )+"'"
...
class NFeListCtrl(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.InsertColumn(27,'Dados do orcamento vinculado',  width=3000)
	...
	def OnGetItemImage(self, item):
		...
		if self.tipo_relatorio and self.tipo_relatorio == '6' and self.itemIndexMap != []:
			...
			if self.itemIndexMap != []:
				...
				index=self.itemIndexMap[item]
				if self.itemDataMap[index][23] in ["301","302","303"]:	return self.e_acr
				...
				if self.itemDataMap[index][4]  ==   '2':
					if self.itemDataMap[index][25] == '1':	return self.processo
					return self.e_tra
...
class nfsCancelamento(wx.Frame):
	...
	def cancela(self,event):
		...
		if len( login.filialLT[self.ffilia] ) >= 31 and len( login.filialLT[self.ffilia][30].split(';') ) >= 3 and login.filialLT[self.ffilia][30].split(';')[2] == "4.00":
			...
			nfe400eventos.cancelamentoNfe( self, filial = self.ffilia, xml = self.arquivo_xml40, motivo = self.moTivo, origem = origem ) 
	...
	def cannfe( self, _pro, _danf, _data, _reT, _Fil, xmlCancel ):
		...
		canu = CancelamentoUltimasVendas()
		conn = sqldb()
		...
		if sql[0] == True:
			...
			prRMA = "UPDATE ccmp SET cc_ndanfe='',cc_numenf='',cc_nfemis='00-00-0000',cc_nfdsai='00-00-0000',cc_nfhesa='00:00:00',cc_ndanfe='',cc_numenf='',cc_protoc='',cc_cancel='"+ TexToCan +"' WHERE cc_contro='"+str( self.numDav )+"'"
			...
			try:
			    ...
			    if sql[2].execute( gerPes ):
				    ...
				    if __r and type( __r ) == str:	__r = __r.decode("UTF-*")
				    
				    _reT += __r if __r else ''
			    ...
			    if self.lancam !='4' and dFisico and self.cnfd.GetValue():
				    ...
				    for i in _devolucao:
					    ...
					    if cancelar and sql[2].execute( consulTa):
					    ...
					    if devolver and i[4] != "" and cancelar:
					    ...
					    """ Cancelamento em ultimas vendas """
					    if self.cnfd.GetValue():
						    ultimas_vendas_atualizada, ultimas_vendas=canu.cancelaUltimasVendas(sql,i[0],self.numDav)
						    if ultimas_vendas and ultimas_vendas_atualizada:	sql[2].execute("UPDATE produtos SET pd_ulvd='"+ ultimas_vendas +"' WHERE pd_codi='"+ i[0] +"'")
...
class CancelaDavEcf(wx.Frame):
	...
	def levanTamento(self):
		...
		if sql[0] == True:
			...
			nao_aberto_receber=sql[2].execute("SELECT * FROM receber WHERE rc_ndocum='"+self.numDav+"' and rc_status !='' ")
			...
			if reTor !=0:
				...
				""" Cancelamento e estorno retroagido """
				if _resu[0][11] < datetime.datetime.now().date():
					self.avanca.Enable(False)
					self.moti.SetValue('Evento retroagido')

					if login.caixa in ["01","02","03","08","09"]:	self.avanca.Enable(True) #--// Apenas para administradores
					if len(login.filialLT[self.fecf][35].split(';')) >= 125 and login.filialLT[self.fecf][35].split(';')[124]=="T":	self.avanca.Enable(True)
				...
				if len(login.usaparam.split(";"))>=32 and login.usaparam.split(";")[31]=="T" and nao_aberto_receber:
					self.avanca.Enable(False)
					self.moti.SetValue(u'Bloqueio para cancelamento de titulos\nDesmenbrados, Liquidados no contas areceber\n{ Esse usuario não tem permissão p/continuar }')
					self.moti.SetBackgroundColour('#8C2020')
					self.moti.SetForegroundColour('#FFFFFF')
		...
		if sql[0] == True:
			...
			try:
			    ...
			    if _IDT == 1:
				    ...
				    if self.d:	_iTem = _iTem.replace("idavs","didavs")
				    ...
				    for i in riTem:
					    ...
					    if _pPro !=0:
						    ...
						    esToque  = ( esTOQ[0] + i[0] )
						    if self.d:	estloja -= i[0]
						    else:	estloja += i[0]
						    
						    if self.d:	esToque = ( esTOQ[0] - i[0] )
					    ...
					    """ Cancelamento em ultimas vendas """
					    if self.cancelarDav.GetValue():
						    
						    ultimas_vendas_atualizada, ultimas_vendas=canu.cancelaUltimasVendas(sql,i[2],_dav)
						    ultimas_vendas=ultimas_vendas.replace("'","")
						    if ultimas_vendas and ultimas_vendas_atualizada:	sql[2].execute("UPDATE produtos SET pd_ulvd='"+ ultimas_vendas +"' WHERE pd_codi='"+ i[2] +"'")
...
class CancelamentoUltimasVendas:
	
	def cancelaUltimasVendas(self, sql, codigo, dav):

		ultimas_vendas=''
		ultimas_vendas_atualizada=False
		if sql[2].execute("SELECT pd_ulvd FROM produtos WHERE pd_codi='"+ codigo +"'"):

			for lv in sql[2].fetchone()[0].split('\n'):

				lis=lv
				if lis and lis.split('|')[1]==dav:
					lis=lis+'|T'
					ultimas_vendas_atualizada=True
				if lis:	ultimas_vendas+=lis+'\n'

		return ultimas_vendas_atualizada, ultimas_vendas
