...
from produtof     import InformarPrecos,ProdutosRelacionar,ProdutosAjustarPreco,manuTencaoSisTema,fornecedores
...
from produtod     import ControleEstoqueItems, ZebraEtiquetas, MensagemQuimicos
from eletronicos.cortecloud import ajustePrecos
import wx.lib.scrolledpanel as scrolled

from produtoalertafiscal import AlertaFiscalTagsConsulta
from eletronicos.woocmc import WoocProdutos
...
fpg=formasPagamentos()
#alerta_fiscal = AlertaFiscalTagsConsulta()
...
class ListCtrl(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.attr6.SetBackgroundColour("#B371B3")
		...
		self.InsertColumn(43,u"Bloqueio pra venda", width=200)
	...
	def OnGetItemAttr(self, item): #Ajusta cores sim/nao
		...
		if self.itemIndexMap != []:
			...
			if self.itemDataMap[index][43].strip():	return self.attr6
...
class ListCtrlPanel(wx.Panel):
	...
	def __init__(self, parent,_frame):
		...
		self.estoque_filiais = wx.ListCtrl(self, 418,pos=(822,274), size=(143,76),
									style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.ListavdCm = wx.ListCtrl(self, 400,pos=(147,389), size=(433,113),
						 			style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.ListavdCm.InsertColumn(0, 'Emissão',            width=120)
		self.ListavdCm.InsertColumn(1, 'QTAnterior',        format=wx.LIST_ALIGN_LEFT,width=65)
		self.ListavdCm.InsertColumn(2, 'QTComprada',        format=wx.LIST_ALIGN_LEFT,width=85)
		self.ListavdCm.InsertColumn(3, 'QTEntrada',         format=wx.LIST_ALIGN_LEFT,width=65)
		self.ListavdCm.InsertColumn(4, 'VlrUnitario',       format=wx.LIST_ALIGN_LEFT,width=110)
		...
		integrar = wx.StaticText(self,-1, u"Integração", pos=(12,353))
		integrar.SetFont(wx.Font(10,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		integrar.SetForegroundColour('#114271')
		...
		self.fsc = wx.StaticText(self,-1, u"Reserva:{}", pos=(170,371))
		...
		""" Integracao """
		self.integracao_corte_cloud = GenBitmapTextButton(self,-1,label=u'  Marcenaria virtual\n  Corte cloud  ',  pos=(10,368),size=(135,32), bitmap=wx.Bitmap("imagens/cloud20.png", wx.BITMAP_TYPE_ANY))
		self.integracao_corte_cloud.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.integracao_corte_cloud.SetBackgroundColour('#D5EAFF')

		self.integracao_woo_commerce = GenBitmapTextButton(self,-1,label=u'  Loja virtual\n  woo-commerce  ',  pos=(10,401),size=(135,32), bitmap=wx.Bitmap("imagens/woo32.png", wx.BITMAP_TYPE_ANY))
		self.integracao_woo_commerce.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.integracao_woo_commerce.SetBackgroundColour('#D0B3D0')
		
		self.integracao_alerta_fiscal = GenBitmapTextButton(self,-1,label=u'  Tributação\n  AlertaFiscal   ',  pos=(10,435),size=(135,32), bitmap=wx.Bitmap("imagens/afiscal40.png", wx.BITMAP_TYPE_ANY))
		self.integracao_alerta_fiscal.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.integracao_alerta_fiscal.SetBackgroundColour('#F9F9F9')

		self.proximo = GenBitmapTextButton(self,-1,label=u'  Integração\n  Parceiros   ',  pos=(10,468),size=(135,32), bitmap=wx.Bitmap("imagens/agrupar16.png", wx.BITMAP_TYPE_ANY))
		self.proximo.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.proximo.SetBackgroundColour('#8BA2B8')
		...
		self.ffilia = wx.CheckBox(self, 501, "Filtrar produtos: "+(" "*20), pos=(147,353 ))
		self.ffilia.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.ffilia.SetForegroundColour('#2865A1')

		self.semFil = wx.CheckBox(self, 502, "Sem Filtro p/Filial, Estoque Físico", pos=(340,353 ))
...
#-------------------: Marcacao,Custos
		...
		#self.corte_cloud.Bind(wx.EVT_BUTTON, self.gerenciaCorteCloud)
		
		self.integracao_corte_cloud.Bind(wx.EVT_BUTTON, self.gerenciaCorteCloud)
		self.integracao_woo_commerce.Bind(wx.EVT_BUTTON, self.gerenciadorWooCommerce)
		...
		self.integracao_alerta_fiscal.Bind(wx.EVT_BUTTON, self.alertaFiscalGerenciador)
	...
	def filialSele(self, _id = 708 ):
		...
		self.ffilia.SetLabel("Filtrar produtos: { "+str( self.ppFilial )+" }")
		...
		#"""  Corte cloud  """
		#corte_cloud=True if len( login.filialLT[ self.ppFilial ][35].split(";") ) >= 145 and login.filialLT[ self.ppFilial ][35].split(";")[144] == "T" else False
		#self.corte_cloud.Enable(corte_cloud)
	...
	def alertaFiscalGerenciador(self,event):
	    
		AlertaFiscalTagsConsulta.editar_produto=ProdutosAlterar
		arq_frame=AlertaFiscalTagsConsulta(parent=self, id=-1 )
		arq_frame.Centre()
		arq_frame.Show()
	...
	def pesquisarProduto(self,event):
		...
		if sql[0]:
			...
			if not self.consultar.GetValue() or self.semFil.GetValue():
			...
			if self.concodigo.GetValue().strip():
				...
				if self.ffilia.GetValue():
					...
					__cn1 = "SELECT t1.*,t2.* FROM produtos t1 inner join estoque t2 on ( t1.pd_codi=t2.ef_codigo and t2.ef_idfili='"+str( self.ppFilial )+"' ) WHERE t1.pd_canc!= '4' and t1.pd_codi='"+ self.concodigo.GetValue().strip() +"' ORDER BY t1.pd_nome"
					__cn2 = "SELECT t1.*,t2.* FROM produtos t1 inner join estoque t2 on ( t1.pd_codi=t2.ef_codigo and t2.ef_idfili='"+str( self.ppFilial )+"' ) WHERE t1.pd_canc!= '4' and t1.pd_codi='"+ self.concodigo.GetValue().strip().zfill(14) +"' ORDER BY t1.pd_nome"
				else:
					__cn1 = "SELECT t1.*,t2.* FROM produtos t1 inner join estoque t2 on ( t1.pd_codi=t2.ef_codigo ) WHERE t1.pd_canc!= '4' and t1.pd_codi='"+ self.concodigo.GetValue().strip() +"' ORDER BY t1.pd_nome"
					__cn2 = "SELECT t1.*,t2.* FROM produtos t1 inner join estoque t2 on ( t1.pd_codi=t2.ef_codigo ) WHERE t1.pd_canc!= '4' and t1.pd_codi='"+ self.concodigo.GetValue().strip().zfill(14) +"' ORDER BY t1.pd_nome"
				...
				if   __saida.upper() == "P":	_consulta = _consulta.replace("ORDER BY t1.pd_nome","and t1.pd_nome like '%"+__pesqu+"%' ORDER BY t1.pd_nome")
					...
					if self.movime.GetValue() !='' and int( self.movime.GetValue().split('-')[0] ) > 3:
						...
						if self.movime.GetValue().split('-')[0] in ["24","25"]:	_consulta = consultaP.replace("ORDER BY t1.pd_nome","and t1.pd_para!='' ORDER BY t1.pd_nome")
						...
						if self.movime.GetValue().split('-')[0] =="27":	_consulta = consultaP.replace("ORDER BY t1.pd_nome","and t1.pd_cont='T' ORDER BY t1.pd_nome")
						if self.movime.GetValue().split('-')[0] =="28":	_consulta = consultaP.replace("ORDER BY t1.pd_nome","and t1.pd_cont!='T' ORDER BY t1.pd_nome")
						__saida = ""
			...
			if reTorno !=0:
				...
				for row in _result:
					...
					if not self.ffilia.GetValue() and nF.fu(self.ppFilial)=='T' and len(row)>=99 and nF.fu(row[98])=='F':	passar=False

					""" Produtos com um determinado PIS/COFINS """
					if self.movime.GetValue().split('-')[0] =="24" and self.consultar.GetValue().strip().isdigit():
					    passar = False
					    codigo_pis=row[89].split('|')[0].split(':')[1].split(';')[0]
					    codigo_cof=row[89].split('|')[1].split(':')[1].split(';')[0]
					    
					    #--// [ o codigo 1000 e considerado com vazio ]
					    if self.consultar.GetValue().strip()=="1000" and "" in [codigo_pis,codigo_cof]:	passar = True
					    else:
						if self.consultar.GetValue().strip() in [codigo_pis,codigo_cof]:	passar=True

					""" Filtros: 25-Vender negativo, 26-Nao vender negativo """
					if self.movime.GetValue().split('-')[0] =="25":	passar=True if row[89] and len(row[89].split('|'))>=9 and row[89].split('|')[8]=='T' else False
					if self.movime.GetValue().split('-')[0] =="26":	passar=True if row[89] and len(row[89].split('|'))>=26 and row[89].split('|')[25]=='T' else False
					...
					if passar:			
						...
						produto_bloqueado='Bloqueado' if row[89] and len(row[89].split('|'))>=12 and row[89].split('|')[11]=='T' else ''
						
						"""  Endereco do produto no estoque da filial  """
						endereco_produto = row[105] if len( row ) >= 106 and row[105] else row[11]
						
						ef = "0.0000"
						vi = "0.0000"
						__rl =  self.movime.GetValue().split('-')[0]
						
						apurar_estoque = True if self.consultar.GetValue().strip() or self.concodigo.GetValue().strip() else False
						if self.semFil.GetValue() == False and apurar_estoque and __rl not in ["18","20"]:	ef = str( row[101] ) #-: Fisico
						if self.semFil.GetValue() == False and apurar_estoque and __rl not in ["18","20"]:	vi = str( row[102] ) #-: Reserva
						if self.semFil.GetValue() == False and apurar_estoque and __rl not in ["18","20"]:	fl = row[98] #--------: Filial

						if lista_apenas_filial and not self.consultar.GetValue():	fl = row[98] #--------: Filial
						if self.concodigo.GetValue().strip():	fl = row[98] #--------: Filial

						lista_fisico = ""
						if sql[2].execute("SELECT ef_idfili, ef_fisico, ef_esloja, ef_endere FROM estoque WHERE ef_codigo='"+ row[2] +"'" ):
	...
	def gerenciaCorteCloud(self,event):

		#corte_cloud=True if len( login.filialLT[ self.ppFilial ][35].split(";") ) >= 145 and login.filialLT[ self.ppFilial ][35].split(";")[144] == "T" else False
		#if self.integracao.GetValue().upper()=='CORTECLOUD':

		    #if not corte_cloud:	alertas.dia(self,u"CorteCloud não implantado\n\n1-Entre em contato com suporte para implantação\n"+(' '*130),"CorteCloud")
		    #else:
	    cort_frame=ajustePrecos(parent=self,id=-1)
	    cort_frame.Center()
	    cort_frame.Show()
	    
	def gerenciadorWooCommerce(self,event):
		#elif self.integracao.GetValue().upper()=='WOO-COMMERCE':
		    
		    #if len(login.usaparam.split(";"))>=47 and login.usaparam.split(";")[46]=="T":
	    WoocProdutos.editar_produto=ProdutosAlterar
	    cort_frame=WoocProdutos(parent=self,id=-1)
	    cort_frame.Center()
	    cort_frame.Show()
	#	    else:	alertas.dia(self,u"Usuario sem acesso ao modulo woo-commerce..\n"+(' '*180),u'Integração')

	#	self.integracao.SetValue(self.web_integracao[0])
	...
	def acessoIncluir(self,lista):
		...
		ProdutosAlterar.filial_relacao = ''
	...
	def pincluir(self,_id):
		...
		if _id in [150,300,617] and self.list_ctrl.GetItemCount() == 0:
			...
			ProdutosAlterar.filial_relacao = self.list_ctrl.GetItem(_in, 10).GetText() if self.list_ctrl.GetItem(_in, 10).GetText() and self.list_ctrl.GetItem(_in, 10).GetText().upper() !='GERAL' else ''
...
#		self.popupmenu.Append(626, "Gerenciador de grupos, sub-grupos, fabricantes e endereços { Substituição dos mesmos }")
		self.popupmenu.Append(626, "Gerenciador de grupos, sub-grupos, fabricantes e endereços p/Substituição {ajuste p/vender negativio e bloqueio p/vender}")
		self.popupmenu.Append(wx.ID_PREFERENCES, "Controle de pedidos p/produção")
		...
		self.popupmenu.Append(632,u"Cadastro de menssagens para produtos químicos")
		self.popupmenu.Append(633,u"Marcar todos os produtos para controlar")
		self.popupmenu.Append(634,u"Desmarcar todos os produtos para não controlar")
		self.popupmenu.Append(635,u"Gravar preços de venda 1 para o terminal de consulta")

		self.popupmenu.Append(636,u"Marcar todos os produtos para { Marcação valor }")
		self.popupmenu.Append(637,u"Desmarcar todos os produtos para { Marcação valor }")
		self.popupmenu.Append(638,u"Replicar códigos dos produtos para o corte-cloud")

		#self.popupmenu.Append(700,u"Alteracao diversas")
		...
		if login.usalogin.upper()!="LYKOS":
		    self.popupmenu.Enable( id = 633, enable = False )
		    self.popupmenu.Enable( id = 634, enable = False )
		    self.popupmenu.Enable( id = 636, enable = False )
		    self.popupmenu.Enable( id = 637, enable = False )
		    self.popupmenu.Enable( id = 638, enable = False )
	...
	def OnPopupItemSelected(self, event):
		...
		if even == 617:	self.pincluir(even)
			...
			gru_frame=GerenciadorGrupos(parent=self,id=-1, ajustes=InformarPrecos)
			gru_frame.Centre()
			gru_frame.Show()
			...
			MensagemQuimicos.importar=False
			MensagemQuimicos.filial=self.ppFilial
			arq_frame=MensagemQuimicos(parent=self, id=even )
			arq_frame.Centre()
			arq_frame.Show()

		elif even in [633,634]:
	
		    marcar_desmarca="Marcar" if even==633  else "Desmarcar"
		    apagar = wx.MessageDialog(self,u"Confirme para "+marcar_desmarca+u" todos os produtos para controlar...\n"+(" "*180),u"Produtos: Marcar Desmarcar controle",wx.YES_NO|wx.NO_DEFAULT)
		    if apagar.ShowModal() ==  wx.ID_YES:

			conn = sqldb()
			sql  = conn.dbc("Produtos: Marcar-Desmarcar produtos para controlar", fil = self.ppFilial, janela = self )
			
			if sql[0]:
			    
			    if sql[2].execute("SELECT pd_regi,pd_codi,pd_cont,pd_imag FROM produtos"):
				
				marcar='T' if even==633 else ''
				usuario = datetime.datetime.now().strftime("%d/%m/%Y %T")+' '+login.usalogin if even==634 else ''
				
				for i in sql[2].fetchall():
				    
				    _d = i[3].decode('UTF-8') if i[3] else None
				    if not _d:	alterado=u'|||'+usuario
				    elif _d and len(_d.split("|"))==1:	alterado=_d.split("|")[0]+'|||'+usuario
				    elif _d and len(_d.split("|"))==2:		alterado=_d.split("|")[0]+'|'+_d.split("|")[1]+'||'+usuario
				    elif _d and len(_d.split("|")) in [3,4]:	alterado=_d.split("|")[0]+'|'+_d.split("|")[1]+'|'+_d.split("|")[2]+'|'+usuario
				    sql[2].execute("UPDATE produtos SET pd_cont='"+ marcar +"', pd_imag='"+alterado+"' WHERE pd_regi='"+ str(i[0]) +"'")
				    
				sql[1].commit()
				conn.cls(sql[1],sql[2])
				
				alertas.dia(self,"Processo finalizado...\n"+(" "*100),"Produtos: Marcar Desmarcar controle")

		elif even==635:

		    apagar = wx.MessageDialog(self,u"Confirme para gravar os preços de venda 1 para o terminal de consulta\n"+(" "*180),u"Produtos: Terminal de consulta",wx.YES_NO|wx.NO_DEFAULT)
		    if apagar.ShowModal() ==  wx.ID_YES:

			conn = sqldb()
			sql  = conn.dbc("Produtos: Terminal de consulta", fil = self.ppFilial, janela = self )
			
			if sql[0]:
			    
			    if sql[2].execute("SELECT pd_regi,pd_tpr1 FROM produtos"):
				
				for i in sql[2].fetchall():
				    
				    _preco = nF.para_reais(i[1])
				    sql[2].execute("UPDATE produtos SET pd_has3='"+ _preco +"' WHERE pd_regi='"+ str(i[0]) +"'")
				    
				sql[1].commit()
				conn.cls(sql[1],sql[2])
				
				alertas.dia(self,"Processo finalizado...\n"+(" "*100),"Produtos: Terminal de consulta")

		elif even in [636,637,638]:

		    marca = "Marcar" if even==636 else "Desmarcar"
		    marcacao = "T" if even==636 else "F"
		    
		    if even==638:
			apagar = wx.MessageDialog(self,u"Confirme para replicar códigos para corte-cloud...\n"+(" "*180),u"Produtos: Replicar códigos",wx.YES_NO|wx.NO_DEFAULT)
		    else:	apagar = wx.MessageDialog(self,u"Confirme para "+marca+u", todos os produtos na opção { Marcacão valor }\n"+(" "*180),u"Produtos: Marcação valor",wx.YES_NO|wx.NO_DEFAULT)
		    if apagar.ShowModal() ==  wx.ID_YES:

			conn = sqldb()
			sql  = conn.dbc("Produtos: Marcacacao valor", fil = self.ppFilial, janela = self )
			
			if sql[0]:
			    
			    if sql[2].execute("SELECT pd_regi,pd_marc,pd_codi,pd_nome FROM produtos ORDER BY pd_nome"):
				
				_mensagem = mens.showmsg("Varrendo cadastro de produtos!!\n\nAguarde...")
				for i in sql[2].fetchall():
				    
				    _mensagem = mens.showmsg("Varrendo cadastro de produtos!!\n\n"+ i[3] +"\n\nAguarde...")
				    if even==638:	sql[2].execute("UPDATE produtos SET pd_clou='"+ i[2] +"' WHERE pd_regi='"+ str(i[0]) +"'")
				    else:	sql[2].execute("UPDATE produtos SET pd_marc='"+marcacao +"' WHERE pd_regi='"+ str(i[0]) +"'")
				
				del _mensagem
				sql[1].commit()
				conn.cls(sql[1],sql[2])
				
				alertas.dia(self,u"Processo finalizado...\n"+(" "*120),u"Produtos: Marcacão valor")
			...
			sql[2].execute("SELECT pd_codi,pd_nome,pd_tpr1,pd_tpr2,pd_tpr3,pd_tpr4,pd_tpr5,pd_tpr6,pd_para,pd_cont FROM produtos")
			lista="Codigo;Descricao dos produtos;Preco_1;Preco_2;Preco_3;Preco_4;Preco_5;Preco_6\n"
			numero=1
			numero1=1
			for i in sql[2].fetchall():
			    lista +=i[0]+';'+i[1].replace(';',' ')+';'+format(i[2],'.2f')+';'+format(i[3],'.2f')+';'+format(i[4],'.2f')+';'+format(i[5],'.2f')+';'+format(i[6],'.2f')+'\n'
			    #if i[8] and len(i[8].split('|'))>=9 and i[8].split('|')[8]=='T':	print str(numero).zfill(4)+" Vender negativo...: ",i[0],i[1],i[8].split('|')[8]
			    #if i[8] and len(i[8].split('|'))>=26 and i[8].split('|')[25]=='T':
			#	sql[2].execute("UPDATE produtos SET pd_cont='T' WHERE pd_codi='"+i[0]+"'")
			 #   else:
			#	sql[2].execute("UPDATE produtos SET pd_cont='' WHERE pd_codi='"+i[0]+"'")
			    if i[8] and len(i[8].split("|"))>=26 and i[8].split("|")[25]=='T':
				
				#sql[2].execute("UPDATE produtos SET pd_cont='T' WHERE pd_codi='"+i[0]+"'")
				numero1+=1
				print(str(numero+1),len(i[8].split("|")),i[0],i[8].split("|"))
			    else:
				#sql[2].execute("UPDATE produtos SET pd_cont='F' WHERE pd_codi='"+i[0]+"'")
				
			    #if i[9]=='T':
				
				#print str(numero).zfill(4)+" Nao Vender negativo: ",i[0],i[1],i[8].split('|')[25],i[9]
				#if i[9]!="T":	print("-"*100)
				#print str(numero).zfill(4)+" Controlando: ",i[0],i[1],i[9]
				#print i[8].split('|')
				numero +=1
			    #print i[0],i[1],i[8].split('|')
			#sql[1].commit()
			conn.cls(sql[1],sql[2])
			print("Controlar: ",str(numero1)," Nao Controlar: ",str(numero))

			#__arquivo = open("telhalar_produtos.csv","w")
			#__arquivo.write(lista)
			#__arquivo.close()
			
			#print lista

		    """
		    if sql[0]: # and sql[2].execute("SELECT pd_nome,pd_cfsc,pd_para,pd_codi,pd_cfis,pd_cfsc FROM produtos"):
			    a1=0
			    a2=0
			    a3=0
			    bb=0
			    ...
			    nao = 0
			    ntt = 0
			    __arquivo = open("srv/codigos_fiscais.csv","r")
			    for i in __arquivo.readlines():
				if i:
				    acha = ''
				    codigo = i.split('|')[0].zfill(14)
				    _cest = ''
				    _p2 = ''
				    if  sql[2].execute("SELECT pd_codi,pd_nome,pd_para,pd_cest FROM produtos WHERE pd_codi='"+codigo+"'"):
					sim +=1
					acha =  'S'
					para = sql[2].fetchone()
					_p1 = para[2].split('|')
					...
					cpis = i.split('|')[7] #--// PIS
					ccof = i.split('|')[9] #--// COFINS

					ppis = i.split('|')[8].replace('%','').replace(',','.') #--// PIS %
					pcof = i.split('|')[10].replace('%','').replace(',','.') #--// COFINS %
					_p1[0] = 'PIS:'+cpis+';'+ppis
					_p1[1] = 'COF:'+ccof+';'+pcof
					_p2 = str(_p1).replace('[','').replace(']','').replace("'","").replace(',','|').replace(' ','')
					
				    else:
					nao +=1
					acha = 'N'

				    if len(i.split('|')) >=9 and acha == 'S':
					nom = i.split('|')[1]
					...
					cest = i.split('|')[3].replace('.','') #--// CEST
					if not cest.strip():	cest=_cest
				
					icms = '0000'
					if cst=='00':	icms='2000'
					cfn = ncm.replace('.','').strip()+'.'+cfo.strip()+'.'+cst.zfill(4).strip()+'.'+icms.strip().zfill(4)
					...
					cfs = ncm.replace('.','').strip()+'.'+cfo.strip()+'.'+cst.zfill(4).strip()+'.'+icms.strip()
					sql[2].execute("UPDATE produtos SET pd_cfis='"+cfs+"',pd_cfsc='"+cfn+"',pd_para='"+_p2+"',pd_cest='"+cest+"' WHERE pd_codi='"+codigo+"'")
	...
	def movimento(self,event):
		...
		if self.list_ctrl.GetItem(indice,33).GetText() != '' and str( self.movime.GetValue().split('-')[0] ) == "12":
			...
			if self.list_ctrl.GetItem(indice,21).GetText() and self.movime.GetValue().split('-')[0] == "1":
				...
				for i in compras:
					...
					if sc[0] != '':
						...
						__filial=self.rfilia.GetValue().split('-')[0]
						__f=sc[17] if len(sc)>=18 else 'SemFilial'
						passar=False
						if __f==__filial or __f=='SemFilial':	passar=True

						if passar:
							self.ListavdCm.InsertStringItem(indic, __f+' '+str(sc[2])+' ['+str(sc[1])+']')	
							...
							if len(sc) >= 19 and sc[18]:	self.ListavdCm.SetItemTextColour(indic, "#E55555")
			...
			if self.list_ctrl.GetItem(indice,22).GetText() and self.movime.GetValue().split('-')[0] == "2":
				...
				for i in vendas:
					...
					if sv[0]:
						...
						__filial=self.rfilia.GetValue().split('-')[0]
						__f=sv[9] if len(sv)>=10 else 'SemFilial'
						passar=False
						if __f==__filial or __f=='SemFilial':	passar=True
						
						if passar:
							self.ListavdCm.InsertStringItem(indic, __f+' '+str(sv[2])+' '+str(sv[3]))	
							...
							if len(sv)>=11 and sv[10]:	self.ListavdCm.SetItemTextColour(indic, "#E55555")
			...
			if self.list_ctrl.GetItem(indice,32).GetText() and self.movime.GetValue().split('-')[0] == "3":
				...
				descri.SetText (u"Descrição da Empresa")
				...
				for i in acertos:
					...
					if sc[0] != '':
						...
						if passar:
							self.ListavdCm.InsertStringItem(indic, __f+' '+str(sc[2])+' '+str(sc[3]))	
							if len(sc) >= 16:	self.ListavdCm.SetStringItem(indic,1, str(sc[15]))
...
class ProdutosAlterar(wx.Frame):
	...
	def __init__(self,parent,id):
		...
		wx.Frame.__init__(self,parent,id,"Produtos: Alterar-Consultar-Incluir",size=(907,650),style=wx.BORDER_SUNKEN|wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.notebook = wx.Notebook(self,-1)
		self.p       = parent

		self.cadastrarProdutos()
		self.cadastrarParametros()
	...
	def cadastrarProdutos(self):
		...
		__Filial     = self.alTF
		self.filial_produto = self.filial_relacao if self.filial_relacao else self.alTF #--// Utilizado para endereco por filial
		...
		if sql[0] == True:
			...
			_mensagem = mens.showmsg("Selecionando dados do produto selecionado\n\nAguarde...")
			alerar_incluir = True
			...
			if sql[2].execute("DESC produtos") != 0:
				...
				if self.modo == 150 or self.modo == 300 or self.modo == 617: #->[ Alteracao ]
					...
					if _retorno:
						...
						if nF.fu( __Filial ) == "T":	esFF = sql[2].execute("SELECT ef_fisico,ef_virtua,ef_trocas,ef_endere FROM estoque WHERE ef_codigo='"+str( _result[0][2] )+"'" )
						else:	esFF = sql[2].execute("SELECT ef_fisico,ef_virtua,ef_trocas,ef_endere FROM estoque WHERE ef_idfili='"+str( __Filial )+"' and ef_codigo='"+str( _result[0][2] )+"'" )
			...
			if self.pAFilial and self.pd_codi and sql[2].execute("SELECT ef_endere FROM estoque WHERE ef_idfili='"+ self.pAFilial +"' and ef_codigo='"+ self.pd_codi+"'"):	self.pd_ende = sql[2].fetchone()[0]
			...
			wx.StaticText(self.painel,-1, u"Corte-cloud",                pos=(743,40)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			...
			wx.StaticText(self.painel,-1, u"1-Unidade",                  pos=(240,145)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1, u"2-Metro Linear",             pos=(240,160)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1, u"3-Metro Quadrado",           pos=(240,175)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1, u"4-Metro Cúbico",             pos=(240,190)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			...
			wx.StaticText(self.painel,-1, u"2",                          pos=(331,237)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1, u"3",                          pos=(260,277)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1, u"4",                          pos=(331,277)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1, u"5",                          pos=(260,317)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			wx.StaticText(self.painel,-1, u"6",                          pos=(331,317)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			...
			_ctrl = wx.StaticText(self.painel,-1, u"",pos=(413,316))
			_ctrl.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			_ctrl.SetForegroundColour("#A52A2A")
			...
			self.pd_codi.SetBackgroundColour('#E5E5E5')
			self.pd_codi.SetForegroundColour('#7F7F7F')

			self.pd_idfi = wx.TextCtrl(self.painel,-1,str( self.filial_produto ),pos=(470,6),size=(60,20),style = wx.ALIGN_RIGHT|wx.TE_READONLY)
			self.pd_idfi.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
			self.pd_idfi.SetBackgroundColour('#E5E5E5')
			self.pd_idfi.SetForegroundColour('#A52A2A')
			...
			self.pd_barr.SetBackgroundColour('#E5E5E5' if self.AltInc else '#EBDFDF')
			self.pd_barr.SetForegroundColour('#7F7F7F' if self.AltInc else '#A52A2A')
			...
			self.pd_nome = wx.TextCtrl(self.painel,-1, str(self.pd_nome),pos=(395,53),size=(340,22))
			self.pd_nome.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
			self.pd_nome.SetBackgroundColour('#E5E5E5')

			self.pd_clou = wx.TextCtrl(self.painel,-1, str(self.pd_clou),pos=(740,53),size=(92,22))
			self.pd_clou.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
			self.pd_clou.SetBackgroundColour('#E5E5E5')
			...
			self.pd_ppis.SetValue( _ppis if _ppis.split('.')[0].isdigit() else '0.00')
			self.pd_pcof.SetValue( _pcof if _pcof.split('.')[0].isdigit() else '0.00')
			...
			self.pd_vdp2 = mkn(self.painel, id = 231, value = str(self.pd_vdp2), pos = (305,250), style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth = 4, fractionWidth = 2, groupChar = ',', decimalChar = '.', foregroundColour = "Black", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
			self.pd_vdp3 = mkn(self.painel, id = 232, value = str(self.pd_vdp3), pos = (225,290), style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth = 4, fractionWidth = 2, groupChar = ',', decimalChar = '.', foregroundColour = "Black", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
			self.pd_vdp4 = mkn(self.painel, id = 233, value = str(self.pd_vdp4), pos = (305,290), style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth = 4, fractionWidth = 2, groupChar = ',', decimalChar = '.', foregroundColour = "Black", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
			self.pd_vdp5 = mkn(self.painel, id = 234, value = str(self.pd_vdp5), pos = (225,330), style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth = 4, fractionWidth = 2, groupChar = ',', decimalChar = '.', foregroundColour = "Black", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
			self.pd_vdp6 = mkn(self.painel, id = 235, value = str(self.pd_vdp6), pos = (305,330), style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth = 4, fractionWidth = 2, groupChar = ',', decimalChar = '.', foregroundColour = "Black", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)

			self.pd_vdp2.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			self.pd_vdp3.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			self.pd_vdp4.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			self.pd_vdp5.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			self.pd_vdp6.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
...
#---------: Endereco da Imagem
			...
			self.imagem_antgerior=self.pd_imag
			...
			""" Mostra a data de ateracao do controle do produto { abaixao de controlar } """
			_ctrl.SetLabel(self.pd_imag.split('|')[3] if self.pd_imag and len(self.pd_imag.split('|'))==4 else "")
			...
			""" Essas opcoes nao serao mais utlizada 05/09/2020 14:19 """
			self.negativo = wx.CheckBox(self.painel, 401 ,"Vender negativo", (19, 203))
			self.naovender= wx.CheckBox(self.painel, 402 ,"Não vender negativo", (130, 203))
			self.negativo.Enable(False)
			self.naovender.Enable(False)
			
			#self.negativo.SetValue(False)
			#self.naovender.SetValue(False)
			""" [ Pode ser utilizada para futuro { deixei gravando normalmente, mais sem uso na venda } ] """
			
			self.pd_bloq = wx.CheckBox(self.painel, -1 , "Bloqueio p/vender", (265, 203))
			self.controle_anterior = self.pd_cont
			...
			if _para and len( _para.split('|') )>=26 and _para.split('|')[25]== "T":	self.naovender.SetValue( True ) #// Nao vender negativo
			...
			if self.pd_acds == "A":	self.acrescim.SetValue(True)
			...
			self.pd_bloq.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			...
			self.naovender.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			...
			if _para and len( _para.split('|') ) >=12 and _para.split('|')[11] == 'T':	self.pd_bloq.SetValue( True )
			...
			self.pd_barr.Bind(wx.EVT_LEFT_DCLICK, self.corteControle)
			
			#self.negativo.Bind(wx.EVT_CHECKBOX, self.vender_nao_vender)
			#self.naovender.Bind(wx.EVT_CHECKBOX, self.vender_nao_vender)
			...
			self.pd_cont.Enable( acs.acsm("240",False) ) #-: Pode marcar a opcao para vender negativo
		...
		""" Filial bloqueda para alterar e incluir produtos e compras """
		if len( login.filialLT[ self.alTF ][35].split(';') ) >= 149 and login.filialLT[ self.alTF ][35].split(';')[148] == "T":
		    salvar.Enable(False)
		    self.remoto.Enable(False)
	
		""" Inibir produto se o mesmo for para controle do corte """
		if self.pd_refe.GetValue() == "CORTE-0000007":
		    
		    self.pd_refe.Enable(False)
		    self.pd_nome.Enable(False)

	#def vender_nao_vender(self,event):
	    
	 #   if event.GetId()==401:	self.naovender.SetValue(False)
	 #   if event.GetId()==402:	self.negativo.SetValue(False)

			#self.negativo = wx.CheckBox(self.painel, 401 ,"Vender negativo", (19, 203))
			#self.naovender= wx.CheckBox(self.painel, 402 ,"Não vender negativo", (130, 203))
	
	def corteControle(self,event):
	    
	    if not self.pd_refe.GetValue() and not self.pd_nome.GetValue() and not self.AltInc:

		produto_corter = wx.MessageDialog(self,"{ Produto para o controle de corte }\n\nConfirme para adicionar o codigo em referencia e descricao do produto\n"+(" "*160),"Produtos CORTE-CONTROLE",wx.YES_NO|wx.NO_DEFAULT)
		if produto_corter.ShowModal() ==  wx.ID_YES:
		
		    self.pd_refe.SetValue('CORTE-0000007')
		    self.pd_nome.SetValue('CORTE-CONTROLE')
		    self.pd_refe.Enable(False)
		    self.pd_nome.Enable(False)

	    else:	alertas.dia(self,'Codigo/Nome do produto ja preenchido e/ou produto para alteracao...\n'+(' '*150),'Codigo para controle do corte')
	    
	def cadastrarParametros(self):

		_mensagem = mens.showmsg("Selecionando dados do produto selecionado ferramentas\n\nAguarde...")

    		mkn  = wx.lib.masked.NumCtrl

		nbl       = wx.NotebookPage(self.notebook,-1)
		self.painel1 = wx.Panel(nbl) #,style=wx.SUNKEN_BORDER)
		abaLista  = self.notebook.AddPage(nbl,"Ferramentas { Conversao, corte-cloud, Desoneracao }")
		self.painel1.Bind(wx.EVT_PAINT,self.desenho)

		__cc = wx.StaticText(self.painel1,-1, u"Utilizado na compra: conversão embalagem p/UN",  pos=(20,2))
		__cc.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		__cc.SetForegroundColour("#4747B4")

		__ca = wx.StaticText(self.painel1,-1, u"Utilizado na compra: conversão de ML,M2,M3 em unidade",pos=(263,2))
		__ca.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		__ca.SetForegroundColour("#33339D")

		__cb = wx.StaticText(self.painel1,-1, u"Utlizado na venda: conversáo M2 em caixas",pos=(662,2))
		__cb.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		__cb.SetForegroundColour("#4747B4")

		wx.StaticText(self.painel1,-1, u"Quantidade p/embalagem", pos=(20,15)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Comprimento",pos=(263,15)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Largura",    pos=(338,15)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Expessura",  pos=(413,15)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Piso QT-M2/Caixa",  pos=(663,15)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))

		wx.StaticText(self.painel1,-1, u"Cartegoria corte-cloud", pos=(20,65)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Codigo messagem\np/venda de quimicos:", pos=(145,70)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Expedição\nindividual:",pos=(375,70)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Ferragens\nPeso/unidade:",  pos=(657,70)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Código Woo-Commerce",  pos=(405,200)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel1,-1, u"Preço p/Terminal de consulta",  pos=(405,240)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))

		qembalagens = wx.StaticText(self.painel1,-1, u"No de Embalagen(s)",  pos=(153,15))
		qembalagens.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		qembalagens.SetForegroundColour("#33339D")

		paletes = wx.StaticText(self.painel1,-1, u"QT Paletes",  pos=(488,15))
		paletes.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		paletes.SetForegroundColour("#BD4040")

		self._finalmetros = wx.StaticText(self.painel1,-1, u"Metros\nEstoque fisico",  pos=(563,5))
		...
		self._finalpisocx = wx.StaticText(self.painel1,-1, u"Piso QT-Caixa(s)",  pos=(763,15))
		...
		__vdm = wx.StaticText(self.painel1,-1, u"Conversao automatica de madeira para M3\nNa retaguarda de vendas [com, larg, exp]",  pos=(20,115))
		__vdm.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		__vdm.SetForegroundColour("#225486")

		__vm3 = wx.StaticText(self.painel1,-1, u"Conversao\nUnidade para M3:",  pos=(488,115))
		__vm3.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		__vm3.SetForegroundColour("#225486")

		__bn = wx.StaticText(self.painel1,-1, u"Codigo beneficio fiscal:",  pos=(20,165))
		__bn.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		__bn.SetForegroundColour("#1C451C")

		_mbn = wx.StaticText(self.painel1,-1, u"Motivo do beneficio fiscal:",  pos=(253,165))
		_mbn.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		_mbn.SetForegroundColour("#1C451C")

		_pbn = wx.StaticText(self.painel1,-1, u"Reducao do beneficio fiscal:{ % }:\nDesoneracao de icms",  pos=(655,160))
		_pbn.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		_pbn.SetForegroundColour("#1C451C")

		_iad = wx.StaticText(self.painel1,-1, u"Informacoes adicionais do produto p/DANFE da nota fiscal [Sem ACENTOS]",  pos=(15,200))
		_iad.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		_iad.SetForegroundColour("#1C451C")

		__mm = wx.StaticText(self.painel1,-1, u"DIRETO { Cadastro }",  pos=(780,115))
		__mm.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		__mm.SetForegroundColour("#BFBFBF")

		__mk = wx.StaticText(self.painel1,-1, u"Composição do KIT"+(' '*20)+u'Preço de venda do KIT:'+(' '*45)+'Custo do KIT:'+(' '*45)+'Maregem:',  pos=(17,287))
		__mk.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		__mk.SetForegroundColour("#2665A2")

		self.preco_kit = wx.TextCtrl(self.painel1,-1, str(self.pd_qtem), pos=(320,283),size=(113,22), style=wx.ALIGN_RIGHT)
		self.preco_kit.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.preco_kit.SetBackgroundColour('#D5E4F2')
		self.preco_kit.SetForegroundColour('#074A8C')

		self.custo_kit = wx.TextCtrl(self.painel1,-1, str(self.pd_qtem), pos=(530,283),size=(113,22), style=wx.ALIGN_RIGHT)
		self.custo_kit.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.custo_kit.SetBackgroundColour('#D5E4F2')
		self.custo_kit.SetForegroundColour('#074A8C')

		self.marge_kit = wx.TextCtrl(self.painel1,-1, str(self.pd_qtem), pos=(725,283),size=(113,22), style=wx.ALIGN_RIGHT)
		self.marge_kit.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.marge_kit.SetBackgroundColour('#D5E4F2')
		self.marge_kit.SetForegroundColour('#074A8C')

		self.pd_qtem = wx.TextCtrl(self.painel1,-1, str(self.pd_qtem), pos=(20,30),size=(113,22), style=wx.ALIGN_RIGHT)
		self.pd_qtem.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.pd_qtem.SetBackgroundColour('#E5E5E5')
		self.pd_qtem.SetMaxLength(10)

		self.resultado_caixaemb= wx.TextCtrl(self.painel1, -1, '', pos=(140,30), size=(110,22), style = wx.ALIGN_RIGHT|wx.TE_READONLY)
		...
		self.resultado_caixapiso = wx.TextCtrl(self.painel1, -1, '', pos=(760,30), size=(95,22), style = wx.ALIGN_RIGHT|wx.TE_READONLY)
		...
		self.comprim = mkn(self.painel1, id = 912,  value = '0', pos = (260,30), size=(70,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 3, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		self.largura = mkn(self.painel1, id = 913,  value = '0', pos = (335,30), size=(70,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 3, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		self.expessu = mkn(self.painel1, id = 914,  value = '0', pos = (410,30), size=(70,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 3, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		...
		self.paletes = wx.TextCtrl(self.painel1, -1, '', pos=(485,30), size=(70,20), style=wx.TE_RIGHT)
		self.paletes.SetBackgroundColour("#C5ADAD")
		self.paletes.SetForegroundColour("#FFFFFF")

		self.pisoqm2 = mkn(self.painel1, id = 915,  value = '0', pos = (660,30), size=(95,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 4, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		...
		self.resultado_metragem= wx.TextCtrl(self.painel1, -1, '', pos=(560,30), size=(95,22), style = wx.ALIGN_RIGHT|wx.TE_READONLY)
...
#-------// Informacoes para produtos quimicos [ Expedicao individual ]
		self.quimicos=wx.TextCtrl(self.painel1, -1, '', pos=(250,75), size=(110,25), style=wx.ALIGN_RIGHT|wx.TE_READONLY)
		self.quimicos.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.quimicos.SetBackgroundColour('#C1D1E2')
		self.quimicos.SetForegroundColour('#1D1DA6')

#-------// Informacoes categoria corte-cloud ]
		self.categoria=wx.TextCtrl(self.painel1, -1, '', pos=(20,77), size=(110,25))
		self.categoria.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.categoria.SetBackgroundColour('#C1D1E2')
		self.categoria.SetForegroundColour('#1D1DA6')

		impres=['']
		simm,impressoras, user, prn = fpg.listaprn(1)
		if simm:
			for i in impressoras:
				if i[4] == 'S':	impres.append(i[0]+'-'+i[2])

		self.exp_individual=wx.ComboBox(self.painel1, -1, '', pos=(435,75), size=(210,27), choices = impres, style=wx.CB_READONLY)

		self.vara_ferro = mkn(self.painel1, id = 720, value = "0.000", pos=(735,75), style = wx.ALIGN_RIGHT|0, integerWidth = 3, fractionWidth = 3, groupDigits = False, decimalChar = '.', foregroundColour = "Black",  signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "#FF0000")
		self.vara_ferro.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))

		self.frg=['1-Ferro peso/unidade { Apenas o valor inteiro desconsiderar casas decimais }','2-Ferro peso/unidade { Manter calculo com 4 casas decimias }','3-Ferro peso/unidade { Arrendondar p/Inteiro }']
		self.calculo_ferro=wx.ComboBox(self.painel1, -1, self.frg[0], pos=(835,75), size=(63,27), choices=self.frg, style=wx.CB_READONLY)

		"""  Comprimento, Largura, Expessura <<< Utilizado na venda >>> """
		self.vcomprim = mkn(self.painel1, id = 612,  value = '0', pos = (250,120), size=(70,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 3, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		self.vlargura = mkn(self.painel1, id = 613,  value = '0', pos = (325,120), size=(70,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 3, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		self.vexpessu = mkn(self.painel1, id = 614,  value = '0', pos = (400,120), size=(70,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 4, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		self.metrocub = mkn(self.painel1, id = 615,  value = '0', pos = (575,120), size=(70,20), style = wx.ALIGN_RIGHT|0, integerWidth = 2, fractionWidth = 4, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)

		self.vcomprim.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.vlargura.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.vexpessu.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.metrocub.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))

		beneficiofs = ['','3-Uso na agropecuária','9-Outros','12-Fomento agropecuário' ]
		self.motivo_beneficio=wx.ComboBox(self.painel1, -1, beneficiofs[0], pos=(400,160), size=(245,27), choices=beneficiofs, style=wx.CB_READONLY)
		self.codigo_woocommerce = wx.TextCtrl(self.painel1,-1,value=self.pd_lvir, pos=(403,213), size=(200,27))
		self.terminal_consulta = wx.TextCtrl(self.painel1,-1,value=self.pd_has3,  pos=(403,253), size=(200,27),style=wx.CB_READONLY|wx.ALIGN_RIGHT)
		self.terminal_consulta.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		
		#self.importar_pedidos = GenBitmapTextButton(self.painel1,102,label=u'  Alerta Fiscal\n  Consultar codigo fiscal  ',  pos=(405,243),size=(175,34), bitmap=wx.Bitmap("imagens/restaurar.png", wx.BITMAP_TYPE_ANY))
		...
		self.codigo_beneficio = wx.TextCtrl(self.painel1, -1, '', pos=(145,160), size=(95,25))
		self.reducacaoicms = mkn(self.painel1, id = 616,  value = '0', pos = (822,160), size=(72,20), style = wx.ALIGN_RIGHT|0, integerWidth = 3, fractionWidth = 2, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = "#E5E5E5", validBackgroundColour = "#E5E5E5", invalidBackgroundColour = "Yellow",allowNegative = False)
		self.codigo_beneficio.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.codigo_beneficio.SetBackgroundColour('#DAEEDA')
		self.codigo_beneficio.SetForegroundColour('#074307')
		self.reducacaoicms.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))

		self.infomacoes_adicionais = wx.TextCtrl(self.painel1,-1,value="", pos=(13, 213), size=(380,60),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
	
		""" Relacionar kits  """
		self.mKit = wx.ListCtrl(self.painel1, -1,pos=(15,307), size=(880,145),
									style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.mKit.InsertColumn(0,  u'Codigo', format=wx.LIST_ALIGN_LEFT,width=120)
		self.mKit.InsertColumn(1,  u'Descrição do Produto', width=480)
		self.mKit.InsertColumn(2,  u'Quantidade', format=wx.LIST_ALIGN_LEFT,width=120)
		self.mKit.InsertColumn(3,  u'Valor custo', format=wx.LIST_ALIGN_LEFT,width=120)
		...
		if self.pd_kitc.GetValue() and self.pd_codk:
		    
		    indi = 0
		    conn = sqldb()
		    sql  = conn.dbc("Produtos: Buscando custos", fil = self.pAFilial, janela = self )
		    if sql[0]:
		    
			venda_preco = Decimal()
			venda_custo = Decimal()
			venda_marge = Decimal()
			for ik in self.pd_codk.split('|'):
			    if ik:

				custo = custom = vendas = Decimal()
				if sql[2].execute('SELECT pd_pcus,pd_cusm, pd_tpr1 FROM produtos WHERE pd_codi="'+ik.split(";")[0]+'"'):
				    custo, custom, vendas = sql[2].fetchone()
				    venda_preco += vendas
				    venda_custo += custo

				self.mKit.InsertStringItem( indi, ik.split(";")[0] )
				self.mKit.SetStringItem(indi,1, ik.split(";")[1] )	
				self.mKit.SetStringItem(indi,2, ik.split(";")[2] )	
				self.mKit.SetStringItem(indi,3, format(custo,','))	
				self.mKit.SetStringItem(indi,4, ik.split(";")[3] )	

				indi +=1
			conn.cls(sql[1])
			
			if venda_preco and venda_custo:
			    venda_marge = self.Trunca.arredonda( 1, (((venda_preco - venda_custo) / venda_custo) * 100 ) )
			    
			self.preco_kit.SetValue(format(venda_preco,','))
			self.custo_kit.SetValue(format(venda_custo,','))
			self.marge_kit.SetValue(format(Decimal(venda_marge),',')+' %')
			
			
		if self.__para and len( self.__para.split('|') ) >=4:	self.comprim.SetValue( str( self.__para.split('|')[3] ) if self.__para.split('|')[3] else "0" )
		if self.__para and len( self.__para.split('|') ) >=5:	self.largura.SetValue( str( self.__para.split('|')[4] ) if self.__para.split('|')[4] else "0" )
		if self.__para and len( self.__para.split('|') ) >=6:	self.expessu.SetValue( str( self.__para.split('|')[5] ) if self.__para.split('|')[5] else "0" )
		if self.__para and len( self.__para.split('|') ) >=7:	self.pisoqm2.SetValue( str( self.__para.split('|')[6] ) if self.__para.split('|')[6] else "0" )
		if self.__para and len( self.__para.split('|') ) >=10:	self.paletes.SetValue( str( self.__para.split('|')[9] ) if self.__para.split('|')[9] else "" )

		if self.__para and len( self.__para.split('|') ) >=13:	self.quimicos.SetValue( self.__para.split('|')[12] )
		if self.__para and len( self.__para.split('|') ) >=14:	self.exp_individual.SetValue( self.__para.split('|')[13] )

		if self.__para and len( self.__para.split('|') ) >=15 and self.__para.split('|')[14].isdigit():	self.vara_ferro.SetValue( self.__para.split('|')[14] )
		if self.__para and len( self.__para.split('|') ) >=16 and self.__para.split('|')[15]:	self.calculo_ferro.SetValue( self.frg[(int(self.__para.split('|')[15])-1)] )
		if self.__para and len( self.__para.split('|') ) >=17 and self.__para.split('|')[16]:	self.categoria.SetValue(self.__para.split('|')[16])

		if self.__para and len( self.__para.split('|') ) >=18 and self.__para.split('|')[17] and self.__para.split('|')[17].upper()!="NONE":	self.vcomprim.SetValue( self.__para.split('|')[17] )
		if self.__para and len( self.__para.split('|') ) >=19 and self.__para.split('|')[18] and self.__para.split('|')[18].upper()!="NONE":	self.vlargura.SetValue( self.__para.split('|')[18] )
		if self.__para and len( self.__para.split('|') ) >=20 and self.__para.split('|')[19] and self.__para.split('|')[19].upper()!="NONE":	self.vexpessu.SetValue( self.__para.split('|')[19] )
		if self.__para and len( self.__para.split('|') ) >=21 and self.__para.split('|')[20] and self.__para.split('|')[20].upper()!="NONE":	self.metrocub.SetValue( self.__para.split('|')[20] )

		if self.__para and len( self.__para.split('|') ) >=22 and self.__para.split('|')[21] and self.__para.split('|')[21].upper()!="NONE":	self.codigo_beneficio.SetValue( self.__para.split('|')[21] )
		if self.__para and len( self.__para.split('|') ) >=23 and self.__para.split('|')[22] and self.__para.split('|')[22].upper()!="NONE":	self.motivo_beneficio.SetValue( self.__para.split('|')[22] )
		if self.__para and len( self.__para.split('|') ) >=24 and self.__para.split('|')[23] and self.__para.split('|')[23].upper()!="NONE":	self.reducacaoicms.SetValue( self.__para.split('|')[23] )
		if self.__para and len( self.__para.split('|') ) >=25 and self.__para.split('|')[24] and self.__para.split('|')[24].upper()!="NONE":	self.infomacoes_adicionais.SetValue( self.__para.split('|')[24] )
		...
		self.quimicos.Bind(wx.EVT_LEFT_DCLICK, self.importacaoQuimicos)

		self.vcomprim.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		self.vlargura.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		self.vexpessu.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		    
		self.calcularMetragem()
	    
		del _mensagem
	
	def importacaoQuimicos(self,event):

		MensagemQuimicos.importar=True
		MensagemQuimicos.filial=self.pAFilial
		arq_frame=MensagemQuimicos(parent=self, id=-1 )
		arq_frame.Centre()
		arq_frame.Show()

	def importacaoQuimicosGravar(self,codigo):	self.quimicos.SetValue(codigo)
	...
	def justarPfcp(self,event):
		...
		if receb.ShowModal() ==  wx.ID_YES:
			...
			if sql[0]:
				...
				lista_produtos = sql[2].execute('SELECT pd_regi, pd_codi, pd_nome, pd_para, pd_cfis , pd_cfir, pd_cfsc FROM produtos')
				gravar = False
				for i in sql[2].fetchall():
					    
					gravar = False
					tamanho = len(i[3].split('|') ) if i[3] else 0
					novo_parametro = i[3].split('|') if i[3] else []
					if tamanho < 11:
					...
					if i[4] and len(i[4].split('.'))>=4 and i[4].split('.')[2]=='0000':	gravar=True
					if i[5] and len(i[5].split('.'))>=4 and i[5].split('.')[2]=='0000':	gravar=True
					if i[6] and len(i[6].split('.'))>=4 and i[6].split('.')[2]=='0000':	gravar=True

					if gravar:
					    
					    sql[2].execute("UPDATE produtos SET pd_para='"+ __n +"' WHERE pd_codi='"+ str( i[1] )+"'")
	...
	def salvaImagem(self,event):
		...
		if sql[0] == True:
			...
			""" Usuario alerou o controle { Controlar produto } """
			if len(self.imagem_antgerior.split('|'))==4:	controle_alterardo=self.imagem_antgerior.split('|')[3]

			imagem_link = self.pd_imag.GetValue()+"|"+self.linkvideo.GetValue()+"|"+self.linkpdf.GetValue()+'|'+controle_alterardo
	...
	def evTRadio(self,event):
		...
		self.calculo.calcularProduto(0,self, Filial = self.pAFilial, mostrar = True, retornar_valor = False, custo=(False,'') )
	...
	def Teclas(self,event):
		...
		self.calculo.calcularProduto( __id, self, Filial = self.pAFilial, mostrar = True, retornar_valor = False, custo=(False,'') )
	...
	def Tvalores(self,valor,idfy):
		...
		try:
			...
			if idfy == 612:	self.vcomprim.SetValue( valor )
			if idfy == 613:	self.vlargura.SetValue( valor )
			if idfy == 614:	self.vexpessu.SetValue( valor )
			
			self.calculo.calcularProduto( idfy, self, Filial = self.pAFilial, mostrar = True, retornar_valor = False, custo=(False,'') )
	...
	def calcularMetragem(self):
		...
		avancar_qembalagen=False
		if self.pd_qtem.GetValue().strip() and len(self.pd_qtem.GetValue().strip().split('.'))==2 and self.pd_qtem.GetValue().strip().split('.')[0].isdigit() and self.pd_qtem.GetValue().strip().split('.')[1].isdigit():	avancar_qembalagen=True
		elif self.pd_qtem.GetValue() and self.pd_qtem.GetValue().strip().isdigit():	avancar_qembalagen=True

		_em = Decimal( self.pd_qtem.GetValue() ) if avancar_qembalagen else Decimal("0")
	...
	def concluir(self,event):
		...
		if self.pd_qtem.GetValue() and not avancar_qembalagen: #self.pd_qtem.GetValue().isdigit() == False:
		...
		if self.pd_qtem.GetValue() and Decimal(self.pd_qtem.GetValue()) and self.vara_ferro.GetValue() and Decimal(self.vara_ferro.GetValue()):

			alertas.dia(self,u"{ Quantidade de embalagens e Ferro peso p/unidade }\n\n1-Embalagen: "+str(self.pd_qtem.GetValue())+u"\n2-Ferro: "+str(self.vara_ferro.GetValue())+u"\n\nApenas uma das opções\n"+(" "*150),"Cadastro de Produtos")
			return
		...
		_eendere = self.pd_ende.GetValue().upper()[0:30]	
		...
		permitir_estoque_negativo = True if len(login.filialLT[___filial][35].split(";"))>=5 and login.filialLT[___filial][35].split(";")[5]=="T" else False
		...
		if sql[0] == True:
			...
			if event.GetId() == 330 and not ___inalte and self.rfilial.GetValue():
				...
				if __rcodigo or __rdescri:
					...
					localizado = ""
					if __rcodigo:	localizado +="Codigo ja consta na filial destino" #"Localizado por codigo do produto\n"	
					if __rdescri:	localizado +=u"Descrição ja consta na filial destino:" #"Localizado por descrição do produto\n"	

					conn.cls(sql[1])
					alertas.dia(self.painel,u"{  ID-Filial remota para inclusão "+ ___filial+"  }\n\n"+localizado+"\n"+(" "*140),u"Validação do código do produto na filial remota")
					return

			if self.pd_clou.GetValue().strip() and sql[2].execute("SELECT pd_codi,pd_nome FROM produtos WHERE pd_clou='"+ self.pd_clou.GetValue().strip() +"'"):
			    
			    resultado=sql[2].fetchone()
			    outro_produto=resultado[1]
			    codigo_produto=resultado[0]
			    
			    if codigo_produto != self.pd_codi.GetValue():

				conn.cls(sql[1])

				alertas.dia(self.painel,u"{ Corte-cloud [ Codigo ja foi utilizado para outro produto ] }\n\nProduto: "+ outro_produto +"\n"+(" "*180),u"Validação do código do produto p/corte-cloud")
				return
			...
			codigo_cortes = self.pd_barr.GetValue()
			...
			try:   
			    ...
			    controle_alterardo = ''
			    _preco_busca_preco = nF.para_reais(self.pd_tpr1.GetValue())

			    """ Usuario alerou o controle { Controlar produto } """
			    if self.imagem_antgerior and len(self.imagem_antgerior.split('|'))==4:	controle_alterardo=self.imagem_antgerior.split('|')[3]
			    if self.controle_anterior=='T' and not self.pd_cont.GetValue():	controle_alterardo=datetime.datetime.now().strftime("%d/%m/%Y %T")+' '+login.usalogin
			    if self.controle_anterior!='T' and self.pd_cont.GetValue():	controle_alterardo=''
			    ...
			    parametro_ajuste = _pis+"|"+_cof+"|"+str( self.vendarp.GetValue() )[:1]+"|"+str( self.comprim.GetValue() )+"|"+str( self.largura.GetValue() )+"|"+str( self.expessu.GetValue() )+\
			    "|"+str( self.pisoqm2.GetValue() )+"|"+str( self.individ.GetValue() )[:1]+"|"+str( self.negativo.GetValue() )[:1]+'|'+str( self.paletes.GetValue() )+\
			    "|"+str( self.pfcp.GetValue() )+"|"+str( self.pd_bloq.GetValue() )[:1]+"|"+self.quimicos.GetValue()+"|"+self.exp_individual.GetValue()+"|"+str(self.vara_ferro.GetValue())+\
			    "|"+str( self.calculo_ferro.GetValue().split('-')[0] )+'|'+self.categoria.GetValue().strip()+\
			    '|'+str( self.vcomprim.GetValue() )+'|'+str(self.vlargura.GetValue())+'|'+str(self.vexpessu.GetValue())+'|'+str(self.metrocub.GetValue())+\
			    '|'+str( self.codigo_beneficio.GetValue() )+'|'+str( self.motivo_beneficio.GetValue() )+'|'+str( self.reducacaoicms.GetValue() )+\
			    '|'+str( self.infomacoes_adicionais.GetValue() )+'|'+str(self.naovender.GetValue())[:1]
			    ...
			    if ___inalte and _gravacao: #--->[ Alteração ]
				    ...
				    pd_marc='"+ mrcp +"',pd_acds='"+ _ad +"',pd_pdof='"+ mdof +"',pd_stcm='"+ str( self.pd_stcm.GetValue() ) +"',pd_qtem='"+ self.pd_qtem.GetValue().strip() +"',pd_altp='"+ ajPreco +"',\
				    pd_alte='"+ str( self.pd_alte.GetValue() )[:1] +"',pd_kitc='"+ str( self.pd_kitc.GetValue() )[:1] +"',\
				    pd_dtal='"+ EMD +"',pd_hcal='"+ DHO +"',pd_salt='A', pd_cest='"+ self.pd_cest.GetValue() +"',pd_para='"+ parametro_ajuste +"', pd_pcfl='"+ flf +"', pd_nvdf='"+ self.pd_nvdf.GetValue().upper() +"',\
				    pd_endd='"+ _endedep +"', pd_nser='"+str( self.pd_nser.GetValue() )[:1]+"', pd_clou='"+self.pd_clou.GetValue().strip()+"',\
				    pd_lvir='"+ self.codigo_woocommerce.GetValue() +"', pd_has3='"+_preco_busca_preco+"' WHERE pd_regi='"+ self.pd_regi.GetValue() +"'"
				    ...
				    pd_cfis='"+ self.pd_cfis.GetValue() +"',pd_cfir='"+ self.pd_cfir.GetValue() +"',pd_mark='"+ self.pd_mark.GetValue() +"',pd_ula1='"+ self.Atualizar +"',\
				    pd_ula2='"+ INC +"',pd_frac='"+ _frac +"',pd_docf='"+ self.pd_docf.GetValue() +"',pd_sug1='"+ _subgru1 +"',pd_sug2='"+ _subgru2 +"',\
				    pd_marc='"+ mrcp +"',pd_acds='"+ _ad +"',pd_pdof='"+ mdof +"',pd_stcm='"+ str( self.pd_stcm.GetValue() ) +"',pd_qtem='"+ self.pd_qtem.GetValue().strip() +"',\
				    ...
				    DHO = datetime.datetime.now().strftime("%T")
				    INC = EMD+' '+DHO+' '+login.usalogin
				    if permitir_estoque_negativo:	_cont='' #--/Permitir estoque negativo
				    else:	_cont='T'
				    ...
				    pd_nvdf,pd_endd,pd_nser,pd_clou,pd_lvir,pd_has3)\
				    ...
				    str( self.pd_cest.GetValue() ), parametro_ajuste, self.pd_nvdf.GetValue().upper(), _endedep, str( self.pd_nser.GetValue() )[:1], self.pd_clou.GetValue().strip(), self.codigo_woocommerce.GetValue(), _preco_busca_preco))

			    """   Atualizacao do endereco da filial no cadastro de estoque fisico  """
			    if self.filial_produto:

				    """ Grava primeiro na filial selecionada, verifica se essa filia estar configurada para replicar, e replica nas outras filias q estao configura para serem replicadas """
				    sql[2].execute("UPDATE estoque SET ef_endere='"+ self.pd_ende.GetValue().upper() +"' WHERE ef_codigo='"+ self.pd_codi.GetValue() +"' and ef_idfili='"+ self.filial_produto +"'")

				    if len( login.filialLT[self.filial_produto][35].split(';') ) >= 123 and login.filialLT[self.filial_produto][35].split(';')[122]=='T':

					    for i in login.ciaLocal:

						    if self.filial_produto != i.split('-')[0]: #--// Para nao gravar de novo

							    if len( login.filialLT[i.split('-')[0]][35].split(';') ) >= 123 and login.filialLT[i.split('-')[0]][35].split(';')[122]=='T':
								    sql[2].execute("UPDATE estoque SET ef_endere='"+ self.pd_ende.GetValue().upper() +"' WHERE ef_codigo='"+ self.pd_codi.GetValue() +"' and ef_idfili='"+ i.split('-')[0] +"'")
	...
	def desenho(self,event):
		...
		dc1.SetFont(wx.Font(15, wx.MODERN, wx.NORMAL,wx.FONTWEIGHT_BOLD,False,"Arial"))		
		dc1.SetTextForeground("#BFBFBF") 	
		dc1.DrawRotatedText("MT", 870, 160, 90)
		dc1.SetFont(wx.Font(15, wx.MODERN, wx.NORMAL,wx.FONTWEIGHT_BOLD,False,"Arial"))		

		dc.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.FONTWEIGHT_BOLD,False,"Arial"))		
		dc1.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.FONTWEIGHT_BOLD,False,"Arial"))		

		dc.SetTextForeground("#385C38") 	
		dc.DrawRotatedText("Físico", 0, 543, 90)
		dc.DrawRotatedText("Copia", 0, 609, 90)
		dc1.SetTextForeground("#385C38") 	
		dc1.DrawRotatedText("M2/M3", 0, 50, 90)
		dc1.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.FONTWEIGHT_BOLD,False,"Arial"))		
		dc1.DrawRotatedText("Quimico", 1, 110, 90)
		...
		dc1.SetTextForeground(cores.boxtexto)
		dc1.SetPen(wx.Pen(cores.boxcaixa, 1, wx.SOLID))
		dc1.SetBrush(wx.Brush('#CCCCFF', wx.TRANSPARENT))
		...
		dc1.DrawRoundedRectangle(12,  0, 885, 59, 3) #-->[ Endereço de Entrega ]
		dc1.DrawRoundedRectangle(12,  110, 885, 79, 3) #-->[ Endereço de Entrega ]

		dc1.DrawRoundedRectangle(12,280, 885, 180, 3) #-->[ KIT ]
...
class expedicaoPrinter(wx.Frame):
	...
	def __init__(self,parent,id,prd):
		...
		self.listaImpressoras = wx.ListCtrl(self.painel, -1, pos=(0,1), size=(450,310),
									style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.todospd = wx.CheckBox(self.painel, -1,  "Gravar impressora\nselecionada para\ntodos os grupos", pos=(315,315))
		self.todospd.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
	...
	def incluiPrinter(self,event):
		...
		if incP.ShowModal() ==  wx.ID_YES:
			...
			if self.todospd.GetValue():
			    for p in range(self.listaImpressoras.GetItemCount()):

				self.listaImpressoras.SetStringItem(p,2, imp)
				self.listaImpressoras.SetStringItem(p,3, fil)
	...
	def listaPrinters(self):
		...
		simm,impressoras, user, prn = self.f.listaprn( 1 )
				...
				if sql[0] == True:
					...
					try:
						...
						for i in range(nRg):
							...
							atualiza = "UPDATE grupofab SET fg_prin='"+impre+"',fg_fila='"+ifila+"' WHERE fg_desc='"+grupo+"' and fg_cdpd='G'"
...
class ProdutosRelatorios(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.nova_relacao = ""
		self.mostrarFisicoCodigo=False
		self.relatorio_vendas_grupo={}
		self.arquivo_csv = ""
		self.gertec_busca_preco=''
		...
		wx.Frame.__init__(self, parent, id, 'Produtos: Relação-Relatorios', size=(950,590), style=wx.CAPTION|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		...
		wx.StaticText(self.painel,-1,u"Fornecedor {Compras p/Fornecedor [Click Duplo]}", pos=(302,445)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,u"Pedido Compra TIPO {Tabelas}", pos=(787,412)).SetFont(wx.Font(7,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.pdDs = wx.StaticText(self.painel,-1,"["+self._cd+"] "+self._ds[:49], pos=(137,310))
		self.pdDs.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		...
		'23-Etiqueta Térmica 40x20 2 carreiras','24-Etiquetas para impressora ZPL','25-Relatorio de margens do KIT','26-Relatorio de produtos e NCMs',\
		'100-Etiqueta argox 107x25 1 carreira','101-Etiqueta argox 25x15 4 carreiras','102-Etiqueta argox 75x25 1 carreira','103-Etiqueta argox 40x20 2 carreiras']
		"""
		
		self.relacao  = ['01-Totalizar Estoque Fisico Inventario','02-Produtos com Estoque Zero','03-Produtos com Estoque Negativo',"04-Reserva Local {Virtual}",\
		'05-Totalizar Produtos Vendidos p/vendedor','06-Estoque Mínimo','07-Giro de Produto(s) { Curva ABC }','08-Ficha de Estoque',\
		'11-Tabela de Preços','12-Relatório de Contagem de Estoque','13-Relatório de Compras',"15-Produtos entregues entre filiais","16-Produtos comprados { Item,Fabricante, Grupo }",\
		'17-Produtos vendidos','18-Compras unidade de manejo extração','19-Produtos sem giro','20-Auxiliar compra { Ordenar fabricante }','21-Relação de produtos para corte-cloud','22-Estoque local { entrada/saida, vendas/saldos }',\
		'24-Etiquetas para impressora [ ZPL e EPL ]','25-Relatorio de margens do KIT','26-Relatorio de produtos e NCMs',u'27-Relatório de preços alterados','28-Tabela de preços { Gertec Busca Preço }']
		
		#'100-Etiqueta argox 107x25 1 carreira','101-Etiqueta argox 25x15 4 carreiras','102-Etiqueta argox 75x25 1 carreira','103-Etiqueta argox 40x20 2 carreiras',\
		#'104-Etiqueta argox 33x23 3 carreiras { Código e descrição }','105-Etiqueta argox 33x23 3 carreiras { Barras,Descricao,Preço }',\
		#'106-Etiqueta argox 100x50 1 carreira','107-Etiqueta argox 100x67 1 carreira','108-Etiqueta argox 83x40 1 carreira {Gondola}']
		...
		self.cmpTipo = wx.ComboBox(self.painel, 602, TipoCompra[0],  pos=(777,422), size=(164,27), choices = TipoCompra, style=wx.CB_READONLY )
		self.tabelas = wx.ComboBox(self.painel, 708, '',  pos=(777,450), size=(164,27), choices = [''], style=wx.CB_READONLY ) #/// N A O  UTILIZADA
		...
		self.fornecedo = wx.TextCtrl(self.painel,-1,value='', pos=(300,462), size=(440,20),style = wx.TE_READONLY)
		self.fornecedo.SetBackgroundColour('#E5E5E5')
		self.fornecedo.SetFont(wx.Font(10, wx.MODERN, wx.NORMAL, wx.BOLD))
		
		self.historico = wx.TextCtrl(self.painel,-1,value='', pos=(303,490), size=(635,60),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
		...
		self.sairfis = wx.CheckBox(self.painel, 508 , "Mostrar EstoqueFisico no código",  pos=(550,305))
		self.rFilial = wx.CheckBox(self.painel, 114 , "Filtrar Filial: { "+str( self.pRFilial )+" }", pos=(550,395))
		self.scodbar = wx.CheckBox(self.painel, 115 , "Sair codigo de barras p/corte-cloud", pos=(550,418))
		self.oendere = wx.CheckBox(self.painel, 115 , "Relatório 12, Ordernar p/Endereço", pos=(551,440))
		self.fixData = wx.CheckBox(self.painel,583,"Manter data inicial/final p/curva ABC", pos=(550,372))
		self.grvCur  = wx.CheckBox(self.painel,583,"Gravar resultado Curva ABC",      pos=(765,393))
		self.rl17gr  = wx.CheckBox(self.painel,584,"Filtrar relatório 17 por grupo",  pos=(15,563))
		self.rFilial.SetValue(True)
		self.sairfis.Enable(False)
		self.oendere.Enable(False)
		self.rl17gr.Enable(False)
		...
		self.scodbar.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.sairfis.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.oendere.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.rl17gr.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))

		self.limpFo = wx.BitmapButton(self.painel, 130, wx.Bitmap("imagens/simapaga16.png", wx.BITMAP_TYPE_ANY), pos=(740,460), size=(32,24))				
		...
		self.sairfis.Bind(wx.EVT_CHECKBOX, self.mudaFisico)
	...
	def mudaFisico(self,event):

	    registros = self.RLTprodutos.GetItemCount()
	    self.mostrarFisicoCodigo=self.sairfis.GetValue()
	    self.configurar()

	    if registros:	self.relacaoProdutos(wx.EVT_BUTTON)
	...
	def rCombobox(self,event):
		...
		if self.relator.GetValue().split('-')[0]=='24':
		    
		    ZebraEtiquetas.lista_produtos=[]
		    zb_frame=ZebraEtiquetas(parent=self,id=-1)
		    zb_frame.Centre()
		    zb_frame.Show()
		    
		else:    
		    ProdutosRelatorios._id = self.relator.GetValue().split('-')[0]
		    ...
		    #self.tabelas.Enable(False)
		    self.scodbar.Enable(False)
		    self.sairfis.SetValue(False)
		    self.sairfis.Enable(False)
		    self.rl17gr.Enable(False)
		    self.rl17gr.SetValue(False)

		    self.oendere.Enable(False)
		    self.oendere.SetValue(False)
		    ...
		    _ps = True
		    if self.relator.GetValue()[:2] in ["08","13","15","18","19","22","25"]:	_ps = False
		    ...
		    if self.relator.GetValue().split('-')[0] in ["22"]:

			    self.rexcel.Enable(False)
			    self.produT.Enable( True )
			    if self.relator.GetValue()[:2] == "22":	self.produT.SetValue( True )

		    if self.relator.GetValue()[:2] in ["11","26"]:
		    ...
		    if self.relator.GetValue()[:2] == "17":
			    ...
			    self.sairfis.Enable(True)
			    self.rl17gr.Enable(True)
		    ...
		    if self.relator.GetValue().split('-')[0] == "21":
			self.rexcel.Enable(False)
			self.scodbar.Enable(True)

		    if self.relator.GetValue().split('-')[0] in ["27","28"]:
			self.dindicial.Enable(True)
			self.datafinal.Enable(True)
			self.rFilial.Enable( False )
			self.rFilial.SetValue( False )
			self.rexcel.Enable(False)
	...
	def configurar(self):
		...
		if self.relator.GetValue()[:2] in ["20","21","22","25","28"]:
	...
	def relatorios(self,event):
		...
		if self.RLTprodutos.GetItemCount() == 0:	alertas.dia(self.painel,u"Sem registro na lista...\n"+(" "*80),"Cadastro de Produtos: Relação e Relatorios")
			...
			if self.relator.GetValue().split('-')[0] in ["21","28"]:

				_mensagem = mens.showmsg("Montando Arquivo de emails de clientes\nNº de Etiquetas: {"+ str( self.RLTprodutos.GetItemCount() ) +"}\n\nAguarde...")
				imprimir  = ''

				if self.relator.GetValue().split('-')[0]=="28":
				    _nomeArq = diretorios.busca_preco+"PRICETAB.TXT"
				    imprimir = self.gertec_busca_preco
				    __arquivo = open(_nomeArq,"w")
				    __arquivo.write(self.gertec_busca_preco)
				    __arquivo.close()
				
				else:
				    _nomeArq = diretorios.usPasta+login.usalogin.lower()+"_cortecloud.csv"
				    c = csv.writer( open( _nomeArq, "wb") )
				    c.writerow(["codigo","descricao","preco","Fabricante"])

				    for i in range( self.RLTprodutos.GetItemCount() ):
					    
					    codigo = self.RLTprodutos.GetItem( i, 1 ).GetText()
					    descricao = nF.acentuacao(self.RLTprodutos.GetItem( i, 2 ).GetText())
					    preco = self.RLTprodutos.GetItem( i, 4 ).GetText()
					    fabricante = nF.acentuacao(self.RLTprodutos.GetItem( i, 5 ).GetText())

					    c.writerow([codigo,descricao,preco,fabricante])
					    imprimir += codigo +','+ descricao +','+ preco +','+fabricante+'\n'
...
#---------:	08-Ficha de Estoque		
				...
				vnd_vendas = "SELECT it_codi,it_nome,it_lanc,it_horl,it_qtan,it_quan,it_nmvd,it_ndav,it_clie,it_canc, it_cdfi,it_pibp,it_inde,it_tped FROM idavs WHERE it_lanc>='"+str(dI)+"' and it_lanc<='"+str(dF)+"' and it_codi='"+str(self._cd)+"' and it_futu!='T' and it_tped='1'"
				dev_vendas = "SELECT it_codi,it_nome,it_lanc,it_horl,it_qtan,it_quan,it_nmvd,it_ndav,it_clie,it_canc, it_cdfi,it_pibp,it_inde,it_tped FROM didavs WHERE it_lanc>='"+str( dI )+"' and it_lanc<='"+str( dF )+"' and it_codi='"+str( self._cd )+"' and it_futu!='T' and it_tped='1'"
				...
				can_vendas = "SELECT it_codi,it_nome,it_dcan,it_hcan,it_aesf,it_quan,it_usac,it_ndav,it_clie,it_canc, it_cdfi,it_pibp,it_inde,it_tped FROM idavs WHERE it_dcan>='"+str(dI)+"' and it_dcan<='"+str(dF)+"' and it_codi='"+str(self._cd)+"' and it_futu!='T' and it_tped='1'"
				can_devolu = "SELECT it_codi,it_nome,it_dcan,it_hcan,it_aesf,it_quan,it_usac,it_ndav,it_clie,it_canc, it_cdfi,it_pibp,it_inde,it_tped FROM didavs WHERE it_dcan>='"+str(dI)+"' and it_dcan<='"+str(dF)+"' and it_codi='"+str(self._cd)+"' and it_futu!='T' and it_tped='1'"
				...
				_mensagem = mens.showmsg("Ficha de estoque: cancelamento de compras!!\nAguarde...", filial =  self.pRFilial )
				...
				if Fichar !=0:
					...
					for f in lFicha:
						...
						historico=f[10]
						if f[8]=='2S' and f[12]=='VND' and sql[2].execute("SELECT cr_tipo FROM cdavs WHERE cr_ndav='"+ f[11] +"'") and sql[2].fetchone()[0]=='9':	historico='Venda consiguinada'
						if f[8]=='3E' and f[12]=='VND' and sql[2].execute("SELECT cr_tipo FROM cdavs WHERE cr_ndav='"+ f[11] +"'") and sql[2].fetchone()[0]=='9':	historico='Cancelamento de venda consiguinada'
						...
						relacao[_registros] = f[11],f[14]+" - "+_lanca,format(f[5],','),_ve,_vs,_sl,historico,f[8],f[12],f[13],_mT
						_registros +=1
...
#---------: 17-Produtos vendidos
				...
				pesquisa_vendas = "SELECT DISTINCT it_codi, it_codi, it_nome  FROM idavs WHERE it_lanc>='"+str( dI )+"' and it_lanc<='"+str( dF )+"' and it_tped='1' and it_canc='' ORDER BY it_nome"
				...
				if q_produtos:
					...
					for i in r_produtos:
					    _mensagem = mens.showmsg("{ Analisando vendas }\n\nProduto: "+i[2]+"\n\nAguarde...")
					    if self.relatorioVendas( i[1], i[2], sql, dI, dF ,_descric):	gravar_vendas = True
					del _mensagem
...
#---------: 19-Produtos sem giro
				...
				self.arquivo_csv = diretorios.usPasta+login.usalogin.lower()+"_semgiro.csv"
				c = csv.writer(open( self.arquivo_csv, "wb"))
				c.writerow(["codigo","produto","cliente","ultima venda","fabricante","endereco","quantidade","preco custo","preco venda","total custo","total venda"])
				...
				for i in [] if not sql[2].execute( sem_giro ) else sql[2].fetchall():
					...
					if entrar_lista and False in entrar_lista and not True in entrar_lista:
						...
						c.writerow([str(i[1]).replace(',',''),i[2].replace(',',''),cliente_lista[0].replace(',',''),datas_lista[0].replace(',',''),i[7].replace(',',''),i[9].replace(',',''),str(qt).replace(',',''),str(i[11]).replace(',',''),str(i[12]).replace(',',''),str(tc).replace(',',''),str(tv).replace(',','')])
						_registros +=1
				...
				self._oc.SetLabel(u"Ocorrências {"+str(_registros)+"}")
				del _mensagem
				self.historico.SetValue(self.arquivo_csv)
				...
				vendas  = "SELECT it_ndav,it_quan,it_lanc,it_horl,it_dcan,it_hcan,it_canc,it_inde, it_nmvd,it_usac,it_tped FROM idavs WHERE it_lanc>='"+ str( dI ) +"' and it_lanc<='"+ str( dF ) +"' and it_eloc!=0 and it_codi='"+ codigo +"'"
...
#--------------------// Relatorio do KIT
			elif self.relator.GetValue()[:2] == "25":
			    
			    _registros = 0
			    relacao = {}

			    if sql[2].execute('SELECT pd_codi, pd_nome,pd_kitc, pd_codk, pd_cokt FROM produtos WHERE pd_kitc="T" ORDER BY pd_nome'):
				...
				for i in sql[2].fetchall():
					...
					for iv in i[3].split('|'):
						...
						custo = custom = vendas = Decimal()
						if sql[2].execute('SELECT pd_pcus,pd_cusm, pd_tpr1 FROM produtos WHERE pd_codi="'+iv.split(';')[0]+'"'):
						    custo, custom, vendas = sql[2].fetchone()
						    venda_preco += vendas
						    venda_custo += custo
					...
					if venda_preco and venda_custo:
					    ...
					    venda_marge = self.T.arredonda( 1, (((venda_preco - venda_custo) / venda_custo) * 100 ) )
					
					relacao[_registros] = i[0],i[1],format(venda_preco,','),format(venda_custo,','),format(Decimal(venda_marge),',')+' %',i[3]
					_registros +=1
			...
			#-----------------------------------// Outros
			    ...
			    if self.relator.GetValue()[:2] == "27":
				_registros = 0
				relacao = {}
				_produto = "SELECT pd_regi, pd_codi, pd_nome, pd_nmgr, pd_fabr, pd_idfi, pd_altp FROM produtos ORDER BY pd_nome"
				if self.pGrupo.GetValue() and _descric:	_produto = "SELECT pd_regi, pd_codi, pd_nome, pd_nmgr, pd_fabr, pd_idfi, pd_altp FROM produtos WHERE pd_nmgr='"+ _descric +"' ORDER BY pd_nome"
				if self.psGru1.GetValue() and _descric:	_produto = "SELECT pd_regi, pd_codi, pd_nome, pd_nmgr, pd_fabr, pd_idfi, pd_altp FROM produtos WHERE pd_sug1='"+ _descric +"' ORDER BY pd_nome"
				if self.psGru2.GetValue() and _descric:	_produto = "SELECT pd_regi, pd_codi, pd_nome, pd_nmgr, pd_fabr, pd_idfi, pd_altp FROM produtos WHERE pd_sug2='"+ _descric +"' ORDER BY pd_nome"
				if self.pFabri.GetValue() and _descric:	_produto = "SELECT pd_regi, pd_codi, pd_nome, pd_nmgr, pd_fabr, pd_idfi, pd_altp FROM produtos WHERE pd_fabr='"+ _descric +"' ORDER BY pd_nome"
				
				if sql[2].execute( _produto ):

				    for i in sql[2].fetchall():
					retorno_validacao, valores_alteracao = nF.validacaoAlteracaoPreco(i[6], dI, dF)
					if retorno_validacao:

					    vlr1 = valores_alteracao[1]
					    vlr2 = valores_alteracao[2]
					    pa_1, pa_2, pa_3, pa_4, pa_5, pa_6 = vlr1[0], vlr1[1], vlr1[2], vlr1[3], vlr1[4], vlr1[5]
					    pl_1, pl_2, pl_3, pl_4, pl_5, pl_6 = vlr2[0], vlr2[1], vlr2[2], vlr2[3], vlr2[4], vlr2[5]
					    relacao[_registros] = str(i[5]),str(i[1]),str(i[2]),str(i[3]),str(i[4]),valores_alteracao[0],pa_1, pa_2, pa_3, pl_1, pl_2, pl_3, str(i[0])
			    ...
			    elif self.relator.GetValue()[:2] == "28":
				_registros = 0
				relacao = {}
				_produto = "SELECT pd_barr, pd_nome, pd_cara, pd_has3 FROM produtos WHERE pd_barr!='' and pd_canc=''  ORDER BY pd_barr"
				if sql[2].execute( _produto ):
					...
					self.gertec_busca_preco+=i[0]+'|'+i[1]+'|'+i[3]+'|10|10|RED|100|150|BLACK|WHITE||||\n'
					_registros+=1
				...
				if self.rFilial.GetValue() == True:
					...
					_produto = "SELECT t1.*,t2.ef_fisico,t2.ef_virtua,t2.ef_idfili,t2.ef_esloja, t2.ef_endere FROM produtos t1 inner join estoque t2 on ( t1.pd_codi=t2.ef_codigo and t2.ef_idfili='"+str( self.pRFilial )+"' ) WHERE t1.pd_canc!= '4' ORDER BY t1.pd_nome"
				...
				if self.pGrupo.GetValue()==True and _descric!='':	_produto=_produto.replace("ORDER BY t1.pd_nome","and t1.pd_nmgr='"+ _descric +"' ORDER BY t1.pd_nome")
				if self.psGru1.GetValue()==True and _descric!='':	_produto=_produto.replace("ORDER BY t1.pd_nome","and t1.pd_sug1='"+ _descric +"' ORDER BY t1.pd_nome")
				if self.psGru2.GetValue()==True and _descric!='':	_produto=_produto.replace("ORDER BY t1.pd_nome","and t1.pd_sug2='"+ _descric +"' ORDER BY t1.pd_nome")
				if self.pFabri.GetValue()==True and _descric!='':	_produto=_produto.replace("ORDER BY t1.pd_nome","and t1.pd_fabr='"+ _descric +"' ORDER BY t1.pd_nome")

				if self.pEnder.GetValue()==True and _descric!='':
				    if self.oendere.GetValue():
					_produto=_produto.replace("ORDER BY t1.pd_nome","and t2.ef_endere like '"+  _descric.upper() +"%' ORDER BY t2.ef_endere")
				    else:
					_produto=_produto.replace("ORDER BY t1.pd_nome","and t2.ef_endere like '"+ _descric.upper() +"%' ORDER BY t1.pd_ende,t1.pd_nome")
				...
				for i in _rca:
					...
					#--// Alteracao da tabela de precos para etiquetas
					valor_venda=format(i[28],',')
					...
					_esf = i[97] #---------------: Estoque Fisico
					_vir = i[98] #---------------: Estoque Virtual
					...
					if _ach == True:
						...
						_relatorio = _fil+"|"+_dsp+"|"+str( _esf )+"|"+_pcm+"|"+_pcu+"|"+_cum+"|"+_pcv+"|"+str( _vir )+"|"+_grp+"|"+_sg1+"|"+_sg2+"|"+_fab+"|"+_end+endereco_deposito+"|"+_ncm+'|'+_cod+'|'+str(estoque_local)
						...
						if self.relator.GetValue().split('-')[0] in ["01","02","03","04"]:
						...
						if self.relator.GetValue()[:2] == "06" and _sug and i[16]:
							
							if _sug and Decimal( _sug ) < 0:	_sug = str( _sug ).split('-')[1]
						...
						elif self.relator.GetValue()[:2] == "26":	relacao[_registros] = str(i[52]),str(i[2]),str(i[3]),str(i[8]),str(i[9]),str( _esf ),format(i[28],','),format(i[24],','),format(i[27],','),i[47],i[53],i[82]
						    ...
						    codigo=_bar if self.scodbar.GetValue() else _cod
						    relacao[_registros]=_fil, codigo,_dsp,_und,format( i[28], ',' ),_fab
	...
	def ordenarVendas( self, event ):
		...
		if sql[0] == True:	
			...
			listar_vendas  = "SELECT tc_usuari, tc_grup, tc_sbg1, tc_sbg2, tc_nome, tc_quantd, tc_infor2, tc_quant1, tc_quant2, tc_quant3, tc_valor2, tc_valor3, tc_davctr FROM tmpclientes WHERE tc_varia1='"+str( login.usalogin )+"' and tc_relat='VENDA' ORDER BY tc_nome"
			...
			if relacao_vendas:
				_mensagem = mens.showmsg(u"Totalizando reletório\n\nAguarde...")

				for i in sql[2].fetchall():

					codigo_estoque = str(i[2])+' ['+str(i[3])+']' if self.mostrarFisicoCodigo else str(i[0])
					relacao[_registros] = str( i[12] ), codigo_estoque,str( i[4] ),str( i[5] ),str( i[7] ),str( i[8] ),str( i[9] ), str( i[10] ),str( i[11] ), str( i[6] ), str(i[2]), str(i[3]),i[1]

					""" Calcular por grupo """
					if i[1] in self.relatorio_vendas_grupo:
					    quantidade, custo, venda, margem, desconto = self.relatorio_vendas_grupo[i[1]]
					    quantidade+=i[5]
					    custo+=i[7]
					    venda+=i[8]
					    margem+=i[9]
					    desconto+=i[10]
					    self.relatorio_vendas_grupo[i[1]]=quantidade, custo, venda, margem, desconto
					else:
					    self.relatorio_vendas_grupo[i[1]]=i[5], i[7], i[8], i[9], i[10]
	...
	def relatorioVendas(self, codigo, produto, _sql, dI, dF, _descric ):
		...
		pesquisa_vendas = "SELECT it_ndav, it_codi, it_nome, it_prec, it_quan, it_subt, it_vdes, it_nmvd, it_usac, it_inde, it_lanc, it_horl, it_cprc, it_ctot, it_clie FROM idavs WHERE it_lanc>='"+str( dI )+"' and it_lanc<='"+str( dF )+"' and it_codi='"+ codigo +"' and it_tped='1' and it_canc='' ORDER BY it_nome"
		...
		_sql[2].execute( pesquisa_vendas )
		lista = _sql[2].fetchall()
		...
		for i in lista:
		    ...
		    lista_devolucao = ""
		    if _sql[2].execute("SELECT cr_ndav FROM dcdavs WHERE cr_cdev='"+ str( i[0] )+"'"):

			    devolucoles =  _sql[2].fetchall()
			    d_margem_unitaria = '0.00'
			    _mensagem = mens.showmsg(u"{ Devolução }\n\nProduto: "+i[2].decode('UTF-8')+"\n\nAguarde...")
			    for ldv in devolucoles:
					    
#				    if _sql[2].execute("SELECT * FROM didavs WHERE it_ndav='"+ str( ldv[0] )+"' and it_codi='"+ str( i[1] )+"' and it_canc!='1'"):
				    if _sql[2].execute("SELECT it_ndav, it_codi, it_nome, it_prec, it_quan, it_subt, it_vdes, it_nmvd, it_usac, it_inde, it_lanc, it_horl, it_cprc, it_ctot, it_clie FROM didavs WHERE it_ndav='"+ str( ldv[0] )+"' and it_codi='"+ str( i[1] )+"' and it_canc!='1'"):

					    for ld in _sql[2].fetchall():

						    d_quantid_grupo += ld[4]
						    d_tlcusto_grupo += ld[13]
						    d_tlvenda_grupo += ld[5]
						    d_tdescon_grupo += ld[6]
						    d_numeros_davs  +=1

						    #d_quantid_grupo += ld[12]4
						    #d_tlcusto_grupo += ld[78]13
						    #d_tlvenda_grupo += ld[13]5
						    #d_tdescon_grupo += ld[28]6

						    d_margem_unitaria = format( ( ( ld[5] -  ld[13] ) / ld[13] * 100 ), '.2f' )  if ld[13] and ld[5] else "0.00"
		    ...
		    margem_unitaria = format( ( ( i[5] -  i[13] ) / i[13] * 100 ), '.2f' )  if i[13] and i[5] else "0.00"
		    rlistas_anali += str( i[9] )+'|'+str( i[0] )+'|'+format( i[10],'%d/%m/%Y')+' '+str( i[11] )+' '+str( i[7] )+'|'+str( i[1] )+'|'+str( i[14] )+'|'+str( i[4] )+'|'+str( i[12] )+'|'+str( i[3] )+'|'+str( i[13])+'|'+str( i[5] )+'|'+str( margem_unitaria )+'|'+lista_devolucao +'\n'
		    
		if quantid_grupo and tlvenda_grupo:
			
			_mensagem = mens.showmsg("{ Calculando }\n\nProduto: "+i[2]+"\n\nAguarde...")
			...
			"""  Busca o estoque fisico """
			fisico = "SELECT ef_fisico FROM estoque WHERE ef_codigo='"+ codigo+"'"
			if self.rFilial.GetValue() and self.pRFilial and nF.fu(self.pRFilial)!="T":
			    fisico=fisico.replace("WHERE","WHERE ef_idfili='"+ self.pRFilial +"' and")
			    _mensagem = mens.showmsg("{ Estoque Fisico }\n\nProduto: "+i[2]+"\n\nAguarde...")

			estoque_fisico=Decimal()
			if _sql[2].execute(fisico):
			    for ef in _sql[2].fetchall():
				estoque_fisico+=ef[0]
				_mensagem = mens.showmsg("{ Calculando Fisico }\n\nProduto: "+i[2]+"\n\nAguarde...")

			estoque_minimo=Decimal()
			nome_grupo=''
			if _sql[2].execute("SELECT pd_estm,pd_nmgr FROM produtos WHERE pd_codi='"+ codigo +"'"):
			    estoque_minimo,nome_grupo=_sql[2].fetchone()
			    _mensagem = mens.showmsg("{ Estoque Minimo e Maximo }\n\nProduto: "+i[2]+"\n\nAguarde...")

			    
			""" codigo,   produto,quantidad,analitico,T-custo,  T-venda,  Margem,   Tipo,    NumeroDavs,usuario,  desconto, descon% """
			_mensagem = mens.showmsg("{ Inserindo dados em temporarios }\n\nProduto: "+i[2]+"\n\nAguarde...")
			ins_vendas = "INSERT INTO tmpclientes (tc_usuari,tc_nome,tc_quantd,tc_infor2,tc_quant1,tc_quant2,tc_quant3,tc_relat,tc_davctr, tc_varia1,tc_valor2,tc_valor3,tc_sbg1,tc_sbg2,tc_grup) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
			_sql[2].execute( ins_vendas, ( codigo, produto, quantid_grupo, rlistas_anali, tlcusto_grupo, tlvenda_grupo, margem, "VENDA", str( numeros_davs ).zfill(5), login.usalogin, tdescon_grupo, descon, str(estoque_fisico), str(estoque_minimo),nome_grupo ) )
			retorno = True
	...
	def FichaEstoque(self,lista,Tipo,sql,TP):
		...
		for i in lista:
			...
			if TP=='CMP' and i[9] in ['8']:	pass #-// Pedido DEPOSITO>--->LOJA
			else:
				moTivo = ""
				__Tp = __Tipo = Tipo
		...
		if nRegis == 0:	alertas.dia(self.painel,u"Sem Registros na Lista...\n"+(" "*90),"Cadastro de Produtos: Relação e Relatorios")
			...
			if   self.relator.GetValue()[:2] == "07":	self.historico.SetValue(self.MeuHisTo)
				...
				for i in range(nRegis):
					...
					if _compra > 0:	self._sTTcp += _compra
					if _custos > 0:	self._sTTcu += _custos
					if _custom > 0:	self._sTTcm += _custom
					if _vendas > 0:	self._sTTvd += _vendas
				...
				"\nValor de Compra.....: "+format(Decimal(format(self._sTTcp,'.2f')),',')+\
				"\nValor de Custo......: "+format(Decimal(format(self._sTTcu,'.2f')),',')+\
				"\nValor de Custo Médio: "+format(Decimal(format(self._sTTcm,'.2f')),',')+\
				"\nValor de Venda......: "+format(Decimal(format(self._sTTvd,'.2f')),',')+\
	...
	def passagem(self,event):
		...
		if self.relator.GetValue()[:2] in ["01","02","03","04"]:
			...
			for i in _saida:
				...
				_mes = i.split("|")
				if _mes[0] !="":	_hs +=_mes[0]+" QTCompra: "+_mes[1]+" QTVendas: "+_mes[2]+u" QTDevolução: "+_mes[3]+"\n"
	...
	def aumentar(self,event):
		...
		if self.relator.GetValue()[:2] == "06":	self.tecladoNumerico( wx.EVT_BUTTON )
			...
			if self.RLTprodutos.GetItem(indice,8).GetText() == "CMP":	self.c.compras( self, NumeroDav, "1", Filiais = self.pRFilial, mostrar = True )
				...
				self.i.impressaoDav(NumeroDav,self,True,True,Devolucao,"",  servidor = self.pRFilial, codigoModulo = "", enviarEmail = "", recibo=False )
				self.i.impressaoDav(NumeroDav,self,True,False,Devolucao,"", servidor = self.pRFilial, codigoModulo = "", enviarEmail = "", recibo=False )
		...
		elif self.relator.GetValue()[:2] == "27": #// [ Editar produto ]
		    
			registro_produto = self.RLTprodutos.GetItem( self.RLTprodutos.GetFocusedItem() ,12).GetText()
			filial = self.RLTprodutos.GetItem( self.RLTprodutos.GetFocusedItem() ,0).GetText()
			ProdutosAlterar.filial_relacao = ''
			ProdutosAlterar.modo   = 150
			ProdutosAlterar.lsta   = ''
			ProdutosAlterar.compra = False
			ProdutosAlterar.alTF   = filial
			ProdutosAlterar.fiFL   = False
			ProdutosAlterar.regi   = registro_produto

			comp_frame=ProdutosAlterar(parent=self,id=-1)
			comp_frame.Center()
			comp_frame.Show()
	...
	def desenho(self,event):
		...
		dc.DrawRotatedText("Cadastro de Produtos: Relatorios { D i v e r s o s }", 0, 585, 90)

		dc.SetTextForeground(cores.boxtexto)
		dc.SetPen(wx.Pen(cores.boxcaixa, 1, wx.SOLID))
		dc.SetBrush(wx.Brush('#CCCCFF', wx.TRANSPARENT))
		dc.DrawRoundedRectangle( 12, 330, 933, 230, 3)
		dc.DrawRoundedRectangle( 300,484, 640, 70,  3)
...
class RLTListCtrl(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.attr2.SetBackgroundColour('#D5B4B9') #2
		...
		self.attr8.SetBackgroundColour('#DECBCB') #8
		...
		if _ID in ["01","02","03","04"]: #,"100","101","102","103","104","105","106","107","108"]:
		...
		if _ID in ["11","26","27"]:
			...
			if _ID=="27":
				self.InsertColumn(5, 'Usuário',   format=wx.LIST_ALIGN_LEFT,width=205)
				self.InsertColumn(6, 'Pc Anterior_1',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(7, 'Pc Anterior_2',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(8, 'Pc Anterior_3',   format=wx.LIST_ALIGN_LEFT,width=105)

				self.InsertColumn(9, 'Pc Alterado_1',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(10,'Pc Alterado_2',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(11,'Pc Alterado_3',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(12,'Registro',   format=wx.LIST_ALIGN_LEFT,width=105)
			else:
				...
				self.InsertColumn(6, 'Preço venda',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(7, 'Valor custo',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(8, 'Comissão',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(9, 'NCM-Primario',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(10,'NCM-NFCe',   format=wx.LIST_ALIGN_LEFT,width=105)
				self.InsertColumn(11,'NCM-Secundario',   format=wx.LIST_ALIGN_LEFT,width=105)
		...
		if _ID == "17":
			...
			if ProdutosRelatorios._sf.mostrarFisicoCodigo:
			    self.InsertColumn(1, 'EstoqueFisico [Minimo]', format=wx.LIST_ALIGN_LEFT, width=160)
			    self.InsertColumn(2, 'Descrição dos produtos', width=250)
			...
			self.InsertColumn(10,'Estoque fisico', width=100)
			self.InsertColumn(11,'Estoque minimo', width=100)
			self.InsertColumn(12,'Grupo', width=100)
		...
		if _ID == "21":
			...
			self.InsertColumn(5, 'Fabricante', width=200)
		...
		if _ID == "25":
			...
			self.InsertColumn(0, u'Código do produto', width=150)
			self.InsertColumn(1, u'Descrição dos produtos', width=400)
			self.InsertColumn(2, u'Preço de venda', format=wx.LIST_ALIGN_LEFT,   width=120)
			self.InsertColumn(3, u'Preço de custo', format=wx.LIST_ALIGN_LEFT,   width=120)
			self.InsertColumn(4, u'Margem', format=wx.LIST_ALIGN_LEFT, width=120)
			self.InsertColumn(5, u'Lista de produtos do kit', width=3000)

		if _ID == "28":

			self.InsertColumn(0, u'Código de Barras', width=150)
			self.InsertColumn(1, u'Descrição dos produtos', width=400)
			self.InsertColumn(2, u'Caracteristica',  width=215)
			self.InsertColumn(3, u'Preço de venda', format=wx.LIST_ALIGN_LEFT,   width=150)
	...
	def OnGetItemAttr(self, item): #Ajusta cores sim/nao
		...
		if self.itemIndexMap != []:
			...
			if _ID in ["11","26","27","28"] and item %2:	return self.attr1
			...
			#if _ID in ["09","10","14","23","25"] and item %2:	return self.attr6
			#if _ID in ["100","101","102","103","104","105","106","107","108","25"] and item %2:	return self.attr6
			if _ID in ["25"] and item %2:	return self.attr6
			...
			if _ID=="17" and Decimal(self.itemDataMap[index][10])<0:	return self.attr2
			if _ID=="17" and Decimal(self.itemDataMap[index][11])>=Decimal(self.itemDataMap[index][10]):	return self.attr8
	...
	def OnGetItemImage(self, item):
		...
		if self.itemIndexMap != []:
			...
			index=self.itemIndexMap[item]
			#if _ID in ["01","02","03","04","09","10","14","23","25"]:
			#if _ID in ["01","02","03","04","100","101","102","103","104","105","106","107","108","25"]:
			if _ID in ["01","02","03","04","25"]:
			...
			elif _ID in ["11","26","27","28"]:	return self.i_orc
...
class AjustarTodosPrecos(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Ajustar percentual p/Acréscimos-Descontos }', size=(380,480), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self)
		self.painel.Bind(wx.EVT_PAINT,self.desenho)
		...
		ajuste=wx.StaticText(self.painel,-1,"{ Gravar novos percentuais para os produtos }\n\n1-Recalcular valores de vendas em cima do preco 1\n2-Calcular p/grupos,sub-grupos,fabricantes e p/todos os produtos", pos=(5,290))
		ajuste.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		ajuste.SetForegroundColour('#7F7F7F')

		wx.StaticText(self.painel,-1,"A-D_2 %", pos=(8,  347)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,"A-D_3 %", pos=(83, 347)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,"A-D_4 %", pos=(158,347)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,"A-D_5 %", pos=(233,347)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,"A-D_6 %", pos=(307,347)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,"A-D_1 %", pos=(8,  387)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.pd_vdp1 = wx.TextCtrl(self.painel, id = 299, value = "", pos = (240, 20), size=(65,20), style = wx.ALIGN_RIGHT|wx.TE_READONLY )
		...
		self.pd_vdp22 = wx.TextCtrl(self.painel, id = 331, value = "", pos = (5,  360), size=(65,25), style = wx.ALIGN_RIGHT|wx.TE_READONLY )
		self.pd_vdp33 = wx.TextCtrl(self.painel, id = 332, value = "", pos = (80, 360), size=(65,25), style = wx.ALIGN_RIGHT|wx.TE_READONLY )
		self.pd_vdp44 = wx.TextCtrl(self.painel, id = 333, value = "", pos = (155,360), size=(65,25), style = wx.ALIGN_RIGHT|wx.TE_READONLY )
		self.pd_vdp55 = wx.TextCtrl(self.painel, id = 334, value = "", pos = (230,360), size=(65,25), style = wx.ALIGN_RIGHT|wx.TE_READONLY )
		self.pd_vdp66 = wx.TextCtrl(self.painel, id = 335, value = "", pos = (305,360), size=(70,25), style = wx.ALIGN_RIGHT|wx.TE_READONLY )
		self.pd_marg1 = wx.TextCtrl(self.painel, id = 336, value = "", pos = (5,  398), size=(70,25), style = wx.ALIGN_RIGHT|wx.TE_READONLY )
		...
		self.pd_vdp22.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.pd_vdp33.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.pd_vdp44.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.pd_vdp55.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.pd_vdp66.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.grvatod = wx.CheckBox(self.painel, -1 , "Marque para selecionar\nTodos os produtos",(5,425))
		self.uscusto = wx.CheckBox(self.painel, -1 , "Opção para ajustar o preço 1 e preencha A-D_1 %\ne utilizar o custo como base para o calculo",(80,386))
		...
		self.grvatod.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.uscusto.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.gravar_percentuais = GenBitmapTextButton(self.painel,704,label=' Gravar novos percentuais',  pos=(200,430),size=(175,30), bitmap=wx.Bitmap("imagens/reler16.png", wx.BITMAP_TYPE_ANY))
		...
		self.gravar_percentuais.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.gravar.SetBackgroundColour("#71A471")
		self.voltar.SetBackgroundColour("#AE6A6A")
		self.gravar_percentuais.SetBackgroundColour("#E8E8D3")
		...
		self.gravar_percentuais.Bind(wx.EVT_BUTTON , self.iniciaProcessoPrecos)
		self.voltar.Bind(wx.EVT_BUTTON , self.sair)

		self.pd_vdp22.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		self.pd_vdp33.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		self.pd_vdp44.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		self.pd_vdp55.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		self.pd_vdp66.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
		self.pd_marg1.Bind(wx.EVT_LEFT_DCLICK, self.TlNum)
	...
	def Tvalores(self,valor,idfy):
		...
		if valor and Decimal(valor.replace(',',''))<=9999.999:
			
			if '.' not in valor:	valor=valor+'.00'
			elif len(valor.split('.'))==2 and len(valor.split('.')[1])==1:	valor=valor.split('.')[0]+'.'+valor.split('.')[1]+'0'
			if valor and not Decimal(valor):	valor=''

			if idfy==331:	self.pd_vdp22.SetValue(valor)
			if idfy==332:	self.pd_vdp33.SetValue(valor)
			if idfy==333:	self.pd_vdp44.SetValue(valor)
			if idfy==334:	self.pd_vdp55.SetValue(valor)
			if idfy==335:	self.pd_vdp66.SetValue(valor)
			if idfy==336:	self.pd_marg1.SetValue(valor)
	
	def iniciaProcessoPrecos(self,event):

		avancar = True
		ajsute_custo = True if self.uscusto.GetValue() and self.pd_marg1.GetValue() else  False
		if not self.fzteste.GetValue() and not event.GetId()==704:

			recalcular = wx.MessageDialog(self,"Ajuste de produtos p/acréscimo-desconto\n\nConfirme para continuar\n"+(" "*130),"Produtos: acréscimo-desconto",wx.YES_NO|wx.NO_DEFAULT)
			if not recalcular.ShowModal() ==  wx.ID_YES:	avancar = False

		if not self.fzteste.GetValue() and event.GetId()==704 and not self.grvatod.GetValue():

			recalcular = wx.MessageDialog(self,"{ Ajuste e gravação dos novos percentuais }\n\n1-Ajuste de produtos p/acréscimo-desconto\n\nConfirme para continuar\n"+(" "*130),"Produtos: acréscimo-desconto",wx.YES_NO|wx.NO_DEFAULT)
			if not recalcular.ShowModal() ==  wx.ID_YES:	avancar = False

		if not self.fzteste.GetValue() and event.GetId()==704 and self.grvatod.GetValue():

			recalcular = wx.MessageDialog(self,"{ Ajuste e gravação dos novos percentuais para todos os produtos }\n\n1-Ajuste de produtos p/acréscimo-desconto\n\nConfirme para continuar\n"+(" "*160),"Produtos: acréscimo-desconto",wx.YES_NO|wx.NO_DEFAULT)
			if not recalcular.ShowModal() ==  wx.ID_YES:	avancar = False

		if self.uscusto.GetValue() and not self.pd_marg1.GetValue():
		    alertas.dia( self, u"{ Opção para utilização do custo com preço 1 }\n\n1-As opções A-D_1 % e opção para utlizar custo precisam estar setadas\n"+(" "*140),u"Ajuste para acréscimo-desconto")
		    return

		if not self.uscusto.GetValue() and self.pd_marg1.GetValue():
		    alertas.dia( self, u"{ Opção para utilização do custo com preço 1 }\n\n1-As opções A-D_1 % e opção para utlizar custo precisam estar setadas\n"+(" "*140),u"Ajuste para acréscimo-desconto")
		    return
		...
		if avancar:
			...
			if sql[0]:
				...
				tipo_ajuste = ""
				if event.GetId()==704 and self.grvatod.GetValue():	pass
				else:
					...
					try:
					    ...
					    for i in lista_produtos:
						    ...
						    if event.GetId()==704:
							    ...
							    if self.pd_vdp22.GetValue():	self.pd_vdp2.SetValue( self.pd_vdp22.GetValue() )
							    if self.pd_vdp33.GetValue():	self.pd_vdp3.SetValue( self.pd_vdp33.GetValue() )
							    if self.pd_vdp44.GetValue():	self.pd_vdp4.SetValue( self.pd_vdp44.GetValue() )
							    if self.pd_vdp55.GetValue():	self.pd_vdp5.SetValue( self.pd_vdp55.GetValue() )
							    if self.pd_vdp66.GetValue():	self.pd_vdp6.SetValue( self.pd_vdp66.GetValue() )
							    
						    _mensagem = mens.showmsg("Verificando produto: "+str( i[3] )+"\n\n"+str( numer_produtos )+"/"+str( nprd )+"\n\nAguarde...")
						    
						    self.calculo.calcularProduto( 0, self, Filial=self.mfilial, mostrar=True, retornar_valor=False, custo=(ajsute_custo,self.pd_marg1.GetValue()) )
						    lista +=i[2]+" "+i[3]+\
						    ...
						    if self.fzteste.GetValue() and numer_produtos >=20:	break
							    ...
							    if event.GetId()==704:
									...
									atualiza = "UPDATE produtos SET pd_tpr2='"+str( self.pd_tpr2.GetValue() )+"',pd_tpr3='"+str( self.pd_tpr3.GetValue() )+"',pd_tpr4='"+str( self.pd_tpr4.GetValue() )+"',\
										    ...
										    pd_vdp1='"+self.pd_vdp1.GetValue()+"',pd_vdp2='"+self.pd_vdp2.GetValue()+"',pd_vdp3='"+self.pd_vdp3.GetValue()+"',pd_vdp4='"+self.pd_vdp4.GetValue()+"',\
										    pd_vdp5='"+self.pd_vdp5.GetValue()+"',pd_vdp6='"+self.pd_vdp6.GetValue()+"',pd_marg='"+self.pd_marg1.GetValue()+"',pd_tpr1='"+str( self.pd_tpr1.GetValue() )+"',\
										    pd_acds='"+str( acrdesco )+"',pd_altp='"+str( ajP )+"' WHERE pd_codi='"+str( i[2] )+"'"

								    else:
									atualiza = "UPDATE produtos SET pd_tpr2='"+str( self.pd_tpr2.GetValue() )+"',pd_tpr3='"+str( self.pd_tpr3.GetValue() )+"',pd_tpr4='"+str( self.pd_tpr4.GetValue() )+"',\
										    pd_tpr5='"+str( self.pd_tpr5.GetValue() )+"',pd_tpr6='"+str( self.pd_tpr6.GetValue() )+"',pd_marc='"+str( marcacao )+"',\
										    pd_vdp2='"+self.pd_vdp2.GetValue()+"',pd_vdp3='"+self.pd_vdp3.GetValue()+"',pd_vdp4='"+self.pd_vdp4.GetValue()+"',\
										    pd_vdp5='"+self.pd_vdp5.GetValue()+"',pd_vdp6='"+self.pd_vdp6.GetValue()+"',\
										    pd_acds='"+str( acrdesco )+"',pd_altp='"+str( ajP )+"' WHERE pd_codi='"+str( i[2] )+"'"
								    ...
								    if ajsute_custo:
									atualiza = "UPDATE produtos SET pd_tpr2='"+str( self.pd_tpr2.GetValue() )+"',pd_tpr3='"+str( self.pd_tpr3.GetValue() )+"',pd_tpr4='"+str( self.pd_tpr4.GetValue() )+"',\
										    pd_tpr5='"+str( self.pd_tpr5.GetValue() )+"',pd_tpr6='"+str( self.pd_tpr6.GetValue() )+"',pd_marc='"+str( marcacao )+"',pd_marg='"+self.pd_marg1.GetValue()+"',pd_tpr1='"+str( self.pd_tpr1.GetValue() )+"',\
										    pd_acds='"+str( acrdesco )+"',pd_altp='"+str( ajP )+"' WHERE pd_codi='"+str( i[2] )+"'"
								    else:
									atualiza = "UPDATE produtos SET pd_tpr2='"+str( self.pd_tpr2.GetValue() )+"',pd_tpr3='"+str( self.pd_tpr3.GetValue() )+"',pd_tpr4='"+str( self.pd_tpr4.GetValue() )+"',\
										    pd_tpr5='"+str( self.pd_tpr5.GetValue() )+"',pd_tpr6='"+str( self.pd_tpr6.GetValue() )+"',pd_marc='"+str( marcacao )+"',\
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(0, 285,  380, 140, 3) #-->[ Lykos ]
