...
from eletronicos.__init__ import CODIGOS_UF
...
#---{ Eventos }-------------------------------------------------// Status do sevidor	
	def status( self, parent, dados ):
		...
		rtn, snh, cer, sis, sie, amb, uf, fus, __cnpj, __nfce, regime_tributario,__amb = informes.certificados( dados[0], parent, nf_modelo=dados[1] )
		if not rtn:	return
		...
		if rtn:
			...
			try:
				...
				rt = informes.retornosSefaz( xml, ["retConsStatServ"], parent, nome_evento = 'STATUS-Lykos' )
	...
	def consultarCadastro(self, parent, filial ):
		...
		rtn, snh, cer, sis, sie, amb, uf, fus, __cnpj, __nfce, regime_tributario,__amb = informes.certificados( filial, parent )
		if not rtn:	return
		...
		if rtn:
			...
			del _mensagem
			rt = informes.retornosSefaz( xml_retorno, ["soap:Reason"], parent, nome_evento = 'CONSULTA CADASTRO-Lykos' )

			if rt[0]:	informes.mostrarInformacoesRetorno( parent, rt[1],   rt[2], 1,'',rt[4],'',filial, '', '', '' )
...
#---{ Eventos }-------------------------------------------------// Inutilizacao da NF	
	def inutilizarNfe(self, parent, filial, ambiente, nota_fiscal, motivo, nfe_nfce, dav, id_lancamento, tipo_pedido, serie ):
		...
		rtn, snh, cer, sis, sie, amb, uf, fus, __cnpj, __nfce, regime_tributario,__amb = informes.certificados( filial, parent, nf_modelo=nfe_nfce )
		if not rtn:	return

		NotaFiscalParametros.ambiente = int( ambiente )
		NotaFiscalParametros.modelo = nfe_nfce
		NotaFiscalParametros.serie = str( serie ).zfill(3)
		informes.gravarRetorno( nfe_nfce, serie, nota_fiscal, ambiente, filial )
		...
		if not erro:
			...
			modulo = 1
			rt = informes.retornosSefaz( xml_retorno, ["retInutNFe"], parent, nome_evento = 'INUTILIZACAO' )
			retorno = rt[1]+ '\n'
			...
			if rt[4] in ['102','563']:
				...
				if dav:	parent.NFEinuTiliza( rT = ( rt[5], rt[6], retorno, motivo, dav, id_lancamento, tipo_pedido, filial, xml_retorno ) )
				
			if rt[0]:

				html = xml_retorno if '!DOCTYPE html' in xml_retorno else ''
				informes.mostrarInformacoesRetorno( parent, retorno, rt[2], modulo, str( nfe_nfce ), rt[4], xml_retorno, filial, '', '', html )
...
#---{ Eventos }-------------------------------------------------// Cancelamento da NF
	def cancelamentoNfe(self, parent, filial='', xml='', motivo='', origem = '', retorno_html = '' ):
		...
		rt = informes.retornosSefaz( xml_retorno, ["retEnvEvento","infEvento"], parent, nome_evento = 'CANCELAMENTO' )
		...
		if rt[0]:
			...
			html = xml_retorno if '!DOCTYPE html' in xml_retorno else ''
			informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulo, modelo, rt[4], string_xml, filial, '', '', html )
	...
	def downloadNfe(self, parent, filial, chave, nsu ):
		...
		rt = informes.retornosSefaz( xml_retorno.encode('utf-8'), ["retDistDFeInt",], parent, nome_evento = 'DOWNLOAD' )
		if rt[0]:
			
			if rt[4] == '653':	parent.cancelamentoNotaFiscalCompras( chave, rt[4] )
			if rt[4] == '138':	parent.relacionarNotasXML( xml_retorno )
			else:	informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], 3, 'nfe', rt[4], '', filial, '', '', html )
	...
	def manifestoNfe(self, parent, filial, chave, evento ):
		...
		#--// Tipo do Evento:210200 - Confirmacao da Operação 210210 – Ciência da Operação 210220 - Deconhecimento da operacao 210240 - Operação não Realizada
		#--// Descrição do Evento: "Confirmacao da Operacao"  "Ciencia da Operacao"  "Desconhecimento da Operacao"	"Operacao nao Realizada"
		time_zone = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S') + '{}:{}'.format(strftime('%z')[:-2], strftime('%z')[-2:])
		...
		rt = informes.retornosSefaz( xml_retorno.encode('utf-8'), ["retEnvEvento","infEvento"], parent, nome_evento = 'MANIFESTO' )
		...
		if rt[4] in ['135']:	parent.gravarManifesto()
			...
			if rt[0]:
				...
				html = xml_retorno if '!DOCTYPE html' in xml_retorno else ''
				informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulo, mod, rt[4], '', filial, '', '', html )
	...
	def cartaCorrecao(self, parent, filial, __i ):
		...
		NotaFiscalParametros.serie = str( numero_serie ).zfill(3)
		time_zone = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S') + '{}:{}'.format(strftime('%z')[:-2], strftime('%z')[-2:])
		informes.gravarRetorno( chave[20:22], numero_serie, numero_nf, ambiente, filial )
		...
		rt = informes.retornosSefaz( xml_retorno.encode('utf-8'), ["retEnvEvento","infEvento"], parent, nome_evento = 'CCE' )
		...
		if rt[0]:
			...
			html = xml_retorno if '!DOCTYPE html' in xml_retorno else '' 
			informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulo, mod, rt[4], xml_retorno, filial, '', '', html )

	def consultaNotaFiscal(self, parent, chave, filial):

		rtn, snh, cer, sis, sie, amb, uf, fus, __cnpj, __nfce, regime_tributario,__amb = informes.certificados( filial, parent )
		mod = 'nfe' if chave[20:22] == '55' else 'nfce'
		modulo = 1
		NotaFiscalParametros.modelo = str( chave[20:22] )
		...
		rt = informes.retornosSefaz( xml_retorno.encode('utf-8'), ["nfeResultMsg"], parent, nome_evento = 'CONSULTAR_NF' )
		if rt[0]:
...
class DadosCertificadoRetornos:
	...
	def dadosIdentificacao(self,filial, modelo=55, gravar = False ):
		...
		aplicativo = i[7]+' '+i[8] #--// Nome do aplicativo + versao
		uf = i[12]
		ambiente = i[9]
		regime = i[11]
		...
		NotaFiscalParametros.modelo = str( modelo )
		NotaFiscalParametros.serie = serie_nfe if modelo == 55 else serie_nfce
		NotaFiscalParametros.cnpj = login.filialLT[filial][9]
		NotaFiscalParametros.uf = uf
		NotaFiscalParametros.versao = i[2]
		NotaFiscalParametros.gravar_envio_retorno = True

		return retorno, uf, ambiente, serie_nfe, serie_nfce, id_csc, numero_csc, aplicativo, regime

	def certificados(self,filial, parent, nf_modelo=55 ):
		...
		try:
			...
			NotaFiscalParametros.caminho_certificado = certificado
			NotaFiscalParametros.senha_certificado = senha
			NotaFiscalParametros.ambiente = int( d[9] )
			NotaFiscalParametros.modelo = str( nf_modelo )
			NotaFiscalParametros.serie = str( serie_nfce ).zfill(3)
			NotaFiscalParametros.cnpj = cnpj
			NotaFiscalParametros.uf = estado.upper()
			NotaFiscalParametros.versao = str( d[2] )

			if not os.path.exists(certificado):
				alertas.dia( parent, u'{ Certificado não localizado [ ' + certificado.split('/')[len(certificado.split('/') )-1]+' ] }\n'+(' '*140),'Nota fiscal')
				r = False
		...
		return r, senha, certificado, sistema, sistema_emissor, homologacao, estado, fuso, cnpj, nfce, regime, ambiente
...
#---{ Dados-Certificados Retornos SEFAZ }--------------------------// Visualizar retornos
	def mostrarInformacoesRetorno(self, parent, informacoes, retornos, modulos, modelo, cStat, xml, filial, filexml, email, html ):
	...
	def gravarRetorno(self, modelo, serie, nota, ambiente, filial):
		...
		diretorio_gravacao = diretorios.usFolder+'nfe400/' if int( modelo ) == 55 else diretorios.nfceacb+'nfce400/'
...
#---------------------------------------------------------------------------------------------------#
class LykosEmissaoNFe:

#	def emissaoNFe400( self, parent = '', filial = '', modelo = 55, tipo_dav = '1', tipo_emissao = '1', tipo_impressao = '0',origem='1', dados={}):
	def emissaoNFe400( self, parent='', filial='', modelo=55, informe_dav={}, dados={}):
    		
		self.p = parent

#-----[ IDENTIFICACAO DA NOTA FISCAL ]
		"""  IDENTIFICACAO DA NOTA FISCAL  """
		"""  emissao tip_dav { 1-Nota fiscal de vendas, 2-Nota fiscal de devolucao de vendas, 3-Nota fiscal de devolucao de compras RMA }  """
		"""  lancamento tipo_emissao {1-emissa, 2-para inutilizar, 3-cancelamento 4-inutilizada, 5-denagada  """
		"""  origem {1-vendas, 2-compras } """

		retorno, uf, ambiente, serie_nfe, serie_nfce, id_csc, numero_csc, aplicativo, regime = informes.dadosIdentificacao( filial, modelo=modelo, gravar = True)
		if not retorno: return
		serie = serie_nfe if modelo == 55 else serie_nfce

		retorno, numero_nota = numeros.VerificaNFE( self.p.numero_dav, Tnf = informe_dav['tipo_nota'], Filial = filial ) #-// Numero de NF gerado ou utilizando um numero ja gerado
		if not retorno:	numero_nota = numeros.numero("5","Numero da NFe",self.p, filial ) #-// Gerando um novo numero
		if not numero_nota:
		...
		if not retorno:
			...
			gravar_gerenciador = str(numero_nota), informe_dav['numero_dav'], '', modelo_nota,\
								 '2', informe_dav['origem'], ambiente, filial, serie, informe_dav['valor_nota'], informe_dav['cliente']
								 
			gravar_retorno.salvarNotaFiscal( emissao = int(informe_dav['tipo_nota']), gravacao = 1, dados = gravar_gerenciador, filial = filial )
		...
		forma_emissao_nota  = informe_dav['tipo_impressao']
		...
		data_emissao = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S') + '{}:{}'.format(strftime('%z')[:-2], strftime('%z')[-2:])
		...
		("dhSaiEnt",dados['dhSaiEnt']), #--// Data e Hora da saída ou de entrada da mercadoria / produto (AAAA-MM-DDTHH:mm:ssTZD)
		...
		("verProc",aplicativo), #--// versão do aplicativo utilizado no processo de emissão { 20 }
		("dhCont",dados['dhCont']), #--// Informar a data e hora de entrada em contingência contingência no formato (AAAA-MM-DDThh:mm:ssTZD) ex.: 2012-09-01T13:00:00-03:00.
		("xJust",dados['xJust']), #--// Informar a Justificativa da entrada
		("refNFe",dados['refNFe']) #--// Chave de acesso das NF-e referenciadas. Chave de acesso compostas por Código da UF (tabela do IBGE) + AAMM da emissão + CNPJ do Emitente + modelo, série e número da NF-e Referenciada + Código Numérico + DV. 
		])
...
#-----[ IDENTIFICACAO DO DESTINATARIO ]
		...
		nome_cliente = 'NF-E EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL' if ambiente == '2' else dc[0]
		...
		if dc[0]:
			...
			("IE",dc[3]), #--// Inscrição Estadual (obrigatório nas operações com contribuintes do ICMS)
...
#-----[ GRUPOS DE PRODUTOS ]
		...
		for i in range( numero_registros ):
			...
			tags_imposto,tags_pis, tags_cofins, tags_imp_dev = self.calculoTributosProduto(dav_tipo=informe_dav['tipo_nota'], indice=i)

			""" Adiciona o imposto em produto { tags_imposto,tags_pis, tags_cofins, tags_imp_dev RETORNO EM DICIONARIO P/MONTAGEM NA LIB } """
			cst_icms = '00' if int( self.p.editdanfe.GetItem(i,33).GetText().strip() ) == 0 else str( int( self.p.editdanfe.GetItem(i,33).GetText().strip() ) )
			cst_orig = self.p.editdanfe.GetItem(i,33).GetText().strip()[:1]
			prdo.append(nfe400imposto.impostosNfe400(icms=cst_icms,origem=cst_orig,tags=tags_imposto,vTotTrib=valor_produto_ibpt,pis=tags_pis,cofins=tags_cofins,crt=crt ))

#-----[ Devolucao de imposto ]
			""" Devolucao do imposto {IPI} em produto """
...
#-----[ TOTALIZACAO DOS TRIBUTOS ]
		nota_fiscal.append(nfe400imposto.totalizacaoNotaFiscal( tags = self.totalizarTributos(dav_tipo=informe_dav['tipo_nota'], valor_ibpt=str( valor_total_ibpt ), crt=crt)))

#-----[ Informacoes tipo de transporte { Modalidade do frete } ]
		nota_fiscal.append( nfe400.modalidadeFrete(tags=informe_dav['modFrete']) )

#-----[ Detalhar pagamentos, dicionario {'02':('2.5','2','0')},{'tPag':('vPag','tpIntegra','indPag')} ]
		nota_fiscal.append( nfe400.detalharPagamentos(tags=self.retornarPagamentos(informe_dav['pag'],informe_dav['tipo_nota'])) )

#-----[ Informacoes adicionais ]
		nota_fiscal.append( nfe400.informacoesAdicionais( tags = OrderedDict([('infAdFisco',''), ('infCpl',self.dadosAdicionais(self.p, filial, informe_dav['tipo_nota']))]) ) )
...
#-----[ PEDIDO DE AUTORIZACAO AO SEFAZ ]
		...
		xml_autorizado, status = msf.autotirzacaoSefaz( tags={'idLote':'1','indSinc':'1'}, xml=nota_fiscal, chave = chave_id )

		rt = informes.retornosSefaz( xml_autorizado, ["protNFe","retEnviNFe"], parent, nome_evento = 'EMISSAO' )
		html = ''
		modulos_nfe = 3
		pasta_xml_gravado = NotaFiscalParametros.gravar_envio_retorno_pasta + '/' +chave_id + '.xml' if NotaFiscalParametros.gravar_envio_retorno_pasta and NotaFiscalParametros.gravar_envio_retorno else ''
		...
		if status in ['100']: 
			...
			if self.p.origem_emissao == '160':	self.p.fecharNfe310( "160", numero_nf, filial )
			...
			gravar_gerenciador = informe_dav['numero_dav'], protocolo, data_recebimento, nchave, numero_nf, modelo_nota, informe_dav['origem'], csTaT, emissao_nota, pasta_xml_gravado, xml_autorizado, rt[1], numero_serie
			gravar_retorno.salvarNotaFiscal( emissao = int(informe_dav['tipo_nota']), gravacao = 2, dados = gravar_gerenciador, filial = filial )

			modulos_nfe = 2 #--// Opcao para habilitar os icones e alternar a cor de fundo { 1-status, 2-emissao, 3-erro na emissao }

		if rt[0]:	informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulos_nfe, '55', rt[4], xml_autorizado, filial, pasta_xml_gravado, self.p.emails, html )

	def totalizarTributos(self, dav_tipo='1', valor_ibpt='', crt='1'):

		_vBC = _vICMS = _vBCST = _vST = _vIPI = _vProd = _vFrete = _vDesc = _vOutro = _vNF = _vPIS = _vCOFINS = '0'
		if dav_tipo == '3':
			pass
		else:
	...
	def calculoTributosProduto(self, dav_tipo = '1', indice = 1):

		_pICMS = _vBC = _vICMS = _pMVAST = _vBCST = _vICMSS = _pIPI = _vBCIPI = _vIPI = _prcPIS = _prcCOF = _bacPIS = _bacCOF = _valPIS = _valCOF = _cstPIS = _cstCOF = ''
		ipi_percentual_devolucao = ipi_valor_devolucao = ''

		if dav_tipo == '3': ##--// Devolucao RMA
			pass
		else:
			_pICMS   = str(self.p.editdanfe.GetItem(indice,17).GetText().strip()) if self.p.editdanfe.GetItem(indice,17).GetText() and Decimal( self.p.editdanfe.GetItem(indice,17).GetText() ) else ''
			_vBC     = str(self.p.editdanfe.GetItem(indice,22).GetText().strip()) if self.p.editdanfe.GetItem(indice,22).GetText() and Decimal( self.p.editdanfe.GetItem(indice,22).GetText() ) else ''
			_vICMS   = str(self.p.editdanfe.GetItem(indice,27).GetText().strip()) if self.p.editdanfe.GetItem(indice,27).GetText() and Decimal( self.p.editdanfe.GetItem(indice,27).GetText() ) else ''
		...
		imposto_pis = {"CST":_cstPIS,"vBC":_bacPIS,"pPIS":_prcPIS,"vPIS":_valPIS}
		imposto_cofins = {"CST":_cstCOF,"vBC":_bacCOF,"pCOFINS":_prcCOF,"vCOFINS":_valCOF}
		imposto_devolucao = {"pDevol":ipi_percentual_devolucao,"vIPIDevol":ipi_valor_devolucao}

		return imposto_diversos, imposto_pis, imposto_cofins, imposto_devolucao

	def retornaProduto( self, modelo_nf = 55, dav_tipo = '1', indice = 0 ):

		_cProd=self.p.editdanfe.GetItem(indice,1).GetText().strip()
		_cEAN=self.p.editdanfe.GetItem(indice,2).GetText().strip() if not self.p.descartar_barras.GetValue() else ''
		...
		produtos = {
			"cProd":_cProd,"cEAN":_cEAN, "xProd":_xProd, "NCM":_NCM, "CEST":_CEST,	"indEscala":"",	"CNPJFab":"",
			"cBenef":"", "EXTIPI":"", "CFOP":_CFOP, "uCom":_uCom, "qCom":_qCom, "vUnCom":_vUnCom, "vProd":_vProd,
			"cEANTrib":_cEAN, "uTrib":_uTrib, "qTrib":_qTrib, "vUnTrib":_vUnTrib, "vFrete":_vFrete, "vSeg":"",
			"vDesc":_vDesc, "vOutro":_vOutro, "indTot":'1'
			}
	...
	def valorAproximadoTributos( self, produto_geral = 1, ibpt_produto = '', modelo = 55 ):
		...
		if produto_geral == 1:
			...
			if modelo == 55:	registros = ibpt_produto.GetItemCount()
			if modelo == 65:	registros = len( ibpt_produto )
			
			for i in range( registros ):
				
				if modelo == 55:	dados_ibpt = str( ibpt_produto.GetItem( i, 43 ).GetText().strip() )
				if modelo == 65:	dados_ibpt = ibpt_produto[i][94]
				if dados_ibpt:
	...
	def enderecos(self, modelo=55, dav_tipo = '1'):
		...
		cli_dados = cli_endere = ''
		endereco_nota = endereco_entrega = dados_cliente = []
		if modelo == 55:	cli_endere, cli_dados = self.p.destinat.GetValue().split('-')[0], self.p.cTClie[0] #-// Endereco para impressao 1-endereco, 2-endereco
		...
		if dav_tipo == '3': #-// Devolucao de compra RMA
			...
			dados_cliente.append( cli_dados[4] ) #// Inscricao estadual
	...
	def numeroCompoeChave(self, numero_nota ):
		...
		rcNF = ""
		for i in numero_nota:	rcNF +=_cNF[i]
	...
	def retornarPagamentos(self, lista, tipo_dav ):
...
#---{ Emissao NF }--------------------------// Confeccao da NF
	def nfe40( self, parent, tipo_emissao = 1, pagamentos = [] ):

		self.p = parent
		self.e = self.p.emails

		self.filial = self.p.filial_notas
		rtn, snh, cer, sis, sie, amb, uf, fus, __cnpj, __nfce, regime_tributario, __amb = informes.certificados( self.filial, self.p )

		if rtn:
			
			self.informacoes_clientes_gravacao = ''
			self.lista_forma_pagamentos = pagamentos
			...
			if '' in [uf_emi.strip(), uf_cli.strip(), uf_ide.strip() ]:
				...
				alertas.dia( self.p, u"{ Unidade federativa }\n\n1-Identificação da nfe\n2-Emitente\n3-Cliente\n\nPode estar vazio...\n"+(" "*120),u"Emissão da NFe 40.00")
				return
				
			"""  Enviar ao sefaz atraves da LIB  """
			if self.adicionandoProdutos( dados_identificacao, regime_tributario, pessoa_juridica_fisica, estado_destino, tipo_emissao ):
				...
				try:
					...
					#print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>: ',etree.tostring(nfe, encoding='unicode',pretty_print=False)	)		
					""" Assinatura """
				...
				if nota_enviada:
					...
					if envio[0] == 0:
						...
						if rt[0]:
							...
							html = envio[1].text if '!DOCTYPE html' in envio[1].text else ''
							informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulos_nfe, '55', rt[4], __xml, self.filial, arquivo_retorno, self.e, html )
...
#---{ Transportadora }--------------------------// Adicionar transportadora
	def adicionarTransportadora(self, documento, nome, dados ):
		...
		if nome and documento and dados:
			...
			endereco = dados[4].strip()
			if dados[5].strip():	endereco +=', '+dados[5].strip()
			if dados[6].strip():	endereco +=' '+dados[6].strip()
			
			transportadora = Transportadora( razao_social = nome,
							 tipo_documento = 'CNPJ' if len( documento ) == 14 else 'CPF',
							 numero_documento = documento.strip(),
							 inscricao_estadual = dados[0].strip(),
							 endereco_logradouro = endereco,
							 endereco_uf = dados[10].strip(),
							 endereco_municipio = dados[8].strip(),
			)

		return transportadora
		
#---{ Emissao NF }--------------------------// Adicionar produtos
	def adicionandoProdutos(self, nota_fiscal, regime_tributacao, pessoa, estado_destino, emissao ):
		...
		for i in range( self.p.editdanfe.GetItemCount() ):
			...
			if emissao == 3:	prod_outras_depesas_acessorias = Decimal( self.p.editdanfe.GetItem(i,64).GetText().strip() )
			...
			pis_percentual = self.p.editdanfe.GetItem(i,45).GetText().strip()
			pis_basecalculo = self.p.editdanfe.GetItem(i,47).GetText().strip()
			pis_valor = Decimal( self.p.editdanfe.GetItem(i,49).GetText().strip() )

			cofins_percentual = self.p.editdanfe.GetItem(i,46).GetText().strip()
			cofins_basecalculo = self.p.editdanfe.GetItem(i,48).GetText().strip()
			cofins_valor = Decimal( self.p.editdanfe.GetItem(i,50).GetText().strip() )
			...
			if indicador_destino == 2 and pessoa == '2' and Decimal( self.p.cTDavs[0][110].split(";")[1] ) and Decimal( self.p.cTDavs[0][110].split(";")[2] ) and Decimal( icms_partilha_fcp[0] ):
				...
				dfal_fcp_aliquota_inserido_uf_destino = icms_partilha_fcp[1] # Decimal('0.00') # pFCPUFDest icms_partilha_fcp[1]
				dfal_valor_icms_fcp_uf_destino = icms_partilha_fcp[5] #Decimal('0.00') # vFCPUFDest icms_partilha_fcp[5]
				dfal_basec_uf_destino = Decimal( icms_partilha_fcp[0] ) # vBCUFDest
				dfal_fcp_basec_uf_destino = Decimal( icms_partilha_fcp[0]) # vBCFCPUFDest
			...
			"""  ICMS 70/90  """
			_modBC = _vBC = _pICMS = _vICMS = _modBCST = _pMVAST = _vBCST = _vICMSST = ''

			if modalidade_icms in ['70','10','900']:

				__pICMS   = str( self.p.editdanfe.GetItem(i,17).GetText().strip() )
				__vBCICMS = str( self.p.editdanfe.GetItem(i,22).GetText().strip() )
				__vICMS   = str( self.p.editdanfe.GetItem(i,27).GetText().strip() )

				__pST    = str(self.p.editdanfe.GetItem(i,20).GetText().strip())
				__vBCST  = str(self.p.editdanfe.GetItem(i,25).GetText().strip())
				__vST    = str(self.p.editdanfe.GetItem(i,30).GetText().strip())
			...
			if Decimal( self.p.editdanfe.GetItem(i,19).GetText().strip() ):

				ipi_percentual = str(self.p.editdanfe.GetItem(i,19).GetText().strip() )
				ipi_base_calculo = str(self.p.editdanfe.GetItem(i,24).GetText().strip())
				ipi_valor = Decimal( self.p.editdanfe.GetItem(i,29).GetText().strip() )
			else:	ipi_percentual, ipi_base_calculo, ipi_valor = '','', Decimal()
			...
			nota_fiscal.adicionar_produto_servico(
				...
				icms_credito = Decimal('0.00'), #--// ICMSSN, vCredICMSSN
				pis_modalidade = mod_pis,
				pis_valor_base_calculo = pis_basecalculo,
				pis_aliquota_percentual = pis_percentual,
				pis_valor = pis_valor,
				cofins_modalidade = mod_cofins,
				cofins_valor_base_calculo = cofins_basecalculo,
				cofins_aliquota_percentual = cofins_percentual,
				...
				icms_70picms = _pICMS,
				icms_70vicms = _vICMS,
				icms_70mdbcst = _modBCST,
				icms_70pmvast = _pMVAST,
				icms_70predbcst = '0',
				icms_70vbcst = _vBCST,
				icms_70picmsst = '0',
				icms_70vicmsst = _vICMSST,
				ipidevolucao_percentual = ipi_percentual,
				ipidevolucao_valor = ipi_valor,
...
#---{ Emissao NF }--------------------------// Identificacao da Nota Fiscal
	def indentificacaoNotaFiscal(self, informar_dados_emitente, informar_dados_clientes, informar_dados_transportadora, emissao, dados_cliente, ambiente, pessoa, endereco_entrega_retira ):
		...
		if emissao in [2,3] and self.p.NumeroChav.GetValue().strip():	referenciar_nota = [self.p.NumeroChav.GetValue().strip()]
		if emissao == 3:	nfe_valor_nota = str( self.p.cTDavs[0][26] ).strip().replace(",","")
		else:
		...
		if not rT:
			...
			self.salvarNotaFiscal( emissao = emissao, gravacao = 1, dados = gravar_gerenciador, filial = self.filial )
		...
		"""  Total da partilha do icms p/consumidor final  p/venda interstadual  { Monfardini ES [ 23-06-2018 ] }"""
		...
		if emissao == 1:	lista_pagamentos = self.retornarPagamentos( self.lista_forma_pagamentos ) #--// Retorna com a relacao de formas de pagamentos ajustado para 4.00
		else:	lista_pagamentos = [] #--// RMA - Devolucao de compras
		...
		"""  Devolucao de vendas - RMA { Forma de pagamento 90-sem pagamento, tipo 2-outros} """
		numero_fatura =  str( int(self.p.numero_dav) ) if emissao == 1 else ''
		if emissao in [2,3]:	lista_pagamentos, ide_forma_pagamento, ide_tipo_pagamento = {'90':('0.00','','0')}, 2, 90
		...
		"""  informacoes complementares do clientes """
		icic = ide_informacoes_complementares_interesse_contribuinte if type( ide_informacoes_complementares_interesse_contribuinte ) == unicode else ide_informacoes_complementares_interesse_contribuinte.decode('utf-8')
	
		"""  ICMS ST na Devolicao RMA """
		if emissao == 3:

			stBc  = Decimal( str( self.p.cTDavs[0][16] ).strip() )
			stVl  = Decimal( str( self.p.cTDavs[0][17] ).strip() )
			vdIPI = Decimal( str( self.p.cTDavs[0][22] ).strip() )

		else:	stBc = stVl = vdIPI = Decimal('0.00')
		...
		nota_fiscal = NotaFiscal(
		   ...
		   informacoes_complementares_interesse_contribuinte = icic,
		   totais_icms_st_base_calculo = stBc,
		   totais_icms_st_total = stVl,
		   totais_icms_total_ipi_dev = vdIPI,
		   ...
		   transporte_veiculo_placa= transp_veiculo_placa,
		   transporte_veiculo_uf = transp_veiculo_uf,
		   transporte_veiculo_rntc = transp_Veiculo_rntc, 
		)
	...
	def informacoesCliente(self, emissao, ambiente, modelo_nf, parent ):
		...
		if emissao == 3: #-// Devolucao de compra RMA
			...
			if enderecamento:
				...
				cli_email    = informes.remover_acentos( cli_dados[16].decode('utf-8').strip() )
...
#---{ Emissao NF }--------------------------// Salvando informacoes de emissao, inutitilizacao, cancelamento e rejeicao nas tabelas de controles
	...
	def retornarPagamentos(self, lista ):
...
class EmissaNfce400:
	...
	def emitirNfce400( self, parent, numero_dav, modulo_emissao, pagamentos = '' ):
		...
		if not rT:
			...
			emissaoNfe400.salvarNotaFiscal( emissao = emissao, gravacao = 1, dados = gravar_gerenciador, filial = self.filial )
		...
		lista_pagamentos = emissaoNfe400.retornarPagamentos( pagamentos ) #--// Retorna com a relacao de formas de pagamentos ajustado para 4.00
		...
		nota_fiscal = NotaFiscal(
		   ...
		   data_saida_entrada = '', #--// em nfce nao pode colocar data de entrada e saida
		...
		if sql[0]:
			...
			for i in self.p.resul_items:
				...
				__pd = nome_produto if type( nome_produto ) == unicode else nome_produto.decode('utf-8')
			...
			if envio[0] == 0:
				...
				if rt[0]:
					...
					html = envio[1].content if '!DOCTYPE html' in envio[1].content else ''
					informes.mostrarInformacoesRetorno( parent, rt[1], rt[2], modulos_nfe, '65', rt[4], __xml, self.filial, arquivo_retorno, '', html )
