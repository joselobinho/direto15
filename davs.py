...
from eletronicos.cortecloud import CorteCloud, AdicionarItemsCloud
from eletronicos.woocmc import Wooc
...
class dav(wx.Panel):
	...
	def __init__(self, parent,_frame,myParente):
		...
		self.calcular_fcp_fixo = False
		self.utilizar_preco_custo = False
		#self.notafiscalcomplementar=False
		self.notafiscalcomplementar_chave=''
		self.table_precos_selecionada = ''
		self.tipo_vendas = ""
		self.endereco_entega = ""
		self.data_emissao_devolucao = ""
		...
		self.frete_proporcional = Decimal("0.00")
		self.acres_proporcional = Decimal("0.00")
		self.desco_proporcional = Decimal("0.00")
		...
		self.pode_consignar = True if len( login.usaparam.split(";") ) >=28 and login.usaparam.split(";")[27] == "T" else False
		self.precos_tabelas = ''
		self.relacao_servico_corte_cloud = []
		self.corte_cloud_servicos_separados=False
		self.corte_cloud_pesquisar_cliente=False
		sb.mstatus("Retaguarda de Vendas",0)

		""" Loja virutal """
		self.ecommerce_pedido = str()
		self.ecommerce_origem = str()
		self.ecommerce_cliente = str()
		self.ecommerce_frete = str()
		self.ecommerce_desconto = str()
		""" Emissao consolidados de Nota """
		self.consolida_emissao_nota_fiscal=False
		...
		self.ListaPro.InsertColumn(97, 'Expedicao grupo individual', width=200)
		self.ListaPro.InsertColumn(98, 'Corte-cloud { P-Produto S-Servico }', width=250)
		self.ListaPro.InsertColumn(99, 'Dados para cubagem automatica do DOF', width=400)
		self.ListaPro.InsertColumn(100,'Valor do corte', width=200)
		self.ListaPro.InsertColumn(101,'Produto bloqueado', width=200)
		self.ListaPro.InsertColumn(102,'Numero do pedido', width=200)
		self.ListaPro.InsertColumn(103,'Alterado o controle do produto', width=200)
		...
		self.informe = wx.StaticText(self,-1,"",pos=(406,367))
		...
		wx.StaticText(self,-1,"Código identificação", pos=(603,418)).SetFont(wx.Font(7.5, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		wx.StaticText(self,-1,"IBPT-Importado", pos=(503,380)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self,-1,"IBPT-Estadual",  pos=(603,380)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self,-1,"IBPT-Municipal", pos=(693,380)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self,-1,"Ajustar Preços {Fixar}", pos=(148,548)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.unpDS = wx.TextCtrl(self,13,value="",pos=(403,430), size=(420,20),style=wx.TE_READONLY)
		self.cdide = wx.TextCtrl(self,-1,value="",pos=(690,410), size=(133,20),style=wx.TE_READONLY)
		...
		self.ibpVFd = wx.TextCtrl(self,-1,value="0.00",pos=(403,392), size=(90,20),style=wx.TE_READONLY|wx.TE_RIGHT)
		self.ibpVFi = wx.TextCtrl(self,-1,value="0.00",pos=(500,392), size=(90,20),style=wx.TE_READONLY|wx.TE_RIGHT)
		self.ibpVEs = wx.TextCtrl(self,-1,value="0.00",pos=(600,392), size=(90,20),style=wx.TE_READONLY|wx.TE_RIGHT)
		self.ibpVMu = wx.TextCtrl(self,-1,value="0.00",pos=(690,392), size=(133,20),style=wx.TE_READONLY|wx.TE_RIGHT)
		...
		self.vpIBPT = wx.StaticText(self,-1,"{ Valor Aprox.Tributos }",pos=(280,549))
		...
		self.bloqueio_filial = wx.StaticText(self,-1,"",pos=(233,490))
		...
		_pt = login.filialLT[self.fildavs][40].split("|")[0].split(',')[0].split(';') if len(login.filialLT[self.fildavs])>=40 and login.filialLT[self.fildavs][40] else [""]
		tb1 = _pt[0]+'-1' if _pt and len(_pt)>=1 and _pt[0] else u"Tabela preço-1"
		tb2 = _pt[1]+'-2' if _pt and len(_pt)>=2 and _pt[1] else u"Tabela preço-2"
		tb3 = _pt[2]+'-3' if _pt and len(_pt)>=3 and _pt[2] else u"Tabela preço-3"
		tb4 = _pt[3]+'-4' if _pt and len(_pt)>=4 and _pt[3] else u"Tabela preço-4"
		tb5 = _pt[4]+'-5' if _pt and len(_pt)>=5 and _pt[4] else u"Tabela preço-5"
		tb6 = _pt[5]+'-6' if _pt and len(_pt)>=6 and _pt[5] else u"Tabela preço-6"
		tabelas_precos = '',tb1,tb2,tb3,tb4,tb5,tb6
		...
		self.tabprec = wx.ComboBox(self, 701, value='', pos=(146,562), size = (130,27), choices = tabelas_precos,style=wx.NO_BORDER|wx.CB_READONLY)
#		self.integra = wx.ComboBox(self, 801, value=login.integracao[0], pos=(831,382), size = (132,27), choices = login.integracao,style=wx.NO_BORDER|wx.CB_READONLY)
		#self.integra = wx.ComboBox(self, 801, value=login.integracao[0], pos=(831,352), size = (132,27), choices = login.integracao,style=wx.NO_BORDER|wx.CB_READONLY)

		""" Integracao """
		self.integracao_corte_cloud = GenBitmapTextButton(self,-1,label=u'  Marcenaria virtual\n  Corte cloud  ',  pos=(828,382),size=(135,32), bitmap=wx.Bitmap("imagens/cloud20.png", wx.BITMAP_TYPE_ANY))
		...
		#self.integracao_corte_cloud.Enable(False)

		self.integracao_woo_commerce = GenBitmapTextButton(self,-1,label=u'  Loja virtual\n  woo-commerce  ',  pos=(828,418),size=(135,32), bitmap=wx.Bitmap("imagens/woo32.png", wx.BITMAP_TYPE_ANY))
		...
		#self.integracao_woo_commerce.Enable(False)

		self.fixar_tabela = wx.CheckBox(self, -1 , "",(255,540))
		self.fixar_tabela.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.procur  = wx.BitmapButton(self, 224, wx.Bitmap("imagens/procurap.png",  wx.BITMAP_TYPE_ANY), pos=(150,500), size=(36,36))
		self.altera  = wx.BitmapButton(self, 225, wx.Bitmap("imagens/alterarm.png",  wx.BITMAP_TYPE_ANY), pos=(190,500), size=(36,36))
		self.regdel  = wx.BitmapButton(self, 226, wx.Bitmap("imagens/apagatudo.png", wx.BITMAP_TYPE_ANY), pos=(230,500), size=(36,36))
		self.cancela = wx.BitmapButton(self, 227, wx.Bitmap("imagens/apagarm.png",   wx.BITMAP_TYPE_ANY), pos=(270,500), size=(36,36))
		self.reimpri = wx.BitmapButton(self, 229, wx.Bitmap("imagens/consultar.png", wx.BITMAP_TYPE_ANY), pos=(310,500), size=(36,36))
		self.salvar  = wx.BitmapButton(self, 228, wx.Bitmap("imagens/savep.png",     wx.BITMAP_TYPE_ANY), pos=(359,500), size=(36,36))
		...
		self.TComa1 = wx.RadioButton(self,-1,u"Pedido   ",   pos=(12,460),style=wx.RB_GROUP)
		self.TComa2 = wx.RadioButton(self,-1,u"Orçamento",   pos=(12,485))
		self.TComa3 = wx.RadioButton(self,-1,u"Devolução",   pos=(12,510))
		self.TComa4 = wx.RadioButton(self,-1,u"Entrega Futura", pos=(12,535))
		self.TComa5 = wx.RadioButton(self,-1,u"Consignação", pos=(12,560))
		if not self.pode_consignar:	self.TComa5.Disable()
		...
		self.TComa5.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
		self.utilizar_preco_custo = False

		self.consultar = wx.TextCtrl(self, 100, "", pos=(20,600),size=(280, 30),style=wx.TE_PROCESS_ENTER)
		self.consultar.SetBackgroundColour('#E5E5E5')
		self.consultar.SetFont(wx.Font(13, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.integracao_corte_cloud.Bind(wx.EVT_BUTTON, self.integracaoApiCorteCloud)
		self.integracao_woo_commerce.Bind(wx.EVT_BUTTON, self.integracaoApiWood)
		...
		#self.integra.Bind(wx.EVT_COMBOBOX,self.integracaoApi)
		...
		self.TComa5.Bind(wx.EVT_RADIOBUTTON,self.evradio)
		self.TComa5.Bind(wx.EVT_RADIOBUTTON,self.utilizarPrecoCusto)
		self.ListaPro.Bind(wx.EVT_RIGHT_DOWN,self.MarcarDesmarcar)

#	def integracaoApiWood(self,event):
#	def integracaoApiCorteCloud(self,event):	self.corteClodPedido()
		...
		"""  Validar para davs remporarios em aberto  """
		if len(login.filialLT[self.fildavs][35].split(';'))>=165 and login.filialLT[self.fildavs][35].split(';')[164]=='T':	self.validacaoDavsAbertoLogin(wx.EVT_BUTTON)
		self.TComa3.Enable( acs.acsm("0603",True) )

		#if len(login.usaparam.split(";"))>=47 and login.usaparam.split(";")[46]=="T":	self.integracao_woo_commerce.Enable(False)
		#if len(login.filialLT[self.fildavs][35].split(";"))>=145 and login.filialLT[self.fildavs][35].split(";")[144]=="T":	self.integracao_corte_cloud.Enable(True)
		
	def validacaoDavsAbertoLogin(self,event):

	    conn = sqldb()
	    sql  = conn.dbc("DAV(s): Pedidos em aberto de dias anteriores", fil = self.fildavs, janela = self )

	    if sql[0]:

		numero_aberto = 0
		hoje = str( datetime.datetime.now().strftime("%Y-%m-%d") )
		if sql[2].execute("SELECT * FROM tdavs WHERE tm_logi='"+login.usalogin+"' and tm_lanc < '"+ hoje +"'ORDER BY tm_lanc"):
		...
		if numero_aberto:
		    ...
		    self.TComa5.Enable(False)
		    self.consultar.Enable(False)
		    self.pcodigo.Enable(False)
		    self.fabrica.Enable(False)

		    alertas.dia(self,u"{ Conferência de davs temporários em aberto }\n\nNúmero de temporários em aberto: [ "+ str(numero_aberto).zfill(3)+u" ]\n1 - Devolva todos os davs temporários, saia e entre na retaguarda para realizar vendas\n"+(" "*190),u"Davs temporários")
		
	def integracaoApiWood(self,event):
	    
	    if self.ListaPro.GetItemCount():	alertas.dia(self,u"Esvazie a lista de produtos para acessar a integração...\n"+(' '*180),u'Integração')
	    else:
		#__i = self.integra.GetValue().split('-')
		#if len(__i) == 2:
		#    if   __i[0] == "1":	self.corteClodPedido()
		 #   elif __i[0] == "2":
			
		#if len(login.usaparam.split(";"))>=47 and login.usaparam.split(";")[46]=="T":
		corte_cloud = Wooc(parent=self,id=-1)
		corte_cloud.Centre()
		corte_cloud.Show()	
		#else:	alertas.dia(self,u"Usuario sem acesso ao modulo woo-commerce..\n"+(' '*180),u'Integração')
		#self.integra.SetValue(login.integracao[0])

	def integracaoApiCorteCloud(self,event):	self.corteClodPedido()
		
	def corteCloudAjustaInclusao(self, ativar):	dav.davCV=ativar
	    
	def utilizarPrecoCusto(self,event):

		if self.TComa5.GetValue() and not self.ListaPro.GetItemCount():

			if len(login.filialLT[self.fildavs][35].split(';'))>=131 and login.filialLT[self.fildavs][35].split(';')[130]=='T':	self.utilizar_preco_custo = False
			else:
			    usar_custo = wx.MessageDialog(self,"Utilizar o valor de custo\n"+(" "*120),u"Pedido de consignação",wx.YES_NO)
			    if usar_custo.ShowModal() ==  wx.ID_YES:	self.utilizar_preco_custo = True
			    else:	self.utilizar_preco_custo = False
	...
	def ajustarPrecosTabelas(self,event):
		...
		__id = 702 if "PyEventBinder".upper() in str( event ).upper() else 701

		if __id == 701 and self.tabprec.GetValue() and self.ListaPro.GetItemCount():
			...
			if receb.ShowModal() ==  wx.ID_YES:	self.ajustarPrecosTabelasCalculo( __id, self.tabprec.GetValue().split('-')[1] )
		else:
			
			valor_tabela = ''
			if len( login.filialLT[self.fildavs][35].split(';') ) >= 124 and login.filialLT[self.fildavs][35].split(';')[123]:
				valor_tabela = self.ajustarPrecosTabelasCalculo( __id, login.filialLT[self.fildavs][35].split(';')[123] ) if login.filialLT[self.fildavs][35].split(';')[123].isdigit() else ''

			return valor_tabela

	def ajustarPrecosTabelasCalculo(self,__id, tabela):
		
		totaliza_tabela = Decimal()
		self.table_precos_selecionada = ''
		tnao = ""
		tsim = ""
		bk_tabela = tabela
		...
		if sql[0]:
			...
			for i in range( self.ListaPro.GetItemCount() ):
				...
				preco_produto_kit = self.ListaPro.GetItem( i, 5 ).GetText()
				kit = self.ListaPro.GetItem( i, 74 ).GetText()
				...
				if sql[2].execute("SELECT pd_tpr1, pd_tpr2, pd_tpr3, pd_tpr4, pd_tpr5, pd_tpr6 FROM produtos WHERE pd_codi='"+str( codigo )+"'"):
					...
					tabela = bk_tabela
					""" kit nao permitir alterar a tabela de preco apenas o preco 1 """
					if kit and len(login.filialLT[self.fildavs][35].split(';'))>=166 and login.filialLT[self.fildavs][35].split(';')[165]=="T":	tabela = "1"
					
					if tabela == '1' and selprecos[0]:	preco = selprecos[0]
					if tabela == '2' and selprecos[1]:	preco = selprecos[1]
					if tabela == '3' and selprecos[2]:	preco = selprecos[2]
					if tabela == '4' and selprecos[3]:	preco = selprecos[3]
					if tabela == '5' and selprecos[4]:	preco = selprecos[4]
					if tabela == '6' and selprecos[5]:	preco = selprecos[5]
					
					self.table_precos_selecionada = tabela

					""" Calculando o preco final em cima do preco cadastrado no KIT """
					if kit and len(login.filialLT[self.fildavs][35].split(";"))>=168 and login.filialLT[self.fildavs][35].split(";")[167]=="T":
					    preco = Decimal( preco_produto_kit.replace(',','') )

					if not preco and __id == 702:	totaliza_tabela +=Decimal( self.ListaPro.GetItem( i, 6 ).GetText().replace(',','') )
					if preco:
						...
						if __id == 701: #--// Atualiza a tabela
							...
							self.ListaPro.SetStringItem( i,64, tabela )

						else:
							if subt:	totaliza_tabela +=subt
			...
			#--// Atualiza a tabela
			if __id == 701:	self.reCalcula(False,True )
			#--// Atualiza a tabela

			if __id == 701:	alertas.dia(self,"{ Produtos atualizados }\n"+tsim+u"\n{ Produtos não atualizados }\n"+tnao+"\n"+(" "*140),u"Atualização de preços")

		if not self.fixar_tabela.GetValue():	self.tabprec.SetValue('')

		return totaliza_tabela
	...
	def recalcularDavCaixa(self):
		...
		self.TComa3.Enable( False )
		self.TComa4.Enable( False )
		if self.pode_consignar:	self.TComa5.Enable( False )
		self.consultar.Enable( False )
	...
	def relFilial(self,event):

	    self.arelFilial( event.GetId() )

	def arelFilial( self, _id ):

		self.fildavs = self.davfili.GetValue().split('-')[0]
		self.corte_cloud_servicos_separados = True if len( login.filialLT[ self.fildavs ][35].split(";") ) >=144 and login.filialLT[ self.fildavs ][35].split(";")[143]=='T' else False
	...
	def MarcarDesmarcar(self,event):
		...
		if self.TComa3.GetValue() or self.TComa2.GetValue() or self.TComa4.GetValue():
	...
	def evradio(self,event):
		...
		if self.TComa1.GetValue():	self.parente.SetTitle("{ PEDIDO } Retaguarda de Vendas Sistemas de DAV(s)")
		if self.TComa2.GetValue():	self.parente.SetTitle("{ ORÇAMENTO } Retaguarda de Vendas Sistemas de DAV(s)")
		if self.TComa3.GetValue():	self.parente.SetTitle("{ DEVOLUÇÂO } Retaguarda de Vendas Sistemas de DAV(s)")
		if self.TComa4.GetValue():	self.parente.SetTitle("{ SIMPLES FATURAMENTO-Entrega Futura } Retaguarda de Vendas Sistemas de DAV(s)")
		if self.TComa5.GetValue():	self.parente.SetTitle("{ Consignação } Retaguarda de Vendas Sistemas de DAV(s)")

		if   self.TComa1.GetValue():	self.abilitar(1)
		elif self.TComa2.GetValue():	self.abilitar(2)
		elif self.TComa3.GetValue():	self.abilitar(3)
		elif self.TComa4.GetValue():	self.abilitar(4)
		elif self.TComa5.GetValue():	self.abilitar(5)

		if self.TComa1.GetValue():	self.TipoVD.SetLabel("PEDIDO-DAV")
		if self.TComa2.GetValue():	self.TipoVD.SetLabel("ORÇAMENTO")
		if self.TComa3.GetValue():	self.TipoVD.SetLabel("DEVOLUÇÃO")
		if self.TComa4.GetValue():	self.TipoVD.SetLabel("Entrega Futura")
		if self.TComa5.GetValue():	self.TipoVD.SetLabel("Consignação")

		passar = True
		if self.TComa1.GetValue():	passar = acs.acsm("601",True) #-: Gerar Pedido
		if self.TComa2.GetValue():	passar = acs.acsm("602",True) #-: Gerar Orcamento
		if self.TComa3.GetValue():	passar = acs.acsm("603",True) #-: Gerar Devolucao
		if self.TComa4.GetValue():	passar = acs.acsm("609",True) #-: Gerar Entregas futuras
		...
		if not self.TComa5.GetValue():	self.utilizar_preco_custo = False	
	...
	def abilitar(self,_nm):
		...
		if _nm == 1: #--:[ Pedido ]
			...
			if _nm in [3,4]:	self.reimprimiDav(wx.EVT_BUTTON)
	...
	def TransFormarDAV(self,event):
		...
		"""  Produto com bloqueio temporario { Utilizado para contagem de estoque }  """
		if self.TComa2.GetValue() and len(login.filialLT[self.fildavs][35].split(";"))>=167 and login.filialLT[self.fildavs][35].split(";")[166]=="T":

		    marcado = 0
		    for i in range( self.ListaPro.GetItemCount() ):

			if self.ListaPro.GetItem(i, 101).GetText() == 'T':	marcado +=1
				
		    if marcado:
			alertas.dia(self,u"Orçamento com produtos bloqueados para venda...\n\n1 - Não permiti transformação para pedido\n"+(" "*160),"Transformar orçamento e pedido")
			return
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(10,  450, 390, 170, 3) #-->[ Pesquisa - Atalhos ]
	...
	def adicionarItem( self, codigo, produto, quantidade, unidade, preco, pcuni, clcom, cllar, clexp, clmet, ctcom, ctlar, ctexp, ctmet, sbToTal, UniPeca, mControle, mCorte, cEcf, _ippt, _fabr, _ende, _cbar, _cf, _edi, iditem, _qTDev, _qTDor, _slDev, _Temp, _Tabela, _pcusTo, _TCusTo, _Ddevolucao, importacao = False, valorManual = "0.000", auTorizacacao = False, moTAuT = "", kiTp = "", kiTv = "0.000", aTualizaPreco = False, codigoAvulso = "", FilialITem="", per_desconto = "0.00", vlr_desconto = "0.00", vinculado = "", estoque_local = "0", produto_servico='' ):
		...
		if not self.validaItem(quantidade, preco, sbToTal):	
		    alertas.dia(self,'{ Dados imcompativeis }\n\nQuantidade....: '+str(quantidade)+'\nValor unitario..: '+str(preco)+'\nSub-Total........: '+str(sbToTal)+'\n'+(' '*120),'Dados incompativeis')
		    return

		iTemFilial = FilialITem if FilialITem else self.fildavs
		dav.lista_reserva_temporaria = None
		...
		"""  Mostrar casas decimais com 2 digitos no dav { Quando for vendas de madeiras em M2,M3 pode da rejeicao na NFe se preco unitario for ex:3.569 } Preco com 2 casas decimais  """
		...
		produto_bloqueado = False
		dados_retorno = None
		expedicao_individual="" #// Impressora para descarregar o produto na expedicao
		cubagem_automatica_dof="" #// Dados para cubagem automatica
		venda_individualizada = ""

		#print 'simples'
		if self.sql[0]:
			#---:[ Pesquisa produto p/resgatar o crupo e busca a impressora para expedicao ]
			if self.sql[2].execute("SELECT pd_nmgr, pd_sug1, pd_sug2, pd_pdof, pd_refe, pd_tpr1, pd_tpr2, pd_tpr3, pd_tpr4, pd_tpr5, pd_tpr6, pd_para, pd_nser, pd_coms, pd_pdsc, pd_imag FROM produtos WHERE pd_codi='"+str( codigo )+"'"):
				...
				nsGrupo1 = _rsGru[0][1]
				nsGrupo2 = _rsGru[0][2]
				controle_alterado=_rsGru[0][15].split("|")[3] if _rsGru[0][15] and len(_rsGru[0][15].split("|"))==4 and _rsGru[0][15].split("|")[3] else ""
				
				#--// Percentual do FCP produto para venda no mesmo estado
				percentual_pFCP = format( Decimal( _rsGru[0][11].split('|')[10] ),'.2f') if len( _rsGru[0] ) >= 15 and _rsGru[0][11] and len( _rsGru[0][11].split('|') ) >= 11 and _rsGru[0][11].split('|')[10] else '0.00'
				controlar_serie = _rsGru[0][12] if _rsGru[0][12] and _rsGru[0][12] == 'T' else ''

				""" Dados para cubagem automatica """
				cuba_comp = cuba_larg = cuba_expe = '0.00'
				cuba_unid = '0.00'

				if _rsGru[0][11] and len( _rsGru[0][11].split('|') ) >=18 and _rsGru[0][11].split('|')[17]:	cuba_comp = _rsGru[0][11].split('|')[17]
				if _rsGru[0][11] and len( _rsGru[0][11].split('|') ) >=19 and _rsGru[0][11].split('|')[18]:	cuba_larg = _rsGru[0][11].split('|')[18]
				if _rsGru[0][11] and len( _rsGru[0][11].split('|') ) >=20 and _rsGru[0][11].split('|')[19]:	cuba_expe = _rsGru[0][11].split('|')[19]
				if _rsGru[0][11] and len( _rsGru[0][11].split('|') ) >=21 and _rsGru[0][11].split('|')[20]:	cuba_unid = _rsGru[0][11].split('|')[20]
				
				cubagem_automatica_dof = cuba_comp +'|'+ cuba_larg +'|'+ cuba_expe
				__ctcom = ctcom if ctcom and Decimal(ctcom) else "0.00"
				__ctlar = ctlar if ctlar and Decimal(ctlar) else "0.00"
				__ctexp = ctexp if ctexp and Decimal(ctexp) else "0.00"

				if cuba_comp and Decimal(cuba_comp):	__ctcom = cuba_comp
				if cuba_larg and Decimal(cuba_larg):	__ctlar = cuba_larg
				if cuba_expe and Decimal(cuba_expe):	__ctexp = cuba_expe

				if Decimal(__ctcom) and Decimal(__ctlar) and Decimal(__ctexp) and UniPeca and Decimal(UniPeca):

				    metragem_cubica = ( Decimal(UniPeca) * Decimal(__ctcom) * Decimal(__ctlar) * Decimal(__ctexp) )

				    __up = nF.eliminaZeros(formata.intquantidade(UniPeca))
				    __cm = nF.eliminaZeros(formata.intquantidade(__ctcom))
				    __la = nF.eliminaZeros(formata.intquantidade(__ctlar))
				    __ex = nF.eliminaZeros(formata.intquantidade(__ctexp))
				    __mc = nF.eliminaZeros(formata.intquantidade(metragem_cubica))

				    cubagem_automatica_dof = "[DOF]  "+__up+" PC X COM: "+__cm+" LAR: "+__la+" EXP: "+__ex+" ( "+__mc+" M3 )"
				
				if cuba_unid and Decimal(cuba_unid):
				    
				    metragem_cubica = nF.eliminaZeros( formata.intquantidade( ( Decimal(UniPeca) * Decimal(cuba_unid) ) ) )
				    cubagem_automatica_dof = "[DOF]  "+" ( "+metragem_cubica+" M3 )"
				...
				expedicao_individual=_rsGru[0][11].split('|')[13].split('-')[1] if _rsGru[0][11] and len(_rsGru[0][11].split('|'))>=14 and _rsGru[0][11].split('|')[13] else ''
				...
				if _rsGru[0][11]:
					...
					produto_bloqueado = True if len( _rsGru[0][11].split('|') ) >= 12 and _rsGru[0][11].split('|')[11] == 'T' else	False
			...
			""" Orçamento - Devolução """
			TipoDC = False
			if self.TComa2.GetValue():	TipoDC = True
			if self.TComa3.GetValue():	TipoDC = True
			if self.TComa4.GetValue():	TipoDC = True

			"""  Produto com bloqueio temporario { Utilizado para contagem de estoque }  """
			passar_bloqueio = True if self.TComa2.GetValue() and len(login.filialLT[iTemFilial][35].split(";"))>=167 and login.filialLT[iTemFilial][35].split(";")[166]=="T" else False
			bloqueio_temporario_produto = 'T' if produto_bloqueado else ''
			
			if produto_bloqueado and not passar_bloqueio:
				
				self.conn.cls( self.sql[1] )
				alertas.dia( self, "{ Produto selecionado com bloqueio temporario... }\n"+(" "*150),"Adicionando items")
				reTo = False
...
#-------------: Baixa do Pedido no Estoque de Reserva-Virutal			
				...
				if rTBx and rTBx[0] and rTBx[1] and rTBx[4] == "":
					...
					##--// Fora do estado com CST 60
					__icms =  rTos[2][0]
					__cst  = rTos[1][2]

					__icms, __cst, percentual_pFCP = self.fcpIcmsSt( rTos, iTemFilial, percentual_pFCP )
					...
					self.ListaPro.SetStringItem(indice,5, nF.eliminaZeros( preco ) if nsREF != "CORTE-0000007" else "0.00")
					self.ListaPro.SetStringItem(indice,6, sbToTal if nsREF != "CORTE-0000007" else "0.00")
					...
					self.ListaPro.SetStringItem(indice,25, str( __icms )) #-: ICMS
					...
					self.ListaPro.SetStringItem(indice,47, __cst) #-: CST
					...
					self.ListaPro.SetStringItem(indice,97,expedicao_individual) #-------: Expedicao individual
					self.ListaPro.SetStringItem(indice,98,produto_servico) #------------: Corte-cloud { P-produto, S-Servico }
					self.ListaPro.SetStringItem(indice,99,cubagem_automatica_dof) #-----: Cubagem automatica para o DOF
					if nsREF == "CORTE-0000007":	self.ListaPro.SetStringItem(indice,100,sbToTal) #-----: Valor total do cortes
					self.ListaPro.SetStringItem(indice,101,bloqueio_temporario_produto) #:Produto com bloqueio temporario
					if self.consolida_emissao_nota_fiscal:	self.ListaPro.SetStringItem(indice,102,vinculado) 
					self.ListaPro.SetStringItem(indice,103,controle_alterado)
				...
				#Mudanca 15-07-2020 Trca de linha. posicao if informe !="" and not listar_reserva:	alertas.dia(self,informe+"\n"+(" "*160),"Incluindo-Alterando Produtos na Relação de Vendas")

				#// Atualiza a lista para mostrar o estoque virtual sendo utilizado
				#print "A: ",listar_reserva,"B: ",rTBx[4]
				if listar_reserva and not rTBx[4]:	dav.lista_reserva_temporaria=listar_reserva,informa,iTemFilial,codigo
				if informe !="":	alertas.dia(self,informe+"\n"+(" "*160),"Incluindo-Alterando Produtos na Relação de Vendas") #Mudanca 15-07-2020 Troca de poiscao para depois
				
		return reTo

	def validaItem(self,q, p, s):

	    if not q:	return False
	    if q and not Decimal(q):	return False

	    if not p:	return False
	    if p and not Decimal(p):	return False

	    if not s:	return False
	    if s and not Decimal(s):	return False
		...
		if numeroReg !=0:
			...
			if sql[0] == True:
				...
				for i in range(numeroReg):
					...
					pFCP = str( self.ListaPro.GetItem( indice, 96 ).GetText() )

					rTos = rcTribu.calcularTributosNFs( self, _fl, sql, 1, codFiscal = _cf.strip(), clEstado = dadosCliente.esta )

					#--// Fora do estado com CST 60
					__icms, __cst, percentual_pFCP = self.fcpIcmsSt( rTos, _fl, pFCP )

					self.ListaPro.SetStringItem( indice, 25, str( __icms ) ) #  self.iICM )
					...
					self.ListaPro.SetStringItem( indice, 47, str( __cst ) ) #  self.iCST )
					self.ListaPro.SetStringItem( indice, 48, str( rTos[3][1].split("|")[0] ) ) #  self.iTIBPT
					self.ListaPro.SetStringItem( indice, 96, str( percentual_pFCP ) ) #  Percentual do FCP
					
					indice +=1

				conn.cls(sql[1])

	def fcpIcmsSt(self, rTos, fl, __pFCP ):

		#--// Fora do estado com CST 60
		__icms = rTos[2][0]
		__cst  = rTos[1][2]
		percentual_pFCP = __pFCP
		"""  Percentual do FCP do estado da filial no caso de destaque de FCP  """
		if len( self.icmsEsTado.split(' ') ) >= 4:	percentual_pFCP = login.aliquotaFcp[ self.icmsEsTado.split(' ')[3] ] if self.calcular_fcp_fixo else '0.00'

		"""
		Passa o CST 60 para 0000 para venda interestadual para clientes REGIME NORMAL { SEFAZ REJEITA SE O ICMS DIFAL NAO FOR CALCULADO }
		Rejeicao se o ICMS no for calculado no difal
		"""
		regime_tributario = login.filialLT[fl][30].split(';')[11]
		if rTos[1][2] and int( rTos[1][2] ) == 60 and self.clD.GetLabel() and len( self.clD.GetLabel().split(' ')[0] ) == 11 and len( self.icmsEsTado.split(' ') ) >= 3 and self.icmsEsTado.split(' ')[2] and regime_tributario in ['2','3']:

			__icms = self.icmsEsTado.split(' ')[2].split('%')[0]

		return __icms, __cst, percentual_pFCP
	...
	def pedidoServicoSeparadoCloud(self):

	    filial=self.relacao_servico_corte_cloud[0].split('|')[4]
    	    self.davfili.SetValue(filial+'-'+login.filialLT[ filial ][14])
	    self.arelFilial(700)

	    conn = sqldb()
	    sql  = conn.dbc("DAVs, Corte-cloud servicos", fil = self.fildavs, janela = self)
	    if sql[0]:

		self.TComa1.SetValue(True)
		for i in self.relacao_servico_corte_cloud:

		    d=i.split('|')
		    dados={'codigo':d[0],'quantidade':d[1],'valor_unitario':d[2],'valor_total':d[3],'produto_servico':'S'}
		    corteItems.adicionaItemsPedido(self, self.fildavs, dados, sql)
	...
	def alterar(self,event):
		...
		if not self.ListaPro.GetItemCount():	alertas.dia(self,'Lista de produtos vazio...\n'+(' '*150),'DAVs: Alterar item')
		else:
	...
	def DevAjustar(self,TAj):
		...
		if TR != 0:
			...
			if self.ListaPro.GetItemCount() == 0:
				...
				self.TComa3.Enable()
				self.TComa4.Enable()
				if self.pode_consignar:	self.TComa5.Enable()
				self.TComa1.SetValue(True)
				self.TComa3.Enable( acs.acsm("0603",True) )
...
#-------------: Pedido { Devolve ao Estoque Virtual - Elimina p/Unidade de Produto }
			...
			if self.ListaPro.GetItemCount() == 0:
				...
				self.arelFilial( 700 ) #--// 19-02-2019
	...
	def finalizaDav(self,event):
		...
		"""  Totalizar valores das tabelas  """
		self.precos_tabelas = self.precos_tabelas = self.ajustarPrecosTabelas(wx.EVT_BUTTON) if self.TComa2.GetValue() else ''
	...
	def corteClodPedido(self):
		
		#CorteCloud.numero_pedido=self.numero_cortecloud.GetValue()
		
		corte_cloud = CorteCloud(parent=self,id=-1)
		corte_cloud.Centre()
		corte_cloud.Show()	
	...
	def gravaDav(self,_codigo,_docume,_fantas,_nomecl,_refere,_pagame,_rclobal,_dEntre,_idfilial,clCadastrado,_SelEndereco,_motivo,_mDev,_auTo,_fPag,_frLocal,_entregas,_comprador, lsTProduTos, sFilial, _varios, __finalizar, _rlFilias, fPagamentos, dav_vendedor, expedicionar ):
		...
		if self.TComa3.GetValue():	NumeroDav = self.NumDav.numero("7","Número da Devolução",self, self.fildavs )
		...
		if NumeroDav: 
			...
			if self.sql[0]:
				...
				try:
					...
					if self.TComa4.GetValue():	DDF = self.enTregaFutura
					...
					if self.TComa2.GetValue():	PDO = "2"
					if self.TComa5.GetValue():	PDO = "9"
					...
					if self.TComa3.GetValue() and self.ddevol:
					...
					Tpedido = "1" #-------------------------------------: Pedido
					if self.TComa2.GetValue():	Tpedido = "2" #-: Orçamento
					_Indice = 0 
					
					#print '1: ',self.ecommerce_pedido,len(self.ecommerce_pedido)
					#print '2: ',self.ecommerce_origem,len(self.ecommerce_origem)
					
					if Devolucao:	NumeroDav = "DEV"+str( NumeroDav ).zfill(10)
					elif self.ecommerce_origem=="WOO" and self.ecommerce_pedido:	NumeroDav="WOO"+str( NumeroDav ).zfill(10)
					...
					fuTuro = ""
					if self.TComa4.GetValue():	fuTuro = "2"
					...
					valor_dav_tabela = self.precos_tabelas if self.precos_tabelas and Decimal(self.precos_tabelas) != Decimal(TON) else ''
					
					""" Observacao no DAV 6-7-2019 Loja SERVER """
					__observacao = self.dadosObservacao.decode("UTF-8") if type( self.dadosObservacao ) == str else self.dadosObservacao
					...
					cr_ecid,cr_ecep,cr_ecfb,cr_cust,cr_vazc,cr_prer,cr_tfat,cr_nfev,cr_comp,cr_comg,\
					cr_dibp,cr_part,cr_mist,cr_vnfe,cr_vnpr,cr_expe,cr_tabe,cr_chcp,cr_vobs,cr_twtp,\
					cr_pdwb,cr_pdor,cr_ecfs)\
					values (%s,%s,%s,%s,%s,\
					%s,%s,%s,%s,%s,\
					%s,%s,%s,%s,%s,\
					%s,%s,%s,%s,%s,\
					...
					if Devolucao:	salvar_dav = salvar_dav.replace('cdavs','dcdavs')
					...
					_cida,_cep,cTrlTemp,vlCusTo,_cVazio,_fPag, fuTuro, DDF, cmpNome, cmpCodigo,\
					dadosIBPT, ToTalizaParTilha, pgToVarios, dv_vin, self.vincularprNFe, expedicao_selecionadas,\
					valor_dav_tabela, self.notafiscalcomplementar_chave,__observacao, self.tipo_vendas,\
					self.ecommerce_pedido,self.ecommerce_origem, str(self.consolida_emissao_nota_fiscal)[:1] ))
					...
					if gravar_dav:
						...
						for iTems in range( itens ):
							...
							"""  Medidas de madeiras p/expedicao individualizada """
							_medidas = formata.intquantidade(Decimal(lsTProduTos[iTems][16]))+" PC "+  str(nF.eliminaZeros(_clcomp))+'CM' if Decimal(_clcomp) else ''
							if _medidas and Decimal(_cllarg):	_medidas += " "+ str(nF.eliminaZeros(_cllarg))+'LG'
							if _medidas and Decimal(_clexpe):	_medidas += " "+ str(nF.eliminaZeros(_clexpe))+'EX'

							medidas_expedicao_individual = lsTProduTos[iTems][16]+';'+_clcomp+';'+_cllarg+';'+_clexpe
							...
							#print (lsTProduTos[iTems]),lsTProduTos[iTems]
							...
							exindi  = str( lsTProduTos[iTems][97] ) #--// Expedicao individual
							numero_pecas = lsTProduTos[iTems][98].strip().split('|')[1]+"|" if lsTProduTos[iTems][98].strip() else "|" #//-Corte cloud
							codIden +=numero_pecas

							if lsTProduTos[iTems][102]:	codIden+=lsTProduTos[iTems][102]+'|' #Numero do pedido para nota com emissao consolidado
							if lsTProduTos[iTems][103]:	_auTorz+="|Usuario alterou o controle do podudo no cadastro\n"+lsTProduTos[iTems][103]
							
							infdof  = lsTProduTos[iTems][99] #--// Informacoes para o DOF
							_Indice +=1
							...
							#relacao_items_expedicao.append( _cdprod +'|' + _unidad +'|'+ _quanti +'|'+ _grupo +'|'+ _produt +'|'+ _fabric +'|'+ _endere +'|'+ str( _EstoqueF ) +'|'+exindi+'|'+_medidas+'|'+_item+'|'+embala)
							relacao_items_expedicao.append( _cdprod +'|' + _unidad +'|'+ _quanti +'|'+ _grupo +'|'+ _produt +'|'+ _fabric +'|'+ _endere +'|'+ str( _EstoqueF ) +'|'+exindi+'|'+_item+'|'+embala+'|'+medidas_expedicao_individual)
							...
							it_part, it_qent, it_reti, it_seri,it_emba,it_comi,it_eloc,it_idof)\
							VALUES(%s,%s,%s,%s,%s,\
							%s,%s,%s,%s,%s,\
							%s,%s,%s,%s,%s,\
							%s,%s,%s,%s,%s,\
							...
							if Devolucao:	grava_items = grava_items.replace('idavs','didavs')
							...
							_auTorz,_codKiT,_qTvKiT,_daibpT,codIden,\
							 parTIC, expedicao_quantidade, expedicao_lancamento, series, embala, comisa, baixa_loja, infdof ) )
							""" 
							...
							if self.TComa1.GetValue() or self.TComa5.GetValue():
						...
						if self.TComa1.GetValue() or self.TComa5.GetValue():	cdMd = "605"
						if self.TComa2.GetValue() == True:	cdMd = "606"
						
						if self.TComa1.GetValue() or self.TComa5.GetValue():	emai = "607"
				...
				if _retorno == True:
					...
					dev = ""
					if Devolucao:	dev = "DEV"
					if _varios == False:

						"""  Nao imprimir dav se for entrega  """
						if impressao:

							#--// Impressao do tichete se tiver configurado, se nao abre o gerenciador
							impressao_tickete = True if len(login.usaparam.split(";")) >=29 and login.usaparam.split(";")[28] == "T" else False
							if venda_dav and impressao_tickete and login.impparao:	impressao_direta.ticketeVendas(self,numero_dav = NumeroDav, cliente=_nomecl, pagamentos=_fPag)
							else:	self.icdavs.impressaoDav( NumeroDav, self.parente, False, True,  dev, "", servidor = sFilial, codigoModulo = cdMd, enviarEmail = emai,recibo=False, vlunitario=True)
				...
				if novo_dav:
					...
					""" Faz automaticamente o pedido de servicos  do corte cloud """
					if self.relacao_servico_corte_cloud and self.corte_cloud_servicos_separados:
					    
					    dadosCliente.docu=_docume
					    dadosCliente.nome=_nomecl
					    
					    self.clC.SetLabel(_codigo)
					    self.clD.SetLabel(_docume)
					    self.clN.SetLabel(_nomecl)
					    self.corte_cloud_pesquisar_cliente=True

					    self.pedidoServicoSeparadoCloud()
	...
	def TotalizaGravacaoDav(self, lsTa ):
		...
		_TON = ( ( _TOP + _vFr + _vAC + _vIP + _vST ) - _vDC ) 
...
class fecharDav(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.cliente_vai_levar = False

		""" Tabelas vinculadas """
		self.usar_tabelas_produtos = False
		self.tabelas_diferentes = False
		self.tabelas_produtos = ""
		self.bloqueio_parcela_cartao_credito = False
		self.bloqueio_parcela_cartao_debito = False
		...
		""" Pagamento """
		self.ListaPaga = wx.ListCtrl(self.painel, 470,pos=(11,490), size=(473,122),
						 			style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		wx.StaticText(self.painel,-1,"Pesquisar: Nome,Código CPF-CNPJ (P:SubString F:Fantasia E:FiltarEstado)",pos=(70,295)).SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
		...
		self.observar  = wx.BitmapButton(self.painel, 230, wx.Bitmap("imagens/edit.png",     wx.BITMAP_TYPE_ANY),  pos=(650, 393), size=(34,30))				
		...
		self.referenci = wx.TextCtrl(self.painel,203,value="", pos=(490, 270), size=(300,40),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
		self.referenci.SetBackgroundColour('#E5E5E5')
		self.referenci.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))

		#// Vendas Presencial. Telefone, Web
		self.parente.tipo_vendas = ""
		self.vnd_web = wx.CheckBox(self.painel, 771, "Tipo-Vendas: Web", pos=(490,311))
		self.vnd_tel = wx.CheckBox(self.painel, 772, "Telefone",   pos=(624,311))
		self.vnd_pre = wx.CheckBox(self.painel, 773, "Presencial", pos=(708,311))
		...
		if self.parente.TComa1.GetValue():	tipos_vendas = True
		else:	tipos_vendas = False
		self.vnd_web.Enable(tipos_vendas)
		self.vnd_tel.Enable(tipos_vendas)
		self.vnd_pre.Enable(tipos_vendas)
		...
		if self.parente.ecommerce_origem.strip()=="WOO":

		    if self.parente.ecommerce_frete and Decimal(self.parente.ecommerce_frete):	self.cvFrete.SetValue(self.parente.ecommerce_frete)
		    if self.parente.ecommerce_desconto and Decimal(self.parente.ecommerce_desconto):	self.cvDesco.SetValue(self.parente.ecommerce_desconto)
		...
		self.calculcfp = wx.CheckBox(self.painel, -1, "{ Venda interestadual-cosumidor final }\nCalcular o FCP fixo para o estado de destino", pos=(170,607))
		self.calculcfp.SetValue( self.parente.calcular_fcp_fixo )
		self.calculcfp.Enable( False )
		...
		self.calculcfp.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,  False,"Arial"))
		...
		self.observar.Bind(wx.EVT_BUTTON,self.observacaoDados)
		...
		self.calculcfp.Bind( wx.EVT_CHECKBOX, self.habilitarFcp )
		
		self.valor_corte = Decimal() #--// Utilizado para produto CORTE-0000007
		self.nomeclien.SetFocus()

		if self.mediad !='' or self.mediaa or self.mediaf:	self.DescontoProporcional()

		adicionar_pagamento_automatico=False
		if self.parente.TComa3.GetValue() or self.parente.TComa4.GetValue() or self.parente.TComa5.GetValue():	adicionar_pagamento_automatico=True
		if self.parente.TComa2.GetValue() and self.parente.notafiscalcomplementar_chave:	adicionar_pagamento_automatico=True
		
		if adicionar_pagamento_automatico:	self.adicionaPagamentoAutomatico()
		...
		if self.parente.utilizar_preco_custo and self.parente.TComa5.GetValue():

			self.cvFrete.Enable( False )
			self.cvAcres.Enable( False )
			self.cvDesco.Enable( False )
			self.prDesco.Enable( False )
		...
		if not self.ListaCli.GetItemCount() and self.parente.TComa3.GetValue():	self.flSalvar.Enable(False)

		if self.parente.notafiscalcomplementar_chave or self.parente.consolida_emissao_nota_fiscal:

		    _fr=str(self.parente.frete_proporcional)
		    _ac="0.00"
		    _dc=str(self.parente.desco_proporcional)
		    self.cvFrete.SetValue(_ac)
		    self.cvDesco.SetValue(_dc)

		    self.davc.atualizavlr(_fr,_ac,_dc)
		    self.vlToT.SetValue("R$"+format(dav.ToTalGeral,','))
			
		    self.ListaPaga.DeleteAllItems()
		    self.ListaPaga.Refresh()
		    self.adicionaPagamentoAutomatico()

		if self.parente.TComa3.GetValue():
		    
		    self.documento.Enable(False)
		    self.nomeclien.Enable(False)

		""" Corte-cloud procuara o cliente de servico """
		if dadosCliente.docu or dadosCliente.nome and self.parente.corte_cloud_pesquisar_cliente:
		    
		    self.parente.corte_cloud_pesquisar_cliente=False
		    self.procuraCliente(wx.EVT_BUTTON)
		    
		""" Verifica as tabelas utilizadas """
		if not self.parente.TComa3.GetValue():
		    
		    self.usar_tabelas_produtos = True if len( login.filialLT[ self.fcFilial ][35].split(";") ) >= 152 and login.filialLT[ self.fcFilial ][35].split(";")[151] == "T" else False
		    self.tabelas_diferentes = False
		    self.tabelas_produtos = self.parente.ListaPro.GetItem(0,64).GetText().strip()
		    for __t in range(self.parente.ListaPro.GetItemCount()):
			
			if self.parente.ListaPro.GetItem(__t,64).GetText().strip() !=self.tabelas_produtos:	self.tabelas_diferentes = True
			if self.parente.ListaPro.GetItem(__t,50).GetText():	self.cliente_vai_levar = True

		    self.alteraFormaTabela(login.pgAviR,self.parente.table_precos_selecionada)

		if self.cliente_vai_levar and len( login.filialLT[ self.fcFilial ][35].split(";") ) >= 160 and login.filialLT[ self.fcFilial ][35].split(";")[159]=="T":
		    novaForma = []
		    for nfp in login.pgAviR:
			if nfp[3:5]=="12":	pass
			else:	novaForma.append(nfp)

		    self.pagamento.SetItems(novaForma)
		    self.pagamento.SetValue(novaForma[0])
		    
		if acs.acsm('623',False):	self.saldodev.Enable(False) #--=// Bloqueio ao acesso do saldo do cliente

		""" Totalizar cortes de produtos configurado p/adicionar em acrescimos """
		for __vc in range(self.parente.ListaPro.GetItemCount()):
			
		    if self.parente.ListaPro.GetItem(__vc,71).GetText() == "CORTE-0000007":	self.valor_corte +=Decimal( self.parente.ListaPro.GetItem(__vc,100).GetText() )

		if self.valor_corte:
		    
		    self.cvAcres.SetValue(str(self.valor_corte))
		    self.cvAcres.Enable(False)

		    self.davc.atualizavlr('0.00',str(self.valor_corte),'0.00')
		    self.vlToT.SetValue("R$"+format(dav.ToTalGeral,','))
		    self.dadosManual()
		    self.Tvalores(str(self.valor_corte),251)

		self.cartaoBloqueioParcelas() #// Bloquio parcelamento do cartao

		""" Atualiza o endereco no retorno """
		if self.parente.endereco_entega and self.nomeclien.GetValue():
		    self.edentrega.SetValue(self.parente.endereco_entega)

		""" Verificar na devolucao se houve entrega e devolucao para o pedido selecionado """
		numero_dav_vinculado = self.parente.ndev.GetLabel().upper().split('VINCULAR:')
		permitir_bloqueio_sem_saldo = True if len( login.filialLT[self.fcFilial][35].split(";"))>=184 and login.filialLT[self.fcFilial][35].split(";")[183]=="T" else False
		if self.parente.TComa3.GetValue() and len(numero_dav_vinculado)==2 and numero_dav_vinculado[1].strip() and self.parente.ListaPro.GetItemCount() and permitir_bloqueio_sem_saldo:

		    lista = self.quantidadeEntregue(numero_dav_vinculado[1].strip())

		    emissao_dav = self.parente.data_emissao_devolucao
		    data_hoje = datetime.datetime.now().strftime("%Y-%m-%d")
		    data_hoje = datetime.datetime.strptime(data_hoje, "%Y-%m-%d").date()
		    passar = True if len(login.filialLT[self.fcFilial][35].split(";"))>=183 and login.filialLT[self.fcFilial][35].split(";")[182]=="T" and emissao_dav < data_hoje else False
		    
		    if lista:
			
			self.ffSalvar.Enable(passar)
			liberado = "\n\n{ LIBERADO PElO CLIENTE PARA FAZER DEVOLUCAO SEM ENTREGA DOS PRODUTOS RELACIONADOS P/DIAS ANTERIORES }" if passar else ""
			
			MostrarHistorico.TP = ""
			MostrarHistorico.hs =lista+liberado
			MostrarHistorico.TT = "Controle de devolucao"
			MostrarHistorico.AQ = ""
			MostrarHistorico.FL = self.fcFilial
			MostrarHistorico.GD = ""
		...
		""" Verifica se o pedido tem origem de e-commerce e valida """
		if self.parente.ecommerce_origem.strip()=="WOO":
		    
		    _fr = str(self.cvFrete.GetValue()).replace(',','')
		    _ac = str(self.cvAcres.GetValue()).replace(',','')	    
		    _dc = str(self.cvDesco.GetValue()).replace(',','')		    
		    self.davc.atualizavlr(_fr,_ac,_dc)
		    self.vlToT.SetValue("R$"+format(dav.ToTalGeral,','))
		    self.dadosManual()
		    self.valirarEcommerce("WOO")
			
	def valirarEcommerce(self, origem):

	    if self.parente.ecommerce_pedido and self.parente.ecommerce_origem and self.parente.ecommerce_cliente:
		
		cidade,nome,bairro,cnpj,aniversario,numero,cpf,telefone,estado,endereco,complemento,ie,email,telefone1,cep=self.parente.ecommerce_cliente.split('<\>')
		documento=cnpj if cnpj else cpf
		self.documento.SetValue(documento)
		self.nomeclien.SetValue(documento)
		...
		if sql[0]:
		    ...
		    localizado = sql[2].execute("SELECT cl_nomecl,cl_docume,cl_lvorig FROM clientes WHERE cl_docume='"+ documento +"'")
		    
		    if localizado:
			
			__nome, __docu, __orig = sql[2].fetchone()
			if __nome.upper().strip()!=nome.upper().strip() and __orig=="WOO":
			
			    alertas.dia(self,"C L I E N T E S Diferentes { "+__docu+" }, com o mesmo CPF/CNPJ...\n\nCliente no woo-commerce: "+nome.upper()+"\nNome no sistema: "+__nome+"\n"+(" "*200),"Clientes diferentes")
			
		    if not localizado:
			
			""" Inclusao automatica do cliente """
			cep=cep.replace('-','')
			cadastro=str( datetime.datetime.now().date() )
			telefone=telefone.replace("(","").replace(")","")
			telefon1=telefone1.replace("(","").replace(")","")
			nome=nome.upper()
			endereco=endereco.upper()
			bairro=bairro.upper()[:20]
			cidade=cidade.upper()
			complemento=complemento.upper()
			estado=estado.upper()
			sql[2].execute("INSERT INTO clientes (cl_nomecl,cl_docume,cl_iestad,cl_cadast,cl_endere,cl_bairro,cl_cidade,cl_cepcli,\
						       cl_compl1,cl_compl2,cl_estado,cl_emailc,cl_telef1,cl_telef2,cl_lvorig)\
						        VALUES('"+nome+"','"+documento+"','"+ie+"','"+cadastro+"','"+endereco+"','"+bairro+"','"+cidade+"','"+cep+"',\
							'"+numero+"','"+complemento+"','"+estado+"','"+email+"','"+telefone+"','"+telefone1+"','"+origem+"')" )
			...
			gRegc = str( nRegc )+'-'+str(self.fcFilial)
		...
		""" Mostra o cliente """
		if localizado:	self.procuraCliente(wx.EVT_BUTTON)
	    
	def quantidadeEntregue(self,vinculado):
		...
		for vd in range(self.parente.ListaPro.GetItemCount()):
			    
		    item = self.parente.ListaPro.GetItem(vd,0).GetText()
		    codigo = self.parente.ListaPro.GetItem(vd,1).GetText()
		    descricao = self.parente.ListaPro.GetItem(vd,2).GetText()
		    quantidade = Decimal(self.parente.ListaPro.GetItem(vd,3).GetText())
		    id_item = self.parente.ListaPro.GetItem(vd,54).GetText()
		    
		    achar = "SELECT it_qent,it_qtdv,it_quan, it_logi FROM idavs WHERE it_ndav='"+ vinculado +"' and it_codi='"+ codigo +"' and it_item='"+ id_item+"'"
		    if sql[2].execute( achar ):
			
			for ri in sql[2].fetchall():

			    saldo_devolucao = ( ( ri[0] + ri[3] ) - ri[1] ) #--[ Quantidade de entrega  - QuantidadesDevolvidas ]
			    #print 'Entrega: ',ri[0], 'Logistica: ',ri[3], 'Devolvido: ',ri[1],'Saldo: ',saldo_devolucao
			    if saldo_devolucao < quantidade:

				if not lista:

				    lista+="Relacao de itens sem entrega suficiente para devolucao\n\n"
				    lista+="Descricao do produto"+(" "*50)+"QT Vendido  QT Entregue QT Devolvido QT p/Devolucao Saldo para devolucao QT Entregue na logistica\n"
				lista +=descricao[:78]+('.'*(67-len(descricao)))+('.'*(13-len(str(ri[2]))))+str(ri[2])+(' '*(13-len(str(ri[0]))))+str(ri[0])+(' '*(13-len(str(ri[1]))))+str(ri[1])+(' '*(15-len(str(quantidade))))+str(quantidade)+(' '*(21-len(str(saldo_devolucao))))+str(saldo_devolucao)+(' '*(25-len(str(ri[3]))))+str(ri[3])+'\n'
	...
	def habilitarFcp(self,event):
	    
		self.parente.calcular_fcp_fixo = self.calculcfp.GetValue()
		self.AtualizaCliente( wx.EVT_BUTTON )
...
#----------: FIM//
	...
	def IncluirClRemoto(self, lsT ):
		...
		if sql[0] == True:
			...
			try:
				...
				cl_dtcomp,cl_dadosc,cl_pgtofu,cl_blcred,cl_compra,cl_codigo,cl_dtincl,cl_hrincl,cl_incalt,cl_uscada)\
				...
				lsT[41],lsT[42],lsT[43],lsT[44],lsT[45],lsT[46],lsT[47],lsT[48],lsT[49], login.usalogin) )
	...
	def adicionaPagamento(self,event):
		...
		semCad = False
		permitir_documento = False if len( login.filialLT[ self.fcFilial ][35].split(";") ) >= 117 and login.filialLT[ self.fcFilial ][35].split(";")[116] == 'T' else True
		if docume.strip() == '' and permitir_documento:	semCad = True
		...
		if not self.pagamento.GetValue().strip() or str(self.pagamento.GetValue().strip()) == '{ Tabelas diferentes }':

			alertas.dia(self.painel,"2-Selecione uma forma de pagamento valida...\n"+(" "*160),u'Pré-Recebimento de Vendas')
			return
		...
		if valor > saldo:	alertas.dia(self.painel,u"Valor de lançamento maior que saldo disponivel...\n",u'Pré-Recebimento de Vendas')
			...
			if saldo > 0:
				...
				if cReg != 0 and self.ListaCli.GetItem(indice,15).GetText() !=None and self.ListaCli.GetItem(indice,15).GetText() == "T":
					...
					"""  Permite vender apenas p/Dinheiro,Cartao Debito, Cartao Credito  e se autorizado em parametros o ( 09 ) """
					lista_vender = ["01","04","05"]
					if len( login.filialLT[self.fcFilial][35].split(";") )>=161 and login.filialLT[self.fcFilial][35].split(";")[160]=="T":	lista_vender.append("09")
					    
					if self.parente.venderNeg == "T" and pCC not in lista_vender:	_bl += u"\n\nCliente { N E G A T I V A D O NÃO V E N D E R }"
	...
	def GravaPagamento( self, _ad, _az, nR, fP, vc, vl, pC, rL, dc, moduloPedindo = 0, MotivoAutorizacao = "", misTFilial = "" ):
		...
		if not self.pagamento.GetValue().strip() or str(self.pagamento.GetValue().strip()) == '{ Tabelas diferentes }':
		    ...
		    alertas.dia(self,'1-Selecione uma forma de pagamento valida...\n'+(' '*160),u'Pré-Recebimento de Vendas')
		    return
		...
		if self.parente.TComa3.GetValue() == True or self.parente.TComa5.GetValue():	pag = fP
		if self.parente.TComa2.GetValue() and self.parente.notafiscalcomplementar_chave:	pag = fP
	...
	def dadosManual(self):
		...
		try:
			...
			if sLan > 0:
				...
				self.rcvTT.SetValue("R$ "+format(dav.ToTalGeral,','))
				self.rcvSL.SetValue(format(sLan,','))
				try:	self.rcvVL.SetValue(str(sLan))
		...
		if self.parente.TComa5.GetValue():	self.ListaCli.Enable(True)
		if self.valor_corte:	self.cvAcres.Enable(False)
	...
	def observacaoDados(self,event):

		adNFe.Titulo  = 'Observação da compra'
		adNFe.sTitulo = 'Observação'

		adn_frame=adNFe(parent=self,id=event.GetId())
		adn_frame.Centre()
		adn_frame.Show()
	...
	def evchekb(self,event):
		...
		if fpg in rl:
			...
			fechar = True if len(login.filialLT[self.fcFilial][35].split(';'))>=163 and login.filialLT[self.fcFilial][35].split(';')[162]=='T' else False
			self.nupar.Enable(fechar)
			self.nupar.SetValue( login.parcelas[0] )
			
		else:
		    self.nupar.Enable(True)

		    """ Bloqueio de parcelas p/cartao credito,debito se tiver nas tabelas de precos selecionadas """
		    if self.bloqueio_parcela_cartao_credito and  self.pagamento.GetValue()[3:5]=="04":
			self.nupar.Enable(False)
			self.nupar.SetValue( login.parcelas[0] )
			
		    if self.bloqueio_parcela_cartao_debito and  self.pagamento.GetValue()[3:5]=="05":
			self.nupar.Enable(False)
			self.nupar.SetValue( login.parcelas[0] )
	...
	def AtualizaCliente(self,event):
		...
		self.parente.endereco_entega = self.edentrega.GetValue()
	...
	def Teclas(self,event):
		...
		if self.ListaCli.GetItem( self.ListaCli.GetFocusedItem(), 6 ).GetText() and self.ListaCli.GetItem( self.ListaCli.GetFocusedItem(), 6 ).GetText() != login.filialLT[self.fcFilial][30].split(';')[12]:

			regime_tributario = login.filialLT[self.fcFilial][30].split(';')[11]
			if len( self.ListaCli.GetItem( self.ListaCli.GetFocusedItem(), 1 ).GetText() ) == 11 and regime_tributario in ['2','3']:	self.calculcfp.Enable( True )
	...
	def procuraCliente(self,event):
		...
		if self.ListaPaga.GetItemCount() and not self.parente.TComa5.GetValue():
		...
		if sql[0] == True:
			...
			#print self.documento.GetValue().strip()
			if self.documento.GetValue().strip().isdigit():	
			    reTorno = sql[2].execute("SELECT * FROM clientes WHERE cl_incalt!='E' and cl_docume='"+str( self.documento.GetValue().strip() )+"' ORDER BY cl_nomecl")
			    ...
			    if codigoClien.isdigit() == True:
				    ...
				    if sTringPesqu in ['P:','F:','E:']:
					    ...
					    if sTringPesqu == 'E:':	reTorno = sql[2].execute("SELECT * FROM clientes WHERE cl_incalt!='E' and cl_estado='"+nomeCliente+"' ORDER BY cl_nomecl")
	...
	def fechamentoDav(self,event):
		...
		if self.parente.TComa5.GetValue() and not self.ListaCli.GetItemCount():
		...
		if self.parente.filial_com_bloqueio and not self.parente.TComa3.GetValue() and not self.parente.notafiscalcomplementar_chave:
		...
		""" Valida para vendedores diferen do usuario logado """
		vendedor_final=self.vende.GetValue().split('-')[1] if self.vende.GetValue() else ''
		if self.parente.TComa1.GetValue() and len(login.usaparam.split(";"))>=51 and login.usaparam.split(";")[50]=="T":
		    if not vendedor_final or vendedor_final==login.usaparam:
			alertas.dia(self,u"Selecione um vendedor diferente do usuário logado...\n"+(' '*140),u"Mudança de vendedores")
			return

		""" Forca a marcacao do tipo de venda Web,Telefone,Presencial """
		if not self.parente.tipo_vendas and len(login.filialLT[self.fcFilial][35].split(";"))>=173 and login.filialLT[self.fcFilial][35].split(";")[172]=="T":
				    
		    if self.parente.TComa1.GetValue() or self.parente.TComa4.GetValue():
			alertas.dia(self,u"Selecione o tipo de venda [ Web, Telefone ou Presencial ]\n"+(' '*140),u"DAV: Fechameto { Forçar tipo de venda }")
			return
		...
		if self.parente.caixaDavRecalculo == True:
			...
			if not nomecl:
			    alertas.dia(self.painel,u"Nenhum cliente registrado para finalizar a venda...\n"+(" "*120),"Finalização-Fechamento do DAV")
			    return
			...
			if dav.ToTalGeral > 0 and dav.DavAV == 'V':
				...
				""" Valida o CPF/CNPJ avulso """
				if self.documento.GetValue().strip() and not nF.cpfcnpj( self.documento.GetValue().strip() )[0]:
					alertas.dia(self,u"{ CPF/CNPJ Avulso "+self.documento.GetValue().strip()+u', inválido }\n'+(' '*140),"DAV: Fechameto")
					return

				if not self.centregar.GetValue() and len( login.filialLT[ self.fcFilial ][35].split(";") ) >= 164 and login.filialLT[ self.fcFilial ][35].split(";")[163] == "T":
				
				    finaliza_entrega = wx.MessageDialog(self,u"{ Finalização sem confimação de entrega }\n\n1-Confirme para voltar para a marcação da entrega\n"+(" "*160),"DAV: Fechamento",wx.YES_NO|wx.NO_DEFAULT)
				    if finaliza_entrega.ShowModal() == wx.ID_NO:	pass
				    elif finaliza_entrega.ShowModal() == wx.ID_YES or finaliza_entrega.ShowModal() == wx.ID_CANCEL:
					self.ffSalvar.Enable( True )
					self.fdbl = False
	...
	def impressaoDavVarios(self,event):
		...
		if self.ListaFilial.GetItemCount() == 0:	alertas.dia(self,"Lista Vazia...\n"+(" "*120),"Retaguarda de Vendas: Impressao")
			...
			if NumeroDav == "" and self.filmT.GetLabel().strip() == "":
				...
				self.parente.icdavs.impressaoDav( NumeroDav, self, False, True,  "", "", servidor =  filialDav, codigoModulo = cdMd, enviarEmail = emai,recibo=False, vlunitario=True )
	...
	def aTualizaDados(self):
		...
		formas = self.relpag.fpg( self.ListaCli.GetItem(indice,13).GetText(),2, rclocal=self.cliente_vai_levar, filial=self.fcFilial)

		self.pagamento.SetItems( formas )
		self.pagamento.SetValue( formas[0] )

		""" Utilizar formas de pagamentos vinculadas a tabela """
		if self.usar_tabelas_produtos and not self.parente.TComa3.GetValue():
		
		    if self.tabelas_diferentes:	
			self.pagamento.SetItems( [] )
			self.pagamento.SetValue( '{ Tabelas diferentes }' )
		    else:

			if self.tabelas_produtos and self.tabelas_produtos in ['2','3','4','5','6'] and login.tabelas_formaspagamentos:	self.alteraFormaTabela(formas, self.tabelas_produtos)
			
		else:
		    if self.parente.table_precos_selecionada and self.parente.table_precos_selecionada in ['2','3','4','5','6'] and login.tabelas_formaspagamentos:	self.alteraFormaTabela(formas, self.parente.table_precos_selecionada)
		
	def alteraFormaTabela(self, f, __tabela_selecionada):
		...
		if self.tabelas_diferentes:	
		    ...
		    continuar = True
		    __tabela_selecionada = self.tabelas_produtos
		    
	    if continuar and __tabela_selecionada and __tabela_selecionada in ['2','3','4','5','6'] and login.tabelas_formaspagamentos:

		r, formas = self.alteraFormaTabelas(f, __tabela_selecionada)
		if r:
		    self.pagamento.SetItems( formas )
		    self.pagamento.SetValue( formas[0] )
	    
	def alteraFormaTabelas(self, f, __tabela_selecionada):
	    
	    r = False
	    novas_formas =['']
	    if __tabela_selecionada and __tabela_selecionada in ['2','3','4','5','6'] and login.tabelas_formaspagamentos:

		for i in f:
		    if self.verificaTabela(i, __tabela_selecionada):
			novas_formas.append(i)
			r = True

	    return r, novas_formas

	def cartaoBloqueioParcelas(self):

	    """ Bloqueio formas de pagamento para cartao de debito,credito para tabelas selecionadas """
	    if len(login.filialLT[self.fcFilial][35].split(";"))>=171 and login.filialLT[self.fcFilial][35].split(";")[170]:

		bloqueio_forma=[]
		bloquear_forma=False
		cr, tb = login.filialLT[self.fcFilial][35].split(";")[170].split('|')
		ccr,ccd = cr.split('-')
		if tb and 'T' in cr:

		    for tbl in tb.split(','):
			if tbl:	bloqueio_forma.append(tbl.zfill(2))
				 
		    for bp in range(self.parente.ListaPro.GetItemCount()):
				    
			if self.parente.ListaPro.GetItem(bp, 64).GetText().zfill(2) in bloqueio_forma:	bloquear_forma=True
			
		    if bloquear_forma:
			for nf in login.pgAviR:

			    if len(nf.split(' '))>=2 and nf.split(' ')[1][:2]=="04" and ccr=="T":	self.bloqueio_parcela_cartao_credito=True
			    if len(nf.split(' '))>=2 and nf.split(' ')[1][:2]=="05" and ccd=="T":	self.bloqueio_parcela_cartao_debito=True

	def verificaTabela(self,f, __tabela_selecionada):
	    
	    retornar = False
	    for i in login.tabelas_formaspagamentos:
		if f:
		    fr = f.split(' ')[1].split('-')[0]
		    tb = i.split(';')[0]
		    nf = i.split(';')[1].split('-')[0]
		    if tb == __tabela_selecionada and nf == fr:	retornar = True
		...
		if sql[0] == True:
			...
			if self.ocorre == 300: #->[ Alteracao ]
				...
				try:
					...
					cl_redeso,cl_dtincl,cl_hrincl,cl_incalt,cl_uscada)\
					...
					'"+self.cl_redeso.GetValue()+"','"+str( EMI )+"','"+str( DHO )+"','A','"+login.usalogin+"')")
...
class dvListCtrlPanel(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.utilizar_preco_custo = self.parente.utilizar_preco_custo
		self.parente.Disable()
		...
		wx.StaticText(self.painel,-1,"Selecione filial remota p/consulta de produtos",pos=(390,316)).SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.estoque_minimo = wx.StaticText(self.painel, -1, '', pos=(622,271))
		self.estoque_minimo.SetFont(wx.Font(11, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
		self.estoque_minimo.SetForegroundColour("#A70000")
		...
		self.cfnor = wx.StaticText(self.painel,-1,'',pos=(443,15))
		...
		self.cdsele = wx.StaticText(self.painel, -1, 'Registro: {}',  pos=(820,32),style=wx.SUNKEN_BORDER)
		self.cdsele.SetFont(wx.Font(6, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
		self.cdsele.SetForegroundColour('#690505')
		
		self.quanti = str(Decimal('1.0000'))
		self.quanti = mkn(self.painel, id = 303, value = self.quanti, pos=(265,427),size=(90,20),style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth = 4, fractionWidth = 4, allowNone=False,groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = '#FFFFFF', validBackgroundColour = '#FFFFFF', invalidBackgroundColour = "Yellow",allowNegative = False)
		self.quanti.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))

		self.chekaute = wx.CheckBox(self.painel, 506, "Checkaute", pos=(10,388 ))
		self.chekaute.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.chekaute.SetForegroundColour('#2865A1')

		self.mostrar_grupos     = wx.RadioButton(self.painel,-1,u"Grupos", pos=(110, 388),style=wx.RB_GROUP)
		self.estoque_deposito   = wx.RadioButton(self.painel,-1,u"Deposito/chão-loja", pos=(180, 388))
		self.virtual_temporario = wx.RadioButton(self.painel,-1,u"Utilizado", pos=(315, 388))

		self.mostrar_grupos.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.estoque_deposito.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.virtual_temporario.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.estoque_deposito.Bind(wx.EVT_RADIOBUTTON, self.estoqueDeposito )
		self.mostrar_grupos.Bind(wx.EVT_RADIOBUTTON, self.estoqueDeposito )  
		self.virtual_temporario.Bind(wx.EVT_RADIOBUTTON, self.estoqueDeposito )
		self.ender.Bind(wx.EVT_LEFT_DCLICK, self.alteraEnderecoEstoque)
	...
	def alteraEnderecoEstoque(self,event):

		if not self.ender.GetValue().upper().strip():	alertas.dia(self,u'Endereço de estoque esta vazio, entre com um endereço valido...\n'+(' '*160),u'Retaguarda: Alteração do endereço de estoque')
		elif self.list_ctrl.GetItemCount() and len(login.usaparam.split(';'))>=34 and login.usaparam.split(';')[33]=='T':

			filial=self.list_ctrl.GetItem(self.list_ctrl.GetFocusedItem(),22).GetText()
			endereco=self.ender.GetValue().strip().upper()
			codigo=self.list_ctrl.GetItem(self.list_ctrl.GetFocusedItem(),0).GetText()
			produto=self.list_ctrl.GetItem(self.list_ctrl.GetFocusedItem(),1).GetText()
			
			confima = wx.MessageDialog(self.painel,u"{ Alterar endereço de estoque ["+codigo+"] }\n\nFilial: "+filial+u"\nProduto: "+produto+u"\nEndereço: "+endereco+u"\n\nConfirme p/Continuar\n"+(" "*160),u"Retaguarda: Alteração do endereço de estoque",wx.YES_NO|wx.NO_DEFAULT)
			if confima.ShowModal()==wx.ID_YES:
		
				conn=sqldb()
				sql=conn.dbc("DAVs, Alterar endereco de estoque { Filial: "+str( filial )+" }", fil = str( filial ), janela = self.painel )

				if sql[0]:
					erro=''
					try:
						if not sql[2].execute("SELECT fg_desc FROM grupofab WHERE fg_desc='"+ endereco +"' and fg_cdpd='E'"):	sql[2].execute("INSERT INTO grupofab (fg_desc,fg_cdpd) VALUES('"+endereco+"','E')")
						gravar=sql[2].execute("UPDATE estoque SET ef_endere='"+endereco+"' WHERE ef_idfili='"+filial+"' and ef_codigo='"+codigo+"'")
					...
					conn.cls(sql[1])
					if erro:	alertas.dia(self,u'Erro na atualização do endereço...\n\n'+erro+'\n'+(' '*160),u'Retaguarda: Alteração do endereço de estoque')
					else:	alertas.dia(self,u'Endereço atualizado com sucesso...\n'+(' '*160),u'Retaguarda: Alteração do endereço de estoque')
		else:	alertas.dia(self,u'Sem acesso para alteração do endereço de estoque...\n'+(' '*160),u'Retaguarda: Alteração do endereço de estoque')
		
	def estoqueDeposito(self,event):

		codigo_produto = self.list_ctrl.GetItem( self.list_ctrl.GetFocusedItem(), 0 ).GetText()
		#print '1: ',dav.lista_reserva_temporaria,'2: ',self.virtual_temporario.GetValue()
		if self.estoque_deposito.GetValue():
			...
			self.ListaCasa.InsertColumn(1, 'Estoque total', width=100)
			self.ListaCasa.InsertColumn(2, 'Estoque filial', width=100)
		...
		elif dav.lista_reserva_temporaria and self.virtual_temporario.GetValue():
			...
			self.ListaCasa.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.simples)
			self.ListaCasa.SetBackgroundColour('#EBEBA5')
			self.ListaCasa.SetForegroundColour('#000000')
			self.ListaCasa.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
			
			self.ListaCasa.InsertColumn(0, 'Filial', width=50)
			self.ListaCasa.InsertColumn(1, 'Data-Hora', width=130)
			self.ListaCasa.InsertColumn(2, 'Usuario', width=90)
			self.ListaCasa.InsertColumn(3, 'Quantidade', format=wx.LIST_ALIGN_LEFT, width=80)
			self.ListaCasa.InsertColumn(4, 'Observacao', format=wx.LIST_ALIGN_LEFT, width=200)

			indice = 0
			if dav.lista_reserva_temporaria[3] == codigo_produto:
			    for i in dav.lista_reserva_temporaria[0].split('\n'):
				    
				if i:
					    
				    _fl, _dt, _hr, _us, _qt = i.split('|')

				    self.ListaCasa.InsertStringItem( indice, _fl )
				    self.ListaCasa.SetStringItem( indice,1, _dt +' '+ _hr)
				    self.ListaCasa.SetStringItem( indice,2, _us )
				    self.ListaCasa.SetStringItem( indice,3, _qt )
				    if indice % 2:	self.ListaCasa.SetItemBackgroundColour( indice, "#DEDEAD")
				    indice +=1
		else:
	...
	def simples(self, event):
	    #print 'a'
	    if dav.lista_reserva_temporaria:
		
		RelacaoReservas.lista = dav.lista_reserva_temporaria[0]
		RelacaoReservas.infor = dav.lista_reserva_temporaria[1]
		RelacaoReservas.filia = dav.lista_reserva_temporaria[2]
		RelacaoReservas.codig = dav.lista_reserva_temporaria[3]
	...
	def comprasClientes(self,event):
		...
		if not self.list_ctrl.GetItemCount():	alertas.dia( self, "Lista de produtos vazia...\n"+(" "*120),"Lista de quem comprou o produto selecionado")
			...
			quemm_frame=QuemComprouSelecionado(parent=self,id=-1)
			quemm_frame.Center()
			quemm_frame.Show()
	...
	def selecionar(self,event):
		...
		if sql[0] == True: 
			...
			if self.concod.GetValue().strip():
				...
				if len( login.filialLT[ self.parente.fildavs ][35].split(';') ) >= 22 and login.filialLT[ self.parente.fildavs ][35].split(';')[21] == "T":
				    _cn1 = _cn1.replace("WHERE","WHERE t1.pd_canc='' and ")
				    _cn2 = _cn2.replace("WHERE","WHERE t1.pd_canc='' and ")
				...
				"""  Nao mostrar produtos marcados  """
				if len( login.filialLT[ self.parente.fildavs ][35].split(';') ) >= 22 and login.filialLT[ self.parente.fildavs ][35].split(';')[21] == "T":
				    _psq = _psq.replace("WHERE","WHERE t1.pd_canc='' and ")
...
#-------------: Venda em KIT
				...
				if row[24] == "T" and row[25]:
					...
					""" Utilizar o valor do KIT """	
					if len(login.filialLT[self.parente.fildavs][35].split(';'))>=168 and login.filialLT[self.parente.fildavs][35].split(';')[167]=="T":
					    vlu = row[4] #-: Valor Unitario
				...
				if passar_filial:
					...
					""" Muda o preco do produto para a tabela do cliente se o valor relacionado estiver preenchido """
					pesquisa_preco_tabela_cliente = True if len(login.filialLT[self.parente.fildavs][35].split(';'))>=181 and login.filialLT[self.parente.fildavs][35].split(';')[180]=="T" else False
					if pesquisa_preco_tabela_cliente and self.parente.TabelaPrc and int(self.parente.TabelaPrc)>1 and sql[2].execute("SELECT pd_tpr2,pd_tpr3,pd_tpr4,pd_tpr5,pd_tpr6 FROM produtos WHERE pd_codi='"+row[0]+"'"):

					    pcs = sql[2].fetchone()
					    pcn = pcs[(int(self.parente.TabelaPrc)-2)]
					    if pcn:	vlu=pcn
					    
					""" Kit individual sair preco 2 """
					if row[26].strip() and sql[2].execute("SELECT pd_tpr2 FROM produtos WHERE pd_codi='"+row[0]+"'"):

					    kit_individual_preco2=sql[2].fetchone()[0]
					    if kit_individual_preco2:	vlu=kit_individual_preco2
	...
	def Tvalores(self,valor,idfy):
		...
		try:	self.quanti.SetValue(str(valor))
		except Exception as erro:	self.quanti.SetValue(str(valor))
	...
	def passagem(self,event):
		...
		if self.virtual_temporario.GetValue():

		    self.estoque_deposito.SetValue(True)
		    self.estoqueDeposito(wx.EVT_BUTTON)
		...
		estoque_minimo  = self.list_ctrl.GetItem(indice, 23).GetText()
		...
		estoque_fisico = self.list_ctrl.GetItem(indice, 31).GetText()
		...
		if estoque_min and ( estoque_fis + estoque_vir ) <= estoque_min:	self.estoque_minimo.SetLabel( "{ Estoque minimo }\n[ "+str( estoque_min )+" ]" )
		...
		if sIPPT !="P":	sIPPT = "T"
		if sICMS and Decimal(sICMS) > 0:	sICMS = "T"+sICMS
		else: sICMS = ""
		...
		if cFili and login.filialLT[ cFili ][30].split(';')[11] in ["2","3"] and self.list_ctrl.GetItem(indice, 28).GetText() !="":

			if login.filialLT[ cFili ][30].split(';')[11]=="2":
			    self.cfnor.SetLabel("2-Regime Simples Nacional\nexcesso sublimite de receita bruta: ["+ str( self.list_ctrl.GetItem(indice, 28).GetText() )+"]" )
			    self.cfnor.SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
			    self.cfnor.SetPosition((443,13))
			    
			if login.filialLT[ cFili ][30].split(';')[11]=="3":	self.cfnor.SetLabel("3-Regime Normal: ["+ str( self.list_ctrl.GetItem(indice, 28).GetText() )+"]" )
	...
	def bAdiciona(self,event):
		...
		if memkiT !="":
		    ...
		    """ Forcar venda de produtos individual pertecente ao KIT """
		    click_medidas="\nPS: Utilize o icone de medidas para venda individualizada de produtos do kit" if len(login.filialLT[self.flProd][35].split(";"))>=140 and login.filialLT[self.flProd][35].split(";")[139]=="T" else ""
		    alertas.dia(self.painel,"2-Produto selecionado pertence ao kit-conjunto: { " +str( memkiT )+" }\nNão pode ser vendido individualmente...\n"+click_medidas+"\n"+(' '*180),"DAVs: Vendas em KIT")
		    return
	...
	def adicionaItem(self,opcao):
		...
		if seguir == True:
			...
			comprar.dQuanti  = str( self.Trunca.trunca(5, self.quanti.GetValue() ) )
	...
	def adiciona(self,event):
		...
		if _memkiT !="":
		    ...
		    """ Forcar venda de produtos individual pertecente ao KIT """
		    if len(login.filialLT[ Tfilial ][35].split(";"))>=140 and login.filialLT[ Tfilial ][35].split(";")[139]=="T":	forcar_kit=True
		    else:
			alertas.dia(self.painel,"1-Produto selecionado pertence ao kit-conjunto: { " +str( _memkiT )+" }\nNão pode ser vendido individualmente...\n"+(' '*140),"DAVs: Vendas em KIT")
			return
	...
	def desenho(self,event):
		...
		dc.DrawRotatedText("S i m i l a r e s", 381, 490, 90)
...
class dvListCtrl(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.attr7.SetBackgroundColour("#B371B3")
		...
		self.InsertColumn(36, "Parametros do produto", width=400)
	...
	def OnGetItemAttr(self, item): #Ajusta cores sim/nao
		...
		if self.itemIndexMap != []:
			...
			if len( self.itemDataMap[index][36].split('|') )>=12 and self.itemDataMap[index][36].split('|')[11]=='T':	return self.attr7
			if minim and minim == fisic:	return self.attr2
			if minim and minim >= fisic:	return self.attr2
...
class comprar(wx.Frame):
	...
	def __init__(self,parent,id):
		...
		if sql[0] == True:
			...
			self.autodes = False
			self.bloqueio_f10_vendas = False
			...
			produto = _result[0][1]
			if self.forcakit:	PrecoTB=['2-'+str(_result[0][15])+' '+self.prc2,'3-'+str(_result[0][16])+' '+self.prc3,'4-'+str(_result[0][17])+' '+self.prc4,'5-'+str(_result[0][18])]
			else:
			...
			"""   Utilizar o preco de custo    """
			if self.parente.utilizar_preco_custo and _result[0][22]:	PrecoTB=['1-'+str(_result[0][22])]

			"""   Pesquisa da relacao do KIT se for alteracao   """
			self.preco_original_kit_tabelas = {1:_result[0][4],2:_result[0][15],3:_result[0][16],4:_result[0][17],5:_result[0][18],6:_result[0][19]}
			...
			if codKiT == "T" and relKiT !=None and relKiT !="":
				...
				for vg in relKiT.split("|"):
					if vg !="":
						...
						if sql[2].execute( _psq ) !=0:
							...
							pk = self.rs[0]
							PrecoTB_KIT = ['1-'+str( pk[4] )+' '+self.prc1,'2-'+str(pk[15])+' '+self.prc2,'3-'+str(pk[16])+' '+self.prc3,'4-'+str(pk[17])+' '+self.prc4,'5-'+str(pk[18])+' '+self.prc5,'6-'+str(pk[19])+' '+self.prc6]
				...
				if codKiT == "T" and PrecoTB_KIT and len(login.filialLT[self.filialc][35].split(';'))>=168 and login.filialLT[self.filialc][35].split(';')[167]=="T":
			...
			if esReserv < 0 and not es_virnegativo:
				...
				vir = wx.StaticText(self.painel, -1, pos=(350,60), label="Ajuste o estoque virtual")
				vir.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
...
#---------: Ajusta p/o Regime Tributario Normal			
			...
			if login.filialLT[ self.filialc ][30].split(';')[11] in ["2","3"] and _result[0][27] !="":	self.cFiscal = str( _result[0][27] )
			...
			auTori = wx.BitmapButton(self.painel, 111, wx.Bitmap("imagens/confere.png", wx.BITMAP_TYPE_ANY), pos=(89,176), size=(20,20))				
			...
			caixas = GenBitmapTextButton(self.painel,104,label='Calcular\nPiso',pos=(357,172),size=(60,31))
			mCopia.SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
			caixas.SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.BOLD,False,"Arial"))
			caixas.SetBackgroundColour('#D0FFD0')

			if not self.parente.TComa3.GetValue():	caixas.Enable(False)
			...
			""" Utilizar tabela fixa """
			if self.dVendas and self.p.fixar_tabela.GetValue() and self.p.tabprec.GetValue():	TTabela = ( int(self.p.tabprec.GetValue().split('-')[1]) - 1 ) 
			if not self.dVendas and self.parente.fixar_tabela.GetValue() and self.parente.tabprec.GetValue() and len(self.parente.tabprec.GetValue().split('-'))>=1:	TTabela = ( int(self.parente.tabprec.GetValue().split('-')[1]) -1 )

			if PrecoTB[TTabela].split("-")[1].split(" ")[0] !="" and Decimal( PrecoTB[TTabela].split("-")[1].split(" ")[0] ) == 0:	TTabela = 0

			""" Nao permiti alteracao da tabela pra kit """
			if codKiT == "T" and len(login.filialLT[self.filialc][35].split(';'))>=166 and login.filialLT[self.filialc][35].split(';')[165]=="T":	TTabela = 0

			self.pcvenda = wx.ComboBox(self.painel, 102, PrecoTB[TTabela], pos=(7,80),size=(238,27), choices = PrecoTB,style=wx.CB_READONLY)

			""" Nao permiti alteracao da tabela pra kit """
			if codKiT == "T" and len(login.filialLT[self.filialc][35].split(';'))>=166 and login.filialLT[self.filialc][35].split(';')[165]=="T":	self.pcvenda.Enable(False)
			...
			self.quanti = mkn(self.painel, id = 190, value = self._quanti, pos=(110,175), style = wx.ALIGN_RIGHT|wx.TE_PROCESS_ENTER, integerWidth=8, fractionWidth=4, groupChar = ',', decimalChar = '.', foregroundColour = "#000000", signedForegroundColour = "Red", emptyBackgroundColour = '#FFFFFF', validBackgroundColour = '#FFFFFF', invalidBackgroundColour = "Yellow",allowNegative = False) #, validator=NumericObjectValidator())
			...
			#_r = wx.StaticText(self.painel, -1,"{ Reservado }", pos=(440,427))
			#_r.SetFont(wx.Font(12,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
			...
			caixas.Bind(wx.EVT_BUTTON, self.calcularPiso)
			...
			if self.dVendas == True and esReserv >= 0 :
			    ...
			    if Decimal(self.dQuanti):	self.enviacompra( wx.EVT_BUTTON )
		...
		if self.rs[0][31]:
		    ...
		    if dav.davCV=="A": ##// [ Alteracao da quantidade { Devolucao } ]
			
			#saldo_devolucao = self.parente.ListaPro.GetItem(self.parente.ListaPro.GetFocusedItem(), 58).GetText()
			#m2caixa = self.rs[0][31].split("|")[6] if len(self.rs[0][31].split("|"))>=7 and self.rs[0][31].split("|")[6] else ""

			numero_caixas = self.parente.ListaPro.GetItem(self.parente.ListaPro.GetFocusedItem(), 91).GetText().split(' ')
			if numero_caixas[0] and Decimal(numero_caixas[0]):	self.quanti.SetValue( numero_caixas[0] )
			
		    nF.vendasPisosCaixas( self.rs[0][31].split('|'), self.quanti.GetValue(), self )
		"""
	
	def calcularPiso(self,event):
	    
	    if not self.quanti.GetValue():	alertas.dia(self,"Entre com a quantidade em caixas\n"+(" "*130),"Retaguarda de vendas")
	    elif self.quanti.GetValue() and int(str(self.quanti.GetValue()).split('.')[1]):	alertas.dia(self,"Apenas numeros inteiros na quantidade\n"+(" "*130),"Retaguarda de vendas")
	    else:

		if len(self.rs[0])>=32 and len(self.rs[0][31].split('|'))>=7 and self.rs[0][31].split('|')[6] and Decimal(self.rs[0][31].split('|')[6]):
		    
		    metros = format( ( Decimal(self.quanti.GetValue()) * Decimal(self.rs[0][31].split('|')[6]) ),'.4f')
		    self.quanti.SetValue(metros)
		    self.quanti.SetFocus()
		    nF.vendasPisosCaixas( self.rs[0][31].split('|'), self.quanti.GetValue(), self )
		else:	alertas.dia(self,"Sem dados de piso no produto\n"+(" "*130),"Retaguarda de vendas")
	...
	def Teclas(self,event):
		...
		if keycode   == wx.WXK_F10 and self.rs[0][24] == "":
		    if not self.bloqueio_f10_vendas:	self.gravarCompra( _volTa=True, kiT = "" )
		    else:	alertas.dia(self.painel,"Botao de vendas estar desabilitado...\n"+(' '*80),"DAVs: Adicionando Produtos")
	...
	def gravarCompra(self, _volTa=False, kiT=""):
		...
		#if self.quantidade_produto_kit:	quantidade = self.MetragemCorte = self.quantidade_produto_kit
		"""
		...
		if Decimal( self.MetragemClien ):	quanDevol = Decimal( self.MetragemClien )
		...
		rT = self.davc.adicionarItem(codigo,produto,\
								...
								aTualizaPreco = False, codigoAvulso = self.codigoa.GetValue(), FilialITem = self.filialc, per_desconto = perdes, vlr_desconto = vlrdes, vinculado = "", estoque_local = "0", produto_servico='' )
		...
		if self.dVendas and dav.lista_reserva_temporaria:

		    self.parente.virtual_temporario.SetValue(True)
		    self.parente.estoqueDeposito(wx.EVT_BUTTON)
	...
	def enviacompra(self,event):
		...
		if Decimal( self.pcvenda.GetValue().split('-')[1].split(' ')[0] ) == 0:

		    alertas.dia(self,u"Preço de venda selecionado estar vazio...\n"+(' '*180),'Preço de  venda')
		    return
		...
		if self.lsKiT:
			...
			if avancar:
				...
				valor_total_kit = self.totalizaKit(self.lsKiT.split("|"), pcv)
				numero_produtos = len(self.lsKiT.split("|")[:-1])
				valor_final_kit = Decimal()

				""" Calculando o preco final em cima do preco cadastrado no KIT """
				if len(login.filialLT[self.filialc][35].split(";"))>=168 and login.filialLT[self.filialc][35].split(";")[167]=="T":
				
				    if not self.validarKitPreco( self.lsKiT.split("|"), pcv ):
					aborTar = True
					alertas.dia(self,'Produto(S) do kit referente a tabela '+pcv+u', estar com valor zero no preço...\n\nO direto precisa desses valores para determinar os preços referenciados\n'+(' '*160),'Validar kit')
				...
				if not aborTar:
				    ...
				    for vd in self.lsKiT.split("|"):
					    ...
					    if vd !="":
						    ...
						    preco_venda_kit = Decimal( self.pcvenda.GetValue().split('-')[1].split(' ')[0] )
							...
							preco_venda = Decimal( self.pcvenda.GetValue().split('-')[1].split(' ')[0] )
							percentual_venda = ( preco_venda / valor_total_kit * 100 )
							valor_final_tabela = self.preco_original_kit_tabelas[int(pcv)]
							valor_final_preco  = self.Trunca.arredonda(2, ( valor_final_tabela * percentual_venda /100 ) )
							titulo_tabela = self.pcvenda.GetValue().split('-')[1].split(' ')[1] if len( self.pcvenda.GetValue().split('-')[1].split(' ') ) >=2 else " "

							""" O ultimo item do kit e subtraido do valor do kit p/evitar diferencas de centavos """
							numero_produtos -=1
							if numero_produtos == 0:	valor_final_preco = ( valor_final_tabela - valor_final_kit )
								    
							valor_final_kit +=valor_final_preco
							novo_valor_venda = pcv+"-"+str( valor_final_preco )+" "+titulo_tabela
							self.pcvenda.SetValue( novo_valor_venda )

							
						    _qT=self.Trunca.trunca(5,_qT)
						    #self.quanti.SetValue(str(_qT)) { Estava dando erro no masked }
						    self.quantidade_produto_kit = str(_qT)
						    self.quanti.SetValue( self.quantidade_produto_kit )
		    ...
		    self.gravarCompra( _volTa=True, kiT= "" )
	...
	def validarKitPreco(self, kit, pvc):
		...
		if vd !="":
		    ...
		    if pvc == '1' and not Decimal( vd.split(";")[2] ):	retorno = False
		    if pvc == '2' and not Decimal( vd.split(";")[4] ):	retorno = False
		    if pvc == '3' and not Decimal( vd.split(";")[5] ):	retorno = False
		    if pvc == '4' and not Decimal( vd.split(";")[6] ):	retorno = False
		    if pvc == '5' and not Decimal( vd.split(";")[7] ):	retorno = False
		    if pvc == '6' and not Decimal( vd.split(";")[8] ):	retorno = False
	    
	    return retorno
	    
	def totalizaKit(self, listakit, pcv):
		...
		if vd !="":
		    ...
		    if pcv == "1":	preco_kit +=Decimal( _vl )
		    if pcv == "2":	preco_kit +=Decimal( _v2 )
		    if pcv == "3":	preco_kit +=Decimal( _v3 )
		    if pcv == "4":	preco_kit +=Decimal( _v4 )
		    if pcv == "5":	preco_kit +=Decimal( _v5 )
		    if pcv == "6":	preco_kit +=Decimal( _v6 )
...
#---[ Calcula Metragem do produto ]
	def calculoMetros(self,idMetros):
		...
		_quant = self.Trunca.trunca(5,Decimal(self.quanti.GetValue()))
