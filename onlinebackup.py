...
from conectar import dialogos,menssagem,sbarra,login,diretorios,cores,MostrarHistorico,gerenciador,sqldb
...
#		self.backup_online.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.downloadArquivo)
		self.backup_online.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.listarPastas)
		...
		if login.usalogin.upper() == "LYKOS":	self.lista_filiais.Enable(True)
		...
		self.lista_filiais.Bind(wx.EVT_COMBOBOX, self.filialSelecionada)
		self.filialSelecionadaPadrao(wx.EVT_BUTTON)
		
		self.conexaoFtp(wx.EVT_BUTTON)

	def filialSelecionada(self,event):
	    
	    if self.lista_filiais.GetValue():

		self.f=self.lista_filiais.GetValue().split('-')[0]
		if self.f in login.websrftp and login.websrftp[self.f] and len(login.websrftp[self.f].split('|'))>=3:

		    self.pp=login.websrftp[self.f].split('|')[0]

		    if login.websrftp[self.f].split('|')[1]=='SQL':	self.backup_sql.SetValue(True)
		    if login.websrftp[self.f].split('|')[2]=='XML':	self.backup_xml.SetValue(True)
		    self.filial_padrao.SetValue(self.f +'  '+ login.websrftp[self.f].split('|')[0].split('_')[1] )
		    self.pasta_atual.SetValue(self.pp)
		    self.reconetarftp(wx.EVT_BUTTON)

		else:	alertas.dia(self,u'Filial selecionada, não estar configurada para backup...\n'+(' '*180),'Backup-cloud')
	    
	def filialSelecionadaPadrao(self,event):

	    cone=False
	    conn=sqldb()
	    sql=conn.dbc("Banckup-Cloud: Consulta de filial padrao", fil = login.identifi, janela = self.painel )
		...
		if rls:
			...
			if passar and i[3] and len(i[3].split('|'))>=3:
			    ...
			    self.pp=i[3].split('|')[0]
			    self.f=i[3].split('|')[0].split('_')[0]
			    if i[3].split('|')[1]=='SQL':	self.backup_sql.SetValue(True)
			    if i[3].split('|')[2]=='XML':	self.backup_xml.SetValue(True)
			    self.filial_padrao.SetValue(self.f +'  '+ i[3].split('|')[0].split('_')[1] )
			    self.pasta_atual.SetValue(self.pp)

			    cone=True
			    break

		    if not login.backup_cloud:
			...
			for i in result:
				...
				fl=i[3].split('|')[0].split('_')[0]
				ft=login.filialLT[fl][14].upper()
				self.filiaisPadroes.append(fl+'-'+ft)

			self.lista_filiais.SetItems(self.filiaisPadroes)
			
	    if cone:	self.reconetarftp(wx.EVT_BUTTON)
	...
	def implantacaoCloud(self,event):
		...
		if self.c:
			...
			conn=sqldb()
			sql=conn.dbc("Banckup-Cloud: Gravacao da filial padrao", fil = login.identifi, janela = self.painel )

			if sql[0]:

			    sql[2].execute('UPDATE cia SET ep_bkcl="'+ arquivo_backup +'" WHERE ep_inde="'+ id_filial +'"')
			    sql[1].commit()
			    conn.cls(sql[1])
		...
		if self.c and self.pasta_padrao:
			...
			try:
				...
				for p in self.relacao_pastas:
					...
					if str( p ) == str( self.pasta_padrao ):	adicionar = False

				if adicionar:	self.relacao_pastas.append(self.pasta_padrao)
				...
				for i in conteudo:
					...
					if s[4] == "0":	tipo = "2" #// Arquivo
					else:	tipo = "1" #// Diretorio
				...
				self.pasta_atual.SetValue(self.pasta_padrao)
...
#		for i in self.pasta_padrao.split('/')[:-1]:
		for i in self.pasta_padrao.split('/'):
