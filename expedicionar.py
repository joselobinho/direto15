...
class RelacionarPedidos(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.relacao_davs = wx.ListCtrl(self.painel, -1,pos=(70,93), size=(sl-82,sc-187),
									style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.relacionarfiliais=wx.StaticText(self.painel,-1, u"", pos=(830,70) )
		self.relacionarfiliais.SetFont(wx.Font(10,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.relacionarfiliais.SetForegroundColour('#1E5567')

		self.simm, self.impressoras, user, prn = formas.listaprn( 1 )
		self.lista_printers = ['']
		...
		adiciona_filial=wx.BitmapButton(self.painel, 300, wx.Bitmap("imagens/simadd20.png", wx.BITMAP_TYPE_ANY), pos=(750,65), size=(30,25))
		apagar_filial=wx.BitmapButton(self.painel, 301, wx.Bitmap("imagens/simapaga16.png", wx.BITMAP_TYPE_ANY), pos=(785,65), size=(30,25))
		...
		adiciona_filial.Bind(wx.EVT_BUTTON, self.relacionarFiliaisAdiciona )
		apagar_filial.Bind(wx.EVT_BUTTON, self.relacionarFiliaisAdiciona )
	...
	def relacionarFiliaisAdiciona(self,event):
	    
	    if event.GetId()==300 and self.relacao_filial.GetValue() and self.relacao_filial.GetValue().split('-')[0] not in self.relacionarfiliais.GetLabel():
		
		if self.relacionarfiliais.GetLabel():	self.relacionarfiliais.SetLabel(self.relacionarfiliais.GetLabel()+','+self.relacao_filial.GetValue().split('-')[0])
		else:	self.relacionarfiliais.SetLabel(self.relacao_filial.GetValue().split('-')[0])
		self.relacionarMaterial(wx.EVT_BUTTON)

	    if event.GetId()==301 and self.relacao_filial.GetValue() and self.relacao_filial.GetValue().split('-')[0] in self.relacionarfiliais.GetLabel():
		
		lista=self.relacionarfiliais.GetLabel().split(',')
		filial=self.relacao_filial.GetValue().split('-')[0]
		relacionar=''
		if len(lista) > 1:

		    for i in lista:
			print i
			if i!=filial:	relacionar+=i+','
			
		    self.relacionarfiliais.SetLabel(relacionar[:-1])
		    
		else:	self.relacionarfiliais.SetLabel('')
		self.relacionarMaterial(wx.EVT_BUTTON)
	...
	def relacionarMaterial(self,event):
		...
		if sql[0]:
			...
			procura = "SELECT cr_ndav,cr_nmcl,cr_udav,cr_urec,cr_edav,cr_hdav,cr_erec,cr_hrec,cr_inde, cr_entr, cr_stat, cr_expe, cr_exse FROM cdavs WHERE cr_edav>='"+ datai +"' and cr_edav<='"+ dataf +"'and cr_reca='1' and  cr_roma='' and cr_entr='00-00-0000' and SUBSTRING_INDEX(cr_stat, '|', 1)!='3' ORDER BY cr_hdav ASC"
			...
			if self.relacionarfiliais.GetLabel():	filiais=self.relacionarfiliais.GetLabel().split(',')
			elif filial:	filiais=[filial]
			else:	filiais=[]
			...
			if numero_registro:
				...
				for i in result:
					...
					avanca=True
					if filiais and i[8] not in filiais:	avanca=False
					if mostrar_dav and avanca:
...
class StatusPedido(wx.Frame):
	...
	def __init__(self, parent,id ):
		...
		self.davs_relacao.SetFont(wx.Font(18, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		
		self.davs_relacao.InsertColumn(0, 'Pedido', format=wx.LIST_ALIGN_TOP,width=130)
		self.davs_relacao.InsertColumn(1, 'Descrição do cliente', width=670)
		self.davs_relacao.InsertColumn(2, 'Status', width=200)
		self.davs_relacao.InsertColumn(3, 'Vendedor', width=200)
		self.davs_relacao.InsertColumn(4, 'Emissão', width=260)
		self.davs_relacao.InsertColumn(5, 'Filial', width=110)
	...
	def relacionarDavs(self,event):
		...
		if self.p.relacionarfiliais.GetLabel():	filiais=self.p.relacionarfiliais.GetLabel().split(',')
		elif _f:	filiais=[_f]
		else:	filiais=[]
		...
		if sql[0]:
			...
			procura = "SELECT cr_ndav,cr_nmcl,cr_udav,cr_urec,cr_edav,cr_hdav,cr_erec,cr_hrec,cr_inde, cr_entr, cr_stat, cr_expe, cr_exse FROM cdavs WHERE cr_edav='"+ hoje +"' and cr_reca='1' and  cr_roma='' and cr_entr='00-00-0000' and SUBSTRING_INDEX(cr_stat, '|', 1)!='3' ORDER BY cr_hdav ASC"
			...
			if relacao_davs:
				...
				for i in results:
				    ...
				    if avanca:
					self.davs_relacao.InsertStringItem(indice, str(int(i[0])))
					self.davs_relacao.SetStringItem( indice, 1, i[1])
					self.davs_relacao.SetStringItem( indice, 3, i[2])
					self.davs_relacao.SetStringItem( indice, 4, str(format( i[4],'%d/%m/%Y')  )+' '+str( i[5]))
					self.davs_relacao.SetStringItem( indice, 5, i[8])
					...
					if i[10] and i[10].split('|')[0] == "1":
					    ...
					    self.davs_relacao.SetStringItem( indice, 2, "SEPARANDO" )
					    self.davs_relacao.SetItemBackgroundColour( indice, "#68DB68")

					if i[10] and i[10].split('|')[0] == "2":
						    
					    self.davs_relacao.SetStringItem( indice, 2, "L O J A" )
