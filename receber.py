...
from conectar   import sqldb,formasPagamentos,dialogos,menssagem,truncagem,cores,socorrencia,numeracao,login,sbarra,TelNumeric,dadosCheque,regBandeira,srateio,consultarCliente,acesso,autorizacoes,diretorios,gerenciador,AbrirArquivos
...
class contasReceber(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.lista_web_server={}
		self.conciliacaoBoletos=ValidacoesReceber()
		...
		""" Relacao de Bancos """
		self.ListaBancos = wx.ListCtrl(self.painel, 820,pos=(692,307), size=(271,85),
						 			style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.boletoba = wx.BitmapButton(self.painel, 240, wx.Bitmap("imagens/boleto60-30.png",  wx.BITMAP_TYPE_ANY), pos=(595, 305), size=(95,35))				
		...
		""" PagHiper """
		self.atualizar_preco = GenBitmapTextButton(self.painel,800,label=u'PagHiper\n& Pagar.me\nBoletos',  pos=(595,343),size=(95,50), bitmap=wx.Bitmap("imagens/qrcode24.png", wx.BITMAP_TYPE_ANY))
		self.atualizar_preco.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL,wx.NORMAL,False,"Arial"))
		self.atualizar_preco.SetBackgroundColour('#729BC2')
		self.atualizar_preco.SetForegroundColour('#FFFFFF')
		...
		self.atualizar_preco.Bind(wx.EVT_BUTTON, self.acessoPagHiper)
...
#		self.relacaoFilial(wx.EVT_COMBOBOX)
	...
	def EmitirBoleto(self,event):
		...
		ws=self.ListaReceber.GetItem( indice, 45 ).GetText().strip().split('|')[0]
		...
		if self.ListaReceber.GetItem( indice, 45 ).GetText().strip():

		    alertas.dia(self.painel,u"{ Boleto ja emitido }\nServidor de gestão de cobrança, boleto emitido para "+ws+"\n"+(" "*180),u"Contas Receber: Emissão de Boletos")
		    return
		...
		isw  = self.ListaBancos.GetItem( __id, 9 ).GetText().split('|')[0]
		...
		if not srvwconexao[0]:
		...
		elif not srvwconexao[1].split('|')[0]:

		    alertas.dia(self.painel,u"Sem dados do webserver para emissão do boleto\n"+(" "*100),"Contas Receber: Emissão de Boletos")
		    return
		    ...
		    cl_idcl = dados_clien[1][14] #-:Numero do documento
		    ...
		    valor_documento=self.ListaReceber.GetItem(indice,5).GetText().replace(",","").replace('.','')
		    vencimento=str(datetime.datetime.strptime(self.ListaReceber.GetItem(indice,3).GetText(),"%d/%m/%Y").date())
		    filial=self.ListaReceber.GetItem(indice,50).GetText().strip()

		    id_lancamento=self.ListaReceber.GetItem(indice,49).GetText().strip() #-: ID-Lancamento

		    cnn=srvwconexao[1].split('|')
		    srvw=login.listaws[cnn[0]]
			
		    token=cnn[3]
		    key=cnn[4]

		    semail='' if cl_emai else u'\n\nP.S Cliente sem email para envio automático'
		    confima = wx.MessageDialog(self.painel,u"Confirme p/enviar boleto para o servidor de gestão de cobrança "+login.listaws[isw]+semail+"\n"+(" "*180),u"Contas Receber: Emissão de Boletos",wx.YES_NO|wx.NO_DEFAULT)
			...
			'filial':filial,
			'servico':'Cobranca de compras'
			}

			if srvw.upper().replace('.','')=='PAGHIPER':	r=pgh.pagHiperBoleto(self, dados_boleto )
			elif srvw.upper().replace('.','')=='PAGARME':	r=pgm.pagarmeBoleto(self, dados_boleto )

			if r[0]:
			    rt=self.levantarBoletos(4,srvw.upper().replace('.','').lower()+'|'+r[3])
			    self.selecionar(wx.EVT_BUTTON)
			    if rt[0]:	alertas.dia(self,u'{ Boleto enviado para webserver '+login.listaws[isw]+' [ '+r[3].split('|')[5]+' ] }\n\n'+r[2]+'\n'+(' '*180),u'Emissão de boletos')

			if not r[0]:	alertas.dia(self,u'{ Retorno da emissão }\n\n'+r[2]+"\n"+(' '*180),u'Emissão de boletos')
	...
	def levantarBoletos( self, opcao, codigo ):
		...
		if sql[0]:
			...
			if opcao == 1: # Clientes
				...
				if sql[2].execute("SELECT cl_codigo,cl_nomecl,cl_fantas,cl_docume,cl_endere,cl_bairro,cl_cidade,cl_cdibge,cl_cepcli,cl_compl1,cl_compl2,cl_estado,cl_emailc,cl_telef1,cl_regist FROM clientes WHERE cl_codigo='"+ codigo +"'"):
			...
			if opcao == 4: # dados de conexao do servidor web
				...
				try:
				    ...
				    id_lancamento=codigo.split('|')[1]
				    sql[2].execute("UPDATE receber SET rc_blweob='"+ codigo +"' WHERE rc_regist='"+ id_lancamento +"'")
				    sql[1].commit()
				    r = True
			...
			if opcao in [5,6]: # cancelamento do boleto
				
				try:
				    #print('Cancelamento: ',codigo,opcao)
				    if opcao==5:	sql[2].execute("UPDATE receber SET rc_blweob='' WHERE rc_regist='"+ codigo +"'")
				    elif opcao==6:	sql[2].execute("UPDATE receber SET rc_blweob='', rc_status='5' WHERE rc_ndocum='"+ codigo +"'")
	...
	def desmembrados(self,event):
		...
		if self.Trelac.GetValue() !='':
			...
			if event.GetId() == 310:
				...
				if Relaca == '' or Relaca == None:	impress.impressaoDav(Numero,self.painel,True,True,"","", servidor = self.filrc, codigoModulo = "", enviarEmail = "",recibo=False )
	...
	def reimpressao(self,event):
		...
		if self.RelacaoTitulo == '':	impress.impressaoDav(self.NumeroComanda,self.painel,True,False,"","",servidor = self.filrc, codigoModulo = "", enviarEmail = "",recibo=False )
	...
	def baixaTitulos(self,event):
		...
		if event.GetId() == 306:
			...
			if clDocumento == '' or clCodigo == '':
				...
				if len( login.filialLT[ self.filrc ][35].split(";") ) >= 117 and login.filialLT[ self.filrc ][35].split(";")[116] == 'T' and clCodigo:	passar = True
				else:
					passar = False
					alertas.dia(self,u"Código e CPF-CNPJ do cliente vazio, apenas liquidção...\n"+(" "*120),u"Contas AReceber [ Baixa de Títulos ]")
	...
	def seleciona(self,_id):
		...
		if sql[0] == True:
			...
			""" Pesquisando davs de origem do check-out """
			__ck = self.consultar.GetValue().upper()
			if __ck and "CK" in __ck:
			    __vd, __nd = __ck.split('CK')
			    if __vd.isdigit() and __nd.isdigit():
				__numdav = __vd.zfill(4)+'CK'+__nd.zfill(7)
				self.consultar.SetValue('D:'+__numdav)
			...
			if _id == 231 or _id == 232:
				...
				if self.period.GetValue() == True and _lan==u"LANÇAMENTO":	Receber = Receber.replace('ORDER BY rc_vencim',"and rc_dtlanc >='"+inicial+"' and rc_dtlanc <='"+final+"' ORDER BY rc_dtlanc")
				...
				# { em 18-07-2019 Posse Ferro } if self.pagamento.GetValue() != '':	Receber = Receber.replace('ORDER BY rc_vencim',"and rc_formap like '"+_fpg+"%' ORDER BY rc_vencim")
				if self.pagamento.GetValue() != '':	Receber = Receber.replace('WHERE',"WHERE rc_formap like '"+_fpg+"%' and")
				...
				#em 18-7-2019 posse ferro, if self.FAbert.GetValue() == True:	Receber = Receber.replace('ORDER BY rc_vencim',"and rc_status='' ORDER BY rc_vencim")
				#print('1: ',Receber,self.FAbert.GetValue())
	...
	def MenuPopUp(self):
		...
		#self.popupmenu.Append(wx.ID_YES,  u"Gerenciador de boletos-cartões")
	...
	def acessoPagHiper(self,event):
	    self.lista_web_server={}
	    for i in range(self.ListaBancos.GetItemCount()):
			
		wsb=self.ListaBancos.GetItem(i,9).GetText().split('|')
		if len(wsb)>=5 and wsb[0]:

		    self.lista_web_server[login.listaws[wsb[0]]+' >--> '+self.ListaBancos.GetItem(i,4).GetText()]=wsb[4],wsb[3]

	    gbl_frame=GerenciadorBoletosCartoes(parent=self,id=-1,relacao_web=self.lista_web_server,modulo='receber',filial=self.filrc)
	    gbl_frame.Centre()
	    gbl_frame.Show()
	...
	def OnPopupItemSelected(self, event):
		...
		#	    self.lista_web_server[login.listaws[wsb[0]]+' >--> '+self.ListaBancos.GetItem(i,4).GetText()]=wsb[4],wsb[3]
		    ...
		    #if not self.lista_web_server:	alertas.dia(self.painel,u"Lista de web-server vazio...\n"+(" "*100),"Contas Receber: Emissão de Boletos")
		    #else:
		 #   gbl_frame=GerenciadorBoletosCartoes(parent=self,id=-1,relacao_web=self.lista_web_server,modulo='receber',filial=self.filrc)
		 #   gbl_frame.Centre()
		 #   gbl_frame.Show()
...
class reimdesvin(wx.Frame):
	...
	def printar(self,event):
		...
		if sel != -1:
			...
			impress.impressaoDav(self.ListaSel.GetString(self.ListaSel.GetSelection()).split('/')[0],self.painel,True,False,"","",servidor = self.fldsv, codigoModulo = "", enviarEmail = "",recibo=False )
...
class baixar(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		wx.Frame.__init__(self, parent, id, 'Contas Areceber: Baixa de títulos', size=(920,505), style=wx.CAPTION|wx.BORDER_SUNKEN|wx.FRAME_FLOAT_ON_PARENT|wx.CLOSE_BOX)
		self.painel = wx.Panel(self,-1,style=wx.BORDER_SUNKEN)
		self.Bind(wx.EVT_CLOSE, self.sair)

		self.ListaBaixa = BaixaLista(self.painel, -1,pos=(10,28), size=(906,altura),
								style=wx.LC_REPORT
								|wx.LC_VIRTUAL
								|wx.BORDER_SUNKEN
								|wx.LC_HRULES
		...
		wx.StaticText(self.painel,-1, "Comissão bandeiras", pos=(801,270)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		wx.StaticText(self.painel,-1,u"Lista de bancos { Lançamentos automático no controle de conta corrente }",    pos=(16,452)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		wx.StaticText(self.painel,-1,u"Referência de baixa", pos=(592,452)).SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.cliente = wx.TextCtrl(self.painel,-1,'',pos=(55,2),size=(640,24), style=wx.TE_READONLY)
		...
		vendedor = wx.StaticText(self.painel,-1,"Vendedor(a)",pos=(717,455))
		vendedor.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		vendedor.SetForegroundColour('#80A4C8')

		self.vendedo = wx.StaticText(self.painel,-1,"",pos=(717,470))
		...
		nT = wx.StaticText(self.painel,-1,"Valor apagar:",pos=(200,345))
		...
		bT = wx.StaticText(self.painel,-1,"Valor liquido:",pos=(200,370))
...
#		self.qBaixar = wx.TextCtrl(self.painel,-1,'',pos=(280,340), size=(100,22),style=wx.TE_READONLY|wx.ALIGN_RIGHT)
		self.qBaixar = wx.StaticText(self.painel,-1,'',pos=(200,358))
		self.qBaixar.SetBackgroundColour('#D3D307')
		self.qBaixar.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))

#		self.vTpagar = wx.TextCtrl(self.painel,-1,'',pos=(280,365), size=(100,22),style=wx.TE_READONLY|wx.ALIGN_RIGHT)
		self.vTpagar = wx.TextCtrl(self.painel,-1,'',pos=(280,340), size=(100,22),style=wx.TE_READONLY|wx.ALIGN_RIGHT)
		...
		self.vliquido = wx.TextCtrl(self.painel,-1,'',pos=(280,365), size=(100,22),style=wx.TE_READONLY|wx.ALIGN_RIGHT)
		self.vliquido.SetBackgroundColour('#E5E5E5')
		self.vliquido.SetForegroundColour('#DC2323')
		self.vliquido.SetFont(wx.Font(9,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		...
		self.fornecedores_bancos = wx.ComboBox(self.painel, 456, "", pos=(14, 463), size=(570, 27), choices = lista_bancos, style=wx.NO_BORDER|wx.CB_READONLY)
		self.data_refencia_baixa = wx.DatePickerCtrl(self.painel,802, pos=(590,463), size=(115,27), style=wx.DP_DEFAULT|wx.DP_ALLOWNONE)

#		self.fornecedores_bancos = wx.ComboBox(self.painel, 456, "", pos=(14, 463), size=(687, 27),  choices = lista_bancos, style=wx.NO_BORDER|wx.CB_READONLY)
...
#		self.voltar. Bind(wx.EVT_BUTTON,self.buscarCnab)
		...
		#self.fornecedores_bancos.Bind(wx.EVT_COMBOBOX,self.baixar)
...
#-------[ Lista debitos do cliente ]		
	...
	def buscarCnab(self,event):

		AbrirArquivos.pastas = diretorios.usPasta
		AbrirArquivos.arquiv = "CNAB (*.ret)|*.ret|Retorno (*.RET)|*.RET|All Files (*.*)|*.*"

		arq_frame=AbrirArquivos(parent=self,id=912)
		arq_frame.Centre()
		arq_frame.Show()
	    
	#def importarCnab(self):

	 #   lista_titulos = cnb.leituraItau400(1, self.a)
	 #   if lista_titulos:
	#	aa=1
	#	for i in sorted(lista_titulos):

	#	    l = lista_titulos[i]
		    
		    #print l[9],l[12],l[22],l[38],l[19],l[42],l[43],l[27],l[32],l[40],l[18]
	#	    aa+=1
		#print lista_titulos
	...
	def passagem(self,event):
		...
		if self.Liquidar == True:
			...
			#self.fornecedores_bancos.SetValue( self.ListaBaixa.GetItem(indice, 23).GetText())
	...
	def liquidarTitulos(self,event):
		...
		if bloqueio_formas and self.ListaBaixa.GetItemCount():
			...
			for lliq in range( self.ListaBaixa.GetItemCount() ):
				...
				if self.ListaBaixa.GetItem( lliq, 0 ).GetText().strip().upper() == "BAIXA" and self.ListaBaixa.GetItem( lliq, 12 ).GetText().strip().split('-')[0] in bloqueio_formas:

					if len( login.usaparam.split(';') ) >= 25 and login.usaparam.split(';')[24] == 'T' and self.ListaBaixa.GetItem( lliq, 12 ).GetText().strip().split('-')[0] == '07':	pass
					else:	prosseguir = False

		if not prosseguir:
			
			alertas.dia(self,u"Existem formas de pagamentos bloqueadas p/liquidação...\n"+(" "*130),u"Formas de pagamentos bloqueadas p/liquidação")
			return

		adiante = True
#		if self.qBaixar.GetValue() == "":	adiante = False
#		elif self.qBaixar.GetValue().split('/')[0] == "0": 	adiante = False
		if self.qBaixar.GetLabel() == "":	adiante = False
		elif self.qBaixar.GetLabel().split('/')[0] == "0": 	adiante = False
			...
			if Finaliza.ShowModal() == wx.ID_YES:
				...
				if baixados_outros_usuarios[0]:	alertas.dia(self, u"[  Titulos em processamento { "+ baixados_outros_usuarios[1]+u" } ]\n\n1-Sistema não pode prosseguir enquanto o problema não for solucionado\n"+ (" "*200), "Contas areceber: Titulos processados por outro usuario")
					...
					if sai == True and gravei == True:
						...
						"""  Incluir no conta-corrente  """
						mensagem = mens.showmsg(u"{ Adicionando titulos no conta correten ["+str( indice + 1 )+"/"+str(itens)+"] }\n"+ self.ListaBaixa.GetItem(indice,14).GetText() +"\n\nAguarde...")
						for cc in range(itens):

							if self.ListaBaixa.GetItem(cc,0).GetText().upper() == "BAIXA" and self.ListaBaixa.GetItem(cc, 23).GetText():
								mensagem = mens.showmsg(u"{ Adicionando titulos no conta correten ["+str( cc + 1 )+"/"+str(itens)+"] }\n"+ self.ListaBaixa.GetItem(cc,14).GetText() +"\n\nAguarde...")

								#--// Numero do documento [ R ContasReceber, 01 Parcela ] { R-0000001/01 }
								numero_dav = 'LR-' + self.ListaBaixa.GetItem(cc, 2).GetText() + '/' + self.ListaBaixa.GetItem(cc, 3).GetText()
								valor_liquido = self.ListaBaixa.GetItem(cc, 11).GetText().replace(',','')
								forma_pagamento = self.ListaBaixa.GetItem(cc, 12).GetText()
								bandeira = self.ListaBaixa.GetItem(cc, 18).GetText()
								filial = self.ListaBaixa.GetItem(cc, 19).GetText()
								tipo_lancamento = "20" #--// Lancamentos de baixa do contas a receber"
								lancamento_longo = "Liquidacao do contas areceber"
								id_banco = str( int( self.ListaBaixa.GetItem(cc, 23).GetText().split('-')[0] ) )
								lancamento_curto = "Liquidacao contas areceber {" + bandeira +' ['+self.ListaBaixa.GetItem(cc, 14).GetText()+'] }' if bandeira else forma_pagamento +"["+self.ListaBaixa.GetItem(cc, 14).GetText()+"]}"
								plano_conta = ""
								credito_debito = u"Crédito"
								modulo = "4-Liquidacao contas areceber"
								banco_origem = ""
								banco_destino = self.ListaBaixa.GetItem(cc, 23).GetText()
								data_lamento_documeto = datetime.datetime.now().strftime("%d%m%Y%T").replace(':','')
								data_referencia = datetime.datetime.strptime(self.data_refencia_baixa.GetValue().FormatDate(),'%d-%m-%Y').strftime("%Y/%m/%d")

								if id_banco:
	
									
									dados = valor_liquido+'|'+lancamento_curto+'|'+lancamento_longo+'|C|'+numero_dav+'|'+plano_conta+\
									'|'+id_banco+'|'+credito_debito+'|'+modulo+'|'+tipo_lancamento+'|'+banco_origem+'|'+banco_destino+'|'+data_lamento_documeto+'|'+data_referencia

									lancamentocontacorrente.gravarLancamentosNovos( dados = dados, parent = self, mostrar = False, numero_apagar='' )
	...
	def impresDav(self,event):
		...
		impress.impressaoDav(NumeroDav,self,True,False,"","",servidor = self.flbxa, codigoModulo = "", enviarEmail = "",recibo=False )
	...
	def baixar(self,event):
		...
		registro = self.ListaBaixa.GetFocusedItem()
		marcar = self.ListaBaixa.GetItem(registro,0).GetText()
		vendas = self.ListaBaixa.GetItem(registro,13).GetText()
		itens  = self.ListaBaixa.GetItemCount()

		lista_itens = self.ListaBaixa
		...
		for i in range(itens):
			#print '1'
			if event.GetId()==100:	selecionado_baixa=['Baixa'] if self.selecao == False else [''] 
			else:
				if i == registro:
					selecionado_baixa=[''] if marcar else ['Baixa']
					selecionado=False if marcar else True
				else:	selecionado_baixa=[lista_itens.GetItem(indice, 0).GetText()]

			for c in range( lista_itens.GetColumnCount() ):

				if i == registro and c == 23:
				    
					#banco=self.fornecedores_bancos.GetValue().strip() if selecionado else ''
					banco = self.ListaBaixa.GetItem(i,23).GetText()
					
					if self.Liquidar and self.fornecedores_bancos.GetValue().strip():	banco=self.fornecedores_bancos.GetValue().strip()
					if not self.Liquidar:
					    banco=self.fornecedores_bancos.GetValue().strip() if selecionado else ''
					
					selecionado_baixa.append(banco) #--// Adicionando o banco

				else:	selecionado_baixa.append( self.ListaBaixa.GetItem( i, c).GetText() ) if c > 0 else ''
    
			relacao[_registros] = selecionado_baixa
			_registros +=1
								
			indice += 1
		self.ListaBaixa.SetItemCount( _registros )
		BaixaLista.itemDataMap  = relacao
		BaixaLista.itemIndexMap = relacao.keys()
	...
	def atualizaLista(self):
		...
		for i in range(itens):
			...
			if self.ListaBaixa.GetItem(indice, 0).GetText().upper() == 'BAIXA':
				...
				vlc += Decimal(self.ListaBaixa.GetItem(indice, 22).GetText().replace(',','')) if self.ListaBaixa.GetItem(indice, 22).GetText() else Decimal()
				
				lista_vendedores.append(self.ListaBaixa.GetItem(indice, 13).GetText())
					
			indice +=1

		""" Valida se tem vendedores diferentes """
		if lista_vendedores:
		    principal = lista_vendedores[0]
		    self.vendedo.SetLabel(principal)
		    for vd in lista_vendedores:
			if principal != vd:	dfv = True
		...
		valor_liquido = ( vlr - vlc )
		self.qBaixar.SetLabel('{ '+str(int(qTd))+"/"+str(itens)+' }')
		self.vTpagar.SetValue(format(vlr,','))
		self.vliquido.SetValue(format(valor_liquido,','))
	...
	def listaDebitos(self):
		...
		if sql[0] == True:
			...
			"""  Custo do boleto  """
			comissao_boletos = ""
			if sql[2].execute("SELECT fg_info FROM grupofab WHERE fg_cdpd='P'"):
			    for bl in sql[2].fetchall():
				if bl[0].split('-')[0] == "06" and len( bl[0].split('|') ) >= 11 and bl[0].split('|')[10]:	comissao_boletos = bl[0].split('|')[10]
			...
			if soma:
				...
				if len( login.filialLT[self.flbxa][35].split(';') )>=151 and login.filialLT[self.flbxa][35].split(';')[150]=='T':	posica.append('09')
				...
				for i in debitos:
					...
					if i[6][:2] in posica and passar == False:
						...
						id_bancos = ''
						if i[27] !=None and i[27] !="" and i[27].split("-")[0] !="":

							comisBand = nF.rTComisBand( i[27] )
							id_bancos = nF.numeroBanco( i[27] )
						...
						"""  Debitando a comissao do boleto do valor nominal  """
						if i[6].split('-')[0]=="06" and comissao_boletos and Decimal(comissao_boletos):
						    vi, vf = comissao_boletos.split('.')
						    valorLiqu = ( apagar - Decimal( vi + '.' +vf.zfill(2) ) )
						    vcomissao = "[0.00] "+format( Decimal( vi + '.' +vf.zfill(2) ),',')

						relacao[_registros] = '',str(ordem).zfill(3),i[1],i[3],str(i[26].strftime("%d/%m/%Y")),format(i[5],','),str(atraso),moraju,\
						vmulta,format(apagar,','),vcomissao,format(valorLiqu,','),i[6],i[56],i[12],i[75]+" - "+i[74],i[35],i[54],i[27],i[24],\
						str( i[0] ),str( comisBand ),str( valorBand ),id_bancos
				...
				self.numeroo.SetLabel("{ "+str( _registros ).zfill(10)+" }")			
	...
	def desenho(self,event):
		...
		dc.DrawRoundedRectangle(10,  450, 696, 45, 3) 
		dc.DrawRoundedRectangle(710, 450, 205, 45, 3) 
		dc.DrawRoundedRectangle(710, 395, 205, 50, 3) 

class BaixaLista(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.attr3 = wx.ListItemAttr()
		self.attr4 = wx.ListItemAttr()
		self.attr1.SetBackgroundColour("#E6E6FA")
		self.attr2.SetBackgroundColour('#FFDFDF')
		self.attr3.SetBackgroundColour('#FFFFC8')
		self.attr4.SetBackgroundColour('#B6ADAD')

		self.InsertColumn(0, 'Baixar',               width=80)
		self.InsertColumn(1, 'ORD',                  width=35)
		self.InsertColumn(2, 'Nº DAV-Título',        width=120)
		self.InsertColumn(3, 'Parcela',              format=wx.LIST_ALIGN_TOP,width=55)
		self.InsertColumn(4, 'Vencimento',           width=80)
		self.InsertColumn(5, 'Valor',                format=wx.LIST_ALIGN_LEFT,width=80)
		self.InsertColumn(6, 'Atraso',               format=wx.LIST_ALIGN_LEFT,width=50)
		self.InsertColumn(7, 'Juros',                format=wx.LIST_ALIGN_LEFT,width=65)
		self.InsertColumn(8, 'Multa',                format=wx.LIST_ALIGN_LEFT,width=65)
		self.InsertColumn(9, 'Valor Apagar',         format=wx.LIST_ALIGN_LEFT,width=75)
		self.InsertColumn(10,'Comissão',             format=wx.LIST_ALIGN_LEFT,width=90)
		self.InsertColumn(11,'Vlr Liquido',          format=wx.LIST_ALIGN_LEFT,width=75)
		self.InsertColumn(12,'Forma de Pagameto',    width=170)
		self.InsertColumn(13,'Vendedor',             width=100)
		self.InsertColumn(14,'Descrição do Cliente', width=400)
		self.InsertColumn(15,'Nº Registro-Descrição do Favorecido', width=700)
		self.InsertColumn(16,'Status',               width=60)
		self.InsertColumn(17,'Estorno',              width=80)
		self.InsertColumn(18,'Bandeira',             width=200)
		self.InsertColumn(19,'Filial',   width=100)
		self.InsertColumn(20,'ID-Lançamento', width=140)
		self.InsertColumn(21,'Comissão da Bandeira', width=200)
		self.InsertColumn(22,'Valor da Comissão da Bandeira', width=230)
		self.InsertColumn(23,'ID-Banco { Lancamento automatico no controle de conta corrente', width=400)
	...
	def OnGetItemAttr(self, item): #Ajusta cores sim/nao
		...
		if self.itemIndexMap != []:
			hoje  = datetime.datetime.now().date()
			index = self.itemIndexMap[item]
			vencimento = datetime.datetime.strptime(self.itemDataMap[index][4], '%d/%m/%Y').date()

			if self.itemDataMap[index][0]:	return self.attr4
			if 	hoje > vencimento:	return self.attr2
			if self.itemDataMap[index][17]:	return self.attr3
	...
	def OnGetItemImage(self, item):
		...
		#		else:	return self.e_sim
			if self.itemDataMap[index][23]:	return self.sim5
		return self.i_orc
...
class desmenbramento(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		if self.pT.dfvd == True:	alertas.dia(self.pT.painel,"{ Títulos selecionados com vendedores diferente }\n\n1-O sistema vai lancar como vendedores diversos\n2-Comissao p/recebimento nao deve utilizar essa opcao\n"+(" "*180),"Contas Areceber: Desmembramento")
		...
		self.vendedor_baixa = wx.StaticText(self.painel,-1,'{ '+self.vendedores+' }', pos=(577,321))
		self.vendedor_baixa.SetFont(wx.Font(7,wx.MODERN,wx.NORMAL,wx.BOLD,False,"Arial"))
		self.vendedor_baixa.SetForegroundColour('#008000')
		if self.vendedores.upper() == 'DIVERSOS':	self.vendedor_baixa.SetForegroundColour('#9B3838')
...
#		self.seleci = wx.TextCtrl(self.painel,-1,str(parent.qBaixar.GetValue()),pos=(200,26), size=(94,22),style=wx.TE_READONLY|wx.ALIGN_RIGHT)
		self.seleci = wx.TextCtrl(self.painel,-1,str(parent.qBaixar.GetLabel()),pos=(200,26), size=(94,22),style=wx.TE_READONLY|wx.ALIGN_RIGHT)
	...
	def saldoTitulos(self):
		...
		filial_padrao = self.filial_finalizacao.GetValue().split('-')[0] if self.filial_finalizacao.GetValue() else login.identifi
		if len( login.filialLT[filial_padrao][35].split(';') ) >= 101 and login.filialLT[filial_padrao][35].split(';')[100] == "T":
	...
	def validaDesmenbramento(self):
		...
		if vsobra > 0:
			...
			for i in range(nLanca):
				...
				indice +=1
				formas = ['06','07','11','12']
				filial = self.filial_finalizacao.GetValue().split('-')[0]
				
				if len( login.filialLT[filial][35].split(';') ) >= 129 and login.filialLT[filial][35].split(';')[128] == 'T':	formas.remove('11')
				if fp in formas:	reTorno = False
...
#-------------------------[ Inclusao no Contas AReceber ]
							...
							for i in range(nLanca):
								...
								lqcD = lqcC = lqcH = lqcP = lqDC = lqTK = "F"
								...
								if len( login.filialLT[ self.fldsm ][35].split(";") ) >=157:	lqTK = login.filialLT[ self.fldsm ][35].split(";")[156] #-:Tickete
								...
								if in_opp =="09" and lqTK == "T":	LiqBaixa = True #-: Tickete
...
class IncluirDocumento(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		if len(login.usaparam.split(";"))>=49 and login.usaparam.split(";")[48]=="T":
				...
				if nLancamento !=0:
					...
					if sql[0] == True:	
						...
						if salv == True and self.contac.GetValue() > 0 and self.transf.GetValue() == True:
							...
							forma.crdb( nLancamento, _codc, _nmcl, _docc, _filc, 'RM','Lançamento Manual Contas AReceber', _vDB, _vCC, _fant, self.painel, Filial = self.flind )
