...
from conectar  import socorrencia,TelNumeric,cores,dadosCheque,regBandeira,srateio,listaemails,acesso,numeracao,AbrirArquivos,configuraistema,autorizacoes,formasPagamentos
...
from prndireta import conexaoDireta
from gerenciamdfe import GerenciadorMdfe

from eletronicos.whatsapp import GerenciadorWhats,EnvioWhatsapp
...
class recebimentos(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		"""Busca Empresas"""
		self.filial = wx.StaticText(self.painel,-1,"{ Informações do DAV: Filial, Cliente }",pos=(510,60))
		...
		self.nfecan = wx.StaticText(self.painel,-1,"{ Informações NFe, NFCe }", pos=(510,79))
		self.nfecan.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.nfecan.SetForegroundColour('#EE0000')

		self.entregar = wx.StaticText(self.painel,-1,"", pos=(862,79))
		self.entregar.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.BOLD,False,"Arial"))
		self.entregar.SetForegroundColour('#1171CF')

		pesquisa = wx.StaticText(self.painel,-1,"Nome,Nº.DAV, P:Expressão, F:Fantasia, N:NoTaFiscal, C:NºCPF-CNPJ, \n$-Valor, pesquisar-Controle Temporario use letra 'p/P' no final do numero", pos=(17,575))
		...
		""" Percentual """
		self.perc_fr = wx.StaticText(self.painel,-1,"",  pos=(750,475))
		self.perc_fr.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.perc_fr.SetForegroundColour('#DC2323')

		self.perc_ac = wx.StaticText(self.painel,-1,"",  pos=(750,495))
		self.perc_ac.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.perc_ac.SetForegroundColour('#DC2323')
		
		self.perc_ds = wx.StaticText(self.painel,-1,"",  pos=(750,515))
		self.perc_ds.SetFont(wx.Font(8,wx.MODERN,wx.NORMAL, wx.NORMAL,False,"Arial"))
		self.perc_ds.SetForegroundColour('#DC2323')
		...
		""" Filiais """
		self.nota_relacao_emissao_pos_recebimento =['Selecione o modelo de nota fiscal p/emissão pos recebimento','55-Emissão da nota fiscal {NFe Modelo [55] }','65-Emissão da nota fiscal { NFCe Modelo [65] }']
		self.relFiliais = [""]+login.ciaRelac
		self.rfilial = wx.ComboBox(self.painel, 900, self.relFiliais[0],  pos=(0,71), size=(500,27), choices = self.relFiliais,style=wx.NO_BORDER|wx.CB_READONLY)
		self.modelo_emissao = wx.ComboBox(self.painel, -1, self.nota_relacao_emissao_pos_recebimento[0],  pos=(509,93), size=(457,27), choices = self.nota_relacao_emissao_pos_recebimento,style=wx.NO_BORDER|wx.CB_READONLY)
		...
		referenc = wx.BitmapButton(self.painel, 232, wx.Bitmap("imagens/referencia16.png",  wx.BITMAP_TYPE_ANY), pos=(320, 520), size=(36,35))				
		...
		filtrar_pedidos = ['1-Filtrar direto','2-Filtrar check-out','3-WOO-Commerce','4-Corte Cloud','10-Notas emitidas','11-Notas não emitidas']
		self.pedido_filtrar = wx.ComboBox(self.painel, -1, filtrar_pedidos[0], pos=(335,572), size = (170,27), choices = filtrar_pedidos, style=wx.NO_BORDER|wx.CB_READONLY)
		...
		self.consultar = wx.TextCtrl(self.painel, -1, pos=(14,597),   size=(318,25),style=wx.TE_PROCESS_ENTER)
		self.pesqperio = wx.CheckBox(self.painel, -1, "Pesquisar cliente período", pos=(335,600))
	...
	def OnClick(self,event):
	    ...
	    self.fdbl=True #-: Click duplo no recebimento
	...
	def MenuPopUp(self):
		...
		self.popupmenu.Append(wx.ID_UNINDENT,  "Gerenciador de nota fiscal de compras { Notas emitidas contra filial, Download }")
		self.popupmenu.Append(wx.ID_NEW,  "Gerenciador de MDFe { Envio, Cancelamento, Download }")
		self.popupmenu.Append(1017,  "Gerenciador de Whatsapp { Envio de menssagens }")

		self.Bind(wx.EVT_MENU, self.OnPopupItemSelected)
		self.Bind(wx.EVT_CONTEXT_MENU, self.OnShowPopup)
		#gr=False if acs.acsm("524",True) else True
		#self.popupmenu.Enable( id = wx.ID_APPLY, enable = gr )

		self.popupmenu.Enable( id = wx.ID_PASTE, enable = acs.acsm("507",True) )
		self.popupmenu.Enable( id = 1013, enable = acs.acsm("518",False) )

		gerenciador_nfe_compras = True if len( login.usaparam.split(";") ) >=26 and login.usaparam.split(";")[25] == "T" else False
		...
		self.popupmenu.Enable( id = 5134, enable = gerenciador_nfe_compras )
		self.popupmenu.Enable( id = 5002, enable=True if len(login.usaparam.split(";"))>=35 and login.usaparam.split(";")[34]=="T" else False )
	...
	def OnPopupItemSelected(self, event):
		...
		if even == 5002:
			...
			dev_frame=GerenciadorMdfe(parent=self,id=even)
			dev_frame.Centre()
			dev_frame.Show()
		...
		if even ==  5134:
			...
			GerenteNfeCompras.modulo = "1-Caixa recebimentos"

			compras_frame=GerenteNfeCompras(parent=self,id=even)
			compras_frame.Centre()
			compras_frame.Show()

		if even ==  1017:

			whats_frame=GerenciadorWhats(parent=self,id=even)
			whats_frame.Centre()
			whats_frame.Show()
	...
	def abrirdav(self,event):
		...
		if   self.cdevol.GetValue() != True:	alertas.dia(self.painel,u"Função exclusiva do modulo de devolução!!","Caixa: Recebimentos")
			...
			if   self.vin.GetValue() == '':	alertas.dia(self.painel,"DAV-Vinculado, estar vazio!!","Caixa: Recebimentos")
				...
				self.impress.impressaoDav( NumeroDav, self, True, True,  "", "", servidor = filialDav, codigoModulo=cdDav, enviarEmail = cdEma,recibo=False )
	...
	def ecfcancela(self,event):
		...
		emissao_nota_complementar=True if self.ListaRec.GetItem(indice,17).GetText()=="2" and self.ListaRec.GetItem(indice,57).GetText().strip() and len(self.ListaRec.GetItem(indice,57).GetText().strip())==44 else False

		if emissao_nota_complementar: # or self.ListaRec.GetItem(indice,48).GetText() == "2":

			alertas.dia(self,"{ I M P O R T A N T E }\n\nOrcamento utilizado para emissao de nota fiscal complementar\n\n1-Cancele a nota fiscal antes de cancelar o orcamento vinculado...\n"+(' '*200),'Nota fiscal complementar')
	...
	def nfeatalho(self,event):
		...
		indice = self.ListaRec.GetFocusedItem()
		NuDav  = self.ListaRec.GetItem(indice,0).GetText()
		tipo_dav = self.ListaRec.GetItem(indice,17).GetText()
		numero_nota = self.ListaRec.GetItem(indice,33).GetText().strip()
		numero_chve = self.ListaRec.GetItem(indice,34).GetText().strip()
		consolidado = self.ListaRec.GetItem(indice,58).GetText().strip() if tipo_dav=='2' else ''
		
		modelo = numero_chve[20:22] if numero_chve else ""
		modelo_emissao_nota = self.modelo_emissao.GetValue().split('-')[0]
		
		emissao_nota_complementar=True if self.ListaRec.GetItem(indice,17).GetText()=="2" and self.ListaRec.GetItem(indice,57).GetText().strip() and len(self.ListaRec.GetItem(indice,57).GetText().strip())==44 else False
		if emissao_nota_complementar:	modelo= modelo_emissao_nota='55'

		__modelo = self.modelo_emissao.GetValue().split('-')
		self.modelo_emissao.SetValue(self.nota_relacao_emissao_pos_recebimento[0])
		
		""" Se ja houve emissao da nota, o direto procuara o tipo automatico p/determinar o modelo, se nao deve selecionar o modelo """
		if numero_nota and not modelo.strip():

		    conn = sqldb()
		    sql  = conn.dbc("Caixa: Consulta de nota emitida", fil =  self.fl, janela = self.painel )
		    if sql[0]:
			if sql[2].execute("SELECT nf_nfesce FROM nfes WHERE nf_numdav='"+ NuDav +"' and nf_nnotaf='"+numero_nota+"'"):

			    tipo_nota_fiscal = sql[2].fetchone()[0]

			    if tipo_nota_fiscal == '1':	modelo = modelo_emissao_nota = '55' 
			    if tipo_nota_fiscal == '2':	modelo = modelo_emissao_nota = '65' 
			    
			conn.cls(sql[1])
		
		if self.ListaRec.GetItem(indice,17).GetText()=="2" and not emissao_nota_complementar and consolidado!='T':

		    alertas.dia(self.painel,u"Orçamento: "+str(NuDav)+u"\n\nProcedimento não suportado\n"+(' '*100),"Caixa: Recebimento")
		    return
	
		elif self.ListaRec.GetItem(indice,39).GetText() != '1' and not emissao_nota_complementar and consolidado!='T':
		    
		    if not self.ListaRec.GetItem(indice,39).GetText():	informe = u"{ DAV não recebido no caixa }\n"
		    if self.ListaRec.GetItem(indice,39).GetText() == '2':	informe = u"{ DAV com marca de estorno }\n"
		    if self.ListaRec.GetItem(indice,39).GetText() == '3':	informe = u"{ DAV com marca de cancelamento }\n"
				
		    alertas.dia(self.painel,informe+(" "*100),u"Caixa: Emissão de NFe-NFCe")	
		    return
		...
		if valorD == '':
		    ...
		    passar = True
		    #--// [ se o cliente nao tiver cadastra o sistema seleciona automaticamente o modelo 55 ]
		    if self.ListaRec.GetItem(indice,43).GetText() != 'S' and not len(__modelo)==2:	modelo_emissao_nota='65'
		    if numero_nota and not numero_chve and len(__modelo)==2 and modelo_emissao_nota:	modelo=modelo_emissao_nota
		    if consolidado=='T':	modelo_emissao_nota='55'
		    
		    if not numero_nota and not numero_chve and modelo_emissao_nota:
			if modelo_emissao_nota in ['55','65']:	modelo = modelo_emissao_nota
			else:
			    alertas.dia(self,u"Selecione um modelo de nota 55-NFe,65-NFCe para continuar\n"+(" "*140),u"Caixa: Emissão de NFE")	
			    passar = False

		    if not modelo: # or not self.ListaRec.GetItem(indice,34).GetText().strip():
			alertas.dia(self,u"Direto não consegue passar o modelo de nota {55,65} c/a chave do dav selecionado\n"+(" "*180),u"Caixa: Emissão de NFE")	
			passar = False
			
		    if self.ListaRec.GetItem(indice,43).GetText() != 'S' and modelo !='65' and not emissao_nota_complementar:
			alertas.dia(self,u"Cliente não cadastrado !!\n"+(" "*100),u"Caixa: Emissão de NFE")	
			passar = False
			    
		    if not self.ListaRec.GetItem(indice,31).GetText().strip() and modelo == '55' and not emissao_nota_complementar:
		    ...
		    if len(self.ListaRec.GetItem(indice,31).GetText().strip())==14 and modelo_emissao_nota=='65' and len(login.filialLT[self.fl][35].split(';'))>=115 and login.filialLT[self.fl][35].split(';')[114]=='T' and self.ListaRec.GetItem(indice,43).GetText().strip().upper()=='S':

			alertas.dia(self,'{ Bloqueio: CNPJ valido com emissão de NFC-e }\n\n1 - Utilize emissão de NFe\n'+(' '*130),'Emissão de NFe/NFC-e')
			passar = False
			...
			editadanfe.modelonfe = modelo
			editadanfe.numechave = numero_chve
			editadanfe.consolidado = consolidado
	...
	def impresDav(self,event):
		...
		self.impress.impressaoDav( NumeroDav, self, True, True,  __Dev, "", servidor =  self.fl, codigoModulo = cdDav, enviarEmail = cdEma,recibo=False )
	...
	def cusTosDav(self,event):
		...
		if self.ListaRec.GetItemCount() == 0:	alertas.dia(self.painel,u"Lista vazia, sem registros para prosseguir!!\n","Caixa: Custo do DAV")
			...
			self.impress.impressaoDav( NumeroDav, self, True, True, __Dev, "CUS", servidor =  self.fl, codigoModulo = cdDav, enviarEmail = cdEma,recibo=False )
	...
	def Teclas(self,event):
		...
		if controle !=None and controle.GetId() == 300:
			...
			if self.ListaRec.GetItem(indice,17).GetText() == '9':	Tipop = u"Consignação"
			...
			valor_produto = Decimal(self._tp.GetValue().replace(',',''))
			valor_frete   = Decimal(self._fr.GetValue().replace(',',''))
			valor_acresci = Decimal(self._ac.GetValue().replace(',',''))
			valor_descont = Decimal(self._dc.GetValue().replace(',',''))

			self.perc_fr.SetLabel('')
			self.perc_ac.SetLabel('')
			self.perc_ds.SetLabel('')
			if valor_produto and valor_frete:	self.perc_fr.SetLabel( '('+ format( ((valor_frete/valor_produto)*100),'.2f') +'%)')
			if valor_produto and valor_acresci:	self.perc_ac.SetLabel( '('+ format( ((valor_acresci/valor_produto)*100),'.2f') +'%)')
			if valor_produto and valor_descont:	self.perc_ds.SetLabel( '('+ format( ((valor_descont/valor_produto)*100),'.2f') +'%)')
			
			if self.ListaRec.GetItem(indice,13).GetText().strip():	self.entregar.SetLabel('Entrega: '+self.ListaRec.GetItem(indice,13).GetText())
			else:	self.entregar.SetLabel('')
			...
			else:	self.nfecan.SetLabel("{ Informações NFe, NFCe }")
	...
	def selecionar(self,event):
		...
		if sql[0]:
			...
			#print("-"*100)
			#print(self.dindicial.GetValue().Format("%d-%m-%Y"))
			#print("Ola: ",self.dindicial.GetValue())
			#print("-"*100)
			...
			for i in _result:
				...
				if filtro_pedidos == '3' and i[123]=='WOO':	avancar = True
				if filtro_pedidos == '4' and i[123]=='CCL':	avancar = True
		
				if filtro_pedidos == '10' and i[8] and i[15] and i[73] and i[74]!='':	avancar=True 
				if filtro_pedidos == '11' and not i[8] and not i[15] and not i[73] and i[74] in ['1','2']:	avancar=True
			...
			self.ListaRec.SetItemCount(_registros)
			self.ocorre.SetLabel("{"+str(_registros)+"}")
	...
	def ecfcaixa(self,event):
		...
		""" Recebimento de DAVS """
		if self.ListaRec.GetItem(indice,17).GetText() == "9":

			alertas.dia(self.painel,u"DAV Nº: "+str( NuDav )+u"\n\nProcedimento não suportado\n\n{ Pedido com marca de consignação/empréstimo }\n"+(' '*120),"Caixa: Recebimento")
			return
		...
		if self.cdevol.GetValue() == False:
			...
			if _recebimento == True:
				...
				if sql[0]:
					...
					""" Bloquio do recebimento, se existir pedidos em aberto """
					if len(login.filialLT[self.fl][35].split(";"))>=174 and login.filialLT[self.fl][35].split(";")[173]=="T":

					    dri = str( datetime.datetime.strptime(( datetime.date.today() + datetime.timedelta(days=-60)).strftime("%Y/%m/%d"),"%Y/%m/%d") ).split(' ')[0].replace('-','/')
					    drf = str( datetime.datetime.strptime(( datetime.date.today() + datetime.timedelta(days=-1)).strftime("%Y/%m/%d"),"%Y/%m/%d") ).split(' ')[0].replace('-','/')
					    paberto= Decimal()
					    abertos = "SELECT SUM(cr_tnot) FROM cdavs WHERE cr_edav >='"+dri+"' and cr_edav <='"+drf+"' and cr_reca='' and cr_tipo='1' and cr_inde='"+self.fl+"'"
					    vaberto = sql[2].execute(abertos)
					    paberto = sql[2].fetchone()[0]

					conn.cls(sql[1])

					""" Bloquio do recebimento, se existir pedidos em aberto """

					if len(login.filialLT[self.fl][35].split(";"))>=174 and login.filialLT[self.fl][35].split(";")[173]=="T" and paberto and ld==hj:

					    alertas.dia(self,"{ Existe pedidos em aberto de dias anteriores ao dia de hoje }\n\nValor: "+format(paberto,'.2f')+"\nEfetue o recebimento desses pedidos e refaça o precesso de recebimento\n"+(" "*200),"Caixa: Recebimentos de Dav(s)")
					    return
...
class RcListCtrl(wx.ListCtrl):
	...
	def __init__(self, parent, ID=wx.ID_ANY, pos=wx.DefaultPosition,size=wx.DefaultSize, style=0):
		...
		self.attr4.SetBackgroundColour("#F9F9F9")
		self.attr5.SetBackgroundColour("#9B6060")
		...
		self.InsertColumn(56,u'Registro no banco',width=200)
		self.InsertColumn(57,u'Chave referenciada da nfe p/emissao de nota fiscal complementar',width=500)
		self.InsertColumn(58,u'Consolidado',width=500)
	...
	def OnGetItemAttr(self, item): #Ajusta cores sim/nao
		...
		if self.itemIndexMap !=[]:
			...
			tipop=self.itemDataMap[index][17]
			consolidado=self.itemDataMap[index][58]

			if consolidado=='T':	return self.attr2
			if tipop.strip() == "9" and not estor.strip() == "3":	return self.attr4 # Consignacao
			if tipop.strip() == "9" and estor.strip() == "3":	return self.attr5 # Consignacao
...
class pagamento(wx.Frame):
	...
	def __init__(self, parent,id):
		...
		self.vda = self.result[0][82].strip() if self.result[0][82] else "" #-:Historico das Autorizações
		...
		self.menssagem_caixa = wx.StaticText(self.painel,-1,"{ Menssagem }", pos=(430,575))
		self.menssagem_caixa.SetFont(wx.Font(8, wx.MODERN, wx.NORMAL, wx.BOLD,False,'Arial'))
		...
		recebimento_local = False
		if len( login.filialLT[ self.pgFil ][35].split(";") ) >= 160 and login.filialLT[ self.pgFil ][35].split(";")[159] == "T":	recebimento_local = self.davComRecebimentoLocal()
		self.formaspgt = self.relpag.fpg(pagCli,2, rclocal=recebimento_local, filial=self.pgFil)
		...
		self.recebn = GenBitmapTextButton(self.painel,229,label='Recebimento Normal', pos=(618,350),size=(123,22), bitmap=wx.Bitmap("imagens/liquidar.png", wx.BITMAP_TYPE_ANY))
		self.recebn.SetFont(wx.Font(7, wx.MODERN, wx.NORMAL, wx.NORMAL,False,"Arial"))
		...
		self.historico = wx.TextCtrl(self.painel,900,value=self.result[0][82] if self.result[0][82] else '', pos=(10,390), size=(414,115),style = wx.TE_MULTILINE|wx.TE_DONTWRAP)
		...
		self.ListaPrer = wx.ListCtrl(self.painel, 770,pos=(428,390), size=(318,180),
									style=wx.LC_REPORT
									|wx.BORDER_SUNKEN
									|wx.LC_HRULES
									|wx.LC_VRULES
		...
		self.recebn.Bind(wx.EVT_ENTER_WINDOW, self.OnEnterWindow)
		...
		self.recebn.Bind(wx.EVT_LEAVE_WINDOW, self.OnLeaveWindow)
		...
		self.ecfemi.Bind(wx.EVT_BUTTON,self.fechanfe)
		#self.ecfemi.Bind(wx.EVT_BUTTON,self.fechaecf)
		self.recebn.Bind(wx.EVT_BUTTON,self.fechaecf)
		...
		if login.filialLT[ self.pgFil ][21] == "1":	self.recebn.Disable()
		
		"""  Permitir Emissao de NFCe, Cadastro da Empresa 2-Normal NFe,NFCe  """
		if login.filialLT[ self.pgFil ][21] == "2" and login.usaenfce == "T":	self.ecfemi.SetBitmapLabel (wx.Bitmap('imagens/backup16.png'))
		...
		""" Validar o CNPJ-CPF, Cliente nao cadastrado inibir o botao de NFe """
		if not self.clresul:
		    nfeemi.Enable(False) #--// Cliente nao cadastrado
		    self.menssagem_caixa.SetLabel(u'{ Cliente não cadastrado }')
		    self.menssagem_caixa.SetForegroundColour('#AC1B1B')
			
		if not self.clresul and self.result and self.result[0][39].strip() and not nF.cpfcnpj( self.result[0][39].strip() )[0]:
		    self.menssagem_caixa.SetLabel(u'{ Não cadastrado com CPF/CNPJ Invalido }')
		    self.menssagem_caixa.SetForegroundColour('#AC1B1B')

		if self.clresul and not self.clresul[0][6].strip():
		    nfeemi.Enable(False)
		    self.menssagem_caixa.SetLabel(u'{ Cliente sem CPF/CNPJ }')
		    self.menssagem_caixa.SetForegroundColour('#AC1B1B')

		if self.clresul and self.clresul[0][6].strip() and not nF.cpfcnpj( self.clresul[0][6].strip() )[0]:
		    nfeemi.Enable(False)
		    self.menssagem_caixa.SetLabel(u'{ CPF/CNPJ Invalido }')
		    self.menssagem_caixa.SetForegroundColour('#AC1B1B')

		csc = login.filialLT[self.pgFil][30].split(';')
		if not csc[20].strip() or not csc[21]: #--// ID ou Numero CSC
		    self.ecfemi.Enable(False)
		    self.menssagem_caixa.SetLabel(u'{ Sem ID-CSC }')
		    self.menssagem_caixa.SetForegroundColour('#AC1B1B')
	...
	def voltar(self,event):
		...
		self.parente.Enable()
		self.Destroy()
	def davComRecebimentoLocal(self):

		conn = sqldb()
		sql  = conn.dbc("Caixa: Recebimento { Verificando recebimento local }", fil = self.pgFil, janela = self )
		rt = False
		if sql[0]:
		    
		    if sql[2].execute("SELECT it_entr FROM idavs WHERE it_ndav='"+self.davn+"'"):
	...
	def alterarValores(self,event):
		...
		if saida:	alertas.dia(self.painel,u"[ Data Incompatível ]\n\nVencimento: "+str(dTVenc.strftime("%d/%m/%Y"))+u"\n  Data Atual: "+str(dTHoje.strftime("%d/%m/%Y"))+u"\n\nCom Pagamento em CHEQUE!!\n"+data_autorizada+(" "*120),u"Caixa-Pagamento")
	...
	def validaPagamentosFuturos(self):
		...
		if self.listaPagamento.GetItemCount():
			...
			permitir_documento = False if len( login.filialLT[ self.pgFil ][35].split(";") ) >= 117 and login.filialLT[ self.pgFil ][35].split(";")[116] == 'T' else True

			if formas_bloqueio and permitir_documento:
				
				alertas.dia( self, u"{ Formas de pagamentos com lançamento no contas areceber }\n\n"+sem_documento+u"\n\nAjuste os dados do cliente e refaça o processo\n"+(" "*150),"Bloqueio com formas de pagamentos futuras")
				retornar = False
	...
	def fechaecf(self,event):
		...
		self.recebn.Enable(False)
		if not self.validaPagamentosFuturos():
	
		    self.recebn.Enable(True)
		    return
		    
		if not self.validaRcNormal():
		    alertas.dia(self,u"{ DAV em processo de inutilização de NOTA }\n\n1 - Voçê pode Reenviar ao SEFAZ p/continuar o recebimento\n2 - Inutilize a NF no gerenciador de notas e refaça o recebimento\n"+(" "*130),u"Caixa: Recebimento Normal")
    		    self.recebn.Enable(True)
	
		elif Decimal(self.slDav.GetValue().replace(',','')):
		    alertas.dia(self.painel,u"4-Consta um saldo de caixa:\n\nSaldo: "+str(self.slDav.GetValue())+"\n"+(" "*100),u"Caixa: Fechamento")
    		    self.recebn.Enable(True)
			...
			if event.GetId()==229:
			    aa=self.fechamento("229",'', filial = self.pgFil )
	
			else:	self.recebn.Enable(True)
	...
	def incluirPagamento(self,_op, vcPre = "" ):
		...
		if Decimal( self.valorapaga.GetValue() ) > 0:
			...
			__forma_pagamento_credito = False
			if self.fpagamento.GetValue()[3:5] == "10":	__forma_pagamento_credito = True
			if len( login.filialLT[ self.pgFil ][35].split(";") ) >= 155 and login.filialLT[ self.pgFil ][35].split(";")[154]=='T' and self.fpagamento.GetValue()[3:5] == "09":	__forma_pagamento_credito = True

			if _receber == True and __forma_pagamento_credito: #self.fpagamento.GetValue()[3:5] == "10":
			...
			if _receber == True and __forma_pagamento_credito and self.clresul !=None and self.clresul !='' and self.clresul[0][5] !=None and self.clresul[0][5] !='' and self.clresul[0][5].split('\n')[0].split('|')[0].upper() == "TRUE":	inf07, _receber = True, False
			...
			if _receber == True:
				...
				if parcelado == True and int( self.nuparcelas.GetValue() ) > 1:
					...
					alertas.dia(self.painel,u"Não é permitido parcelamento para a forma de pagamento selecionado\n"+(" "*180)+u"Forma de Pagamento: "+self.fpagamento.GetValue()[6:],u"Contas AReceber: Desmenbramento de Títulos")
					return
	...
	def inluiRecebimento(self,opcao, mudanca_forma_pagamento ):
		...
		if self.fpagamento.GetValue()[3:5] == "10":	pagamento_credito = True
		if len( login.filialLT[ self.pgFil ][35].split(";") ) >= 155 and login.filialLT[ self.pgFil ][35].split(";")[154]=='T' and self.fpagamento.GetValue()[3:5] == "09":	pagamento_credito = True

		#print('Aqui!! 3')
		#pagamento_credito = True if self.fpagamento.GetValue()[3:5] == "10" else False
		#if self.fpagamento.GetValue()[3:5] == "09":	pagamento_credito = True
	...
	def fechanfe(self,event):
		...
		modelo_nf = '65' if event.GetId() == 225 else '55'
		if not self.validaPagamentosFuturos():	return
		ic = self.parente.ListaRec.GetFocusedItem()
		
		rT = False
		if event.GetId() in [160,225]:	rT = self.fechaTeste('160')

		if modelo_nf == 55 and self.parente.ListaRec.GetItem(ic,43).GetText() != 'S':
			alertas.dia(self.painel,u"Cliente não cadastrado !!\n"+(" "*80),u"Caixa: Emissão de NFE")	
			rT = False
				
		elif modelo_nf == 55 and self.parente.ListaRec.GetItem(ic,31).GetText() == '':
		...
		if rT == True:
			...
			editadanfe.tiponfrma = ""
			editadanfe.modelonfe = modelo_nf
			editadanfe.numechave = ''
			editadanfe.consolidado = ''
	...
	def fechamento( self, idNF, nfe, filial = '' ):
		...
		if baixar == True:
			...
			if sql[0]:
				...
				"""  Verifica se esse titulo ja se encontra no contas areceber { para evitar q recebe com numero de parcela ja recebida }  """
				Numero_lancamentos_contas_receber=sql[2].execute("SELECT rc_ndocum FROM receber WHERE rc_ndocum='"+self.davn+"'")
				
				"""  Relacionar produtos para expedicao por departamento  """				
				relacao_items_expedicao = []
				if sql[2].execute("SELECT it_codi,it_nome,it_unid,it_fabr,it_ende,it_quan,it_grup,it_estf,it_item,it_emba,it_unpc,it_clcm,it_clla,it_clex FROM idavs WHERE it_ndav='"+ self.davn +"'"):

				    for ri in sql[2].fetchall():

					expedicao_individual='' #--// Expedicao individual p/produto { monfardini 31-01-2019 }
					if sql[2].execute("SELECT pd_para FROM produtos WHERE pd_codi='"+ri[0]+"'"):
					    
					    exp=sql[2].fetchone()[0]
					    if exp and len(exp.split('|'))>=14 and exp.split('|')[13]:	expedicao_individual=exp.split('|')[13].split('-')[1]

					relacao_items_expedicao.append( ri[0] +'|' + ri[2]+'|'+ str( ri[5] ) +'|'+ ri[6] +'|'+ ri[1] +'|'+ ri[3] +'|'+ ri[4] +'|'+ str( ri[7] )+'|'+expedicao_individual+'|' +str(ri[8])+'|'+ri[9] +'|'+ str(ri[10]) +';'+str(ri[11])+';'+str(ri[12])+';'+str(ri[13]) )
				...
				if idNF == '160':
					...
					cr_guap='"+str( guardar )+"',cr_vpar='"+str( parcela )+"',cr_cavu='"+str( avu )+"' WHERE cr_ndav='"+self.davn+"'"
				...
				try:
					...
					lqcP = "F"
					lqDC = "F"
					usando_conta_corrente = False #--// Usando pagamento com credito para a forma de pagamento [09]
					forma_pagamento_09 = ""
					...
					""" Utilizado para quando o titulo ja existir no contas areceber [ Para incrementar as parcelas ]"""
					nova_parcela = Numero_lancamentos_contas_receber
					for i in range( Nregistro ):

						if Numero_lancamentos_contas_receber:
						    nova_parcela +=1
						    _parc = str(nova_parcela).zfill(2)
						else:	_parc = str(self.listaPagamento.GetItem(_indice,0).GetText()[1:])
						...
						if CDB and len( login.filialLT[filial][35].split(";") ) >= 155 and login.filialLT[filial][35].split(";")[154]=='T' and _fpag.split('-')[0]=="09":

						    usando_conta_corrente = True
						    forma_pagamento_09 = _fpag
						    Faturado = False
					...
					if Decimal( CDB ) !=0:
						...
						if usando_conta_corrente:	_his = u"["+forma_pagamento_09+"] recebido no caixa"
						if self.numer_TicheT.GetValue().strip() !="":
				...
				impressao_recibo_venda=True if len(login.filialLT[filial][35].split(";"))>=135 and login.filialLT[filial][35].split(";")[134]=='T' else False
				impress = impressao()

				if fina and simm and idNF == '229' and len( login.usaparam.split(";") ) >=30 and login.usaparam.split(";")[29] == 'T' and login.impparao and len(user) >=13:

					""" 
					#--// [ Opcao para Impressao direta ]
					arquivo = impress.impressaoDav( self.result[0][2], self.parente, True, True,  "", "", servidor = filial, codigoModulo='TERM', enviarEmail =[] )
					pasta = arquivo['bematech'+user[12]] if user[12] in ['1','2'] and 'bematech'+user[12] in arquivo else ''
					ipp, porta = user[11], user[10]
					if pasta and ipp and porta:	termica_direta.enviarImpressao(self.painel, open(pasta,"r").read(), user, ipp, porta, filial )
					"""
					if impressao_recibo_venda:	impress.impressaoDav( self.result[0][2], self.parente, True, True,  "", "", servidor = filial, codigoModulo='501', enviarEmail =[],recibo=True )
					else:	impress.impressaoDav( self.result[0][2], self.parente, True, True,  "", "", servidor = filial, codigoModulo='501', enviarEmail =[],recibo=False )

					self.parente.selecionar(wx.EVT_BUTTON)
					self.voltar(wx.EVT_BUTTON)

				elif idNF == '229' and impressao_recibo_venda and self.result[0][21]:	impress.impressaoDav( self.result[0][2], self.parente, True, True,  "", "", servidor = filial, codigoModulo='501', enviarEmail =[],recibo=True )
				else:
...
class sangria(wx.Frame):
	...
	def ajustafrn(self,_dc,_ft,_nm,_ie,_im,_cn,_id,_rp, _pc ):
		...
		self.dados_fornecedor = str( _id )+"|"+str( _dc )+"|"+str( _ft )+"|"+ _nm +"|"+str( _pc )
		self.planoc.SetValue( _pc )
		self.fornec.SetValue( str( _id )+" "+ _nm )
		...
		if len(self.histo.GetValue()) > 9 and valor !=0:
			...
			if addsan.ShowModal() ==  wx.ID_YES:
				...
				if sql[0]:
					...
					conn.cls(sql[1])
					if sair:	self.sair(wx.EVT_BUTTON)
